...
Notice how the counter does not reset now when the webview is hidden and then restored. No extra code required! With `retainContextWhenHidden`, the webview acts similarly to a background tab in a web browser. Scripts and other dynamic content keep running even when tab is not active or visible. You can also send messages to a hidden webview when `retainContextWhenHidden` is enabled.
