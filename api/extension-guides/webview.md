...
[Web workers](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers) are supported inside of webviews but there are a few important restrictions to be aware of.
...
If you do need to load worker code from a JS file in your extension, try using `fetch`:
...
Worker scripts also do not support importing code using `importScripts` or `import(...)`. If your worker loads code dynamically, try using a bundler such as webpack to package the worker script into a single file.

With `webpack` for example, you can use `LimitChunkCountPlugin` to force the compiled worker JavaScript to be a single file:
