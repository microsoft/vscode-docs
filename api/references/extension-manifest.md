...
| `extensionKind`                                 |          | `array`                                    | An array that indicates where the extension should run in remote configurations. Values are `ui` (run locally), `workspace` (run on remote machine), `web` (run in [web extension host](/api/extension-guides/web-extensions)), with the order setting the preference. For example: `[ui, workspace]` indicates the extension can run in either location but prefers to run on the local machine. **Note** The `web` extension kind is usually inferred based on [rules](/api/extension-guides/web-extensions#web-extension-enablement) and `web` should be added at the end of the list. See [here](https://code.visualstudio.com/api/advanced-topics/remote-extensions#incorrect-execution-location) for more information on `extensionKind`..                                                                          |
