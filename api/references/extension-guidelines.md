...
These guidelines cover the best practices for creating extensions.

## Architecture

The UI has two parts: groups and sections. Groups refers to the outer layers, which include: [Activity Bar](#view-container), Side Bar, Editor, Panel, Status Bar, and [Notifications](#notifications):

![images/guidelines/Untitled.png](images/guidelines/architecture-groups.png)

Sections refers to items  inside of the groups, which include: Sidebar Toolbar, [View](#views), View Container, View Toolbar, [Editor Toolbar](#editor-actions), Panel Toolbar, and [Status Bar Item](#status-bar):

![images/guidelines/Untitled%201.png](images/guidelines/architecture-sections.png)

## Notifications

[Notifications](/api/extension-capabilities/common-capabilities#display-notifications) are small bits of information that is surfaced from the bottom right. You can send three types of notifications: [Information](/api/references/vscode-api#window.showInformationMessage), [Warning](https://code.visualstudio.com/api/references/vscode-api#window.showWarningMessage), and [Error](https://code.visualstudio.com/api/references/vscode-api#window.showErrorMessage). It's important to limit the amount of notifications sent in order to respect the user's attention.
