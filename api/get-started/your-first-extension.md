...
If the `Hello World` command cannot be found from the Command Palette (`kb(workbench.action.showCommands)`), check if there is any error message showing up in the lower right corner. The most common problem is that Visual Studio Code requires an update since the sample extension project generated by `yo code` would specify to use the latest version of the `vscode` engine, which would be specified by the `engines` field in `package.json`.
