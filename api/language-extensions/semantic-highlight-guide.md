...
In the example above, an extension declares a new type `templateType` and a new modifier `native`. By naming `type` as the `superType`, theme styling rules for `type` will also apply to `templateType`:
...
`"#ff0011"` above applies to both `type` and all it's subtypes, including `templateType`.
