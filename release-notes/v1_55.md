...
Additionally, `kbstyle(Shift)`+Click range selections behave more deterministically.
...
We support selecting multiple cells in a notebook through mouse (`kbstyle(Shift)`+Click) or keyboard shortcuts (`kbstyle(Shift Arrow)`). Once multiple cells are selected, you can then copy/cut/paste/move/duplicate selected cells.

![noteook multiple cells selection](images/1_55/notebook-multi-cells-selection.gif)

You can also drag and drop multiple cells together.

![noteook multiple cells selection drag anddrop](images/1_55/notebook-multi-cells-selection-dnd.gif)

**Limitations**: When there are multiple distinct ranges of cells are selected, most commands will only be executed against the first range or the focused range. We will continue improving this experience in following releases.
...
![Notebook cells being mult-selected via border instead of backgorund colors](images/1_55/notebook-multiselect-border.gif)

### Show/hide output and metadata differences in the diff editor

We have heard issue reports from users that when reviewing changes in a notebook, they found the modifications in notebook cell metadata or outputs distracting and irrevelant. VS Code now allows you to control the visibility of metadata or output changes in the diff editor through settings `notebook.diff.ignoreMetadata` and `notebook.diff.ignoreOutputs`. They can also be toggled by using **Show Outputs/Metadata Differences** command from the editor title toolbar.
