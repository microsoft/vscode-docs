...
New settings `terminal.integrated.profiles.<platform>` define Integrated Terminal profiles that can be accessed via the terminal dropdown, where `<platform>` is `osx`, `linux`, or `windows`.
