...
Last month, we introduced Custom Labels, which let you personalize the labels of your editor tabs. This feature has been designed to help you easily identify which `index.tsx` file, or any other file, you've opened without any guesswork. Building on that, we've extended Custom Labels to Quick Open `(Ctrl + P)`. Now, you can search for your files using the custom labels you've created, making file navigation more intuitive.

![Custom Labels in Quick Open](images/1_89/custom-labels-quick-open.png)

### Customize Keybindings

Customizing keybindings for action items is now more straightforward. Just right-click on any action item in your workbench, and select "Customize Keybinding". If the action has a `when` clause, it will be automatically included, making it easier to set up your keybindings just the way you need them.
...
We have addressed an issue where the Find Widget was frequently being opened unintentionally in tree widgets, such as the Explorer, when users actually intended to use it within the editor. To reduce these accidental activations, we have changed the default keybinding for opening the Find Widget in tree widgets to `Ctrl + Alt + f`. Users who prefer the previous setup can easily revert to the original keybinding of `Ctrl + f` for the `list.find` command using the Keyboard Shortcuts Editor.
...
A new layout for the notebook error renderer can be used with the setting <code codesetting="notebook.output.minimalErrorRendering:true">notebook.output.minimalErrorRendering</code> which will display just the error and message and allow the full error stack to be expanded into view.
