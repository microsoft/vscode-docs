...
Line signals are now debounced with a higher delay than positional signals. Also, aria alert signals have a higher debounce delay than audio cue signals.
...
Now, when you navigate to the next `(kb(workbench.action.terminal.accessibleBufferGoToNextCommand))` or previous `(kb(workbench.action.terminal.accessibleBufferGoToPreviousCommand))` command in the terminal Accessible View, you can hear if the current command failed. This can be configured with the setting <code codesetting='accessibility.signals.terminalCommandFailed'>accessibility.signals.terminalCommandFailed</code>.
...
For extensions that use the language model, you can now track their usage of the language model in the Extension Editor and Runtime Extensions Editor. For example, you can view the number of language model requests, as demonstrated for the Copilot Chat extension in the following screenshot.
...
![Local Workspace Extension](images/1_89/local-workspace-extension.png)

Please note that you should include the unpacked extension in the `.vscode/extensions` folder and not the `VSIX` file. You can also include only sources of the extension and build it as part of your workspace setup.

### Custom Editor Labels in Quick Open

Last month, we introduced custom labels, which let you personalize the labels of your editor tabs. This feature is designed to help you easily identify which `index.tsx` file, or any other file, you've opened without any guesswork. Building on that, we've extended custom labels to Quick Open `kbstyle(Ctrl + P)` (`kbstyle(Cmd + P)` on macOS). Now, you can search for your files using the custom labels you've created, making file navigation more intuitive.

![Custom Labels in Quick Open](images/1_89/custom-labels-quick-open.png)

### Customize keybindings

Customizing keybindings for action items is now more straightforward. Just right-click on any action item in your workbench, and select **Customize Keybinding**. If the action has a `when` clause, it's automatically included, making it easier to set up your keybindings just the way you need them.

<video src="images/1_89/customize-keybinding.mp4" title="Customize keybinding of action items" autoplay loop controls muted></video>

### Find in trees keybinding

We have addressed an issue where the Find control was frequently being opened unintentionally for a tree control, such as the Explorer view, when users actually intended to use it within the editor. To reduce these accidental activations, we have changed the default keybinding for opening the Find control in a tree control to `kb(list.find)`. If you prefer the previous setup, you can easily revert to the original keybinding for the `list.find` command using the Keyboard Shortcuts editor.


### Auto detect system color mode improvements

If you want your theme to follow the color mode of your system, you can do this by enabling the setting <code codesetting="window.autoDetectColorScheme:true">window.autoDetectColorScheme</code>.

When enabled, the current theme is defined by the setting <code codesetting="workbench.preferredDarkColorTheme">workbench.preferredDarkColorTheme</code> when in dark mode and setting <code codesetting="workbench.preferredLightColorTheme">workbench.preferredLightColorTheme</code> when in light mode.

<code codesetting="workbench.colorTheme">workbench.colorTheme</code> is then no longer read or modified. It is only used when <code codesetting="window.autoDetectColorScheme">window.autoDetectColorScheme</code> is off.

What's new is that now the theme picker dalog (Command: ** Preferences: Color Theme **) is aware of this and configures the currently active setting.

Here. the theme selection dialog is configuring the theme when in Dark mode:
![The theme selection dialog configuring the theme for Dark mode](images/1_89/configuring_dark_mode.png)

The dialog also has a new button (![The theme selection dialog configuring the theme for Dark mode](images/1_89/configure-detect-mode.png)) to directly bring you to the <code codesetting="window.autoDetectColorScheme">window.autoDetectColorScheme</code> setting.

### Paste Markdown links in Comments

In the input editor of the Comments widget, pasting a link will provide the same behavior as pasting a link in a markdown file. The paste options will show and you can choose to paste a Markdown link instead of the raw link that you copied.
...
This milestone we have addressed a long-standing feature request to save/restore editors when switching between source control branches. Use the <code codesetting="scm.workingSets.enabled:true">scm.workingSets.enabled</code> setting to enable this feature. To control the open editors when switching to a source control branch for the first time you can use the <code codesetting="scm.workingSets.default">scm.workingSets.default</code> setting.

### Dedicated commands for viewing changes

To make it easier to view your staged changes, changes, and untracked changes in the multi-file diff editor we have added a set of new commands to the command palette: **Git: View Staged Changes**, **Git: View Changes**, and **Git: View Untracked Changes**.

## Notebooks

### Minimal error renderer

You can use a new layout for the notebook error renderer with the setting <code codesetting="notebook.output.minimalErrorRendering:true">notebook.output.minimalErrorRendering</code>. This new layout displays only the error and message, and enables the full error stack to be expanded into view.
...
Note that all Quick Search commands and settings no longer have the "experimental" keyword in their ID. For example, the command ID `workbench.action.experimental.quickTextSearch` became `workbench.action.quickTextSearch`. This may be relevant if you have settings or keybindings that use the old IDs.
...
The JavaScript debugger will now automatically look for binaries that appear in the `node_modules/.bin` folder in the `runtimeExecutable` configuration. Now, it resolves them by name automatically:
...
Want a quick preview of an image or video in some Markdown without opening the full [Markdown preview](../docs/languages/markdown.md#markdown-preview)? Now you can hover over an image or video path to see a small preview of it:
...
The two `## Example` headers have the same text but can each be linked to individually by using a unique ID (`#example` and `#example-1`). Previously, if you renamed the first `## Example` header to `## First Example`, the `#example` link would be correctly changed to `#first-example` but the `#example-1` link would not be changed. Howeverm `#example-1` is no longer a valid link after the rename because there are no longer duplicated `## Example` headers.

We now corretly handle this scenario. If you rename the first `## Example` header to `## First Example` in the document above for instance, the new document will be:
...
Now rename suggestions can be triggered using an icon within the rename widget to get suggestions.

![Rename widget now has a button to trigger rename suggestions from copilot](images/1_89/rename-suggestions-button.mp4)
...
When a file is excluded by content exclusions, Copilot Chat will be unable to see the contents or the path of the file and it will not be used in generating an LLM suggestion.
...
We now support inserting new cells with inline chat activated automatically in the notebook editor. We show a **Generate** button on the notebook toolbar and the insert toolbar between cells when the setting <code codesetting="notebook.experimental.generate">notebook.experimental.generate</code> is set to `true`. It can also be triggered by pressing `kbstyle(Cmd+I)` on macOS (or `kbstyle(Ctrl+I)` on Windows/Linux) when the focus is on the notebook list or cell container. This feature can help simplify the process of generating code in new cells with the help of the language model.
...
The hex editor now has an "insert" mode in addition to its longstanding "replace" mode. The insert mode allows new bytes to be added within an at the end of files, and it can be toggled using the `kbstyle(Insert)` key or from the status bar.

The hex editor now shows the currently-hovered byte in the status bar.
...
The [window activity API](https://github.com/microsoft/vscode/issues/181569) has been finalized. This API provides a simple additional `WindowState.active` boolean that extensions can use to determine if the window has been interacted with recently.
...
There is [a proposal](https://github.com/microsoft/vscode/issues/209491) for a `preserveFocus` boolean on test run requests triggered by extensions. Prerviously, test runs triggered from extension API never caused the focus to move into the **Test Results** view, requiring some extensions to reinvent the wheel to maintain UX compatibility. This new option can be set on `TestRunRequest`s to ask the editor to move focus as if the run was triggered from in-editor.
