...
The terminal link provider API is now stable. This new API allows extensions to give custom links for terminal lines when it requests them. The following example creates a link for the first instance of the word `"test"` on each line:
...
This API enables language/lint extensions to handle more obscure link formats that are not already supported and even support language-specific behavior. Given the following Java Exception for example:
