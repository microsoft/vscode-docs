...
For questions that are not tied to a folder, you can start a quick chat directly from the Agents window without opening a workspace first. Quick chats appear in the **Chats** section and open focused and ready for input.
...
### Single-pane detail panel for the Agents window (Experimental)

This release refines the Agents window workspace so reviewing agent output takes less layout management.

When `setting(sessions.layout.singlePaneDetailPanel)` is enabled, the editor area and detail panel render as one card with a shared tab bar. You can move between **Changes**, **Files**, and editor content from a single tab strip, resize the docked detail panel, and keep each session's layout state when you switch sessions or reload the window. The setting is experimental, disabled by default, and requires a window reload to take effect.
...
Multimodal support is now generally available in VS Code, starting with this latest release. Attach images by pasting them into Chat, dragging and dropping them, or using the context menu. Images may also be read by the agent via a tool call.
