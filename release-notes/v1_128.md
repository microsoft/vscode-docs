...
* [Multi-chat agent sessions](#multiple-chats-in-a-session-now-supports-claude-harness): Run several related chats in one Claude session to compare approaches and work in parallel.

* [Quick chats](#chat-without-a-selected-workspace-in-the-agents-window): Ask a question in the Agents window without opening a workspace first.
...
<video src="images/1_128/claude-multi-chat-session.mp4" title="Video showing multiple chats running in parallel within a single Claude agent-host session." autoplay loop controls muted></video>

This experience is available when using the Claude agent via the agent host. Ensure the agent host is enabled and can be selected for the Claude agent (via the harness picker) with `setting(chat.agentHost.enabled)` and `setting(chat.agents.claude.preferAgentHost)`.
...
Chats without a workspace are supported only by agent host sessions, so this experience is available when the agent host is enabled (and selected via the harness picker) with `setting(chat.agentHost.enabled)`.
...
This experience is available when using Copilot via the agent host. Ensure the agent host is enabled (and selected via the harness picker) with `setting(chat.agentHost.enabled)`.
...
* Create a chat with `sessions.chatCompositeBar.addChat`. Try this keybinding live while in the Agents window with `kb(sessions.chatCompositeBar.addChat)`.
* Reopen the last closed chat with `sessions.chatCompositeBar.reopenLastClosedChat`. Try it with `kb(sessions.chatCompositeBar.reopenLastClosedChat)`.
* Go to the next or previous chat with `sessions.chatCompositeBar.navigateNextChat` and `sessions.chatCompositeBar.navigatePreviousChat`. Try it with `kb(sessions.chatCompositeBar.navigateNextChat)` and `kb(sessions.chatCompositeBar.navigatePreviousChat)`.
* Quickly switch between open chats with `sessions.quickSwitchNextChat` and `sessions.quickSwitchPreviousChat`. Try it with `kb(sessions.quickSwitchNextChat)` and `kb(sessions.quickSwitchPreviousChat)`.
* Close the active chat tab with `sessions.chatCompositeBar.closeChat`. Try it with `kb(sessions.chatCompositeBar.closeChat)`.
* Delete the active non-main chat with `sessions.chatCompositeBar.deleteChat`. Try it with `kb(sessions.chatCompositeBar.deleteChat)`.
* Open a searchable picker for open and closed chats with `sessions.showChatsPicker`. Try it with `kb(sessions.showChatsPicker)`.
...
This feature is experimental and is still under active development. Ensure the agent host is enabled and can be selected with `setting(chat.agentHost.enabled)`.
...
When you use a bring your own key (BYOK) model as the main agent model, you can change the default behavior used by built-in utility flows, such as generating a chat title or a commit message. Set `setting(chat.byokUtilityModelDefault)` to use the main agent model, use a model provided by GitHub Copilot, or not use a default utility model.
