...
Without async stacks, we can't see what happened that led the program to end up in the `setTimeout` callback:
![Async Frames in Call Stack](images/1_11/async-stack-before.png)

But with the async stacks, we can see all of the calls that led to calling `setTimeout`.
