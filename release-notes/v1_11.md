...
* **[Preview: Workbench theming](#workbench-theming-part-1)** - We're working to make the entire VS Code workbench colors customizable.
...
* **[Panels maximize and restore](#maximize-and=-restore-the-panel-size)** - Quickly open panels (Output, Terminal, Debug Console) to full size.
...
However, if you want to play around with new colors, use the setting `workbench.experimental.colorCustomizations` to customize the currently selected theme.
 ...
 ![color customization](images/1_11/color-settings.png)
...
With better support from the debug adapter protocol, it became possible to provide extensive information about the occurred exception. This information can include the exception id, break mode, description, and additional details (for example, stack trace).

![column breakpoints](images/1_11/exception-widget.png)

## Node Debugging

### Transitioning to the "inspector" protocol

In the last milestone, we've folded the two Node.js debuggers ('node' and 'node2') into a single Node.js debugging experience and introduced a new launch configuration attribute `protocol` to control which protocol to use (for details see the [section in the February release notes](https://code.visualstudio.com/updates/v1_10#_node2-transitioning).
