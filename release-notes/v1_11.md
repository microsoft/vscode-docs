...
The editor's "Peek UI" has been enhanced to show additional information for thrown exceptions, e.g. VS Code's built-in node debugging now shows the stacktrace:

![Exception Peek UI show Stack Trace](images/1_11/exception-peekui.png)

Since this feature requires some adoption effort by debug extensions, do not expect to see additional exception information in other debug extensions immediately.
...
The **Debug: Open Loaded Script** action gives you access to loaded scripts while a debug session is active. In the Quick Pick, you can filter and select the script to open. The script is then loaded into a read-only editor where you can set breakpoints. These breakpoints are remembered across debug sessions but you only have access to the script content while a debug session is running.
...
A stable Source Control API was released this month which lets you create source control extensions integrated within VS Code. You can learn all about it in the [Source Control in VS Code](/docs/extensionAPI/api-scm.md) documentation page.
...
* **ExceptionInfoRequest now used by VS Code**<br>
