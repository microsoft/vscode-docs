...
When debugging async Node.js code, we now show the async frames that led to the current callstack. This is currently only supported when debugging with the [inspector protocol](v1_10.md#node2-transitioning) via `"protocol": "inspector"`. It can be disabled by setting `"showAsyncStacks": true` in the launch config.
