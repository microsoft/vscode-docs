...
VS Code now includes a `objective-cpp` language mode for working with Objective-C++. Prevously, `.mm` files were treated as regular C++. The release also improves syntax coloring for Objective-C++ files.
