...
With this release, VS Code provides a new and simple keyboard shortcuts editing experience using a new **Keyboard Shortcuts** editor. You can now find a command and change / remove / reset its keybinding easily. Most importantly, you can see keybindings according to your keyboard layout. For example, keybinding `cmd+\` in US keyboard layout will be shown as `ctrl+shift+alt+cmd+7` when keyboard layout is changed to German. For more advanced customizations like editing `when` context of a keybinding, you can open the `keybindings.json` file just like before and edit.
