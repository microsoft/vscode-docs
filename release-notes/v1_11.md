...
When debugging async JavaScript in Node.js or Chrome, we now show the async frames that led to the current callstack. This is currently only supported when debugging with the [inspector protocol](v1_10.md#node2-transitioning) via `"protocol": "inspector"`. It can be disabled by setting `"showAsyncStacks": true` in the launch config.
...
* [Manoj Patel (@nojek)](https://github.com/nojvek): Support source mapping of stack traces in the Debug Console (and exception widget) [PR #190](https://github.com/Microsoft/vscode-chrome-debug-core/pull/190)
* [Manoj Patel (@nojek)](https://github.com/nojvek): Nicer error messages when a source map fails to parse [PR #188](https://github.com/Microsoft/vscode-chrome-debug-core/pull/188)
