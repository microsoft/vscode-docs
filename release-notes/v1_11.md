...
* **[Faster search](#text-search-improvements)** - Faster text search and respecting .gitIgnore files.
...
* **[Async callstacks](#async-callstacks)** - See async JavaScript callstacks in the debugger.

The release notes are arranged in the following sections related to VS Code focus areas. Here are some further updates:

* **[Editor](#editor)** - Set default language mode, better file encoding detection.
...
You can now let VS Code guess the encoding of a file automatically via the new `files.autoGuessEncoding` setting which is disabled by default. Once enabled, VS Code attempts to guess the encoding from the file contents and use that information if there is a suitable encoding detected.

In addition, if you select the encoding picker on a file to change its encoding, we will now present you with the guessed encoding (if any) at the top of the list. This makes it very easy to pick the right encoding in the case when the file contents are not displayed correctly.
...
When debugging async JavaScript in Node.js or Chrome, we now show the async frames that led to the current callstack. This is currently only supported when debugging with the [inspector protocol](v1_10.md#node2-transitioning) via `"protocol": "inspector"`. It can be disabled by setting `"showAsyncStacks": false` in the launch config.
