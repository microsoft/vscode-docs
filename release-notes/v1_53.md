...
Users can switch to new themes with `Preferences: Product Icon Theme`
...
A `viewsWelcome` contribution from an extension in which an `enablement` expression is specified will now render command links inactive when the expression evaluates as false. Thanks to [@gjsjohnmurray](https://github.com/gjsjohnmurray) for this contribution.
