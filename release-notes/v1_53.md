...
* **[Apple Silicon builds](#apple-silicon)** - VS Code is now available for Apple Silicon.
...
Emmet now works much faster in HTML and CSS files. For example, the gif below shows a 20x performance improvement on expanding an abbreviation in a large HTML file!

The extension also uses the latest Emmet, meaning that features such as countdowns are now supported.
