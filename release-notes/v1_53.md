...
* **[Customize search mode](#default-search-mode)** - Configure what default search UI commands and Explorer menu entires use for search.
* **[New JS debugger features](#javascript-debugger)** - Support for conditional exception breakpoints and debugging of Node.js `worker_threads`.
* **[PowerShell 7](#better-powershell-7-discovery)** - VS Code will use PowerShell 7 as the default shell if you have it installed.
* **[Notebook UX updates](#notebooks)** - Outline view for Notebook cells, and breadcrumbs for improved navigation.
* **[Performance improvements](#performance-improvements)** - Building VS Code now takes half as much time as before.
* **[Electron 11 and Apple Silicon support](#electron-11-update)** - Electron 11 is now bundled into VS Code, and our first stable Apple Silicon builds are available. 
* **[New Remote Development resources](#documentation)** - Check out the video series to get, create, and configure a container-based development environment.
...
Here's an example of Emmet expanding with a countdown in a large HTML file:
...
- Condition editing can not only initiated via the context menu, but there is now an inline **Edit Condition** action as well.

![condition editing in breakpoint view](images/1_53/breakpoints.gif)

Please note that while the new UI supports editing of all breakpoints types, conditions and hit counts are only respected if a debug extension actually supports them. For now we are not aware of any debuggers supporting conditions and hit counts for function breakpoints, but we expect some support soon. For exception breakpoints, condition support has been added to the built-in JavaScript debugger in this release (please see next section **JavaScript debugger** below).
...
* [Ikko Ashimine (@eltociear)](https://github.com/eltociear): Fix typo in diff.ts [PR #1167](https://github.com/microsoft/vscode-eslint/pull/1167)
