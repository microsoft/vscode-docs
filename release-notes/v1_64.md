...
This milestone, we introduce the Side Panel, a new surface in the workbench to house views from the Side Bar or the bottom Panel appearing opposite the Side Bar. Unlike the historical ability to move the bottom Panel to the left or the right of the editor, the new Side Panel works in addition to the bottom Panel so you can see more sets of views at once.

To use the Side Panel, you'll need to move some views over to it. Based on one of our most upvoted feature requests, you might want to move _Outline_ from the Side Bar to Side Panel. You can do this by dragging and dropping the view into the Side Panel. If the Side Panel is not open, just like the bottom Panel, dragging a view to the edge of the editor area, will pop it open. Alternatively, you can use the `Move View` command for something more keyboard friendly.
...
Please note that with all options the Activity Bar is considered the edge of the window.
...
![Experimental Layout Control](images/1_64/customize_layout.gif)


Please try it out, and let us know if you have any feedback in [this issue](https://github.com/microsoft/vscode/issues/137633).
...
### Explorer: Confirm & Disable Undo

The file explorer has long supported an undo stack to revert file and folder edits (rename, cut/copy/paste, move, import, etc.). Some find this functionality unexpected, so we now provide a setting to disable Undo in the explorer (`explorer.enableUndo`).

Additionally, a new setting (`explorer.confirmUndo`) has been added to configure how frequently the explorer will prompt for confirmation when performing an Undo. By default the explorer will prompt more frequently than before to ensure destructive undoing doesn't accidentally happen, but this can be changed to the old behavior, `"light"`, or set to `"verbose"` to prompt before all undo actions.

![recording of file explorer under different undo configurations](images/1_64/explorer-undo.gif)

## Source Control

### More control on repository discovery

Based on community feedback, we have introduced two new settings that allow users to better control the process of discovering Git repositories to accommodate more diverse folder structures. One can use the `git.repositoryScanMaxDepth` setting to control the depth used when scanning, as well as the `git.repositoryScanIgnoredFolders` setting to control the list of folders that should be ignored during scanning.

### Changes list sort improvements

This milestone we have merged a community contribution that introduces a new setting (`scm.defaultViewSortKey`) to control the default sort key that is used for the changes list in the SCM viewlet. The choice for the sort key is also being remembered for each folder/workspace.

### New Git stash command

A new command, **Git: Drop All Stashes...**, was added to the Command Palette that allows users to clear all stash entries.
...
A new setting `editor.foldingMaximumRegions` allow users to change the maximum number of folding ranges that are shown in an editor. By default the limit is 5000. Note that larger number can result in a reduced performance.

### Audio cues

Audio cues indicate if the current line has certain markers, such as errors, breakpoints, or folded text regions.

They are played when the primary cursor changes its line or the first time a marker is added to the current line.
Audio cues are enabled automatically when a screen reader is attached, but can also be turned on manually by setting `audioCues.enabled` to `on`.

More audio cues are to come and we will iterate on the sound design.

### Unicode highlighting improvements

To improve the user experience of the unicode highlighting feature introduced in the November release, we introduced new settings to better deal with false positives.

When setting `editor.unicodeHighlight.includeStrings` to `false`, no characters in strings are highlighted.

The new setting `editor.unicodeHighlight.allowedLocales` can be used to allow characters that are common in one or many configured locales. By default, this includes the current VS Code display language and the current OS language. At the moment, only locales translated in [vscode-loc](https://github.com/microsoft/vscode-loc) are supported.

### Editor adopts to language feature timings

Extensions power almost all language features, also ambient features like folding regions, code lens, inlay hints, or semantic tokens. These features aren't triggered by user gestures but usually with a timeout after typing. For instance, code lens updates as you type, and so does semantic tokens. The chosen timeout values used to be static and more or less arbitrary. With this release, the editor adapts to the observed timings so that slower machines or network connections aren't overwhelmed with request and faster machines are utilized better.
...
Currently there are limitations of searching text in outputs, for example, we are not able to search on streaming/text output rendered by VS Code itself. We plan to improve this in next iteration.
...
Third, if code cells are visible in the outline or breadcrumbs via the `notebook.outline.showCodeCells` and `notebook.breadcrumbs.showCodeCells` settings, they will get an animated running icon when they are executing.

![executing spinner](images/1_64/cell-executing-spinner.gif)

## Debugging

### Viewing and editing binary data

VS Code's generic debugger now supports viewing and editing binary data (aka "memory"). Variables for which memory viewing and editing is available show an inline binary icon in the **Variables** view, and clicking on it opens the [Hex Editor](https://marketplace.visualstudio.com/items?itemName=ms-vscode.hexeditor):

![](./images/1_64/debug-memory.png)

_Theme: [Codesong](https://marketplace.visualstudio.com/items?itemName=connor4312.codesong)_

Memory viewing and editing relies on the memory-related features of the Debug Adapter Protocol and debug extensions have to implement these features in order to enable the corresponding UI in VS Code. For details please see section "Debugger extension authoring" below.
...
Sometimes you might set a breakpoint or add a debugger statement, but only want to pause there when called from a certain code path. DOM event handlers like `mousemove` are a good example for this. In this release, the JavaScript debugger added the ability to "exclude a caller" from pausing at a breakpoint or debugger statement.
...
![](./images/1_64/js-debug-exclude-caller.gif)

**Edge Support on Linux**

[Microsoft Edge](https://www.microsoft.com/en-us/edge?r=1) debugging is now available on Linux, via the `edge` debug type.
...
![Surrounding selected code with an if statement](images/1_64/markdown-strike-through.png)
...
When typing `=` after a property in HTML, the language service now automatically inserts quotes and places the cursor between the quotes. The setting `html.completion.attributeDefaultValue` lets you specify what type of quotes (single or double).
...
VS Code for the Web has adopted the PWA model and can now be proposed to be installed as such in the host OS. Thanks to that adoption, it was also possible to enable some offline functionality. Having once visited [vscode.dev](https://vscode.dev) or [insiders.vscode.dev](https://insiders.vscode.dev), users can now use it to edit local files even while offline.

![Azure Repos picker](./images/1_64/pwa.png)
...
VS Code for the Web now bundles pre-release versions of both GHPR and RemoteHub extensions.

## Contributions to extensions

### Hex Editor

The [Hex Editor](https://marketplace.visualstudio.com/items?itemName=ms-vscode.hexeditor) has undergone some significant architectural refactors. Notably, in most cases it's now able to open files of any size without performance issues. Additionally, its layout width is now configurable, and it has a more capable find/replace implementation. Continued improvements will be forthcoming in future iterations.

### Jupyter

**Remote Jupyter Servers**
Reloading VS Code is no longer necessary when switching between local and remote Jupyter servers. In addition, we now display both local and remote kernels together in the kernel picker.
...
To improve our support for Anaconda users we have implemented support for `conda run`, using the `-no-capture-output` parameter which does not capture output and has caused errors in the past. This feature is implemented for executing the linter, testing, and formatting functions.

There is a known bug at [#10972](https://github.com/conda/conda/issues/10972) that causes an error on Windows when running linting and testing with a special character in the parameters, this is due to how `conda run` reads parameters with special characters, so it can affect any command line arguments you have specified in your config.json.

Anaconda is already working on the issue and hope to have a fix out soon. You can update your version of anaconda [here](https://docs.conda.io/) once the new version is released.
...
The Explorer has experimental new support for nesting together files in the same directory. This can be helpful for visually grouping together related files and collapsing files into a "root" file to reduce clutter. Several new settings have been added to control this behavior:
...
For example, in many repositories `.ts` files sit next to their derived `.js`, `.d.ts`, and/or `.js.map` files. While hiding derived files entirely has long been possible via `files.exclude`, it can be nice to have a quick way of accessing derived files, which is now possible with nests. By default, a folder may appear collapsed (based on the `explorer.experimental.fileNesting.expand` setting):
![explorer with collapsed nests](images/1_64/nesting-collapsed.png)

But its entries can then be expanded to show derived files:
![explorer with expanded nests](images/1_64/nesting-collapsed.png)
...
- A new `API.add` method has been added, to enable the ability to stage files.
- The `API.tag` and `API.deleteTag` methods were added to enable the ability to create and delete tags.
...
We're proposing an official way to implement a "refresh" handler for tests, check out the [vscode.proposed.testRefresh.d.ts](https://github.com/microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.testRefresh.d.ts). This will allow extensions to unify behind a standard "refresh" button in VS Code and avoid implementing their own.

Additionally, we propose that `TestItem` have a `sortText` attribute in [vscode.proposed.testSortText.d.ts](https://github.com/microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.testSortText.d.ts).
...
The tree drag and drop proposal is almost ready to begin finalization now supports dragging from a tree view into an editor when the `"resourceurls"` mime type is supported by the `TreeDragAndDropController`. Details are available in https://github.com/microsoft/vscode/issues/141418#issuecomment-1021184968. Please try it out and provide feedback for [vscode.proposed.treeViewDragAndDrop.d.ts](https://github.com/Microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.treeViewDragAndDrop.d.ts).

### Comment timestamp

The commenting API has a new `timestamp` property on `Comment`. Along with the API is a new setting to control how comment timestamps are displayed: `comments.useRelativeTime`. Please try it out and provide feedback for [vscode.proposed.commentTimestamp.d.ts](https://github.com/Microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.commentTimestamp.d.ts).
