...
![folded cell hint](./images/1_64/cell-fold-hint.png)
...
Path IntelliSense can also help you link to headers within the current file or within another markdown file. Simply start the path with `#` to see completions for all the headers in the file (depending on your settings, you may need to use `kb(editor.action.triggerSuggest)` to see these):
...
We now support rendering `~~strikethrough~~` text in both the markdown editor and in the built-in markdown preview:
...
To use surround with, first select some text and then run `Insert Snippet`. Instead of replacing the selected text, snippets such as `if` or `forof` will now use the the selected text as the body of the new statement.
