...
VS Code now include UI for manipulating memory in sessions (via the `readMemory` and `writeMemory` requests in the [Debug Adapter Protocol](https://microsoft.github.io/debug-adapter-protocol/specification#Requests_ReadMemory).) Variables for which memory viewing is available have an inline binary icon in the **Variables** view, and clicking on it opens the Hex Editor.

![](./images/1_64/debug-memory.png)

_Theme: [Codesong](https://marketplace.visualstudio.com/items?itemName=connor4312.codesong)_

The built-in JavaScript debugger supports memory editing of binary types in this release.

### JavaScript debugging

**Exclude Caller**

Sometimes you might set a breakpoint or add debugger statement, but only want to pause there when called from a certain code path. DOM event handlers like `mousemove` are a common scenario. In this release, the JavaScript debugger added the ability to "exclude a caller" from stopping at the current location.

When paused at a location, you can right-click on a frame in the **Call Stack** view and choose the **Exclude Caller** option. A tree view will be shown to allow you to view and manage excluded callers, and afterwards your program won't pause at the current location if the selected caller is anywhere in its call stack.

![](./images/1_64/js-debug-exclude-caller.gif)

**Edge Support on Linux**

[Microsoft Edge](https://www.microsoft.com/en-us/edge?r=1) debugging is now available on Linux, via the `edge` launch type.
