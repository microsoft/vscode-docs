...
* **[New Side Panel](#new-side-panel)** - See move views at once when placed in the Side Panel.
* **[Settings editor search](#settings-editor)** - Search now prioritizes whole word matches.
* **[Audio cues](#audio-cues)** - Hear when the cursor runs to folded regions, errors, and breakpoints.
...
* **[VS Code for the Web](#remote-repositories)** - Better GitHub support with signed commits in vscode.dev.

>If you'd like to read these release notes online, go to [Updates](https://code.visualstudio.com/updates) on [code.visualstudio.com](https://code.visualstudio.com).

**Join us live** at the [VS Code team's livestream](https://code.visualstudio.com/livestream) on Thursday, February 3 at 8am Pacific (4pm London) to see a demo of what's new in this release, and ask us questions live.
...
With all these new layout options, we have been exploring ways to better expose layout configuration for discoverability and ease of use. That's why you might notice a new experimental layout control in your title bar. This only appears if your `window.titleBarStyle` setting is `custom` and `workbench.experimental.layoutControl.enabled` is set to `true`. The enablement is controlled by an experiment, but if you want to enable it manually, you can do so in the Settings editor.
...
The File Explorer has long supported an Undo stack to revert file and folder edits (rename, cut/copy/paste, move, import, etc.). Some find this functionality unexpected, so there is now a setting to disable Undo in the Explorer (`explorer.enableUndo`).
...
Extensions power almost all language features, also ambient features like folding regions, CodeLens, inlay hints, or semantic tokens. These features aren't triggered by user gestures but usually with a timeout after typing. For instance, CodeLens updates as you type, and so does semantic tokens. The chosen timeout values used to be static and more or less arbitrary. With this release, the editor adapts to the observed timings so that slower machines or network connections aren't overwhelmed with request and faster machines are utilized better.
...
Memory viewing and editing relies on the memory-related features of the Debug Adapter Protocol and debug extensions have to implement these features in order to enable the corresponding UI in VS Code. For details please see section "Debugger extension authoring" below. In this release only the built-in JavaScript debugger supports viewing and editing of binary data, but we expect other debug extensions to follow soon.
...
To improve our support for [Anaconda](https://www.anaconda.com) users, we have implemented support for `conda run`, using the `--no-capture-output` parameter, which does not capture output and has caused errors in the past. This feature is implemented for executing the linter, testing, and formatting functions.
...
Anaconda is already working on the issue and hope to have a fix out soon. You can update your version of Anaconda on the [Conda website](https://docs.conda.io), once the new version is released.
...
The tree drag and drop proposal is almost ready to begin finalization now supports dragging from a tree view into an editor when the `"resourceurls"` mime type is supported by the `TreeDragAndDropController`. Details are available in [issue #141418](https://github.com/microsoft/vscode/issues/141418#issuecomment-1021184968). Please try it out and provide feedback for [vscode.proposed.treeViewDragAndDrop.d.ts](https://github.com/Microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.treeViewDragAndDrop.d.ts).
...
The scripts to run Code Web & Server from sources moved to the `scripts` folder
* `./scripts/code-web.sh|bat` starts Code for the Web (a.k.a serverless) from sources and opens a browser on it. Use `--help` for more options.
* `./scripts/code-server.sh|bat` starts a code server from sources. Add `--launch` to additionally open the web UI in a browser. Use `--help` for more options.
