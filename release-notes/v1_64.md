...
Based on community feedback, we have introduced two new settings that allows users to better control the process of discovering Git repositories to accomodate more diverse folder structures. One can use the `git.repositoryScanMaxDepth` setting to control the depth used when scanning, as well as the `git.repositoryScanIgnoredFolders` setting to control the list of folders that should be ignore during scanning.
...
A new command, `Git: Drop All Stashes...`, was added to the command pallette that allows users to clear all stash entries.
...
Memory viewing and editing relies on the `readMemory` and `writeMemory` requests in the [Debug Adapter Protocol](https://microsoft.github.io/debug-adapter-protocol/specification#Requests_ReadMemory) and debug extensions have to opt into this feature in order to enable it in VS Code.
In this release only the built-in JavaScript debugger supports editing of binary data, but we expect other debug extensions to follow soon.
...
When typing `=` after a property in HTML, the language service now automatically inserts quotes and places the currsor between the quotes. The setting `html.completion.attributeDefaultValue` lets you specify what type of quotes (single or double).

The feature can be disable by `"html.autoCreateQuotes": false`
...
```
The icon is shown if the current file icon themes only has a generic file icon for the language.

File icon themes like `Minimal` or `None` that do not show file icons also will not use the language icon. Also, if a file icon theme has a icon for an extension or file name, these will be preferred.
...
### VS Code now supports the memory related features of the Debug Adapter Protocol
...
The tree drag and drop proposal is almost ready to begin finalization now supports dragging from a tree view into an editor when the `"resourceurls"` mime type is supported by the `TreeDragAndDropController`. Details are availabe in https://github.com/microsoft/vscode/issues/141418#issuecomment-1021184968. Please try it out and provide feedback for [vscode.proposed.treeViewDragAndDrop.d.ts](https://github.com/Microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.treeViewDragAndDrop.d.ts).
