...
When searching in the terminal, the result count and currently selected match index are now displayed in the find widget.

![A term is searched for in the terminal and the find widget is labeled match 12 of 14 to indicate that match 12 is currently selected](images/1_67/terminal-find-count.png)

### Improved `workbench.action.quickOpenTerm`

To aid in terminal selection, the quick open terminal quickpick now features `cwd`.

![The terminal quickpick lists terminals and their cwd](images/1_67/terminal-quick-pick.png)
