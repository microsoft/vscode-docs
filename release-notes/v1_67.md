...
### Windows Group Policy: `update.mode`

The `update.mode` setting can now be overwritten via Windows Group Policy. Inside the Windows ZIP release, there is now a `policies` folder containing the necessary ADMX/ADML files for GPO configuration.
...
![Old behavior](./images/1_67/unbalanced-brackets-shell-old.png)
...
![New behavior](./images/1_67/unbalanced-brackets-shell-new.png)
...
Old bracket matching behavior (notice that the colors and the highlight decoration does not match):
...
Users that have the `git.untrackedChanges` setting set to `hidden` will experience better performance when working with large repositories as in this scenario we are now passing the `-uno` parameter when calling `git status`.
...
A new setting `github.gitProtocol` lets you change the default protocol when cloning a repository from GitHub.
...
![Dropping a image from a web browser into VS Code to add a markdown image](images/1_67/md-drop-link.gif)
...
We now have a [new extension](https://marketplace.visualstudio.com/items?itemName=ms-python.black-formatter) that provides formatting support for python using [black](https://pypi.org/project/black/) formatter. Pre-release version of the extension is available to install.

**Import organization with isort**

We also have a [new extension for isort](https://marketplace.visualstudio.com/items?itemName=ms-python.isort), which provides import organization using the [isort](https://pypi.org/project/isort/) library. Pre-release version of the extension is available to install.
