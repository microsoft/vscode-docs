...
To get started, place your cursor either on the header itself or on the fragment part of a link to the header (e.g. `/link/to/file` in `[link text](/path/to/file#some-header)`) and run `Find all references`.

<!-- TODO: mjbvz add image -->

This will show all references in the references view. If you prefer a different view, uou can also use the `Peek References` or `Go to References` commands.

### Markdown: Find all references to reference links

We also support finding all references to [reference links](https://spec.commonmark.org/0.30/#reference-link) with in the current file. This is supported for:
...
Ever change a header in a markdown file only to realize you also accidentally broke all links to it? That's where our new rename support for markdown can help.
...
You can trigger renames on the header itself or on the header component of a link to the header (e.g. `some-header` in `[link text](/path/to/file#some-header)`)
