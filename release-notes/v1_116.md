...
Also fixed an issue where rapid bursts of tool invocation updates would cause brief hangs in the extension host.

* **Chat send performance**: Fixed an issue where chat message sending would be blocked by loading chat customizations. The message should now visually appear in the chat conversation immediately even if prompts are still loading.
...
* Added a chat tool for creating pull requests.
