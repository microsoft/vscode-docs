...
Welcome to the 1.116 release of Visual Studio Code. This release continues to make working with chat and agents more powerful and seamless. Here are some highlights of what's new:

* [Agent Debug Logs](#view-historical-sessions-in-agent-debug-logs-panel): view logs from previous agent sessions to understand and debug agent interactions.
...
* [Terminal agent tools](#terminal-agent-tools): interact smoothly with any terminal session from your agent sessions.
...
<div class="toc-nav-layout">
  <nav id="toc-nav">
    ...
    <ul>
      ...
      <li><a href="#terminal-agent-tools">Terminal agent tools</a></li>
...
You can now view the log for the current session as well as previous sessions, with logs persisted locally on disk. This allows you to review and debug past agent interactions even after the session has ended.

![Screenshot showing the Agent Debug Logs panel with a list of historical session logs.](images/1_116/agent-debug-logs.png)

We have merged the setting to enable the Agent Debug Logs panel into the troubleshooting setting `setting(github.copilot.chat.agentDebugLog.fileLogging.enabled)`.
...
We have made several improvements to the experience for terminal input in agent sessions:

* **Detect terminal input**: We removed the LLM-based prompt-for-input detection. Previously, every terminal output chunk triggered an extra LLM call to classify whether the terminal was waiting for input, which added latency and used extra tokens. The agent now handles terminal input directly via `send_to_terminal` and uses the question carousel to defer to you when needed.
...
In this release, we enabled background terminal notifications by default. This means that when the agent runs a command in a background terminal, it will automatically receive notifications when the command completes, times out, or requires input. This allows the agent to respond more quickly and accurately without needing to poll for terminal output.
...
The new builtin JS/TS Chat Features extension enhances Copilot's skills to work with TypeScript and JavaScript. For this first release, the extension contributes skills for setting up modern TypeScript projects. We plan to enhance and expand its functionality in future release.
...
GitHub Copilot is now a built-in extension in VS Code. New users no longer need to install any extension to start using Copilot features such as chat, inline suggestions, and agents. Copilot is available out of the box as part of the standard VS Code installation.
