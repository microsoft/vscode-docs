...
If you have settings that allow users to customise some executable paths and they require these paths to exist on the machine they are running in. You can now calssify such settings as `machine` scoped when you contribute to `configuration` extension point. Machine specific configuration can be configured only in the User settings.
...
- selection ranges: to compute selection ranges for an array of positions. Send from client to server.
- call hierarchy: to compute a call hierarchy for a given symbol. Send from client to server.
- progress: to initiate progress reporting from the server. Send from server to client.

The new features are available in next versions of the `vscode-languageclient` and `vscode-languageserver` npm modules.
