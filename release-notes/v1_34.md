...
### Lower rank for CSS property value with vendor prefix in auto completion

CSS property values prefixed with `-` (such as `-moz-grid` or `-ms-grid`) now come after `grid` in auto completion.
