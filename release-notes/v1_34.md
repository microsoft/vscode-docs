...
Welcome to the April 2019 release of Visual Studio Code. During April, we were busy with the Preview release of the [Remote Development](https://code.visualstudio.com/docs/remote/remote-overview) extensions. Three new extensions let you work with VS Code over SSH into a remote machine or VM, in Windows Subsystem for Linux (WSL), or inside a Docker container.

Read the [Remote Development with Visual Studio Code](https://code.visualstudio.com/blogs/2019/05/02/remote-development) blog post to learn more.

![Remote Development extensions](images/1_34/remote-extensions.png)
...
The editor now optimistically caches CodeLens locations and restores them immediately after switching editors. This fixes an issue where CodeLens lines would shift slightly when switching between editors.
...
The **Tasks: Terminate Task** command has a new option to terminal all tasks if there are multiple tasks running. If this is something you do often, you can create a keyboard shortcut for the command:
...
The extensions in the [Remote Development extension pack](https://aka.ms/vscode-remote/download/extension) run in the context of the remote workspace located either in a container, in WSL, or on a remote machine while VS Code feels like it does when you run locally.
...
The [Vetur](https://github.com/vuejs/vetur) extension now offers diagnostics, hover information, jump to definition and find references for the JavaScript expression inside Vue's template interpolation region:

![vetur-interpolation](images/1_34/vetur-interpolation.gif)
...
VS Code Remote Development is still in preview (and requires the [Insiders](https://code.visualstudio.com/insiders) build) but there is extensive [documentation](https://code.visualstudio.com/docs/remote/remote-overview) to help you get started.
...
* [Remote - Containers](https://code.visualstudio.com/docs/remote/containers.md) - Work with a sandboxed toolchain or container-based application inside a container.
...
There is also a helpful [Tips and Tricks](https://code.visualstudio.com/docs/remote/troubleshooting) guide as well as [FAQ](https://code.visualstudio.com/docs/remote/faq).

### Python Azure Functions

There is a new [Deploy Python to Azure Functions](https://code.visualstudio.com/docs/python/tutorial-azure-functions) tutorial that describes how to create and deploy a Python serverless Azure Function.
