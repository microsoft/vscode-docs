...
VS Code now allow users to sponsor their favourite extensions. When an extension can be sponsored, VS Code will render a **Sponsor** button in the extension details page like below:

![sponsor extension](images/1_68/sponsor-extension.png)

> VS Code theme: [GitHub Dark Dimmed Theme](https://marketplace.visualstudio.com/items?itemName=GitHub.github-vscode-theme)

Please refer to [Extension Sponshorship](#extension-sponsorship) to learn how to opt into this feature for your extension.
...
We're actively working on improving this feature so give it a try in your codebase and [please share your feedback](https://github.com/microsoft/TypeScript/issues/49003).
...
While working with Markdown, it's easy to mistakenly add an invalid file link or image reference. Perhaps you forgot that the file name used a `-` instead of an `_`, or perhaps the file you are linking to has since been moved to a different directory. Often you only catch these mistakes after viewing the markdown the the markdown preview or even after publishing it. VS Code's new experimental markdown link validation can help catch these mistakes.
...
We have moved away from pushing changes directly to the `main` branch and are now pushing all changes to VS Code exclusively using Pull Requests. We require that each PR gets at least one approval from another team member. Taking advantage of this, we now also require that some basic checks pass before a PR can be merged. These are tasks like TypeScript compilation, formatting rules, unit tests and integration tests, which typically don't take longer than 10 minutes. Switching to this flow has allowed us to reduce the number of times our Insiders build was broken due to a programming mistake.
...
Extension authors for enabling extensions that run code as [web extensions](https://code.visualstudio.com/api/extension-guides/web-extensions) (the list below is between May 2 2022 and June 6 2022):
