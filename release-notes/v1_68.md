...
In this milestone, we have added support for deprecated extensions in VS Code. An extension can be just deprecated or deprecated in favor of another extension or when its functionality is built into VS Code. VS Code will render extensions as deprecated in the UI, as shown below.
...
Refer to [Extension sponsorship](#extension-sponsorship) to learn how to opt into this feature for your extension.
...
Since last release find in the terminal would show a border around all matches, this was a temporary solution until support for changing the background of cells dynamically was possible. Background is now the default for themes and the overall experience should look similar to the editor now.
...
You can read all about dev containers and the specification at [https://containers.dev/](https://containers.dev/).
...
[Shell integration](https://code.visualstudio.com/docs/editor/integrated-terminal#_shell-integration) (enabled with the `terminal.integrated.shellIntegration.enabled` setting) and command decorations have been polished and improved upon this iteration.
...
We have started to work on a better merge experience. It is still super early and we aren't yet ready for feedback but you can give it a try via `git.experimental.mergeEditor`. With this enabled, files under conflict open in a new merge editor to make resolving conflicts simpler.

We will continue to work on this. Use [Insiders](https://code.visualstudio.com/insiders/) to follow our progress.

We would like to sincerely thank [Mingpan](https://github.com/Mingpan) and our friends at Google that continously help us with this effort ❤️
...
If an extension opts-in to this, VS Code will render a **Sponsor** button in the extension details page as shown [here](#sponsoring-extensions).

**Note:** Make sure to use the latest vsce `>=2.9.1` to publish your extension with sponsorship enabled.
...
* [@Mingpan Mingpan](https://github.com/Mingpan): [Unpolished prototype] 3 way merge for Git [PR #150388](https://github.com/microsoft/vscode/pull/150388)
