...
* A `How does this work` action in the command context menu enables easy access to the [shell integration docs](https://code.visualstudio.com/docs/editor/integrated-terminal#_shell-integration)![](images/1_68/shell-integration-how.png)
* Works with `bash-git-prompt` and other programs that rely upon shell status ![Bash git prompt is successfully used when shell integration is enabled](images/1_68/shell-integration-git-prompt.png)
* Allows the use of a custom `ZDOTDIR` for `zsh`
