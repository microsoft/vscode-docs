...
We have moved away from pushing changes directly to the `main` branch and are now pushing all changes to VS Code exclusively using PRs. We require that each PR gets at least one approval from another team member. Taking advantage of this, we now also require that some basic checks (like TypeScript compilation, formatting rules, unit tests and integration tests), which typically don't take longer than 10 minutes, pass before a PR can be merged. Switching to this flow has allowed us to reduce the number of times our Insiders build was broken due to a programming mistake.
