...
VS Code will not automatically migrate or uninstall a deprecated extensions instead it will guide users by providing UI like below
...
In this milestone, we have introduced a `sponsor` field in the extension's `package.json` to allow extensions to opt into sponsorship.
...
**Note:** Please make sure to use latest vsce `>=2.9.1` to publish your extension with sponsorship enabled.
