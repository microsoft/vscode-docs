...
- Nerd font symbols should now apply minimum contrast ratio to blend in with nearby text while powerline symbols and box drawing characters will _not_ apply minimum contrast ratio as they are often adjacent to inverted cells without any foreground characters.
...
When you haven't set up a launch.json file and press F5 or click the "Run and Debug" button in the debug viewlet, then we pick a debugger based on the language in the currently active file. If you don't have a file open, then we have to ask you which debugger you want to use. This experience can be a little confusing, so we've made a couple improvements.
...
The Chrome/Edge debuggers have been renamed to Web App (Chrome) and Web App (Edge) to try to avoid confusion with other debuggers like Flutter's which also run apps in a browser.
...
For users who set their browser to one of our core supported languages, https://vscode.dev will automatically apply translations in that language. The languages we support is documented in the [vscode-loc](https://github.com/microsoft/vscode-loc) repository.
...
Once that is set, I can go to https://vscode.dev (or https://insiders.vscode.dev) and see that it is in German:
