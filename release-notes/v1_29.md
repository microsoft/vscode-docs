...
* **[Preview: List All References](#references-view)** - List All References view shows reference search results.
...
There is a new command **List All References**, which displays a dedicated Side Bar view showing reference search results. This complements the existing Peek view presentation and provides a more stable list of references. In the context menu, select **List All References** (`kb(references-view.find)`) and references are shown in a separate view, just like search results.

![References View](images/1_29/references-view.gif)

Results are stable and individual results can be cleared, which is great if you use the view as a To Do list. You can use `kb(references-view.showNextReference)` and `kb(references-view.showPrevReference)` to navigate through results without taking your hands off the keyboard.
