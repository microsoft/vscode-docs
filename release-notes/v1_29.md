...
Breakpoint widget can now expand to multiple lines, use `Shift+Enter` to insert an additional line, or simply paste a multiline condition or log message. This should help when editing more complex conditions and log messages.
...
When debugging Node (or Chrome using the "Debugger for Chrome" extension), you can now style messages from `console.log` using `%c`, similarly to what Chrome Devtools supports. Supported properties are `color`, `background`, `font-weight: bold`, and `text-decoration: underline`.
...
### Graceful handling of unknown properties in CSS
...
If VS Code is unable to download a JSON schema used for IntelliSense and error-checking JSON files, a warning will be shown in the Status Bar:
...
### TypeScript TS Lint plugin

The new [TypeScript TSLint plugin](https://github.com/Microsoft/typescript-tslint-plugin) enables TS Lint as a TypeScript language service plugin. These plugins work in any editor that uses the TypeScript language service including VS Code, Visual Studio, Atom, and Sublime.

We've also shipped [an extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-tslint-plugin) that bundles the TypeScript TS Lint plugin so that you don't have to configure the plugin in your workspace.

## Extension Authoring

### Normalized extension samples

The samples at [vscode-extension-samples](https://github.com/Microsoft/vscode-extension-samples) have been updated for consistency.
...
* Listing of the `vscode` API or Contribution Points used in each sample.

### Develop extension without publisher

In an effort to simplify the extension development experience, the `publisher` field was removed from the [Yeoman Extension Generator](https://code.visualstudio.com/docs/extensions/yocode) template. For extensions without a `publisher` field, VS Code now loads them with `undefined_publisher` as their publisher. However, the `vsce` publishing tool will warn you if you are missing the publisher when publishing extensions.
...
Below are some of the key features that are now supported when you use the new [Go modules](https://github.com/golang/go/wiki/Modules) in Go 1.11
...
- The auto completion widget now shows documentation for the suggested symbols. Press `kb(editor.action.triggerSuggest)` to expand/collapse the docs part of the auto-completion widget.
...
- Context menu with `Copy Value`, `Copy as expression` and `Add to Watch` options are now available in the Variables pane of the debug viewlet.
...
* [Maksym Dogadailo (@mdogadailo)](https://github.com/mdogadailo): fix: items in split view are not draggable without separator-border [PR #59830](https://github.com/Microsoft/vscode/pull/59830)
