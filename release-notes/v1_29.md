...
There is a new command and viewlet to see reference search results. This complements the existing peek view presentation and provides a more stable work list of references. In the context menu, select "List All References" (`kb(references-view.find)`) and see how references are shown in a separate viewlet, just like search results.

![References View](images/1_29/references-view.gif)

Results are stable and individual results can be cleared which is great if you use the view as todo list. You can use `kb(references-view.showNextReference)` and `kb(references-view.showPrevReference)` to navigate through results without taking your hands off the keyboard. The viewlet will hide again when results are cleared.
...
In order to preserve horizontal space and reduce clutter we have removed the toggle panel location from the panel title area. This action is now available in the panel title area context menu and also in View > Appearance main menu.
Another reason for removing this item from such a prominent location is that we believe users trigger this action once based on their layout and than never trigger it again.
...
The VS Code tree widget control is used in many places but has been hitting some severe performance constraints on some specific use cases (Problems panel, Outline view). There has been some effort to create faster virtual tree widget implementations which can accomodate different levels of performance versus API convenience, read [here](https://github.com/Microsoft/vscode/wiki/Lists-And-Trees) to learn more.

The new `ObjectTree` widget is now being used in the Problems panel and we've already seen quite some improvements. We were able to dramatically raise the number of problem items the workbench can handle as well as increase the speed of certain operations (such as **Collapse All**) and advanced filtering scenarios. These improvements now make it feasible to work with larger collection sizes.

### Improved workbench navigation
...
- `workbench.action.nextPanelView`: Go to next View in Panel.
- `workbench.action.previousPanelView`: Go to previous View in Panel.
...
The `view` command now shows each group's parent group to better represent the view hierarchy. For example, `Explorer` group now show as `Side Bar / Explorer`, and `Terminal` group as `Panel / Terminal`.
...
Sometimes call stacks in the debugger contain a lot of stack frames which are not interesting to the user (external code, skipped files etc...). To reduce clutter and save on vertical space we now render those uninteresting stack frames as collapsed.
...
When debugging multiple debug sessions we now show their output in different debug consoles. This makes it easier to distuingish which debug session is responsible for which output and evaluation.

![multi repl](images/1_29/multi-repl.png)


### Breakpoint widget expands to multiple lines
Breakpoint widget can now expand to multiple lines, use `shift+enter` to insert an additonal line, or simply paste a multi line condition or log message. This should help when editting more complex conditions and log messages.

![breakpoint](images/1_29/breakpoint-widget.png)


###  Start debugging with stop on entry
For node debugging we have added a new command Debug: Start Debugging and Stop On Entry (`extension.node-debug.startWithStopOnEntry`). Use this command if you would like to start debugging and immediatly stop on the entry of your program. The default keybinding is `kb(extension.node-debug.startWithStopOnEntry)`. Note that is command is only available if your selected launch configuration is of type `node`.
...
This release dishes up TypeScript 3.1.4, a small update that [fixes a few important bugs](https://github.com/Microsoft/TypeScript/milestone/81?closed=1).

### Markdown snippet improvements

The built-in markdown snippets, such as those for links and code blocks, now use the currently selected selected text by default. This allows you to []create keybindings](https://code.visualstudio.com/docs/getstarted/keybindings) that turn the currently selected text into a link or header:
...
The CSS hover helps you winning the Specificity War.

![css-specificity](images/1_29/css-specificity.png)

### Graceful handling of unkonwn properties in CSS

With CSS pre-processors such as PostCSS, you might use invalid properties that are compiled to valid properties during development time. The `[css/less/scss].lint.validProperties` setting prevents VS Code from marking those invalid properties as errorneous.

### Improved CSS zero unit warning

CSS now lints zero units used in CSS shorthand property values. For example,
...
Thanks to a [Pull Request](https://github.com/Microsoft/vscode/pull/60219) from [@xxyy](https://github.com/xxyy), now if VS Code is unable to download a JSON schema used for IntelliSense and error-checking JSON files, a warning will be shown in the Status Bar:
...
Enabling strict null checks is a major engineering effort but we think the payoff in terms of code and product quality will be worth it. You can track our strict null checking progress [here](https://github.com/Microsoft/vscode/issues/60565).
...
The samples at [vscode-extension-samples](https://github.com/Microsoft/vscode-extension-samples) used to suffer from different code format, insufficient documentation and redundant files. Such differences make it hard for users to consume the samples. In this milestone, we have normalized a majority of the samples. Each normalized sample includes:

- Code of the same coding style and structure.
- README that explains the sample's functionality with gif.
- Listing of vscode API or Contribution Points used in each sample.

### Develop extension without publisher

In an effort to simplify the extension development experience, the `publisher` field is removed from the [Yeoman Extension Generator](https://code.visualstudio.com/docs/extensions/yocode) template. For extensions without a `publisher` field, VS Code now loads them with `undefined_publisher` as their publisher. However, `vsce` will warn you for missing publisher when publishing extensions.
...
We've updated our documentation on [how to setup a continuous integration solution for your extension](/docs/extensions/testing-extensions#_continuous-integration) using Azure Pipelines. The new guide provides a configuration with which you can run your extension tests simultaneously on Windows, macOS and Linux.
...
The content includes: what action caused signature help to be triggered, the trigger character (if any), and if this was a re-trigger (signature help was already showing):
...
* [Brendon Crawford (@brendoncrawford)](https://github.com/brendoncrawford): Fixes tests error with faulty OS check when OS is Fedora [PR #60917](https://github.com/Microsoft/vscode/pull/60917)
...
* [Gary Mathews (@garymathews)](https://github.com/garymathews): Fix #27393: Fix explorer inputbox alignment [PR #60539](https://github.com/Microsoft/vscode/pull/60539)
* [Guillaume Poussel (@gpoussel)](https://github.com/gpoussel): Fix #62207 ('null' tooltips on Quick Open view) [PR #62268](https://github.com/Microsoft/vscode/pull/62268)
...
* [Matt Q (@irrationalRock)](https://github.com/irrationalRock): Fixed #59395 - Emmet Syntax Profiles tag_nl produces no extra space [PR #60108](https://github.com/Microsoft/vscode/pull/60108)
* [John Graham (@J-Graham)](https://github.com/J-Graham): Fixes for 58176: Middle clicking files in breadcrumbs explorer opens … [PR #59797](https://github.com/Microsoft/vscode/pull/59797)
* [Jason Bright (@jgbright)](https://github.com/jgbright): `NOT MERGED - PLS REVIEW` Open Workspace button in .code-workspace files, fixes #59305 [PR #59586](https://github.com/Microsoft/vscode/pull/59586)
* [karan verma (@karanisverma)](https://github.com/karanisverma): Fix(#61582): enablePreview description change [PR #61603](https://github.com/Microsoft/vscode/pull/61603)
...
* [Samuel Attard (@MarshallOfSound)](https://github.com/MarshallOfSound): feat: enable mojave dark mode support [PR #59742](https://github.com/Microsoft/vscode/pull/59742)
* [Maksym Dogadailo (@mdogadailo)](https://github.com/mdogadailo): `NOT MERGED - PLS REVIEW` fix: items in split view are not draggable without separator-border [PR #59830](https://github.com/Microsoft/vscode/pull/59830)
...
* [mnikn (@mnikn)](https://github.com/mnikn): Add accept all incoming or current changes in popup-menu. Fixes: #53133  [PR #53649](https://github.com/Microsoft/vscode/pull/53649)
* [Matthew Knox (@mrkno)](https://github.com/mrkno): Fix #59323 Problems view filters have massive delay [PR #60080](https://github.com/Microsoft/vscode/pull/60080)
...
* [Karthikayan (@ParkourKarthik)](https://github.com/ParkourKarthik): include priority in requestItem and add to request queue accordingly [PR #60582](https://github.com/Microsoft/vscode/pull/60582)
...
* [Roy Tang (@roytang)](https://github.com/roytang): Fixes Microsoft/vscode#15043 Format selection keyboard shortcut will … [PR #61206](https://github.com/Microsoft/vscode/pull/61206)
...
* [Prabhanjan S Koushik (@skprabhanjan)](https://github.com/skprabhanjan)
  * Fixes #60724 - Terminal:SelectBox: Terminal select box should have main ARIA label [PR #61466](https://github.com/Microsoft/vscode/pull/61466)
  * Fix #59364 : Respect global .gitignore when searching [PR #59717](https://github.com/Microsoft/vscode/pull/59717)
...
* [Jeremy Shore (@w9jds)](https://github.com/w9jds)
  * Resolves #57507 - Allows clicking of Modified In labels in setting editor view [PR #60045](https://github.com/Microsoft/vscode/pull/60045)
  * Fix #59774 - Handle accessibility events on "Open Settings" link in searchview [PR #60058](https://github.com/Microsoft/vscode/pull/60058)
