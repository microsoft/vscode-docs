...
* **[File icons in IntelliSense](#file-and-folder-icons-in-intellisense)** - Quickly distinquish file types in IntelliSense.
* **[Easier formatting](#format-selection)** - Format Selection on current line if no editor selection.
...
* **[Workbench](#workbench)** - Platform specific end-of-line, highlight modified file tabs, workbench navigation commands.
* **[Integrated Terminal](#integrated-terminal)** - Split terminal can inherit working directory, Cmd+Backspace to delete to start of line.
* **[Languages](#languages)** - Markdown snippet improvements, CSS Specificity on hover, handle unknown CSS properties.
* **[Debugging](#debugging)** - Start with stop on entry, multiple debug consoles, style console.log messages.
...
It is also now possible to use [backreferences](https://www.regular-expressions.info/backref.html) and [lookahead](https://www.regular-expressions.info/lookaround.html) assertions in regex searches, by setting `"search.usePCRE2": true`. This configures [ripgrep](https://github.com/BurntSushi/ripgrep) to use the PCRE2 regex engine. While PCRE2 supports many other features, we only support regex expressions that are still valid in JavaScript, because open editors are still searched using the editor's JavaScript-based search.
...
There are new VS Code debugging [recipes](https://github.com/Microsoft/vscode-recipes) for [PHP](https://github.com/Microsoft/vscode-recipes/tree/master/PHP), [Python](https://github.com/Microsoft/vscode-recipes/tree/master/debugging%20python), and [Ruby on Rails](https://github.com/Microsoft/vscode-recipes/tree/master/debugging-Ruby-on-Rails)e.
