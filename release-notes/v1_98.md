...
Behind the scenes, this runs `git --no-optional-locks branch -a --no-color --sort=-committerdate` in order to get the list of branches before processing them into completions. Similar is done to also fetch tags to checkout.
