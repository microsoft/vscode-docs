...
**Setting**: `setting(editor.inlineSuggest.model)`
...
When using the [simple file picker](https://code.visualstudio.com/docs/getstarted/tips-and-tricks#_simple-file-dialog) (either when connected to a remote or when using `setting(files.simpleFilePicker.enabled:true)`), you can now hide dot files by using the **Show/Hide dot files** button.
