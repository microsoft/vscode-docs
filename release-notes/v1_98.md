...
### Attach problems to chat context
...
Previously, you could attach folders as context using drag and drop from the Explorer view. Now, you can also attach a folder by clicking the paperclip 📎 button or by typing `#folder:` followed by the folder name in chat.

### Next Edit Suggestions (NES): Show Collapsed

We've added a `Show Collapsed` mode for NES. When enabled, suggestions are shown by the NES gutter indicator on the left-hand side. The suggestion is revealed only when you navigate to it by pressing `kb(editor.action.inlineSuggest.jump)`. Consecutive suggestions are shown immediately until a suggestion is not accepted. `Show Collapsed` is disabled by default and can be enabled by configuring `setting(editor.inlineSuggest.edits.showCollapsed:true)` or can be toggled in the NES gutter indicator menu.
...
You could already change the language model for Copilot Chat and Copilot Edits, and now you can also change the model for inline suggestions. This is done via the `setting(editor.inlineSuggest.model)` setting.

You can change the model that is used for code completions via **Change Completions Model** command in the Command Palette or the **Configure Code Completions** item in the Copilot menu in the title bar.
...
We're quickly rolling out end-to-end vision support in this version of GitHub Copilot Chat. This lets you attach images and interact with images in Copilot Chat prompts. For example, if you're encountering an error while debugging, quickly attach a screenshot of VS Code and ask Copilot to help you resolve the issue, or attach some UI and Copilot can provide some HTML and CSS.
...
We have added a new Settings editor search algorithm that prioritizes more relevant key matches. The search algorithm attempts to match the setting ID and labels in more ways than before, but it also filters down the results more so that only the best match types are shown. You can try out the preview feature by enabling the `setting(workbench.settings.useWeightedKeySearch:true)` setting.
...
When using the simple file picker (either when connected to a remote or when using setting(files.simpleFilePicker.enabled:true)), you can now hide dot files using the "Show/Hide dot files" button
...
### Occurrences Highlight Delay 0 by default

The delay for occurrence highlighting within the editor is now set to 0 by default. This results in an overall more responsive editor feel, and the delay can still be controlled with the setting(editor.occurrencesHighlightDelay) setting.
...
Notebook Inline Values now have their decoration truncated to fit the width of the viewport and have a rich hover to show the full value, maintaining whitespace formatting. This maintains the shape of varaibles like dataframes, making values easier to read at a glance.

![Cursor hoving above a dataframe inline decoration, rich value hover is shown](images/1_98/nb-inline-values-rich-hover.png)
...
![Cursor hoving above a dataframe inline decoration, rich value hover is shown](images/1_98/debug-inline-values-rich-hover.png)
...
### Configuring allowed extensions in Windows Group Policy as a multiline string

You can now configure allowed extensions in the group policy on Windows using a multiline string. This allows for more flexible and extensive configuration of allowed extensions.
