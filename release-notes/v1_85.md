...
You can now enable Code Actions on Auto Save in the editor, and Editor: Code Actions On Save (editor.codeActionsOnSave) settings have now been migrated over to corresponding enum values. When set to `always`, Code Actions can be run when you save or Auto Save with window change (onWindowChange) or focus change (onFocusChange). To enable this feature, check Editor: Code Actions On Save (editor.codeActionsOnSave) and change each Code Action's setting to `always`.
...
This milestone we have introduced a new "Incoming/Outgoing" section in the Source Control view to display incoming and outgoing changes for the current branch compared to its remote. The new section will display both the individual changes with the number of resources changed along with insertions and deletions, as well as an "All Changes" entry that will summarize all resources across all changes. The visibility of the new section can be controlled using the `scm.showIncomingChanges`, and `scm.showOutgoingChanges` settings. Both settings support the following values: `always`, `auto` (default), and `never`.

![Incoming/Outgoing changes in the Source Control view](images/1_85/scm-incoming-outgoing.png)

### Input max lines setting

Previously, the source control input would auto-grow to display at most 6 lines of text which was sufficient space for most commit messages though there were cases in which more space would have been helpful.  This milestone we have added a new setting, `scm.inputMaxLines`, that one can use to control the maximum number of lines that the source control input will auto-grow to.
...
#### Inline chat for fixes shows summary and followup action

When using inline chat for fixes, you now also get a short summary what the fix does. If there are remaining errors, you can refine the fix by clicking on the offered followup action.

![inline chat with summary](./images/1_85/inline-chat-message.png)

#### Progressive rendering of message in inline chat

Inline chat now uses the same progressive rendering as in the panel chat:
...
Copilot Chat now sees cross-file context from your codebase when you ask it to explain Rust code and have installed an extension contributing Rust language services like [rust-analyzer](https://marketplace.visualstudio.com/items?itemName=rust-lang.rust-analyzer). To use this, select some code in your active editor, then use the **Copilot > Explain This** from the context menu or `/explain` from inline or panel chat.

![Copilot > Explain This in the editor context menu](images/1_85/copilot-explain-rust.png)
...
We want to make sure that users are aware of any possible issues with code that Copilot generates for you, so we are now running the code in chat panel codeblocks through a code vulnerability detection model and flagging any possible issues that are detected. You may not see this feature at first, but we will be gradually rolling it out to Copilot Chat users, and also tuning the types of vulnerabilities that are detected.

When a codeblock is determined to possibly contain a vulnerability, it will be annotated at the bottom of the codeblock.

![Copilot code vulnerabilities](images/1_85/copilot-code-vulnerabilities.png)
...
Building on the success of sticky scroll in the editor, we've extended this beloved feature to all tree views, enabling users to navigate their project trees effortlessly. Sticky Scroll for Trees can be enabled by setting `workbench.tree.enableStickyScroll: true`. To ensure sticky scroll does not take to much of your trees space, we ensure it can only take up to 40% of the view height. Additionally, users can customize the maximum number of sticky elements by configuring `workbench.tree.stickyScrollMaxItemCount` which is set to 7 by default.
...
For a more enjoyable tree navigation experience, users can simply click on a sticky element to jump directly to it within the tree, or press the twistie of a parent element to hide all its child elements. Additionally, accessing checkboxes and action items is made effortless through the sticky scroll feature.

<video src="images/1_85/sticky-scroll-extension-trees.mp4" autoplay loop controls muted title="Sticky Scroll in Extension Views"></video>
...
`1.85` is the last release that supports macOS 10.13 (macOS High Sierra) and 10.14 (macOS Mojave). Refer to our [FAQ](https://code.visualstudio.com/docs/supporting/faq#_can-i-run-vs-code-on-old-macos-versions) for additional information.
