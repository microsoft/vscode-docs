...
The easiest is to just drag an editor out of the window and drop it on some empty space on your screen:
...
You can configure the new `workbench.editor.dragToOpenWindow` setting to control this behaviour.

We also added global and contextual commands to move or copy editors and editor groups into their own windows:
...
Please give this new feature a try and report back any issues you encounter. Make sure to [review our existing issues](https://github.com/microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-auxwindow) and cast your vote on missing features.
...
You can now choose which extensions to auto update. This is helpful if you do not want to auto update all extensions but selectivly choose which ones to auto update. You can either select an extension or all extensions from a publisher to auto update. You can also choose to auto update all extensions from a publisher and unselect individual extensions from that publisher.
...
<video src="images/1_85/show-type-hierarchy-pylance.mp4" autoplay loop controls muted title="Show Type Hierachy with Pylance"></video>
...
When attempting to use smart send via shift+enter on a Python file that contains invalid or deprecated code, there is now a warning message and a setting to deactivate REPL smart send. Users are also allowed to change and set their user and workspace specific behavior for REPL smart send.

<img width="826" alt="Screenshot 2023-12-01 at 7 56 34 PM" src="https://github.com/microsoft/vscode-docs/assets/62267334/d6adfc11-cc4b-4066-925a-b8a7924ed3c5">
...
There is a new setting to hide problem decorations in the editor and throughout the workbench (excluding the problems view). Problems: Visibility (`problems.visibility`) is enabled by default to show all problems.

Outline > Problems: Badges (`outline.problems.badges`), Outline > Problems: Colors (`outline.problems.colors`), Outline > Problems: Enabled (`outline.problems.enabled`), and Problems > Decorations: Enabled (`problems.decorations.enabled`) will be overwrtitten when Problems:Visibility is off. A warning will be shown in the status bar when this setting is off.
