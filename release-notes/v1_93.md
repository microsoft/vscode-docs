...
Out of the box, we have not yet enabled custom title by default on Linux, but we plan to do so eventually. Once you turned on custom title, native window control overlays will appear automatically. We very much welcome any feedback you find when using this new feature!
...
Based on user feedback, we have moved the history graph that was enabled last milestone from the "Source Control" view into a new view called "Source Control Graph". This will reduce the information overload from the main view, enable users to hide/collapse/move the new view as they see fit, as well as lays the foundation for a full-featured history graph.

The "Source Control Graph" view is currently showing the current branch, it's remote/base. Next milestone we are working on adding the capability to filter the graph to any repository references as well as improve the experience for users that have multiple repositories in their workspace.
...
[Git 2.45](https://github.blog/open-source/git/highlights-from-git-2-45/#preliminary-reftable-support) added preliminary support for the new reference storage backend called "reftable". This milestone we have updated the built-in Git extension to support this new reference storage backend.

Using Git 2.45 you can either create or clone a repository that uses the new reference storage backend with the `--ref-format=reftable` flag. Git 2.46 also added support for migrating an existing repository to use the new reference storage format using the [`git refs migrate`](https://git-scm.com/docs/git-refs/2.46.0) command.

Please note that the new reference storage backend is still considered experimental.

### Compact folders setting

Thanks to a community contribution, we have added a new setting, <a href="vscode://settings/scm.compactFolders">`scm.compactFolders`</a>, to control whether folders are being rendered in compact form in the main Source Control view when changes are viewed as a tree.

## Terminal

#### Julia and NuShell support

There's now support shell integration support for Julia and NuShell. This enables features such as [command decorations](/docs/terminal/shell-integration.md#command-decorations-and-the-overview-ruler) and [run recent command](/docs/terminal/shell-integration.md#run-recent-command) for these shell types.
...
The <a href="vscode://settings/git.openDiffOnClick" codesetting="true">`git.openDiffOnClick`</a> setting is now honored when browsing a GitHub repository on https://vscode.dev or on desktop with the [GitHub Repositories extension](https://marketplace.visualstudio.com/items?itemName=GitHub.remotehub).
...
#### Automatic panel chat participant detection

GitHub Copilot has built-in chat participants such as `@workspace` which also contribute commands to the panel chat view. Previously it was necessary to manually specify the chat participant and command to use for a request.
...
![Participant detection in panel chat](images/1_93/participant-detection.png)

If the automatically-selected participant is not appropriate for your question, you can select the `rerun without` button at the top of the chat response to resend your question. You can also manually enable or disable this feature by configuring <a href="vscode://settings/chat.experimental.detectParticipant.enabled" codesetting="true">`chat.experimental.detectParticipant.enabled`</a>.

#### Attach Context in Quick Chat

When using Quick Chat, you can now use the `Attach Context` action to attach context like files and symbols for Copilot.

![Attach context in quick chat](images/1_93/attach-context-quick-chat.gif)

#### Improvements to chat history

You can open previous chat sessions with the "Show Chats" button at the top of the chat panel. These sessions now have nicer names that are computed by the LLM. You can also click the pencil icon on each row to rename the session manually.

![the chat history picker](images/1_93/chat-history.png)

These entries are also now sorted by the date of the last request, and are labeled with their age.

> Note: Only new chat sessions will get the LLM-generated name, any chat sessions that you already have won't be renamed.

#### Chats saved in empty windows

VS Code previously wasn't saving chat sessions in empty windows (that don't have a folder or workspace open). Now, these will be saved as expected, and previous chats from an empty window can be loaded via the "Show Chats" button. But, you should avoid opening and using the same chat session in multiple empty windows simultaneously.

#### Thumbs down details

Did you get a response from Copilot Chat that isn't what you expected? Help us out by clicking the "thumbs down" button on the toolbar in the chat response. Now, it will show a dropdown with a few detailed options for you to describe the issue. You can also open the issue reporter from this menu.
...
We are excited to announce support for one of our most [requested features](https://github.com/microsoft/vscode-python/issues/73): you can now  discover and run Django unit tests through the Test Explorer! For set up instructions on how to enable this functionality, [check our documentation](/docs/python/testing.md#django-unit-tests).
...
We made additional improvements and bug fixes to the new Native REPL experience. We are rolling out the native REPL as the default target for [Smart Send](https://devblogs.microsoft.com/python/python-in-visual-studio-code-november-2023-release/#improvements-to-shift-enter-run-line-in-terminal) as an experiment, but if you are interested in trying this out, you can set <a href="vscode://settings/python.REPL.sendToNativeREPL" codesetting="true">`python.REPL.sendToNativeREPL`</a> in your User `settings.json` and reload your VS Code window.

#### Go to definition from inlay hints with Pylance
When [inlay hints are enabled](/docs/python/settings-reference.md#intellisense-engine-settings) with Pylance, you can now more conveniently navigate to a type's definition by hovering over it and clicking while holding `kbstyle(Ctrl)` on Windows and Linux or `kbstyle(Cmd)` on macOS:
...
You can now restart the debugger when debugging tests through the debug control widget.
...
1.0 versions of the `wasm-wasi-core` extension together with the `@vscode-wasm-wasi` and `@vscode/wasm-component-model` npm modules have been released. Usage samples can be found in the [vscode-extension-samples](https://github.com/microsoft/vscode-extension-samples) repository in the `wasm-*` sub folders. There are also serveral posts about how to use WebAssembly code inside a VS Code exension in [VS Code's blog](https://code.visualstudio.com/blogs)
...
We've continued to iterate on our `LanguageModelTool` API. The API comes with two major parts:
...
One of the changes this month adds the ability for the user to manually attach certain tools to their chat request. An example of this would be for a tool that computes some dynamic context that the user wants to use in their request.
