...
### Lightbulb Widget Imrpovements

In addition to improvments last iteration to the lighbulb widget's location, we've decided to show the lightbulb widget less by setting <a href="vscode://settings/editor.lightbulb.enabled" codesetting="true">`editor.lightbulb.enabled`</a> to `onCode` by default. 

### Action widget theme key

The color of the action widget (such as the code action menu) can be configured by themes with the `editorActionList-background`, `editorActionList-foreground`, `editorActionList-focusForeground`, and `editorActionList-focusBackground` keys. By default, the action widget will match with the quick pick and command palette.
...
Working with JavaScript and TypeScript on <vscode.dev> just got a whole lot better. To start off, we've implemented package IntelliSense so you can see suggestions and documentation from any imported package, such as `react`:

![IntelliSense and error reporting for an imported package on vscode.dev](images/1_93/ts-web-full-project-intellisense.png)

This works much like it does on Desktop VS Code. In TypeScript files, we offer IntelliSense for packages listed in your `package.json`. JavaScript files are a little more flexible and use [automatic type acquisition](../docs/nodejs/working-with-javascript.md#typings-and-automatic-type-acquisition), which offers IntelliSense for any package imported into the current file even if it's not listed in the `package.json`.

Thanks to package IntelliSense, we've also enabled full project IntelliSense for JavaScript and TypeScript projects. This greatly improves navigation through code, letting you correctly **Go to Definition** and **Find All References** to any symbol in your project. It also enables type error reporting. We even now support auto-imports while writing code.

![Finding all references of a symbol in a TypeScript project](images/1_93/ts-web-peek-refs.png)

Package typings and full project IntelliSense are supported in in Chrome, Edge, and Firefox. Safari is not yet supported as it does not implement [`ReadableByteStreamController`](https://developer.mozilla.org/en-US/docs/Web/API/ReadableByteStreamController).
...
#### Preview: Start debugging from Chat
...
#### Preview: Automatic chat participant detection in Chat view
...
1. Enable the <a href="vscode://settings/github.copilot.editor.enableInlineChatCompletionTrigger" codesetting="true">`github.copilot.editor.enableInlineChatCompletionTrigger`</a> setting, so that Copilot makes suggest to start inline chat once a line consists of mostly text and not source code.

The following video shows how Copilot suggests to start inline Chat once a line consists of mostly text and not only source code.
...
Inline Chat can now use recently seen or edited code as context. This is currently experimental but also good to be tested by everyone. You can enable it via the <a href="vscode://settings/github.copilot.chat.experimental.temporalContext.enabled" codesetting="true">`github.copilot.chat.experimental.temporalContext.enabled`</a> setting.
...
### Aux Window based Issue Reporter

This iteration, we are experiementing with a new version of the issue reporter. Functionality remains the same and is enabled by default via <a href="vscode://settings/issueReporter.experimental.auxWindow" codesetting="true">`issueReporter.experimental.auxWindow`</a> in desktop versions. Issue reporting will now also be available by default in web versions.
