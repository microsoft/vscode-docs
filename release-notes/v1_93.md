...
* [Experiment: Inline Chat](#copilot-inline-chat-context) - Use recent coding files for Inline Chat context.
...
<h3 id="github-copilot">GitHub Copilot<div id="_github-copilot"/></h3>
...
GitHub Copilot enables you to generate tests by using the **Generate Tests using Copilot** Code Action via the Editor context menu, or by using `/tests` in inline Chat. We made improvements to the test generate flow. Now, Copilot tries to identify if there's an existing test file and starts generating tests directly into that file, appending them at the end. If there is no existing test file, Copilot creates a new test file and generates tests into it.
...
<h4 id="copilot-inline-chat-context">Experimental: Use recent coding files as Inline Chat context<div id="_copilot-inline-chat-context"/></h4>
...
If test coverage information is available, GitHub Copilot can offer a CodeLens **Generate tests using Copilot** for functions and methods that are not yet covered by tests. This experimental feature is available via the setting <a href="vscode://settings/github.copilot.chat.experimental.generateTests.codeLens" codesetting="true">`github.copilot.chat.experimental.generateTests.codeLens`</a>.
