...
* [GitHub Copilot](#github-copilot) - Attach context in Quick Chat, user-friendly browsing of chat history.
* [Experiment: Inline Chat](#thumbs-down-feedback-details) - Use recent coding files for Inline Chat context.
...
#### Improvements to test generation

GitHub Copilot allows to generate tests by using a code action "Generate Tests using Copilot", via editor context menu, or using `/tests` in inline chat. We made improvements to the test generate flow. Now Copilot tries to identify an existing test file and starts generating tests directly into that file, placing the new tests under the last existing test. If there is no existing test file, Copilot will create a new test file and generate tests into it.
...
GitHub Copilot offers code actions to generate tests and documentation that are offered when cursor is placed on an identifier, e.g., a method name. Previously, these code actions were called "Test using Copilot" and "Document using Copilot". To better reflect their purpose, we renamed these code actions to "Generate Tests using Copilot" and "Generate Documentation using Copilot".

![New code action names for test and documenation generation](images/1_93/new-code-action-names.png)

#### Experimental: Use recent coding files as Inline Chat context
...
#### Experimental: Use current editor line as inline Chat prompt
...
The following video shows how Copilot suggests to start inline Chat after it detects that there's mostly text on the current line.

<video src="images/1_93/inline-with-line.mp4" title="Start Inline Chat via Completions" autoplay loop controls muted></video>

#### Experimental: Start debugging from Chat
...
#### Experimental: Automatic chat participant detection in Chat view
...
#### Experimental: Code generation instructions
...
#### Experimental: Code lens to generate tests based on test coverage information

If test coverage information is available, GitHub Copilot can offer code lens "Generate tests using Copilot" for functions and methods that are not yet covered by tests. The feature is available with a setting <a href="vscode://settings/github.copilot.chat.experimental.generateTests.codeLens" codesetting="true">`github.copilot.chat.experimental.generateTests.codeLens`</a>.
...
We believe that this is the clearest way to han
