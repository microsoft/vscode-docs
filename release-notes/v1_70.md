...
Selecting an item in the picker will focus the editor on the selected debug session's context. This is helpful if you want to perform debug actions (e.g., pause, step, restart, stop) from the context of a specific session. Selecting a session also shows the session's debug console.
