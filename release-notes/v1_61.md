...
* **[Virtual Workspaces extension guide](#virtual-workspaces-extension-guide)** - Learn how to update your extension for VS Code virtual environments.
...
**Join us live** at the [VS Code team's livestream](https://code.visualstudio.com/livestream) on Thursday, October 7 at 8am Pacific (4pm London) to see a demo of what's new in this release, and ask us questions live.
...
The editor now supports rudimentary bracket pair guides. Bracket pair guides use the same colors and the [same algorithms](https://code.visualstudio.com/blogs/2021/09/29/bracket-pair-colorization) as the bracket pair colorization feature. Similar to indentation guides, the active bracket pair guide is highlighted.
...
![New sync button](images/1_61/scm-sync-button.png)
...
Prior to this release, the Git extension had a hard-coded limit of 5000 changes that it could show in the Source Control view. The limit was to prevent the user from having to wait for too long while VS Code processed all the changes reported by Git. This limit is now increased to 10,000 changes, and there is a new `git.statusLimit` setting to allow users to customize the limit (per repository if desired). This setting can also be set to `0` to disable the limit completely, but be aware this could cause updates to take a very long time if there are lots of changes.
...
You can now statically set terminal dimensions via the `workbench.action.terminal.setDimensions` command. This is useful for cases when line wrapping can be jarring or disruptive.
...
### IntelliSense across files on github.dev

IntelliSense now works across JavaScript and TypeScript files on [github.dev](https://github.dev) when VS Code is running in a browser.

![Cross file JavaScript IntelliSense on github.dev](images/1_61/ts-cross-file-github.png)

Keep in mind that github.dev only has access to your currently opened files, so VS Code cannot provide project-wide IntelliSense features such as auto imports.
...
Terminal processes get disposed of when a VS Code window is closed, in this version there is a new opt-in setting for recording and restoring the buffer and recreating the processes under. Set `terminal.integrated.persistentSessionReviveProcess` to enable this.

![The buffer gets restore on application restart with a message indicating when the snapshoit was taken](images/1_61/buffer-restore.png)
...
Jupyter Notebook renderers have been split out into a separate extension ([Jupyter Notebook Renderers](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-renderers)), allowing users to view Notebook outputs such as plotly, vega, latex and the like on [github.dev](https://github.dev).

### Python

**New Python walkthrough**

The Python extension now offers a walkthrough with some basic set up steps to improve the getting started experience for Python in VS Code.

![New Python walkthrough](images/1_61/new-python-walkthrough.png)

**Improvement on the debugging experience**

When working with workspaces with no launch.json configuration present, the [Python extension](https://marketplace.visualstudio.com/items?itemName=ms-python.python) would display a debugger configuration menu everytime you would debug your Python file or project. This could be particuarly annoying when debugging a web application  with custom arguments (like Flask, Django or FastAPI, for example).
...
Extensions can now publish different VSIXs for each platform (Windows, Linux, macOS) VS Code is running on. We call such extensions  **platform-specific extensions**. Starting with version 1.61.0, VS Code looks for the extension package that matches the current platform. Starting with version 1.99.0, VSCE allows publishing platform-specific extension.
...
The new `enableForms` property on `enableForms` allows you to enable or disable forms inside webviews. For backwards compatibility, `enableForms` defaults to true if you have already set `enableScripts`. Otherwise it defaults to false.

We recommend disabling forms unless your webview content needs them.

### Terminal state

A new `state` property is exposed on `Terminal` in addition to a corresponding `window.onDidChangeTerminalState` event, this currently indicated whether or not a terminal has been interacted with by the user. For example, an extension might want to wait until a user has interacted with the terminal to run an action.
...
This global symbol was not very discoverable and also didn't give VS Code much control over the renderer scripts themselves. To fix that, renderer script now use JavaScript modules that export an `activate` function. This function is passed the VS Code API:
...
There is a new section covering the [Remote - Containers](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers) extension's [advanced container configuration](https://code.visualstudio.com/remote/advancedcontainers/overview).
