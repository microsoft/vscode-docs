...
The editor now supports rudimentary bracket pair guides.
Bracket pair guides use the same colors and the [same algorithms](https://code.visualstudio.com/blogs/2021/09/29/bracket-pair-colorization) as the bracket pair colorization feature.
Similar to indentation guides, the active bracket pair guide is highlighted.

![](./images/1_61/bracket-pair-guides.png)
...
`editor.renderIndentGuides` and `editor.highlightActiveIndentGuide` have been deprecated in favor of `editor.guides.indentation` and `editor.guides.highlightActiveIndentation`.
...
Prior to this release, the Git extension had a hard-coded limit of 5000 changes that it could show in the Source Control view. The limit was to prevent the user from having to wait for too long while we processed all the changes reported by Git. We've now increased this limito to 10,000 changes, and added a new `git.statusLimit` setting to allow users to customize the limit (per repository if desired). This setting can also be set to `0` to disable the limit completely, but be aware this could cause updates to take a very long time if there are lots of changes.
...
This better matches the behavior of on-disk files in the workspace. Simple add an `import` or `export` to an untitled file to turn it into a module, which will hide its global variables from other files.

### IntelliSense across files on Github.dev

IntelliSense now works across JavaScript and TypeScript files on [GitHub.dev](https://github.dev)

![Cross file JavaScript IntelliSense on Github.dev](images/1_61/ts-cross-file-github.png)

Keep in mind that GitHub.dev only has access to your currently opened files, so we cannot provide project-wide IntelliSense features such as auto imports.
...
- More accurate reference counts for the references code lens.
...
This global symbol was not very discoverable and also didn't give VS Code much control over the renderer scripts themselves. To fix that, renderer script now use JavaScript modules which export an `activate` function. This function is passed the VS Code api:
