...
The current TypeScript version and IntelliSense state is now shown in the language status item:
...
The language status item also shows the current file's jsconfig/tsconfig and will indicate if you are currently in [partial mode](/docs/nodejs/working-with-javascript#partial-intellisense-mode).
...
### QuickPick keepScrollPosition property

The `keepScrollPosition` property on the `QuickPick` object that you get back from `window.createQuickPick()` has been added as proposed API. This allows you to control whether or not the scroll position (`cursorTop`) in the quickpick moves back to the top of the list. There are a few usecases we had in mind for this API:
