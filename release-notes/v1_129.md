...
Prompt files (`*.prompt.md`) are used to describe custom slash commands. They are only supported in the Local agent harness while other hernesses express slash commands with skills. For compatibility across harnesses, we recommend to migrate all prompt files to skills.

With setting(chat.customizations.promptMigration.enabled) enabled, if you select a harness running on the (agent host)[#the-agent-host] and you have migratable prompt files, you'll now see a 'Migrate Prompts' entry in the AI Customizations overview
