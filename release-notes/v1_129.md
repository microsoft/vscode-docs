...
Welcome to the 1.129 release of Visual Studio Code. This release brings a dedicated agent host, a new editor panel in the Agents window,  GitHub Enterprise sign-in for Copilot and prompt file migration to skills.
...
* [Migrate prompt files to skills (Experimental)](#migrate-prompt-files-to-skills-experimental): Convert existing prompt files into reusable skills.

* [GitHub Enterprise support for Copilot in the agent host](#github-enterprise-support-for-copilot-in-the-agent-host): Sign in to Copilot with a GitHub Enterprise instance.
...
![Screenshot showing running a terminal command from chat with the ! prefix.](images/1_129/bang-commands.png)

### Migrate prompt files to skills (Experimental)

Prompt files (`*.prompt.md`) are currently supported only in the Local agent harness. If you already use them, you can migrate them to skills and keep the same prompt content in a reusable format.

With `setting(chat.customizations.promptMigration.enabled)` enabled, you'll see a migration card in the AI Customizations overview when you have migratable prompt files. The migration interface lets you:
...
![migrate prompt files](./images/1_129/migrate-prompt-files.png)
