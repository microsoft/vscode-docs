...
We added a new command ("Report Performance Issues" from the command pallette) that makes it very easy to open a new GitHub issue with lots of details about your environment and performance timings. If you think that VS Code is starting slow on your system, please use this new action to tell us about it!
...
The Interactive Playground available from the Welcome document introduces you in a step-by-step guide to some of our advanced code editing features with interactive examples to try next to the instructions:
...
**Note**: The file contents are cached. Therefore, you will need to reload the Visual Studio Code window the first time you add these files and everytime you modify them.
...
Due to potential security risks, VS Code no longer supports using `php.validate.executablePath` as a workspace setting. You are still able to configure VS Code to use PHP executable to lint your PHP programs. To do so either sepcifiy the `php.validate.executablePath` on a user level or click on the `Path` status bar item in the lower right corner in the status bar to enter the path to the PHP executable to use:
