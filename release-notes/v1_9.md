...
* **[Faster Integrated Terminal](#terminal-improvements)** - We improved the integrated terminal's performance and as well as Windows support.
* **[Synchronized Markdown Preview](#markdown-preview-and-editor-integration)** - Coupled view of a Markdown preview and its editor.
* **[Settings improvements](#user-and-workspace-settings)** - Easily switch between user and workspace settings.
...
* **[TypeScript 2.1.5 and CodeLens references](#typescript)** - VS Code ships with TypeScript 2.1.5 and now includes References CodeLens.
* **[Single file debugging](#debugging-without-a-launch-configuration)** - You can debug without a configuration file to enable quick debugging of single files.
* **[Inline variables display](#inline-variable-values-in-source-code)** - See variable values inline while debugging.
* **[Expanded Node.js debugging configurations](#launch-configuration-snippets-for-node-scenarios)** - Launch configuration snippets to easily debug Mocha tests, gulp tasks and even Yeoman generators. 
...
* **[Editor](#editor)** - Format on Paste, keybind to snippets, Go to Implementation, fast search navigation.
...
We have added a Welcome page (**Help** > **Welcome**) with **Quick links** to help new users as well as an MRU list of recent folders and online documentation links for fast navigation. Have the **Show welcome page on startup** option checked to keep it handy:

![Welcome document](images/1_9/welcome-page.png)

### Interactive Playground

The **Interactive Playground** on the **Welcome** page introduces you in a step-by-step guide to some of our advanced code editing features with interactive examples to try next to the instructions:
...
* `"zenMode.hideTabs"`
* `"zenMode.fullScreen"` - had been `workbench.fullScreenZenMode`
...
VS Code gives you the flexibility of customizing the editor through Settings. But quite often users work with different file types (languages) within or across the workspaces. This brings the requirement of more detailed editor configuration. With this release, you can customize your editor by language through language based settings. This is one of the most wanted features requested by our users for a long time.
...
- If you want to customize for any language, run the global command `Preferences: Configure language specific settings` (id: `workbench.action.configureLanguageBasedSettings`) from the **Command Palette** (`kb(workbench.action.showCommands)`) which opens the language picker. Selecting the language you want, opens the Settings editor with the language entry where you can add applicable settings.
...
**NOTE:** All editor settings and some non-editor settings can customized. IntelliSense in the Settings editor will help you in finding the allowed settings.
...
Below a screen cast that first starts the tsc compiler in background (Using Ctrl+Shift+B) and then executes the dir command in another terminal.
...
We have added a new setting to enable format on paste. Simply update your settings to include `editor.formatOnPaste: true`. Note that this works with existing formatters that support formatting a selection such as the TypeScript formatter or other formatters from the Marketplace.
...
Here are the multicursor actions:
...
![TypeScript tsdk in workspace error](images/1_9/ts-workspace-tsdk-error.png)
...
### cut - Support for suggestions and formatting in unsaved JavaScript and TypeScript files
...
### cut- Icons For TypeScript file and folder completions
...
### Deprecation of workspace `php.validate.executablePath` setting

Due to potential security risks, VS Code no longer supports using `php.validate.executablePath` as a workspace setting. You are still able to configure VS Code to use PHP executable to lint your PHP programs. To do so either specify the `php.validate.executablePath` in your user settings or click on the `Path` item in the lower right corner in the Status Bar to enter the path to the PHP executable to use:

![Configure PHP executable](images/1_9/configure-php.png)

If you have configure a `php.validate.executablePath` on workspace level VS Code will help you to migrate the existing setting to the new settings store.
...
In order to make the transition easier, we made changes to how you specify the PHP executable path. In both cases, the first time you open a PHP file in a workspace that still has a workspace level setting, we will guide you through the migration process. 

The `typescript.tsdk` setting created a similar security exposure which has been addressed separately. 
...
We added a new command, **Report Performance Issues** in the **Command Palette**, that makes it very easy to open a new GitHub issue with lots of details about your environment and performance timings. If you think that VS Code is starting slow on your system, please use this new action to tell us about it!

![Report Performance Issue](images/1_9/performance.png)
