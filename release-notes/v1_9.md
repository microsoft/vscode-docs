...
### Deprecate of Workspace typescript.tsdk Setting

Due to potential security issues, VSCode 1.9 no longer supports the `typescript.tsdk` setting in project workspaces.

![TypeScript tsdk in workspace error](images/1_9/ts-workspace-tsdk-error.png)

You can still easily use your own version version of TypeScript, either globally by setting `typescript.tsdk` in your user settings or in individual workspaces. To use the a preview of TypeScript 2.2, first install a nightly build of TypeScript in your workspace: `npm install typescript@next`. Then open a JavaScript or TypeScript file and look in the lower right corner in the status bar to see which version of TypeScript is being used by VSCode for JavaScript and TypeScript language features

![TypeScript status bar version](images/1_9/ts-status-bar-version.png)

Click on the TypeScript version number in the status bar switch between VS Code's version of TypeScript and the version installed in your workspace.

![TypeScript version selector](images/1_9/ts-select-ts-version-message.png)

After switching TypeScript versions, be sure to restart VS Code to the change take effect.
