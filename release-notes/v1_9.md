...
In order to make the transition easier, we made changes to how you specify the PHP executable path as well as the TypeScript `.tsdk` file. In both cases, the first time you open a PHP or TypeScript file in a workspace that still has a workspace level setting, we will guide you through the migration process.
