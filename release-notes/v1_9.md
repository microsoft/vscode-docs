...
* The Markdown preview will now attempt to maintain the currently visible content in view when resized. 
* When the Markdown preview is first opened, it will automatically scroll to the selected line in the editor.
* A light gray bar in the left margin of the Markdown preview indicates the currently selected line.
...
### Added new Type Implementation Provider API

A new `TypeImplementationProvider` interface was added to support the new `go to implementation` feature in VSCode 1.9. A `TypeImplementationProvider` maps a location in a text document to a list of implementations locations, for example going from an interface to classes that implement this interface or from an abstract method to a list of implementing methods. 

Extensions that wish to support `go to implementation` can implement `TypeImplementationProvider` and register the provider using `vscode.languages.registerTypeImplementationProvider`
