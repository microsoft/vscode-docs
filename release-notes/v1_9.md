...
**NOTE:** Intelli-sense in Settings editor will help you in finding allowed language based settings. All editor settings (except following) and some non-editor settings are supported. Following settings are not supported because of discovering them very late in the release and considering the technical risk of supporting them in the last minute, we decided to drop them. Please refer to [#19511](https://github.com/Microsoft/vscode/issues/19511) for more information. Good news is that these are already implemented in upcoming release and you can start using them with our insiders builds.
...
We started large refactorings around source control in order to [provide SCM functionality from different contributable providers](https://github.com/Microsoft/vscode/issues/2824). A the same time, all Git functionality is being ported over to an [SCM provider](https://github.com/Microsoft/vscode/blob/23c4c8d1b881cfbf48e76fbe67e350b4efecba68/src/vs/vscode.proposed.d.ts#L143).

The new behaviour is available for you to try out with via the `SCM: Enable Preview SCM` command, which will replace the current Git functionality with the experimental one (run `SCM: Disable Preview SCM` to undo). Be aware that this is not feature complete nor without issues. You can track progress in [#18615](https://github.com/Microsoft/vscode/issues/18615).
