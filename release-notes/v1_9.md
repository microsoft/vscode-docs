...
Due to potential security risks, VS Code ask for confirmation of the `php.validate.executablePath` workspace setting before executing the program the setting is pointing to. 

![Confirming PHP executable](images/1_9/confirming-php.png)

If the setting is set on user level no confirmation is necessary.
