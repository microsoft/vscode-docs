...
Since diagnostic error codes are now surfaced in [Problems panel](#problem-code), it is no longer necessary for the extensions to add them to the [`Diagnostic.message`](https://github.com/Microsoft/vscode/blob/master/src/vs/vscode.d.ts#L4074) property.
...
We have continued our [webpack](https://webpack.js.org) effort and now most bundled extensions are webpack'ed. As a re-fresher: this helps to reduce the startup time for extensions because there are fewer files to load and less source to parse and it helps to reduce the install time because fewer files need to be extracted and written to disk.

This milestone we have trimmed the bundle by another 2803 files. In July we have started with 9000 files, for August we had 4650, and in September we have 1847!
