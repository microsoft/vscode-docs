...
* **[Easier keyboard shortcut search](#record-and-search-keyboard-shortcuts)** - Search for keybindings by keystroke instead of key name.
...
* **[Rename via import path](#rename-import-path)** - Rename a JS/TS file through it's import path.
* **[Convert to async refactoring](#convert-to-async-function)** - Quickly update a function to use async and await.
* **[Better Markdown folding](#markdown-code-block-list-and-HTML-folding)** - Collapse code blocks, multi-line lists, and embedded HTML.
...
You can also enter into the **Recording Keys** mode using `kb(keybindings.editor.recordSearchKeys)` and `Esc` key exists from that mode.
...
### Markdown code block, list, and HTML folding
...
Since diagnostic error codes are now surfaced in [Problems panel](#problems-panel), it is no longer necessary for the extensions to add them to the [`Diagnostic.message`](https://github.com/Microsoft/vscode/blob/master/src/vs/vscode.d.ts#L4074) property.
