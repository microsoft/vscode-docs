...
For a long time, VSCode used to resolve a symbolic link of a folder when opening. This meant, that even though the symbolic link was there for a reason, VSCode was ignoring it and always opened the resolved path the link was pointing to. We now decided to change this behaviour and preserve the symbolic link.

**Note:** certain features will work with reduced functionality when opening a folder as symbolic link (for example [debug](https://github.com/Microsoft/vscode/issues/57954) and [git](https://github.com/Microsoft/vscode/issues/5970)).

### File icons per file type

On macOS and Windows, VSCode registers as default editor for well known file types. We are now shipping icons for the top file types VSCode is being used for. These icons will show up in the OS explorer when the size of the icons is configured to be medium to large. For small icon sizes we decided to preserve the VSCode logo as icon.

![File Icons](images/1_28/file-icons.png)

### Toggle panel changes

We decided to revisit the behaviour of our commands for toggling panels (Terminal, Output, Problems, Debug Console). You can follow [issue 19400](https://github.com/Microsoft/vscode/issues/19400) for the background why we do this change.
...
To get back the previous behaviour, simply configure a keybinding to close the panel when the related panel is active, e.g. for output:
...
### New setting to exclude recently opened files from quick open

A new setting `search.quickOpen.includeHistory` allows to control if files that were recently opened should be part of the file picker. By default, files that were recently opened will appear to the top of the search and other files from the workspace below. If you find this behaviour annoying, consider to change this setting to `false`.

### New setting to disable trash when deleting files

If you are seeing issues with deleting files from the explorer by moving them to the OS trash, you can now set `files.enableTrash` to `false` to never use the OS trash when deleting. Doing so will change the "Delete" action and keybinding on the file explorer to bypass the OS trash and delete permanently. You will still see a prompt before the delete happens.

### Prompt to open workspace file if detected

When opening a folder that contains one or more `.code-workspace` files in the root, we now show a notification to ask to open the workspace file instead. A workspace file is a strong indicator that the project provides a much better experience if the workspace file is opened, instead of the folder. You can learn more about multi-root workspaces in our [documentation](https://code.visualstudio.com/docs/editor/multi-root-workspaces).

![Workspace Prompt](images/1_28/workspace-prompt.png)

### New setting to not restore view state when reopening a file

VSCode has always remembered the view state (cursor location, folding, etc.) of a file even after the file was closed. There is now a new setting `workbench.editor.restoreViewState` to drop this view state when a file is closed. This allows to always start scrolled to the top of the file when opening it after it was closed.

### New command to close a file across all editor groups

A new command "Close Editor in all Groups" (`workbench.action.closeEditorInAllGroups`) allows to close the currently active file across all editor groups.

## Editor

### New command to navigate to last edit location
...
### New command to save without formatters

The new command "Save without Formatting" (`workbench.action.files.saveWithoutFormatting`) can be used to save a file without triggering any of the save participants (e.g. formatters, remove trailing whitespace, final newline). The default keybinding is `kb(workbench.action.files.saveWithoutFormatting)`.

### New setting to restrict encodings from being guessed
...
---|-------|----------
`kb(workbench.action.navigateToLastEditLocation)`|Navigate to the last location that was edited.|`workbench.action.navigateToLastEditLocation`
