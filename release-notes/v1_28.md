...
The custom title bar and themable menus described in previous release notes are now enabled by default. The setting `"window.titleBarStyle"` has defaulted to `"custom"` for the entire last month on Windows in insiders, and we are now ready to make this the default experience.
