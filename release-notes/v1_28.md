...
As announced on the [September iteration plan](https://github.com/Microsoft/vscode/issues/58224) we focussed on housekeeping our GitHub issues and Pull Requests. Across all of our VS Code repositories, we closed (either triaged or fixed) 3918 issues. While we closed issues, you created 2187 new issues. This resulted in a net reduction of  1731 issues. The the main [vscode repository](https://github.com/Microsoft/vscode/issues) now has 2472 open feature requests and 625 open bugs. In addition, we closed 212 Pull Requests.
...
As a side effect of the grooming we have addressed several feature requests and the Pull Requests also resulted in some new features. Therefore the release notes do not end here.

## Workbench

### Custom title and menu bar on Windows by default

The custom title bar and themable menus described in previous release notes are now enabled by default. The setting `"window.titleBarStyle"` has defaulted to `"custom"` for the entire last month on Windows, and we are now ready to make this the default experience.
...
Thanks to a PR from [Sardorbek (@oneslash)](https://github.com/oneslash), VS Code now includes syntax highlighting and basic langauge support for [TOML](https://github.com/toml-lang/toml).

![TOML syntax highlighting](images/1_28/toml.png)

For more advanced TOML language support, try installing a TOML language extensions such as [this one](
https://marketplace.visualstudio.com/items?itemName=bungcip.better-toml).

### Markdown fenced code block, list, and html folding
...
![Renaming an JavaScript import to rename a file](images/1_28/markdown-preview-link.gif)
