...
<video src="images/1_135/agent-host-vscode.mp4" title="Video showing the Agent Host feature in VS Code, demonstrating how agent sessions persist and sync across different projects, web interfaces, and machines." loop controls muted></video>

### Continue external agent sessions in VS Code

**Setting**: `setting(chat.agentSessions.showExternal)`

Recent sessions created in other applications with a Copilot or Claude agent can appear in the Sessions list. Open one to continue the conversation in VS Code with your Copilot subscription. By default, VS Code shows up to two recently updated external sessions.
...
### Single-pane side layout is the default

**Setting**: `setting(sessions.layout.singlePaneDetailPanel)`

The single-pane layout keeps session details and editors together in one side pane, with a shared tab bar next to chat. After introducing this layout in the previous release, it is enabled by default for the Agents window on desktop.
...
![Screenshot showing the simplified action bar for the Changes editor in the single-pane layout.](images/1_135/agents-single-pane-action-bar.png)

Disable `setting(sessions.layout.singlePaneDetailPanel)` and reload the window to use the classic layout.

### Simplified session controls and information

The session header is less crowded, so you can identify the active session and focus on the conversation.

The session title has a prominent position, while session and chat actions such as **New Chat** and pinning are grouped in an overflow menu next to the title. Search remains available in the sessions list. When a session contains multiple chats, chat tabs replace the single-chat header.

![Screenshot showing session and chat actions grouped in the simplified Agents session header.](images/1_135/agents-simplified-session-header.png)
...
![Screenshot showing changes, pull request, and artifact pills above the chat input, with the artifact list expanded.](images/1_135/agents-session-artifacts-pill.png)
...
![Screenshot showing the context menu for choosing which session pill types are visible.](images/1_135/agents-session-pill-visibility.png)
...
Chat sticky scroll lets you keep the current prompt visible as you scroll through and review long responses. We've further refined the behavior to make it more consistent with the editor sticky scroll experience. Enable both settings to try the redesigned experience.
...
To give you better insight into your chat usage, we've redesigned the chat response footer. When you hover over it, it now shows a breakdown of the tokens used in that chat turn by model for input, cached input, and output.

![Screenshot showing detailed token usage for each model in a chat turn.](images/1_135/chat_turn_info.png)

### Sandboxing for the Local Harness

We rolled sandboxing out to 50% to validate it in real-world usage. While we didn’t identify a specific blocking issue, continuing the rollout would increase the likelihood of support and follow-up work at a time when we want to preserve focus on higher-priority investments, especially in the area of the Agent Host and the Copilot Harness. We’ve therefore returned the default rollout to 0% for now.

Sandboxing for the Local Harness remains available as an opt-in through the UI, so users who want to try it can still enable it.
