...
### New "List: Automatic Keyboard Navigation" setting

When disabled, lists and trees won't automatically capture lexicographic keypresses in order to navigate through its elements. Instead, keyboard navigation becomes modal and the `list.toggleKeyboardNavigation` command will toggle its state. You can assign a specific keyboard shortcut to that keybinding. For example, one can set it to be <kbd>/</kbd> and only when pressing <kbd>/</kbd> then you'd be allowed to type anything to filter lists/trees. This setting defaults to enabled.
...
The command `expandLineSelection` is now bound by default to `kbstyle(Cmd+L)` (`kbstyle(Ctrl+L)`) and not to `kbstyle(Cmd+I)` (`kbstyle(Ctrl+I)`).

### Improved column selection

When configuring `"editor.multiCursorModifier": "ctrlCmd"`, and thus having the `kbstyle(Alt)` modifier used only for jump to definition, it is now possible to press the `kbstyle(Alt)` modifier when dragging a selection in the editor to toggle between a regular selection or a column selection. In the following gif, the selection begins as a regular selection and then `kbstyle(Alt)` is pressed and held until the mouse button is released:
...
### Debug Console wraps all elements

Thanks to the new tree implementation the debug console now supports dynamic height wrapping for all its elements - also for attributes inside objects.
...
Here's an example from the extension's `package.json`:
```json
"contributes": {
  "commands": [
    {
...
![debugtoolbar](images/1_32/debug-toolbar.png)
