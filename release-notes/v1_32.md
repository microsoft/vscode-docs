...
Alternatively, the `action` can be set to `debugWithChrome`. In this case, VS Code starts a Chrome debug session for the URI. In this mode, a `webRoot` property can be added that is passed to the Chrome debug session.
