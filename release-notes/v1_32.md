...
- A status bar with `Quick Fix...` and `Peek Problem` actions is added to the problems hover. `Quick Fix...` action will allow you applying quick fixes from hover and `Peek Problem` action will open the problem peek view in the editor, thereby making it eay to discover.
