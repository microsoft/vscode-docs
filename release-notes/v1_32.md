...
Removed the link that opens the `keybindings.json` file from the Keyboard Shortcuts Editor in favour of the button `{}` in the editor title area that does the same. Also changed the behaviour to open only `keybindings.json` file by default. You can change it to open `Default Keybindings` along with `keybindings.json` by setting `workbench.settings.openDefaultKeybindings` to `true`.
