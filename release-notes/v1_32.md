...
* **[Vue.js interpolations IntelliSense](#vetur)** - Vetur extension supports smart completions in Vue.js interpolations.
...
When disabled, lists and trees won't automatically capture lexicographic keypresses in order to navigate through its elements. Instead, keyboard navigation becomes modal and the `list.toggleKeyboardNavigation` command will toggle its state. You can assign a specific keyboard shortcut to that keybinding. For example, one can set it to be `/` and only when pressing `/` then you'd be allowed to type anything to filter lists/trees. This setting defaults to enabled.
