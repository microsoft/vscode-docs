...
* **Configure VS Code**
  ...
  * Staged changes now easier to manage ([more...](#quick-diff-decorations-for-staged-changes)).
...
You can tailor your AI experience in VS Code to your specific coding practices and technology stack by using instructions and prompt files. Although they're similar and use the same Markdown file format, each of these has its specific use cases, so we now more clearly distinguish between instructions and prompt files.
...
To address a long-time feature request, this milestone we have added quick diff editor decorations for staged changes. Now you can view and manage staged changes directly from the editor, without needing to open the Source Control view or a full diff editor.
...
These new quick diff decorations provide better visibility to the staged changes, and enable editor actions like Unstage Changes and Unstage Selected Ranges that were previously only available in the diff editor.
