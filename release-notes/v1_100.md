...
* Done button: we heard your feedback about the "Done"-button and we removed it! No more confusion about unexpected session endings. Now, only the New Chat `kb(workbench.action.chat.newChat)` command starts a new session.
...
When your conversation gets long, or your context gets very large, you may see a "Summarized conversation history" message in your agent mode session:
...
You can tailor your AI experience in VS Code to your specific coding practices and technology stack by using instructions and prompt files. Although they're similar and use the same Markdown file format, each of these has their specific use cases, so we now more clearly distinguish between instructions and prompt files.

### Instruction files

**Instruction files** (aka custom instructions, rules) provide context and rules, typically describing code guidelines or specifying which framework to use. You can then apply these instructions to your chat requests.

Instruction files use the `.instructions.md` file suffix. They can be located in your user data folder or in the workspace. The `setting(chat.instructionsFilesLocations)` setting lists the folders that contain instruction files.

You can attach instructions files manually to a request, or they can be automatically added:

- To add them manually, use the **Add Context** button in the Chat view, and then select **Instructions...**.
  Alternatively use the **Chat: Attach Instructions File...** command from the Command Palette. This brings up a picker that lets you select existing instructions files or create a new one to attach.

- To automatically add instructions to a prompt, add the `applyTo` frontmatter header to the instructions file to indicate which files the instructions apply to. If a chat request contains a file that matches the given pattern, the instructions file will be attached.
...
**Prompt files** describe a standalone, complete chat request, including the user query, mode and tools to use. Prompt files are useful for creating reusable chat requests for common tasks.
...
Prompt files can have the following frontmatter headers to indicate how they should be run:
...
The following example shows a prompt file for generating release notes, the prompt will run in agent mode and will use a set of tools:
...
- Instructions/prompt files that are open in an active editor are now correctly treated as "prompt file" attachments (for example, nested file references are resolved properly). This allows you to iterate on the instructions/prompt files quickly while you keep tuning them in the file editor.
- Instructions and prompt files now have their own language IDs, configurable in the *language mode* dialog for any file open document ("Prompt" and "Instructions" appropriately). This allows, for instance, using untitled documents as temporary prompt files before saving them as files to disk.
...
- The `applyTo` header is currently supported only in instruction files, and is considered invalid in prompt files.
- The new `/save` chat command lets you quickly store the current chat history as a prompt file. If agent mode is used, any tools invoked during the session are automatically summarized into the `tools` prompt file metadata and represents the minimal set of tools required for the stored prompt to work.
...
We leverage compact mode by default for when you create a chat in a new window. Combined with the option to have the window always on top, you can always keep the Chat view around for asking questions!
...
### Control Secondary Side Bar default visibility
...
To address a long-time feature request, this milestone we have added quick diff editor decorations for staged changes. These new quick diff decorations provide better visibility to the staged changes, and enable editor actions like "Unstage Changes" and "Unstage Selected Ranges" that were previously only available in the diff editor.
...
You can learn more about these features in the [Remote Development release notes](https://github.com/microsoft/vscode-docs/blob/main/remote-release-notes/v1_100.md).
...
![Gif showing coverage run using the Test Explorer and branch coverage displayed.](images/1_100/python-branch-coverage.gif)
...
![Image showing Quick Create in the Python: Create Environment quick pick.](images/1_100/python-envs-quick-create.png)
...
![Gif demoing the get environment information tool call implicitly by the model in copilot agent mode.](images/1_100/python-get-env-tool.gif)

![Gif demoing the get install package tool call for numpy version 2.2.](images/1_100/python-install-pkg-tool.gif)
