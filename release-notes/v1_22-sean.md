...
* **[Syntax aware folding support](#fold-markdown-by-heading-level)** - Syntax aware folding in CSS, HTML, JSON, and Markdown files.
* **[Cross file Error, Warning and Reference navigation](#error-and-reference-navigation-across-files)** - it's now possible to navidate across files within your workspace.
* **[New Hint suggestions](#hints-in-the-editor)** - make it easy to spot suggested fixes or refactorings in the editor.
* **[Convert to ES6 refactoring](#suggestion-code-actions-for-javascript-and-typescript)** - we've added a new Code Action to convert to ES6 classes.
* **[Auto attach to process](#node-debugging)** - Automatically attach the debugger to running Node.js processes.
* **[Log Points](#log-points)** - Log diagnostic messages without breaking on your debug session.
* **[Better large file support](#easier-way-to-open-large-files)** - Faster syntax highlighting, improved Find and Replace and increase memory for very large files.
* **[Multi-line links in the terminal](#better-support-for-multi-line-links)** - Paths and URLs can span lines.
* **[Emmet Wrap Preview]()** - we now provide a live preview if you leverage Emmet's wrap with abreviation functionality.
* **[Updates](TODO)** - Less downtime for updates on Windows.
* **[PREVIEW: Organize JS/TS imports](#organize-imports-for-javascript-and-typescript)** - Quickly remove unused imports and sorting them.

>>>
Other candidates
File a bug on an extensions
...
* **[Workbench](#workbench)** - Windows OS: scrolling improvements and improved auto update support.
...
> Tip: In the languages section we cover off how this is used for [folding of headers in Markdown](#fold-markdown-by-heading-level).
...
> Tip: In this release we have added several hints for JS/TS refactorings - these are covereed off in the XXX section of htese docs. <TODO>
...
Previously, opening large files (~2GB+) in the editor would prompt you to relaunch from the command line with the flag `--max-memory=NEWSIZE`. To reduce this friction, when you try to open a file which requires more memory than the application is currently allowed to use, a new notification will appear that lets you quickly relaunch with more memory available to the application. The max memory available after relaunch is configurable with a new setting `files.maxMemoryForLargeFilesMB` and defaults to `4096` (4GB).
...
We shipped a new text buffer implementation in last release (read our [blog post](https://code.visualstudio.com/blogs/2018/03/23/text-buffer-reimplementation) for more information), we continued the optimization for Find and Replace for large files.

### Configurable Format On Save timeout

VS Code can run formatters just before saving a file to disk. Due to that, speed matters because a slow extension could slow down saving. VS Code is very strict about saving and cancels a format-on-save-request after 750ms. For some extensions, especially when dealing with large files, that's not enough time, and so we have added a new setting `"editor.formatOnSaveTimeout"` to configure the timeout. In order to monitor saving, there is a progress indicator on the Status Bar. This setting enables that a language contribution that is known to need more time for formatting can tweak the timeout as a default [language specific setting](https://code.visualstudio.com/docs/getstarted/settings#_language-specific-editor-settings).
...
* When using either the **Emmet: Wrap with Abbreviation** or **Emmet: Wrap Individual Lines with Abbreviation** commands, there is now a preview of what the wrapped text will look like.

  ![Wrap text with abbreviation preview](images/1_22/wrap.gif)

## Workbench

### Windows: Automatic Background Updates

We've been rolling out automatic background updates for our Insider users for quite some time now. We've decided to enable this for everyone for the March release.
...
### Better support for multi line links
...
This milestone features our first take towards **Log Points**. A Log Point is a variant of a breakpoint that does not "break" into the debugger but instead logs a message to the console. Log points are especially useful for debugging production servers which cannot be stopped.
...
**Note:** Existing `registerTreeDataProvider` API is still supported to contribute data to the view. You can use either `registerTreeDataProvider` or `createTreeView` to contribute `TreeDataProvider` but not both.

**Reveal**

At present, `TreeView` exposes `reveal` API to reveal and select an item in the view.
...
More examples are [here](https://github.com/Microsoft/vscode-extension-samples/blob/master/tree-view-sample/src/ftpExplorer.fileSystemProvider.ts#L254).

**Note:** To access reveal API, the contributed `TreeDataProvider` is required to implement the `getParent` method.
...
The JSON language server that is part of the VS Code JSON extension is based on the [Language Server Protocol (LSP)](https://microsoft.github.io/language-server-protocol/) and can be used in other editors and IDEs as well. To facilitate the usage, the server is now available as a separate [node module](https://www.npmjs.com/package/vscode-json-languageserver). The plan is to also publish the HTML and CSS language servers soon.
...
* [Raghav Katyal (@rakatyal)](https://github.com/rakatyal):  Clear context immediately to avoid sending multiple remove eve… [PR #318](https://github.com/Microsoft/vscode-chrome-debug-core/pull/318)
