...
During November, we spent significant time in the profiler investigating VS Code performance. We identified and addressed several [startup performance issues](https://github.com/Microsoft/vscode/issues?q=is%3Aissue+label%3Aperf-startup+milestone%3A%22November+2017%22+is%3Aclosed) as well as [general performance issues](https://github.com/Microsoft/vscode/issues?q=is%3Aissue+milestone%3A%22November+2017%22+is%3Aclosed+label%3Aperf).

Some noteworthy optimizations that we have done:

- Delay the spawhing of VS Code helper processes until the editor is running so that these processes do not compute with the main process ().

- Cache the manifest extension files (`package.json`) [#28331](https://github.com/Microsoft/vscode/issues/28331).

- Use the faster List widget instead of the tree widget.

- Improve the overall workbench layout by skipping layouts for hidden views and doing better caching of results of expensive computations.

We have improved the entire workbench layout for better performance. Improvements are mostly around skipping layouts for hidden views and doing better caching of results of expensive computations.

bloat removal

### OPEN EDITORS improved

The **OPEN EDITORS** view has been reimplemented to use the list widget instead of the tree for improved performance.
