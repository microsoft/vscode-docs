...
* Due to an issue with wrongly assuming that Code should read via stdin from another program, we now require to pass in a "`-`" as argument to explicitly signal that output is coming in through stdin. For example: `echo Hello World | code -` (Windows) and `ps aux | grep code | code -` (macOS, Linux).
