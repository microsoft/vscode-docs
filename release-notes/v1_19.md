...
### Pipe Output directly into Code

We added support to pipe the output of a command directly into VS Code and have it open as editor:
...
A new setting `workbench.editor.tabSizing` was added to control how tabs should size depending on the space that the outer container provides. The default (`fit`) works as before and will always ensure that the tab shows the entire name of the file. If space gets too little, a scrollbar will appear.

You can change this behaviour by setting this new option to `shrink`. When space is little, tabs will shrink up to a minimum and thus make it less likely that a scrollbar appears.

![Shrinking Tabs](images/1_19/shrink.gif)

### Compare File to Clipboard

You can now compare the contents of the currently opened file with the contents of the clipboard. The command is `workbench.files.action.compareWithClipboard`.
