...
A new command line argument was added to print some useful diagnostics information to the console. The idea is that you can share this information with us via GitHub issues and we are getting a better understanding of how VS Code is running at the time of an issue.

Just run `code --status` from the command line while VS Code is running and you will get something like this:

![Status](images/1_19/status.png)

The status includes information about the environment, all running processes and the counts of some typical files of the workspace.

## Editor

### Smarter IntelliSense

Since a while IntelliSense re-sorts suggestions as you type. This ensures that more relevant suggestions come closer to the top. However, there are cases in which multiple suggestions match equally well, e.g. `con` matches on `confirm`, `console`, `const`, and `constructor` equally well and lexicographic sort decides which suggestion is pre-selected.

Now, IntelliSense remembers what you have selected in such cases. Select `const` for the `con` prefix once and it will be the top match in the future.

![IntelliSense Memory](images/1_19/suggest_mem.gif)

When typing it happens that you mix up neighbouring characters, for instance instead of `console` you might type `cnosole` or `reslut` instead of `result`. With this release we have made IntelliSense more relaxed and it will try a few permutation when scoring suggestions.

![IntelliSense Relaxed](images/1_19/suggest_relax.png)

In the sample above, IntelliSense favours the `console`-suggestion by changing `cnos` to `cons` as that match is a lot stronger then the match of `cnos` on `RTCIceTransportStateChangedEvent`
...
New commands have been added:
   * Fold Regions (`kb(editor.foldAllMarkerRegions)` folds all regions from markers (e.g. `#region`)
   * Unfold Regions (`kb(editor.unfoldAllMarkerRegions)` unfolds all regions from markers
   * Fold All Block Comments (`kb(editor.foldAllBlockComments)`) folds all regions that start with a block comment token (e.g `/*`)
...
A new mode *JSON with Comments* has been added to distinguish between JSON-like files that allow comments and JSON files that follow the JSON spec.
All VSCode configuration file use the new mode an allow comments, while `.json` files default to the standard JSON mode that will report comments as errors.
