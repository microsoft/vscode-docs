...
For simplifying this we have introduced two more fine-grained activation events:
...
In this release `null` has been added to the environment variable's value type with the semantics "remove the environment variable from the environment before launching the target".
...
As a by-product of the new VS Code command line option `--status`, we've created an extension ["vscode-processes"](https://github.com/weinand/vscode-processes) that shows all child processes of VS Code in a custom view and dynamically updates as processes come and go.

To better understand the role of those processes, the extension analyses the process arguments and labels the processes accordingly.

In addition the extension tries to identify processes that can be debugged by VS Code itself and offers for those a debug action in the context menu. All processes can be terminated or killed from the menu as well.
