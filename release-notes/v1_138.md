...
![Screenshot showing a Codex session handed off from the ChatGPT app to VS Code and generating an image.](images/1_138/codex-session-handoff-and-image-generation.png)
...
Create pull requests from [Agent Host sessions](https://code.visualstudio.com/docs/agents/concepts/agent-host) in the {% data variables.copilot.agents_window %} using one form to review and edit generated titles and descriptions, choose draft status, and configure available merge options. Create the pull request directly or send the request to your agent, with your preferred options remembered for next time.

> **Note**: The **Agent Merge** option is experimental and is only available when the setting above is enabled.

![Screenshot showing the Create PR form with editable title and description, draft and merge options, and a split button for creating the pull request.](images/1_138/create-pull-request-form.png)

### Mark completed sessions as done (Preview)

**Setting**: `setting(chat.agentSessions.archiveNudge.enabled)`

When all of an inactive session's pull requests have merged, the {% data variables.copilot.agents_window %} can suggest marking the session as done, keeping finished work out of the way while preserving the conversation for later. A first-use guide shows you where to find **Mark as Done** in the sessions list.

Enable the preview setting above to show these suggestions.

![Screenshot showing a suggestion to mark a session as done after its pull request has merged.](images/1_138/mark-session-as-done.png)
...
**Setting**: `setting(sessions.showApplicationBadge)`

See when agent sessions need your attention without switching back to {% data variables.product.prodname_vscode_shortname %}, with a badge on the macOS dock, Linux launcher, or Windows taskbar. The badge highlights sessions with new results, requests for input, or pull request checks that need attention.

Enable the preview setting above to show the badge.

![Screenshot showing the application icon on the Windows taskbar with a badge indicating one session needs attention.](images/1_138/sessions-application-badge.png)
...
**Setting**: `setting(chat.agentHost.devContainer.enabled)`
...
### Chat background improvements (Experimental)

**Settings**: `setting(chat.agentSessions.preferredDarkBackgroundImageLayout)`, `setting(chat.agentSessions.preferredLightBackgroundImageLayout)`
...
![Screenshot showing the Codicons pattern used as the chat background in the Agents window, with the agent response staying readable on top of it.](images/1_138/agents-chat-background-codicons.png)
...
Response and request surfaces were also reworked so agent responses appear more "bubble-like" making backgrounds with higher contrast feel better.
...
When models are already available for that agent, the notification explains the second path as well: you can select **Download**, or just send a message and let the SDK download as part of the turn. The wording updates in place if models become available while the notification is showing.
