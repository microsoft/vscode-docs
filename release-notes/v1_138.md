...
**Settings**: `setting(chat.automations.enabled)` (Agents window only)

Automations are now enabled by default, making it easier to streamline repetitive tasks. You can also export and import automations to share them across different environments or with your team. Learn more about [Automations](https://code.visualstudio.com/docs/agents/run/automations) in the VS Code docs.
...
* **Choose your subscription**: use Codex with a GitHub Copilot subscription or a ChatGPT subscription. If you sign in to both, switch between Copilot-backed and ChatGPT-backed models from the model picker without losing the current conversation.
  * If you are not signed in to GitHub Copilot and want to use the Codex harness just with your ChatGPT subscription, you also have to enable`setting(chat.agentHost.allowSignedOutWhenUsable)`.
