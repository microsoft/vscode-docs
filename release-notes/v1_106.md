...
Complex features require careful planning, whether you're working solo or delegate it to an agent. This release, we're introducing a [**built-in planning agent**](#plan-agent) that helps you break down tasks step-by-step and build a detailed implementation plan before any code is written. For even finer control, you can create [**custom agents**](#chat-modes-renamed-to-custom-agents) and configure handoffs to other agents, shaping how the AI works for your project's needs and workflows.

To confidently delegate more tasks to AI, **security and transparency** are key. This release, we invested further in making agent tools safer with [**post-approval for external data**](#tool-approvals-and-trust), improved terminal tool auto-approvals, and adoption of the latest [MCP authentication standards](#mcp). For organizations that need governance at scale, the new [**custom MCP registry**](#mcp-server-access-for-your-organization) lets you control which MCP servers can be used across your team, and with [**Linux policy support**](#linux-policy-support), you can now manage VS Code policies across all major platforms.

As a developer, your productivity depends on a [**great editor experience**](#code-editing). This release brings several (and sometimes long-awaited) improvements to make your day-to-day coding smoother. Selecting and copying from deleted lines in the diff editor is now possible, the go to line experience is further enhanced, and comparing [**source control**](#source-control) branches and tags will be a huge time saver. And speaking of feeling great, the entire editor experience has been refreshed with **modernized iconography**.
