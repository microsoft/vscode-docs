...
Learn more about the [Agent Sessions view](/docs/copilot/chat/chat-sessions.md/#agent-sessions-view) in the VS Code documentation.
...
You can now trust MCP servers and extension tools at the source level through the **Allow...** dropdown. This means that you can approve all tools from a specific MCP server or extension at once, rather than having to approve each tool individually.
...
You can now save your chat conversations as reusable prompts with the `/savePrompt` command. When you invoke `/savePrompt` in an active chat session, VS Code generates an prompt file containing a generalized prompt based on your conversation. The editor displays a blue button that lets you save this prompt to a valid location, either at the user level or workspace level.

![Save prompt](images/1_106/save-prompt-prompt.png)

This feature replaces the previous `/save` command and provides a more streamlined workflow for capturing and sharing useful conversation patterns. The generated prompts can be easily reused in future chat sessions or shared with your team.

### Chat modes renamed to custom agents

Chat modes have been renamed to custom agents throughout VS Code.

![Agent Picker](images/1_106/agent-picker.png)

Custom agents are now located in `.github/agents`. The can use the `.agents.md` suffix and can also be used as Github Copilot Cloud Agents and CLI Agent.

Use **Chat: New Custom Agent...** to create a new agent and **Chat: Configure Custom Agents...** to manage them.

If you have existing custom chat modes (`.chatmode.md` files in `.github/chatmodes`), they continue to work and are automatically treated as custom agents. When you open a chat mode file in the editor, a info marker appears on the first line with a quick fix to migrate it to a custom agent file.

### Custom agent metadata

Custom agent `.agent.md` files now accept additional frontmatter so you can describe how an agent should run across environments. Use the `target` attribute to declare the preferred runtime:
- `target: vscode` optimizes the agent for local chat and unlocks `name`, `description`, `argument-hint`, `model`, `tools`, and `handoffs`. Any tool installed in VS Code can be used.
- `target: github-copilot` prepares it for Copilot cloud agents or the GitHub CLI with support for `name`, `description`, `tools`, `mcp-servers`, and `target`. Tools can be `edit`, `search`, `shell` and `custom-agent` as well tools from mcp servers

All agents can be run in all environment. Each environment will ignore unknown attributes and tools.
...
The agent file editor provides validation, code completions, hivers and code actions.
...
We have added a new command to the Source Control Graph context menu, "Compare with..." that allows users to compare a history item in the graph with an arbitrary branch/tag. This allows users to view changes that are in the history item but not in the branch/tag. In the context menu there are shortcuts commands, "Compare with Remote", "Compare with Merge Base" that allows to compare a history item with the remote branch and merge base respectively.

### Repositories selection mode

The Source Control Repositories view shows the list of opened repositories in the workspace and it used to control the repositories shown in the Source Control Changes view. We are looking to expand the functionality of the Repositories view and in preparation for that, we are introducing a setting, `setting(scm.repositories.selectionMode)`, to control the selection mode in the Repositories view. Apart from the new functionality in the Repositories view this also allows us to remove the repository picker in the Graph view's title and have a global repository picker across all source control views. Users can toggle the selection mode using the setting, or from the "..." menu of the Repositories view.

### Repositories explorer (experimental)

We are looking at enhancing the Repositories view and showing additional information for each repository. Users who would like to try out this experimental feature need to set `setting(scm.repositories.selectionMode:single)` and `setting(scm.repositories.explorer:true)`. In the first iteration we have focused on branches and tags. Users can create new branches, tags, view the list of branches and tags and take various actions of each branch, tag (ex: checkout, etc.). In the upcoming milestone we will be adding more information (ex: stashes, remotes, etc). Please give this experimental feature and try and let us know what you think.
