...
We will be removing the `classic` option for `setting(microsoft-authentication.implementation)`. This means that 1.106 is the final version that will have the `classic` option.

The `setting(microsoft-authentication.implementation)` setting has been around to let users opt-out of native brokered authentication for Microsoft accounts if they experienced issues. The values for this setting have been:
...
Based on usage, we will remove the `classic` since it's usage is very low and most issues with the `msal` option are due to the brokering which is remedied with `msal-no-broker`.
...
You can now also use the `setting(github-authentication.preferDeviceCodeFlow)` setting to always try the device code flow for GitHub authentication first.
