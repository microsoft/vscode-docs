...
The **Go to Line** command (`kbstyle(Ctrl+G)`) now supports navigating to a specific character position in a file using the `::` syntax. This is useful when tools report errors at specific character offsets, such as "error at position 599".
...
A toggle in the input box allows you to switch between 1-based (default) and 0-based offset calculations.

TODO@dmitriv - video

### Go to Line improvements

The **Go to Line** command has been improved to handle out-of-range values more gracefully:

- **Line numbers**: Typing a line number larger than the file's line count now navigates to the last line instead of doing nothing.
- **Column numbers**: You can now use negative column numbers to navigate from the end of a line. For example, `:12:-1` takes you to the last character on line 12. Typing a column number larger than the length of the line will now take you to the end of line instead of doing nothing.
...
You can now snooze inline completions directly from the gutter icon. When you hover over the gutter icon for an inline completion, a widget appears with a **Snooze** option. Select Snooze and choose a duration to pause inline completions. The Copilot icon in the status bar shows the snooze status.

TODO@benvillalobos - video
...
The Command Palette now ignores character accents when searching for commands, making it easier to find what you need regardless of your keyboard layout or language preferences. For example, if you're searching for a command containing word `Générer` (French for `Generate`), you can now type `generer` without accents and matching commands will still appear in the results.

This is helpful in cases when using different keyboard layouts or simply mistyping while looking for a command. The filtering is based on [Unicode Normalization Form D](https://www.unicode.org/reports/tr15/#Norm_Forms) and supports all Unicode languages.
...
You can now manage your authentication accounts directly from the Command Palette using the new **Accounts: Manage Accounts** command. This command provides access to account management features even when the Account menu is hidden or not easily accessible.
...
We've enhanced the Quick Pick and Quick Input APIs with several new capabilities that give extension developers more flexibility in creating interactive user interfaces.

TODO@dmitriv - video

#### Proposed API: Toggle button support

Extensions can now add toggle buttons to Quick Pick and Quick Input interfaces through the new `toggles` property on `QuickInput`. This enables scenarios like showing a password visibility toggle in the input box area, allowing users to interact with controls without leaving the quick pick interface.
...
Quick Pick now supports a `prompt` property, similar to the one available in Input Box. The prompt displays persistent text beneath the input box that remains visible while users type, providing helpful guidance or instructions that doesn't disappear when the user starts entering text.
...
Quick Pick items can now display file-type-specific icons through the new `resourceUri` property on `QuickPickItem`. When you provide a resource URI, VS Code automatically derives the appropriate label, description, and icon based on the resource type, matching your current theme's file icon set. This is particularly useful when building file or folder selection interfaces, as users can quickly identify items by their familiar file type icons.
