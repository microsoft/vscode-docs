...
![A chat message showing a terminal command attachment with npm run my_script displayed in a code block, followed by error output text that reads npm error Missing script: my_script. The attachment includes a Show Terminal button to reveal the command in the terminal panel.](images/1_106/terminal-attachment.png)

#### View terminal output inside chat (Experimental)

The new `setting(chat.tools.terminal.outputLocation)` setting controls whether chat-created terminals render automatically. The default `hidden` value prevents the terminals from cluttering the panel.

Every chat terminal invocation now surfaces two actions on the progress element:

- **Show Terminal** or **Focus Terminal** reveals hidden sessions, and with rich shell integration, scrolls directly to the relevant command. With basic or no shell integration, the action still focuses the correct terminal tab.
- **Show Output** opens a color-aware inline viewer that mirrors the terminal output. The output view expands automatically when a command exits with a non-zero code.
...
In this release, we have also shipped an initial integration with the Copilot CLI.

You can create new CLI agent sessions in a chat editor or an integrated terminal.
...
We also refreshed the configuration experience. Provider descriptions now surface directly inside `setting(terminal.integrated.suggest.providers)`, and the Settings UI exposes each provider as a checkbox so you can toggle completions without editing JSON.

When you expand IntelliSense details (`kbstyle(Ctrl+/)`), Git branches display their latest commit message to help you confirm the right target at a glance.
...
The terminal completion provider API is now stable, so providers no longer require the proposed API flag.

```ts
import * as vscode from 'vscode';

export function activate(context: vscode.ExtensionContext) {
  const provider: vscode.TerminalCompletionProvider = {
    provideTerminalCompletions(completionContext) {
      const checkoutPrefix = 'git checkout ';
      if (!completionContext.commandLine.startsWith(checkoutPrefix)) {
        return undefined;
      }

      const typedBranch = completionContext.commandLine.slice(checkoutPrefix.length);
      const start = completionContext.cursorIndex - typedBranch.length;
      const range: readonly [number, number] = [start, completionContext.cursorIndex];

      const mainBranch = new vscode.TerminalCompletionItem('main', range, vscode.TerminalCompletionItemKind.Module);
      mainBranch.detail = 'Switch to the main branch';

      return { items: [mainBranch] };
    }
  };

  context.subscriptions.push(
    vscode.window.registerTerminalCompletionProvider('example.git-helper', provider, {
      triggerCharacters: [' '],
      description: 'Adds completions for git checkout commands'
    })
  );
}
```
