 ...
 **Note:** we are still thinking about other ways of how to present pinned tabs. If you have an opinion, feel free to throw your ideas into the existing issues for [showing a secondary tab bar](https://github.com/microsoft/vscode/issues/98160) or having a setting to [show more context for pinned tabs](https://github.com/microsoft/vscode/issues/98161).
 ...
 ### explorer.autoReveal focus without forcing a scroll
...
* Now it is possible to save an automatically detected debug configuration by clicking on the `Configure Debug Configuration in launch.json` button in the quick pick
* Debug dropdown now remembers the previously chosen automatic debug configuration
...
The VS Code debugging UI shows the `Step Into Targets` action in the context menu when debug session is stopped for debug extensions that support this capability. After this command is executed a user can choose from a list of targets into which exact target to step into.

For now, only the Mock Debug extension implements Step Into Target support, but soon other debug extensions will follow - like the Julia language debugger.
...
Once a breakpoint is hit and the VS Code tab is not focused we update the tab title to include 🔴.
This should make it easier for users to discover that a breakpoint is hit in one of the inactive tabs where VS Code is running. As soon as the tab is focused we no longer show the breakpoint indicator.
