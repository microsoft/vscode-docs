...
With the custom editor API finalized with support for binary editors, we have developed a [hex editor extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode.hexeditor) to provide a native hex editing experience.

![Hex Editor](images/1_46/hex-editor.gif)

This extension is still under development and currently only supports opening and navigating files as hex as we work on the navigational experience and then move on to support editing. You can try out the extension and provide feedback or file issues against the [vscode-hexeditor  repository](https://github.com/microsoft/vscode-hexeditor).
