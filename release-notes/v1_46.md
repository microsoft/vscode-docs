...
Enabling the `workbench.list.smoothScrolling` setting will make scrolling in lists and trees much smoother than before, in hardware which lacks smooth scrolling (e.g. discrete mouse wheel on Windows).
...
You can now use the `screencastMode.fontSize` setting to configure the font size in pixels that is being used in screencast mode.

## Integrated Terminal

### Improved link support

The link preview from last month has replaced the old system. Links now use a more robust implementation behind the scenes enabling::
...
* `font family name` - name of the font family to be used in the input field (ex: `Cascadia Code`)
...
A warning triangle in the status bar will appear when the current editor would like to use schemas that can not be downloaded. Schema based features like validation, code completion and hovers will then be limited.
...
In previous iterations, we introduced a preview of a new accounts management UI alongside the preview of settings sync. Now, this UI will show up by default. Extensions using the new proposed [Authentication Provider API](https://github.com/microsoft/vscode/issues/88309) can surface requests to sign in here, and you can view your accounts and manage extension access. There are currently a limited number of extensions using this API, we will work on increasing adoption once the API is stable.
...
We also have new color theme option for inputs. You can now set the foreground color for active input options via `inputOption.activeForeground`. These appear in the Search viewlet and Find widget (editor, terminal, debug console).
...
The `ExtensionContext.EnvironmentVariableCollection` API is now stable, this API allows extensions to contribute to terminal environments.
...
The Language Server libraries maintained in the [vscode-languageserver-node repository](https://github.com/microsoft/vscode-languageserver-node) have been split into a common, node and browser part to support using the LSP client and server npm modules in a Web browser via [webpack](https://webpack.js.org/). Note that this is a breaking change. Please constult the [Readme](https://github.com/microsoft/vscode-languageserver-node/blob/master/README.md) for further explanations.
...
There are new official Insider builds for Windows for ARM 64-bit, available in https://code.visualstudio.com/insiders/

These go great with the Microsoft Surface Pro X, give a try and let us know what you think.
