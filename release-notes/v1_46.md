...
* **[Add GitHub remotes](#add-remote-from-github)** - Easily add GitHub remotes to your local repository.
...
* **[Windows ARM64 support](#windows-arm64-insiders)** - Try out the new VS Code Insiders ARM64 builds.
* **[VS Code at Build 2020](#documentation-and-extensions)** - Sessions on Tips & Tricks, Remote Development, and the history of VS Code.
...
Perhaps you really would prefer a view from the Side Bar to be in the Panel or vice versa. To do this, simply drag a view by its header or an entire group by its icon or title from its current location and move it to the desired location. Before this work was done, there was a setting for moving Search from the Side Bar to the Panel. Now, that setting is obsolete and drag and drop can be used instead. From the keyboard, the commands **View: Move View** (`workbench.action.moveView`) and **View: Move Focused View** (`workbench.action.moveFocusedView`) can be used.
...
The workbench will now dynamically allow opening links to any GitHub remotes in the workspace. Additionally, if you have signed in with GitHub, all links to pages under your GitHub profile will be trusted.
...
Continuing with the recent tasks performance improvements, **Run Build Task** no long always asks extensions for tasks. When you have a default build task configured in your folder's `tasks.json`, **Run Build Task** will try to resolve only that task instead of finding all tasks from all extensions before running the default build task.
