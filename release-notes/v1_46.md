...
In this milestone VS Code has started to leverage this feature (for debug extensions that support it). When a debugging has stopped on a statement the context menu has a new action **Step Into Targets**. This action presents a list of all targets (e.g. functions) that are available at the current stack frame. Selecting one of them will step into that target.
