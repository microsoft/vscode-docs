...
For several iterations, we have announced progress on making our layout more flexible. As of this iteration, the set of features we have been working on are now ready for general use. Below is an overview of these features.

#### Moving Views Between Side Bar and Panel

Perhaps you really, would prefer a view from the Side Bar to be in the Panel or vice versa. To do this, simply drag a view by its header or an entire group by its icon or title from its current location and move it to the desired location. Before this work was done, there was a setting for moving Search from the Side Bar to the Panel. Now, that setting is obsolete and drag and drop can be used instead. From the keyboard, the commands **View: Move View** (`workbench.action.moveView`) and **View: Move Focused View** (`workbench.action.moveFocusedView`) can be used. Below is a demonstration of dragging Search to the Panel and Problems into the Side Bar.

![Moving Views Between Panel and Side Bar](images/1_46/move-views-panel-sidebar.gif)
...
Alternatively, you may want some views together that come from different extensions or the default groups of built-in views aren't quite right for you. You can both move views into existing groups or create new groups for a select set of views. This works across the Side Bar and Panel just as before. Below are a couple of examples of this.

![Moving Timeline to Source Control](images/1_46/timeline-to-scm.gif)
...
*Dragging the Watch view from Run next to Debug Console in Panel*

![Custom View Group in Side Bar](images/1_46/custom-view-group.gif)

*Creating a group in the Side Bar with several views from different locations*
...
#### Updates for Extension Authors Contributing Views or View Containers

When views are moved around the workbench, they sometimes need to be presented differently, either with an icon or extra context if they they aren't in their default location. When contributing a view, authors may now provide an `icon` property and a `contextualTitle`. If not provided, these will default to the icon and title of the view container to which they are contributed.

Lastly, extension authors can now start contributing view containers directly to the `panel` as opposed to `activitybar` as outlined [here](https://code.visualstudio.com/api/extension-guides/tree-view#view-container).
...
The workbench will now allow dynamically allow opening links to any GitHub remotes in the workspace. Additionally, if you have signed in with GitHub, all links to pages under your GitHub profile will be trusted.
...
VS Code currently recognizes CR (Carriage Return), LF (Line Feed) and CRLF as line terminators. But programming languages have different definitions for what constitutes a line terminator. This varies across languages, for example LS (Line Separator) and PS (Paragraph Separator) are line terminators in C# and JS, but not in HTML, PHP or Java. This difference can introduce problems when VS Code communicates with a language server, since various concepts are communicated between VS Code and the language server using `(line;char)` coordinates, which will point to different locations in the file based on what each party believes to be a line terminator.
...
Continuing the with the recent tasks performance improvements: **Run Build Task** no long always asks extensions for tasks. When you have a default build task configured in your folder's `tasks.json`, **Run Build Task** will try to resolve only that task instead of finding all tasks from all extensions before running the default build task.
...
Refactorings for JavaScript and TypeScript, such as **Extract to method** and **Move to new file**, now try to better preserve the original formatting of the refactored source code. This includes any newlines present in the selection:
...
The **Adding missing import** Quick Fix for JavaScript and TypeScript can now be marked as the [preferred](https://code.visualstudio.com/docs/editor/refactoring#_keybindings-for-code-actions) fix for an error. This lets you add the missing import with a single keypress using `kb(editor.action.autoFix)`.
...
* **Better tab disambiguation:** The debug session names more accurately disambiguate multiple tabs. Additionally, we added a **Focus Tab** action in the context menu for the debug session in the Call Stack view, which you can use to make sure you're looking at the right page.
...
![Problems panel](images/1_46/notebook-problems.png)
...
![Join/Split Notebook cells](images/1_46/notebook-join-split-cells.gif)

*Theme: [GitHub Dark Theme](https://marketplace.visualstudio.com/items?itemName=GitHub.github-vscode-theme)*

**Retained Notebook**

Pinned or dirty Notebook editors are now persistent in the workspace, users won't lose the working status in the Notebook no matter whether the Notebook is moved to the background or moved to another editor group.
...
Work continues on the [GitHub Pull Requests and Issues](https://marketplace.visualstudio.com/items?itemName=GitHub.vscode-pull-request-github) extension, which allows you to work on, create, and manage pull requests and issues. Some highlights for this release include:

- Better looking labels in issue hovers.
...
To learn about all the new features and updates you can see the full [changelog for the 0.17.0](https://github.com/microsoft/vscode-pull-request-github/blob/master/CHANGELOG.md#0170) release of the the extension.

### Remote Development

Work continues on the [Remote Development extensions](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.vscode-remote-extensionpack), which allow you to use a container, remote machine, or the [Windows Subsystem for Linux](https://docs.microsoft.com/windows/wsl) (WSL) as a full-featured development environment.

Feature highlights in 1.46 include:
...
With the custom editor API being finalized with support for binary editors, we have developed a hex editor extension to provide a native hex editing experience.

![Hex Editor](images/1_46/hex-editor.gif)

This extension is still under development and currently only supports opening and navigating files as hex as we aim to solidify the navigational experience and then move on to support editing. You can currently try out the extension and provide feedback or file issues against [this repository](https://github.com/microsoft/vscode-hexeditor).
...
The built in Git extension [offers API](https://github.com/microsoft/vscode/blob/master/extensions/git/src/api/git.d.ts) for extension to use. The existing API exposed a way to get `log` results for a `Repository`. Now, that `log` can be constrained by a `path`.
...
There is also now Git extension API to `getBranches` based on a `BranchQuery`:
...
Our [extension samples](https://github.com/Microsoft/vscode-extension-samples) have all been updated to use [ESLint](https://eslint.org) for linting instead of the now deprecated TSLint. If your  extension is still using TSLint, take a look at the [TypeScript ESLint guide](https://eslint.org) and our newly updated extension samples to see how you can migrate to use ESLint instead.

## Documentation and extensions

### Visual Studio Code at Build

The VS Code team was busy at the [Microsoft Build 2020](https://mybuild.microsoft.com/) conference and you can find an overview of the VS Code sessions and online talks in this [VS Code at Build](https://code.visualstudio.com/blogs/2020/05/14/vscode-build-2020) blog post.

Don't miss these talks by VS Code team members:
...
![Azure Databases extension](images/1_46/azure-databases-extension.png)
