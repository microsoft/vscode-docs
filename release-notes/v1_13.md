...
Set `emmet.useNewEmmet` to `true` to start using this new feature. This feature is best used with the suggestion documentation fly-out expanded where you can preview the expanded abbreviation as you type.

![Emmet abbreviation expansion in autocomplete](images/1_13/emmet.gif)

When using this setting, the `kbstyle(Tab)` key will no longer expand the abbreviation. You can still use the **Emmet: Expand Abbreviation** command or bind a new keyboard shortcut to the `editor.emmet.action.expandAbbreviation` command.
