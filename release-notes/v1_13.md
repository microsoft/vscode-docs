...
* **[Changes to settings defaults](#changed-setting-defaults)** - We're enabling several useful features by default such as extensions auto-update, editor drag and drop and minimaps.
...
* **[File links in exception stack traces](#file-link-detection-in-exception-peek-ui)** - Jump directly to source code from debugging stack traces.
...
* **[Tasks](#tasks)** - Autodetect and customize Gulp and Grunt tasks to run in VS Code.
...
We received a lot of feedback for our workbench [theming](https://code.visualstudio.com/docs/getstarted/theme-color-reference) support and are happy to see more and more themes adopting the workbench colors! During this milestone, we added almost 50 new colors as well as did some tweaks to existing colors. These colors can be set by themes or directly by the user with the `workbench.colorCustomizations` setting.
