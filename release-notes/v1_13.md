...
In the meantime, you can now scroll beyond the first line when the Find Widget is visible and then The Find Widget won't cover the matched results. We'll automically scroll the window a little bit to make sure the matched results are not covered by the Find Widget when you are navigating between them.

![Uncover macthed results](images/1_13/uncover-matched-results.gif)
...
Besides, we added a new command `toggleFindInSelection` (`kb(list.focusFirstChild)`) for Toggle Find In Selection then you can keep your hands on the keyboard when switching all Find options.
...
Unicode characters in the terminal are now sized explicitly which means that applications like [vtop](https://www.npmjs.com/package/vtop) which make extensive use of these characters should now render correctly.
...
In the same time, congratulations and thank you to Brazilian Portuguese community localization team! With the effort from team, localization is now completed in Transifex. Translation has been integrated into [Insider build](https://code.visualstudio.com/insiders) for testing. Based on the progress of validation, in near future, we will integrate into release for Brazilian customers.
