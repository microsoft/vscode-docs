...
Once autocomplete/suggestions is triggered, press `kb(editor.action.toggleSuggestionDetails)` to view the docs for the suggestion item in focus as before. The docs will now expand to the side instead of being overlaid on the suggest widget, enabling you to read the docs and navigate the list at the same time.

Once the docs are expanded, they stay expanded (across VS Code windows, sessions and updates) everytime autocomplete/suggestions is triggered, until you explicitly close them either using the close button or by pressing `kb(editor.action.triggerSuggest)` again.

For keyboard centric users, who want to navigate up and down the long docs, press `kb(editor.action.triggerSuggestionFocus)` to move the focus to the docs such that it can now recieve keyboard shortcuts for Page Up/Down actions.

For screenreader users, once the docs are expanded, navigating the suggestion list will read out the label and docs (if any) of the item in focus.

### Emmet abbreviation expansion now in suggestion list

Until now, the default behavior for expanding an emmet expansion has been using the tab key. There were two issues with this design:
- Many unexpected emmet expansions occured when the user wanted to just add an indent. 
...
Both of these issues are now solved by having the expanded emmet abbreviations show up in the suggestion list and freeing up the tab key for what it was meant to do i.e indenting.
...
After using this setting, the tab key will no longer expand the abbreviation. You can still use the `Emmet: Expand Abbreviation` command or bind a new short cut for it.
