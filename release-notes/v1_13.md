...
One thing you may notice right away in the May release is that we've changed the default value of several settings. Features such as minimap, icon themes, and indent guides have been off by default and we think they are so useful, we want to showcase them.
...
------- | ------- | ----
`editor.minimap.enabled` | true | Show file minimap on right gutter
...
`window.openFilesInNewWindow` | false | Open files in the running VS Code instance

Of course, you can still configure VS Code to your preference with user or workspace [settings](https://code.visualstudio.com/docs/getstarted/settings).

### Adding multiple cursors with Ctrl/Cmd + Click

We have introduced a new setting, `editor.multiCursorModifier`, to change the modifier key for applying multiple cursors to `kbstyle(Cmd + Click)` on macOS and `kbstyle(Ctrl + Click)` on Windows and Linux. The setting can be set to `ctrlCmd` or the existing default `alt`. There's also a new menu item **Use Ctrl+Click for Multi-Cursor** in the **Selection** menu to quickly toggle this setting.
...
`tab.unfocusedActiveForeground`|Active tab (tabbed header) foreground color in an inactive editor group.
`tab.unfocusedInactiveForeground`|Inactive tab (tabbed header) foreground color in an inactive editor group.
...
### Autocomplete/Suggestion list and docs side by side

Once autocomplete/suggestions is triggered, press `kb(toggleSuggestionDetails)` to view the documentation for the suggestion item in focus as before. The documentation will now expand to the side instead of being overlaid on the suggest widget, enabling you to read the documentation and navigate the list at the same time.

![auto expanded docs in autocomplete](images/1_13/suggest.gif)

Once the documentation fly-out is expanded, it will stay expanded (across VS Code windows, sessions and updates) every time autocomplete/suggestions is triggered, until you explicitly close it either using the close button or by pressing `kb(editor.action.triggerSuggest)` again.
...
### Emmet abbreviation expansion now in suggestion list
...
Both of these issues are now solved by having the expanded Emmet abbreviations show up in the suggestion list and freeing up the `kbstyle(Tab)` key for what it was meant to do, indenting.

Set `emmet.useNewEmmet` to `true` to start using this new feature.
This feature is best used with the suggestion docs expanded where you can preview the expanded abbreviation as you type.

![emmet abbreviation expansion in autocomplete](images/1_13/emmet.gif)

After using this setting, the `kbstyle(Tab)` key will no longer expand the abbreviation. You can still use the `Emmet: Expand Abbreviation` command or bind a new short cut for it.

You will see 2 kinds of suggestions in html like files
...
- All possible abbreviation whose prefix is the text that has been typed (you can turn this off by setting `emmet.showAbbreviationSuggestions` to `false`). For eg: `a`, `a:link`, `a:mail`, `area` etc are suggested when you type `a`. This is educational and helps in learning all the emmet abbreviations.

In css/less/scss files you will see just the first kind above.

To implement this feature, we replaced the use of the single monolithic [Emmet library](http://github.com/emmetio/emmet) with the use of smaller re-usable [Emmet modules](https://www.npmjs.com/%7Eemmetio).
In doing so, most of the Emmet commands have been re-implemented. If you see any changes in the behaviour of any of the Emmet commands, please log an issue. We hope to complete this work in the next milestone, after which the setting `emmet.useNewEmmet` will be deprecated and the new model will be made default.
...
You can opt into the new task version 2.0.0, please note that this version is currently a preview and still under active development. We make it available to get early feedback. To enable the preview set the `version` attribute to "2.0.0". :
...
The initial recipes cover [Debugging TypeScript in a Docker Container](https://github.com/weinand/vscode-recipes/tree/master/Docker-TypeScript) and [Developing the MERN Starter in VS Code](https://github.com/weinand/vscode-recipes/tree/master/MERN-Starter).
...
You find the a list of all known theme colors [here](https://code.visualstudio.com/docs/getstarted/theme-color-reference).
...
As part of this milestone we automated [the smoke test](https://github.com/Microsoft/vscode/wiki/Smoke-Test) that used to be carried out manually. Every previously manual step of it was automated with the use of [Spectron](https://electron.atom.io/spectron) testing framework with detailed progress tracked in [this issue](https://github.com/Microsoft/vscode/issues/25291).

Smoke test assists us in verifying if VS Code basic features are working before publishing a release. Its automation enables us to save more time on testing process, instead putting more resources and focusing on development of new features.
...
* [ESLint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint): Major performance improvements to lower CPU load when linting large JS files with lots of code actions.
* [VSCodeVim](https://marketplace.visualstudio.com/items?itemName=vscodevim.vim): Code [improvements](https://github.com/VSCodeVim/Vim/pull/1642) to make it easier to contribute to the project.
* [PHP language server](https://github.com/felixfbecker/php-language-server): We are getting close to merge [the pull request](https://github.com/felixfbecker/php-language-server/pull/357) to adopt the [tolerant PHP parser](https://github.com/Microsoft/tolerant-php-parser).
