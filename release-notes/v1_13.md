...
Of course, you can still configure VS Code to your preference with user or workspace [settings](https://code.visualstudio.com/docs/getstarted/settings) (**File** > **Preferences** > **Settings** or keyboard shortcut `kb(workbench.action.openGlobalSettings)`).
...
If you have `editor.quickSuggestions` turned off, then you can try one of the below options to get Emmet expansion.
...
* Run the command **Emmet: Expand Abbreviation** explicity.
* `kbstyle(Tab)` key will no longer expand the abbreviation by default. Bind a keyboard shortcut for the command **Emmet: Expand Abbreviation** and use it.
