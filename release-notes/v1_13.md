...
You can opt into task version 2.0.0 by changing the version number of `tasks.json` file to `"2.0.0"`. Please note that task version 2.0.0 is still under active development and that things might change in future releases. We decided to provide early access to it to get feedback as early as possible. If you don't have a `tasks.json` and are fully relying on task autodetection, you can simply create a `tasks.json` file with the following minimal content:
