...
### Suggestion list and docs side by side

You can now view the docs for the suggestion item in focus side by side to the suggestion list. Once the suggestion list is triggered, press `kb(editor.action.triggerSuggest)` to view the docs for the suggestion item in focus as before. The docs will now expand to the side instead of getting overlaid on the suggest widget, enabling you to see the docs and navigate the list at the same time.

Once the docs are expanded, they stay expanded (across VS Code windows, sessions and updates) until you explicitly close them either using the close button or by pressing `kb(editor.action.triggerSuggest)` again.
...
Until now, the default behavior for expanding an emmet expansion has been using the tab key. The downside of this was there were many unexpected emmet expansions happening when the user wanted to just add an indent. On the other hand, there were also items from the suggestion list getting inserted when the user was expecting the emmet abbreviation to be expanded. Both of these issues are now solved by having the emmet abbreviation expansion show up in the suggestion list and freeing up the tab key for what it was meant to do i.e indenting.

This is an opt-in feature at the moment. Set `emmet.useNewEmmet` to `true` to start using this new feature and give us feedback.
