...
### Adding multiple cursors with Cmd + Click (macOS) or Ctrl + Click (Windows and Linux)

We have introduced a new setting, `editor.multiCursorModifier`, which can be set to `ctrlCmd` or `alt` to change the modifier key for adding multiple cursors. There's also a new menu item **Use Ctrl+Click for Multi-Cursor** in the **Selection** menu to quickly toggle this setting.
...
Until now, the default behavior for expanding an Emmet expansion has been using the `kbstyle(Tab)` key. There were two issues with this design:
...
This is an opt-in feature at the moment. Set `emmet.useNewEmmet` to `true` to start using this new feature.
...
To implement this feature, we replaced the use of the single monolithic [Emmet library](http://github.com/emmetio/emmet) with the use of smaller re-usable [Emmet modules](https://www.npmjs.com/%7Eemmetio). We were also able to extract the Emmet features into an extension (built-in) and move away from the tight coupling of Emmet in VS Code.

Note: In using these modules, most of the Emmet commands have been re-implemented. Please give these commands a try and provide us feedback. This work will be completed in the next milestone after which we will be using the new Emmet model by default for all.
