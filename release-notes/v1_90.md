...
This milestone we have added a set of workbench commands to help set keybindings to focus the next/previous Source Control input field - `workbench.scm.action.focusNextInput`, `workbench.scm.action.focusPreviousInput` as well as commands to focus the next/previous resource group within a repository - `workbench.scm.action.focusNextResourceGroup`, `workbench.scm.action.focusPreviousResourceGroup`.
...
We now support basic IntelliSense for code blocks generated by Copilot. This lets you use many of the same IntelliSense tools you use in normal VS Code editors to learn more about copilot responses:

![Hover IntelliSense in Copilot chat code block](images/1_90/copilot-hover-intellisense.png)

Currently support IntelliSense features include:

- Go to definition using `kbstyle(ctrl)` `kbstyle(click)` / `kbstyle(cmd)` `kbstyle(click)` or `kb(editor.action.revealDefinition)`.
- Hovers
...
IntelliSense for TypeScript, JavaScript, HTML, and CSS code blocks is avalible by default of the box. For additional language support, try installing an extension for that language although at this time not every language extension currently supports code block IntelliSense. Po please file feature requests for any languages that aren't working.

#### Better links chat responses
...
Clicking on these will take you to the corresponding file or symbol.
...
We continued improving our support for the upcoming TypeScript 5.5 rleaes. Check out the [TypeScript 5.5 beta blog post](https://devblogs.microsoft.com/typescript/announcing-typescript-5-5-beta/) and [iteration plan](https://github.com/microsoft/TypeScript/issues/57475) for details on this release.
...
To enable this feature, make sure you are using TypeScript 5.5 in your workspace and then set `javascript.experimental.updateImportsOnPaste` `typescript.experimental.updateImportsOnPaste`

## API

### Chat and Language Model API

We have finalized APIs that allow extensions to participate in chat and to access language models. See the [extension sample](https://github.com/microsoft/vscode-extension-samples/tree/main/chat-sample) and the [chat extensions doc page](https://code.visualstudio.com/api/extension-guides/chat) for more information.

> ‼️ Please note that these APIs are finalized but currently only available in VS Code Insiders ‼️

#### Chat Participants

This API allows extensions to extend GitHub Copilot Chat with a chat participant that can be invoked in the chat window with `@`. The participant can reply to user requests with markdown, a file tree, buttons to run VS Code commands, and other types of content.
