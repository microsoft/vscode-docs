...
We made first steps in significantly improving the performance of Quick Open particularly for large workspaces such as Chromium. On MacOS (<kbd>⌘+P</kbd>) and on Linux (<kbd>ctrl+P</kbd>), quick open on a Chromium workspace takes now less than half the time it took before. On Windows (<kbd>ctrl+P</kbd>) it originally took twice as long as on MacOS and now is as fast as on MacOS. So, it's now 4 times faster than before. And we are not done yet; expect more gains in the next iteration.
...
Double clicking on the debug toolbar drag icon will center the debug toolbar. This makes it easy to restore the default state.
...
- **Move cursor:** We have added a new command that moves the cursor to different logical positions in the editor - [9143](https://github.com/Microsoft/vscode/issues/9143)
```
commands.executeCommand('cursorMove', {to: 'up', by: 'wrappedLine', value: '2'})
```

- **Move active editor:** We have added a new command that moves the active editor across groups or across tabs with in its group  - [8234](https://github.com/Microsoft/vscode/issues/8234#issuecomment-234573410)
```
...
* [Georgios Kalpakas](https://github.com/gkalpak): docs(LanguageConfiguration): fix typo (interfaces --> interface) [#8703](https://github.com/Microsoft/vscode/pull/8703)
