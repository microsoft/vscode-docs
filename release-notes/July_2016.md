...
We made the first steps in improving the performance of Quick Open, particularly for large workspaces such as Chromium. On Mac OS X (`kbstyle(⌘+P)`) and on Linux (`kbstyle(Ctrl+P)`), quick open on a Chromium workspace takes now less than half the time it took before. On Windows (`kbstyle(Ctrl+P)`), it originally took twice as long as on Mac OS X and now it is just as fast. We are not finished; expect more gains in the next iteration.
...
* [Giorgos Retsinas (@elemongw)](https://github.com/elemongw): [Mac] Implement `kbstyle(Ctrl+P)` and `kbstyle(Ctrl+N<)` for up and down navigation. [PR #7316](https://github.com/Microsoft/vscode/pull/7316)
