...
* [Python environments](#python-environment-discovery-using-python-environment-tools) - Enhanced environment discovery with python-environment-tools.
...
We've added more options to give you more flexibility when installing extensions:

* Install an extension without syncing it
* Install a specific version of an extension even before installing it.
...
### Quick toggling off of read-only files

Toggle off read-only for files that have been configured as read-only through `files.readonlyInclude` from the read-only editor message.

![alt text](images/1_91/quick-toggle-readonly.png)
...
The Notebook Find Widget now ships with "Find in Selection" for textual selections as well as cell selections. This behavior is enabled by default, and can be controlled via the button within the widget. Based on the context of your selection, toggling this button on while scope your find query to the selected cell, cells, or multiple lines within a cell.

<video src="images/1_91/notebook-find-in-selection.mp4" title="Find in Selection using Notebook Find Widget" autoplay loop controls muted></video>
...
### New custom glyphs for branch, line number and lock Powerline symbols

The terminal now supports custom glyphs for the branch, line number, and lock Powerline symbols. Like the [other custom glyphs](https://code.visualstudio.com/docs/terminal/appearance#_custom-glyphs) we support, these symbols will work without the need to configure a font when [GPU acceleration](https://code.visualstudio.com/docs/terminal/appearance#_gpu-acceleration) is enabled:

![Characters 0xe0a0, 0xe0a1 and 0xe0a2 will now render with their Powerline symbols](images/1_91/terminal-powerline.png)

They will also scale perfectly with the cell when font size, line height or letter spacing is adjusted:
...
![](images/1_91/debug-shadowed.png)
...
This will only display when the Copilot extension is installed and there is a single terminal that has not had any interaction. It's easy to dismiss the error permanently by right clicking it and it will also dismiss when terminal inline chat is used for the first time.
...
Extensions that start a debug session from a test run can now pass that run into the `DebugSessionOptions` when they call `vscode.debug.startDebugging`. When passed, lifecycle actions such as restarting the debug session will create a new test run as well.
...
* Addition of an source location to `evaluate` requests. VS Code implements this and will pass the location when evaluating hovers and inline values.
