...
You can now hide actions from tool bars. Simply right-click on any action in a tool bar and select its hide-command or any of the toggle commands. Hidden actions will be moved to the `...` menu and can be invoked from there. To restore a menu select the "Reset Menu" command, to store all menus select "F1 > Reset All Menus".

[Hide actions from editor toolbar](images/1_72/hide-actions.mp4)
...
VS Code now shows extensions updated during the last 7 days in the **Recently Updated** section of **Updates** view.

![Recently Updated](images/1_72/extensions-recently-updated.png)

### Extensions requiring attention

VS Code now shows extensions requiring attention sorted to the top of the default **Installed** section of **Extensions** view. This includes extensions that require update or have been updated or disabled and require VS Code reload.
...
Please note that installing a specific version of an extension will also ignore updates for that extension.
...
You can now view search results in a tree! Just toggle between the list and tree view by clicking on the list/tree icon in the top corner of the search viewlet.
![search-tree-view](images/1_72/search-tree-view.gif)
_Theme: [Night Owl Light](https://marketplace.visualstudio.com/items?itemName=sdras.night-owl)_
...
VS Code shows Settings Profiles indicator at the bottom of the Activity bar. By default, the first two characters of the profile name is shown in the indicator. You can also customise the short name to any two characters or an emoji or a codicon. A codicon can be referenced by using the `$(codicon-id)` syntax.

![Settings Profiles Indicator](images/1_72/settings-profiles-indicator.png)

Following menu is shown when you click on this indicator.
...
#### Increased threshold for for issues to 7500 characters
...
1. Certain browsers that have a hard limit of the number of characters a URL could be are no longer supported (EOL).
...
If you open the running extensions using the command: `Developer: Show Running Extensions` and click on the `Report Issue` button, this will now take you to the built-in issue reporter. Before, this would take you to a GitHub issue and ask you to paste and it caused a lot of issue spam in extension repositories. For example, [this issue](https://github.com/ritwickdey/vscode-live-server/issues/2600) along with many many others in many other extension repositories. Now, these issues will go through the built-in issue reporter and will give the user the ability to make quality issues for extensions.
...
"Cells" with a python file denoted with `# %%` markers can now be folded in the editor.
...
Feel free to provide your [feedback here](https://github.com/microsoft/vscode-python/discussions/18888). Note the API is still not finalized yet, so may still change without providing backwards compatibility based on the feedback.
...
#### 1. Editing notebooks with `WorkspaceEdit`
...
#### 2. Performing snippet edits with `WorkspaceEdit`
...
[Refactoring using Snippet Edit](images/1_72/workspace-edit-snippet.mp4)

#### 3. Creating files with contents with `WorkspaceEdit`
...
The new webview context menu API makes it easy for advanced [webviews](https://code.visualstudio.com/api/extension-guides/webview) to customize the context menus that shows when a user right-clicks inside a webview. This new contribution point uses VS Code's normal context menus, so custom menus fit right in with the rest of the editor. Webviews can also show custom context menus for different sections of the webview.
...
The log ouput channels created by extensions are shown as separate entries in the Output dropdown view. Messages written to this log output channel are represented in the same format as the VS Code log messages.
...
* run VS Code desktop with the `--enable-coi`-flag, e.g `code --enable-coi`, or
* in web append the `vscode-coi`-query

To check that cross origin isolation is enabled use the Application-tab of devtools or check the value of the [`crossOriginIsolated`](https://developer.mozilla.org/en-US/docs/Web/API/crossOriginIsolated) global variable.
...
To analyse performance we make heavy use of browser's devtool and OS inspection tools. However, we also have some tools that are baked into VS Code. We took time to document them here so that more people can benefit from them: https://github.com/microsoft/vscode/wiki/Perf-Tools-for-VS-Code-Development
