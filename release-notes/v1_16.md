...
* **[JavaScript/TypeScript refactoring](#extract-method-and-extract-function)** - Easily extract source code into methods and functions.
* **[HTML tag auto close](#html)** - Improved editor productivity with automatic close tags.
* **[Color picker in HTML](#color-picker)** - You can now use the VS Code color picker in HTML files.
* **[Integrated Terminal Quick Open](#terminal-quick-open-improvements)** - Create or jump to terminals from VS Code Quick Open.
* **[Emmet preferences](#emmet-preferences)** - Customize your Emmet abbreviation expansions.
...
* **[Preview: Multiple SCM providers](#preview-source-control)** - Along with multiple folders, you can now have multiple source control providers (Insiders build).
...
* **[Workbench](#workbench)** - Drag and drop from Search and Problems, more theme colors.
* **[Editor](#editor)** - Smooth scrolling, undo stack detects file changes.
...
The setting `emmet.preferences` as documented in [Emmet Preferences](https://docs.emmet.io/customization/preferences/) was discontinued in the last release as the new modular approach in Emmet 2.0 did not support them.

We are bringing back support for a few of these preferences:
...
The Integrated Terminal can now be launched via the **Terminal: Switch Active Terminal** command or by typing "term " (with a trailing space) in **Quick Open** now has the ability to create new terminals and kill existing terminals.

![Terminal quick open improvements](images/1_16/terminal-quick-pick.png)

### Session names on Windows

The mechanism for extracting the session name from the terminal shell which was added in 1.15, no longer needs to spin up a new process. This should reduce CPU usage when using the terminal.
...
### Updated Node.js Deployment

We continue to refine the [Node.js Deployment to Azure](https://code.visualstudio.com/docs/nodejs/nodejs-deployment) tutorial so you can get your application running in the Cloud.
...
### Multi Root Workspace API adoption

We've created a guide for how to make your extension ready for multi-root workspaces. See [the guide](https://github.com/Microsoft/vscode/wiki/Extension-Authoring:-Adopting-Multi-Root-Workspace-APIs) for more information. All the APIs discussed are considered stable and available. Note however that multi-root workspaces are currently only enabled in VS Code Insiders release.
...
In additiion to the the guide, there are new samples from our [samples repository](https://github.com/Microsoft/vscode-extension-samples) that exercise the new APIs.
...
The following built-in languages have been updated to support folder specific settings in a multi-root workspaces setup: CSS, Sass (SCSS), Less, JSON, and HTML. In addition the [ESLint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint), [TSLint](https://marketplace.visualstudio.com/items?itemName=eg2.tslint), and [Go](https://marketplace.visualstudio.com/items?itemName=lukehoban.Go) extensions have been updated to support multi-root workspaces.

### Multi-root and the Language Server Protocol

Proposed protocol has been added to the LSP to support multi-root workspaces. Both the [VS Code language client](https://www.npmjs.com/package/vscode-languageclient) and the [VS Code language server](https://www.npmjs.com/package/vscode-languageserver) have been updated to support the proposed protocol. In addition, VS Code's language client has been enhanced to support a server per workspace folder. A corresponding example can be found [here](https://github.com/Microsoft/vscode-extension-samples/tree/master/multi-lsp-sample).

## Preview: Source Control

There is now support for having multiple source control providers simultaneously. This feature aligns with the product-wide multi-root support active development.
