...
In May we've started to collect recipes for some non-standard or complex debugging setups in a "[VS Code Recipe](https://github.com/Microsoft/vscode-recipes)" repository. This repository has now moved to the new location https://github.com/Microsoft/vscode-recipes.
...
We started to document a guide for how to make your extension ready for multi-root workspaces. See [the guide](https://github.com/Microsoft/vscode/wiki/Extension-Authoring:-Adopting-Multi-Root-Workspace-APIs) for more information. All the APIs discussed are considered stable and are available. Note however that multi-root workspaces are currently only enabled in VS Code Insiders release. The guide touches on:
* the basic APIs
* settings
* impact on extensions that have a language client/server model

The guide is joined by new samples from our [samples repository](https://github.com/Microsoft/vscode-extension-samples) that exercise the new APIs. 
...
There is a new Visual Studio Code application logo available on Insiders.
