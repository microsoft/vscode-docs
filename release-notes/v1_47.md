...
The arguments `performSearch` and `focusResults` have been added to search editors to allow keybindings to configure wheather a saarch should be run automatically, and further if the results of that search should be start out focused rather than the query.
...
The field `enable-proposed-api` is now supported in `argv.json`, which allows extension authors to anable proposed API for their under-development extensions without needing to pass the `--enable-proposed-api` CLI flag.
