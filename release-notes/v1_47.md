...
The arguments `triggerSearch` and `focusResults` have been added to search editors to allow keybindings to configure wheather a search should be run automatically, and further if the results of that search should be start out focused rather than the query.
...
All repositories are rendered in a single view, so you can get a better overview of the entire workspace status. Addittionally, the Source Control view can now be moved to the panel and other views can be moved to the Source Control view container.
