...
Additionally, comment widgets have been made accessible. A help dialog (`editor.action.accessibilityHelp`) is also available in that context to inform a user of what to expect and how to utilize the feature. We added the commands **Comments: Go to Next Commenting Range** and **Comments: Go to Previous Commenting Range** to allow for navigation between commenting ranges.
...
A new setting `workbench.editor.pinnedTabsOnSeparateRow` has been added. When activated, pinned tabs are displayed on a separate row above other tabs. The editor tabs automatically pin or unpin when dragged between the tab rows.
