...
We now offer the ability to customize the display label for editor tabs and for the Open editors view. This functionality can be useful to distinguish between editors for files with the same name.
...
To enable or disable the application of these custom labels, use the <code codesetting="workbench.editor.customLabels.enabled">workbench.editor.customLabels.enabled</code> setting. This allows you to view the original editor names at any time without needing to remove your custom patterns.
...
You can now sync your scrolling across all visible editors in Visual Studio Code just by using the **View: Toggle Locked Scrolling Across Editors** command. This means that when you scroll in one editor, all the other editors scroll by the same amount, keeping everything aligned.

If you want more control for enabling and disabling locked scrolling, you can choose to only activate the scrolling sync when you're holding down a specific keybinding. Just set up a keyboard shortcut for the `workbench.action.holdLockedScrolling` command, and you're able to temporarily lock the scrolling across editors whenever you need it.
...
Along with the option to place the activity bar at the top of the side bar, you can now also move it to the bottom. To do this, change the <code codesetting="workbench.activityBar.location">workbench.activityBar.location</code> setting to `bottom`. We've also improved the look and feel of the activity bar when it's positioned at the top, to make sure it fits in nicely with the rest of the interface.
...
The right-click context menu on a comment thread in the Comments view now includes a "Reply" action when the comment thread allows replies. This allows you to quickly jump to the reply input box and start typing a reply.
...
The minimap now picks up sections defined by folding markers, such as `//#region` in TypeScript, or comments that use `MARK:`, and renders them in the minimap.
...
The diff editor now has a separate gutter for stage and revert buttons.
These actions allow to stage or revert changed code blocks.
If changes are selected, these buttons allow to stage/revert the selected changes (i.e. all changed characters within the selection).

![Stage/Revert Button Demo](images/1_88/diffEditor-stage-revert-demo.gif)

The gutter can be hidden by setting `"diffEditor.renderGutterMenu"` to  `false`.
...
### Filter support for Notebook Outline

You now have filters available in the Notebook Outline, which enable you to control the inclusion of Markdown headers, Code Cells, and Code Cell Symbols. The filters correspond to the following settings:
...
By enabling `interactiveWindow.promptToSaveOnClose`, you can be prompted to save the content in an Interactive Window when it is closed, ensuring that you don't lose any work. The only currently supported file format is `.ipynb`.
...
The `@workspace /new` command now offers sample projects curated from GitHub repositories as suggestions when a suitable match is detected for the query.

![Panel chat with @workspace new with project templates](images/1_88/new-workspace-project-templates.png)

The `@workspace /new` command has also been enhanced to more effectively manage context and history. This allows for refining suggested workspaces structure and file contents through follow-up queries.

#### @terminal /explain slash command

A new `@terminal /explain` slash command is available which is optimized for explaining something, rather than suggesting a fix which is what `@terminal` is optimized for. This is used in the `Explain using Copilot` quick fix or the explain selection actions.
...
A preview of the terminal Inline Chat is available in this release by enabling the `terminal.integrated.experimentalInlineChat` setting and invoked in a terminal (`kb(inlinechat.start)`). This feature allows convenient access to Copilot's capabilities directly in the terminal.
...
Once a command is suggested, `kb(workbench.action.terminal.chat.runCommand)` can be used to run the command in the terminal or `kb(workbench.action.terminal.chat.insertCommand)` can be used to insert the command into the terminal. The command can also be edited directly in Copilot's response before running it (currently `kbstyle(Ctrl/Cmd+down)`, `kbstyle(Tab)`, `kbstyle(Tab)`).
...
The chat submit button now has a dropdown for easy access to additional actions.
...
When you use `/explain` without a selection in your active editor and there are multiple scopes of interest, we've added support for prompting to clarify which symbol or block scope to explain. This behavior is currently opt-in behind the <code codesetting="github.copilot.chat.scopeSelection">github.copilot.chat.scopeSelection</code> setting.

### Python

#### Improved Debug config selection for Flask and Django

Creating launch configurations for Flask and Django apps just got easier! Improvements have been made to detect possible startup files in the workspace. For Django, the Python Debugger extension looks for `manage.py` or `app.py` files in the root or a subdirectory one level lower in your workspace. For Flask, the extension looks for `wsgi.py`, `app.py`, or `init.py` files that contain the declaration of a Flask application (for example, `app = Flask(`). If those files are not found in the project, the dropdown shows a **Default** option for the corresponding project type, even though that file may not be present.
...
- Outdated comments have are displayed differently from current comments in the Comments view.
- The new `auto` value for <code codesetting="githubPullRequests.createDefaultBaseBranch">githubPullRequests.createDefaultBaseBranch</code> will use the upstream's default branch as the base branch for fork repositories.
...
To enable this feature, you need to install the latest [Pylance](https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-pylance) prerelease in VS Code Insiders, and enable the `jupyter.executionAnalysis.enabled` and `notebook.consolidatedRunButton` settings.
...
This feature is available for preview in the [Insiders](https://code.visualstudio.com/insiders/) release via <code codesetting="extensions.experimental.supportWorkspaceExtensions">experimental setting</code>. Try it out and let us know your feedback by creating issues in the [VS Code repository](https://github.com/microsoft/vscode/issues)
...
The new comment thread `applicability` lets comment threads be marked as outdated in the Comments view:
...
The `commentsView/commentThread/context` proposed menu allows actions to be added to the right-click context menu of a comment thread in the Comments view. The usual `inline` group is also respected so that actions are shown in the Comments view inline.
...
In this milestone, we are promoting the Electron 28 update to users on our stable release. This update comes with Chromium 120.0.6099.291 and Node.js 18.18.2. We want to thank everyone who self-hosted on Insiders builds and provided early feedback.
