...
You can now sync your scrolling across all visible editors in Visual Studio Code just by using the `View: Toggle Locked Scrolling Across Editors` command. This means when you scroll in one editor, all the other editors will scroll by the same amount, keeping everything aligned.

If you prefer a bit more control, you can choose to only activate this scrolling sync when you're holding down a specific keybinding. Just set up a keyboard shortcut for the `workbench.action.holdLockedScrolling` command, and you'll be able to temporarily lock the scrolling across editors whenever you need it.
...
Along with the option to place the activity bar at the top of the sidebar, you can now also move it to the bottom. To do this, just change the <code codesetting="workbench.activityBar.location">workbench.activityBar.location</code> setting to `bottom`. We've also improved the look and feel of the activity bar when it's positioned at the top, making sure it fits in nicely with the rest of the interface.

![Activity Bar Positions](images/1_88/activity-bar-positions.png)
...
The minimap will now pick up sections defined by folding markers (like `//#region` in TypeScript) or comments that use `MARK:` and will render them in the minimap.

![The minimap picks up a folding marker region](./images/1_88/minimap-sections.png)
...
### Run Cells in Section for Notebook Outline and Sticky Scroll

Users can now execute cells that have been grouped together by a markdown section header via an action `Run Cells in Section` that has been contributed to both the notebook outline and Sticky Scroll elements. Within sticky scroll elements, users can right click the header of their choice, and execute the section via the action in the context menu. Within the outline view, users can select the toolbar icon that appears on hover or selection, and execute a single cell or section of cells via the presented actions.
...
Users now have filters available to them within the Notebook Outline, allowing them to control the inclusion of Markdown headers, Code Cells, and Code Cell Symbols. The filters correspond to the following settings:
