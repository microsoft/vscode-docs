...
A sound now indicates when voice recording starts and can be disabled with `accessibility.signals.voiceRecordingStarted`.

A sound to indicate when the recording stops can be configured with `accessibility.signals.voiceRecordingStopped`.

### Improved diff editor accessibility

An announcement is made to screen reader users when a diff editor becomes the active editor. This can be disabled with `accessibility.verbosity.diffEditorActive`.

Information about a helpful command for keyboard users, `Diff Editor: Switch Side`, has been added to the accessibility help dialog.

Accessibility signals formerly only worked on the modified side of the diff editor. Now they also work in the original side.

### Accessible view chat code block commands

When a user inspects a chat response in the accessible view, they can now use the code block commands and keybindings that are available in the panel chat view.
...
**Note:** due to a technical limitation, moving a webview based editor between windows will require the contents of that editor to reload. It is up to the editor then to restore the state you had previously accumulated but in some cases your state might be reset as if you had opened the editor for the first time.

### Search Editor Single-Click Behavior

You can now configure <code codesetting="search.searchEditor.singleClickBehaviour">search.searchEditor.singleClickBehaviour</code> in your settings, which drives what happens when you single-click on a Search Editor entry. Currently, it only supports opening a Peek Definition.

### Quick Search Improvements

#### Sticky file paths

To make it clear which file a search result is associated with, we now have stickiness to the file name separators in Quick Search:
...
The following link format is now detected in terminals, event if the path contains spaces:
