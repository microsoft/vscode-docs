...
We added information about the **Diff Editor: Switch Side** command, which is helpful for keyboard users, to the [accessibility help dialog](https://code.visualstudio.com/docs/editor/accessibility#_accessibility-help).

Previously, [Accessibility Signals](https://code.visualstudio.com/docs/editor/accessibility#_accessibility-signals) only worked on the modified side of the diff editor. Now they also work in the original side.
...
Aria labels for notebook cells will now update to inform the user if the cell is currently executing or pending execution.
...
#### File path separators buttons show up when focus is in that file's results

Whether you hover into file results of a particular file, or if you arrow down to a result, you'll see the buttons for the file path appear.
...
We received feedback that it would be nice to be able to navigate between separators in a quick pick. This iteration we've added a keybinding to do just this. On Windows & Linux, it is `Alt+Up/Down` and on macOS it is `Cmd+Up/Down`. In this example video, you can see the active item moving between:
...
### Quick Pick clear disablement of checkbox items

This iteration, we made it clear when a quick pick wants items to be displayed, but disabled. An example of this can be found in the "Managed Trusted Extensions" quick pick which can be accessed for any of the accounts that you're logged in to.
...
When utilizing Rename Symbols (When Copilot is enabled, it will prompt Copilot Rename Suggestions), accessible via `F2`, the keybinding for previewing edits has been updated to `CtrlCmd + Enter` (previously `Shift + Enter`) to maintain consistency with previewing refactorings in other contexts, such as Code Actions.

When hovering over Code Actions, `CtrlCmd + Enter` will also open the Refactor Preview panel in the workbench.
...
Shell integration is now automatically enabled in terminals launched when debugging, providing enhanced functionality to the user and extensions.

### Run recent command improvements

The shell integration-powered run recent command `kb(workbench.action.terminal.runRecentCommand)` will now scroll to and display the last time the command was run where possible. Running the command or cancelling the quick pick will return the terminal to its previous state.
...
The open detected link feature `kb(workbench.action.terminal.openDetectedLink)` will now preview the link result in the editor underneath the link quick pick as well as showing the link source in the terminal.
...
Additionally, duplicate links will now be filtered out of the list and all links are presented in a consistent format.
...
[Word links](https://code.visualstudio.com/docs/terminal/basics#_links) are defined by the `terminal.integrated.wordSeparators` setting and are the fallback when files/folder/URIs cannot be found. When activated, these links will now include additional surrounding context to add line and column information for the search that occurs.

Notice in the screenshot below that the link `terminalLinkParsing` was clicked, but the resulting search included the line number following the link:
...
[Sticky scroll in the terminal](https://code.visualstudio.com/updates/v1_85#_sticky-scroll) now supports transparency. A theme can use this by setting the `terminalStickyScroll.background` theme color to a transparent value, or by setting an override in your settings.json for example:
...
VS Code now includes TypeScript 5.4. This major update brings new improvements to type checking and IntelliSense, as well as fixing a number of bugs. See the [TypeScript 5.4 release blog post](https://devblogs.microsoft.com/typescript/announcing-typescript-5-4/) for more details.

### Smarter inserting of images and links in Markdown

When you drop/paste an image or file into a Markdown file, VS Code automatically inserts Markdown image or link syntax for it. We now also smartly disable this behavior when inserting into code blocks and other contexts that don't support Markdown syntax:
...
You can always switch back to inserting the Markdown syntax using the drop/paste widget. You can also configure this behavior using the `markdown.editor.drop.enabled` and `markdown.editor.filePaste.enabled` settings
...
* Selecting output from the current selection point to top or bttom of the output can be achieved with the keyboard with the `kbstyle(Ctrl+Shift+Up)` and `kbstyle(Ctrl+Shift+End)` keybindings respectively (`kbstyle(Shift+Cmd+Up)` and `kbstyle(Shift+Cmd+Down)` on macOS).

#### Cell Error Diagnostics

An extension can now provide error details for a failed cell so that an error diagnostic will show within the cell. While focused on the cell container, `notebook.cell.openFailureActions` (`kbstyle(ctrl+.)`) will jump to the quick actions menu for that error. The diagnostic will only show when an LLM is available to provide quick actions.
...
For the <code codesetting="accessibility.voice.speechLanguage">accessibility.voice.speechLanguage</code> setting, `auto` is the new default. In this mode, the VS Code speech extension will follow the display language of VS Code and pick the right model if supported.

### GitHub Copilot

#### Inline chat improvements

Inline chat now starts as a floating widget making it more lightweight. When a request has been made it takes up more space between your code lines. We have also adjusted its rendering to be more consistent with other chat locations like panel or quick chat.
...
Inline chat with new positions for the voting and re-run action, also the action to toggle the diffs is now a prominent button next to "Accept" and "Discard"
...
When you are in a notebook, the kernel state (e.g., variables, avaialble packages) is now included as context in the Inline Chat. This helps Copilot understand the current state of the notebook and provide more relevant completions.
...
Clarity around authentication is very important. We want to be clear about how we authenticate and what we ask for. With that, if you open a private repository in VS Code, and we don't have the right authentication for this scenario, we'll present an auth dialog with a description why and a "learn more" button to find out more about these requirements:
...
Previously, the "Used References" section only showed context that was pulled in implicitly. Now, it will also show variables like `#file` that were used explicitly, and indicate the name of the variable that included the reference. If a variable is missing from the "Used References" section, it may have been too large and didn't fit into the context window for the LLM.
...
After improvements were made to the `Help: Report Issue` API, the [Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python) and [Python Debugger](https://marketplace.visualstudio.com/items?itemName=ms-python.debugpy) extensions have adopted these changes making it easier for you to report issues to our repos! Filing an issue through the Report Issue command (`workbench.action.openIssueReporter`) does much of the heavy lifting and prompts you for details so that our team can most efficiently triage the problem you are encountering. To file an issue using the Report Issue command for [@vscode-python](https://github.com/microsoft/vscode-python) or [@vscode-python-debugger](https://github.com/microsoft/vscode-python-debugger), choose **Python** or **Python Debugger** respectively from the Extension dropdown.
