...
We've improved caching for local file inside a webview. If a webview reloads a file from disk, then we now use [etags](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/ETag) to avoid reading the file again if it has not changed. This can eliminate data transfer and improves response time, especially for remote workspaces.

This caching currently only applies we an existing webview reloads a resources. It does not apply to newly created webviews, even if that webview belongs to an extension that previously created a similar webview.
...
VS Code now ships with TypeScript 4.2.2. This major update includes a number of TypeScript language improvements, along with many improvements and bug fixes for JavaScript and TypeScript tooling.
...
Note that to use this quick fix in JavaScript, you must []enable type checking](https://code.visualstudio.com/Docs/languages/javascript#_type-checking).
...
Deprecated dom methods are now marked as deprecated in suggestion and in code:

![document.clear being marked as deprecated in code](images/1_54/ts-deprecated-dom.png)

You can disabled showing deprecated properties and functions in the editor by setting `"editor.showDeprecated": false`.
...
In order to make our markdown renderer extensible, this iteration we explored using [Markdown It](https://github.com/markdown-it/markdown-it) to render all notebook markdown. We also explored moving the rendered markdown into the backlayer webview of the notebook.

While we aren't quite ready to open up an official API for this, we plan on continuing this exploration in March. You can also try out the new extensible markdown renderers today with the undocumented `"notebook.experimental.useMarkdownRenderer": true` setting.
...
VS Code now is built using a nightly build of TypeScript 4.3. This lets validate the TypeScript team's latest work and also provide feedback on new changes.
