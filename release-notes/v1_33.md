...
A call hierarchy view shows all calls from or to a function and allows to drill into callers of callers and calls of calls. The capture below shows that function `foo` is being called by `bar` and `bang`, then `bar` is being called by `bang` and `fib`.

![Call Hierarchy](images/1_33/call-hierarchy.png)

This is a preview feature and there aren't any extension providing real data for this. However, if you are an extension auther we invite you to validate our proposed API. For users keen on trying this, there is a sample extension that allows for a dry-run: https://github.com/jrieken/demo-callhierarchy
...
Are you an insider? Come on and jump over to the [Insiders snap package](https://snapcraft.io/code-insiders):
