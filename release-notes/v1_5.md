...
The `More Information` action takes you to a web site explaining how to configure VS Code to always use a local installed TypeScript version. VS Code checks now also whether the used TypeScript language server is different than the globally installed `tsc` compiler. If so a corresponding information message will be shown.
