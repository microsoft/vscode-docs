...
By default, no file icon set is configured and the File Explorer doesn't render icons. Once an Icon Theme is selected, the choice will be persisted and used again on restarts of VS Code.
In this release file icons are only shown in the File Explorer, but it is planned to use file icons in other places, e.g. in Editor tabs.
...
Credits to Jesse Weed and all contributors to the popular 
[Seti UI](https://github.com/jesseweed/seti-ui) theme for the fantastic Seti UI icons.

### Keybinding support within the Integrated Terminal

A new setting `terminal.integrated.commandsToSkipShell` was added that contains an array of commands that skip processing by the shell and instead are processed by VS Code's keybinding system. This allows keys such as `kbstyle(F1)` to now work while focus is in the terminal.
...
On our development machines Quick Open went from around 30s in version 1.3 to 3-4s (depending on your platform) in version 1.5.
...
### macOS: Cmd+E no longer opens Quick Open
...
The reason behind this change is that `kbstyle(Cmd+E)` on macOS is typically associated with running a search in the current active file.
...
>**Note:** The August release of VS Code still bundles the official version of Typescript which is `1.8.10`. Extension developement should be done using this version. We will support TypeScript `2.0.x` for extension develeopment in a future release.
...
* [9822](https://github.com/Microsoft/vscode/issues/9822): The terminal IME composition view appears on top of the text when at the bottom of the screen
...
We will create monthly releases of the Monaco Editor, in the same rhythm with VS Code. Changelog [here](https://github.com/Microsoft/monaco-editor/blob/master/CHANGELOG.md).
