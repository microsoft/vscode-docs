...
With this release, VS Code shows open files as Tabs in the title area above an editor. You can move Tabs around via drag and drop to organize your files or right-click on a Tab to perform useful actions like **Switch to Changes View**.
...
As a preparation for enabling Tabs in the workbench, we revisited how you can interact with editors in VS Code.

Many users coming from other tools were confused by some of the editor (open file) behavior in VS Code.
...
* Performance of the terminal has improved significantly and should no longer perform poorly when running commands that produce massive output.
...
[Global Search and Replace](https://github.com/Microsoft/vscode/issues/1690) is one of the features that most users wanted to have. With this release, we enhanced our Search view to include Replace functionality so that you can replace text across multiple files. You can replace all files or pick file by file or result by result. You can also exclude results or files and perform replace on the rest. In addition, we also provide a nice preview inside the Search view and a Diff editor view showing the file before and after replace.
...
* [Giorgos Retsinas (@elemongw)](https://github.com/elemongw): Open empty window on activate when all windows are closed [PR #7547](https://github.com/Microsoft/vscode/pull/7547)
* [Andrew Arnott (@AArnott)](https://github.com/AArnott): Language Server Protocol-Clarifications regarding JSON-RPC header [PR #15](https://github.com/Microsoft/language-server-protocol/pull/15)
...
* [kisstkondoros (@kisstkondoros)](https://github.com/kisstkondoros):
