...
* **Languages**: Better, more complete Emmet support, pluggable Atom JavaScript grammar
...
<<<<<<< 0bd5ffeb07f638b5d21e45f6b00ebbf58a58e9b8
```
=======
   ```
- the variable can now be used as `${command.RemoteHost}` in any string typed value of a launch configuration.
...
- **Editing variables**: if a debug adapter returns the [capability `supportsSetVariable`](https://github.com/Microsoft/vscode-debugadapter-node/blob/master/protocol/src/debugProtocol.ts#L594) VS Code supports to set the value of unstructured (leaf) variables in the Variables view by calling the [`setVariable` request](https://github.com/Microsoft/vscode-debugadapter-node/blob/master/protocol/src/debugProtocol.ts#L476) request.
...
* [Denis Malinochkin (@mrmlnc)](https://github.com/mrmlnc): Emmet - support all the features [PR #8155](https://github.com/Microsoft/vscode/pull/8155)
