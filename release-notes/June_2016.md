...
The integrated terminal that was introduced in v1.2.0 has seen many improvements this release, the primary feature being the ability to use launch and use multiple terminals at the same time. Terminal instances can be added by hitting the plus icon on the top-right of the terminal panel or triggering the `kb(workbench.action.terminal.new)` command. This creates another entry in the dropdown list that can be used to switch between them.
...
- Copy and paste on Linux and Windows is now available using `Ctrl+Insert` and `Shift+Insert` respectively, this is the temporarily non-configurable until we can [route custom copy/paste keybindings](https://github.com/sourcelair/xterm.js/issues/118) to the xterm.js library 
...
- Performance of the terminal has improved significantly and should no longer perform so badly when using commands that produce massive output
