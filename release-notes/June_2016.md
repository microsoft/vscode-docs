...
* **Extension Authoring**: A new contribution point for menu bars and context menus. New APIs to open non-text resources and add editor decorator attachments. The preview HTML pane now supports titles and theming.
...
* `workbench.editor.enablePreviewFromQuickOpen` to enable or disable preview editors when opened from the quick box/quick open.
...
* `kb(workbench.action.terminal.focusNext)`: Focuses the next terminal instance.
* `kb(workbench.action.terminal.focusPrevious)`: Focuses the previous terminal instance.
* `kb(workbench.action.terminal.kill)`: Remove the current terminal instance.

The ability to run selected text in the current terminal has been added through the `kb(workbench.action.terminal.runSelectedText)` command. 
...
### CSS/LESS/SCSS and JSON language services are now available in the Monaco editor
