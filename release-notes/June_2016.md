...
Python environment discovery is **faster and more reliable**. Over the past month, we've cut the rate of slow Python environment startups by about 15% on average, and worst-case waits for the **slowest sessions are down by roughly a third**. Improvements include caching the discovered system Python across sessions, deferring background work that doesn't need to block environment selection, and a coordinated set of fixes in the underlying python-environment-tools binary.
