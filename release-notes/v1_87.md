...
### Settings in the release notes

When you view the release notes from within VS Code (command "Show Release Notes") you might see some settings mentioned that have a gear. Clicking on that setting or gear will let you enable and view that setting quickly from the release notes.
...
You can track language model usage of an extension in the Extension Editor and Runtime Extensions Editor. Following images show the number of requests made by the Cat Extension to the GitHub Copilot Chat language model.
...
This iteration we have set to true the default enablement state of the editor sticky scroll. You may change the setting here <code codesetting="editor.stickyScroll.enabled">editor.stickyScroll.enabled</code>.
...
This iteration we have added support for multi-cursor inline completions. Now the inline completion at the primary cursor position are previewed and applied to the secondary cursor positions too.
...
Refactor preview changes are now shown in a multi diff editor, which facilitates seeing all changes at a glance.
...
## For extension authors: Contributing Additional Data in Issue Reporter

Last iteration, we added fields for `data` and `uri` in the `workbench.action.openIssueReporter` command. This allowed extensions to directly open the native issue reporter with more pre-filled information.
...
We have been working on allowing extensions to contribute to the Chat panel and general reuse of language models. We are now in the final stages of finalizing these APIs and would love to hear your feedback. It is about these two proposals:

- Chat participantion - [`vscode.proposed.chatParticipant.d.ts`](https://github.com/microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.chatParticipant.d.ts)
