...
**Per file**
Perhaps the easiest way to start type checking your JavaScript is by adding a `// @ts-check` comment to the top of a file.
...
**Using a Setting**
To enable type checking for all Javascript without changing any code, try setting `"javascript.implicitProjectConfig.checkJs": true` instead.

This setting enables type checking for any JavaScript file that is not part of a `jsconfig.json` or `tsconfig.json` project. You can opt individual files out of typechecking with a `//ts-nocheck` comment
...
* [TSLint](https://marketplace.visualstudio.com/items?itemName=eg2.tslint). We have contributed to the [tslint language server plugin](https://github.com/angelozerr/tslint-language-service) and there is now a [preview](https://github.com/angelozerr/tslint-language-service#vscode) available to try out for VS Code.
