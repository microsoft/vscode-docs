...
During this milestone, we closed 2199 issue across all of the VS Code repositories. However, while we were busy closing issues, you were busy opening them. 1925 new issues, to be exact, and we decreased our total issue count by 274. We're still very happy with our results for April. By comparison in March, we ended with 329 more issues than we started with.
...
* New Yeoman generator to create workbench themes.
* Daniel Imms (@Tyriar) theme generator.
* Built-in themes adopting new colors.
* New colors for: list/tree, diff editor, activity foreground, notifications, scrollbar, splitview, buttons.
* Settings moved out of the experimental API.
...
TypeScript 2.3 brings type checking to plain old JavaScript files. These type checks are a great way to catch common programming mistakes and they also enable some exciting quickfixes for JavaScript.

![](images/1_12/ts-checkjs-example.gif)

Variable types and function signatures are defined using JSDoc comments. You can read more about how JSDocs are used [here](https://github.com/Microsoft/TypeScript/wiki/Type-Checking-JavaScript-Files). 

Type checking of JavaScript is optional and opt-in. Existing JavaScript validation tools such as ESLint can be used alongside the new built-in type checking functionality. You can get started with type checking a few different ways depending on your needs.

**Per file**
The easiest way to enable type checking in a JavaScript file is by adding a `// @ts-check` comment to the top of a file.
...
This opts the entire file into type checking. Using `// @ts-check` is a good approach if you just want to try type checking in a few files but not enable it for an entire codebase.

**Using a Setting**
To enable type checking for all Javascript files without changing any code, just add `"javascript.implicitProjectConfig.checkJs": true` to your workspace or user settings. This setting enables type checking for any JavaScript file that is not part of a `jsconfig.json` or `tsconfig.json` project.

You can opt individual files out of type checking with a `// ts-nocheck` comment
...
JavaScript type checking requires TypeScript 2.3. If you are unsure what version of TypeScript is currently active in your workspace, simply run the `Select TypeScript Version` command.
