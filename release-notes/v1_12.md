...
**Per file** : Perhaps the easiest way to start type checking your JavaScript is by adding a `// @ts-check` comment to the top of a file.
...
This opts the entire file into type checking. Using `// @ts-check` is a good approach if you just want to try type checking in a few files but not enable it for an entire project.

**Using a Setting** : To enable type checking for all Javascript files without changing any source code, you can set `"javascript.implicitProjectConfig.checkJs": true`.

This setting enables type checking for any JavaScript file that is not part of a `jsconfig.json` or `tsconfig.json` project. You can opt individual files out of type checking with a `// @ts-nocheck` comment.
...
**Using a JSConfig or TSConfig** : To enable type checking for javascript files that are part of a `jsconfig.json` or `tsconfig.json`, simply add `"checkJs": true` to the project's compiler options.
...
This enables type checking for all JavaScript files in the project. Again you can use `// @ts-nocheck` to disable type checking per file.
