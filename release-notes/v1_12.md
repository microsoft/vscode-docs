...
It is now possible to edit watch expressions also using the context menu.
...
One consequence of the Electron update is that promise rejections are now treated as uncaught exceptions by the node runtime even if the reject is caught later. Since this behavior is annoying when debugging extensions, we've introduced a new exception option to control promise rejections independent from regular exceptions:
...
Since this approach does not work for all languages, we've introduced the `evaluateName` attribute for variables some time ago. If the `evaluateName` attribute is available for a variable VS Code will use 'as is' it for the evaluate request. If `evaluateName` is missing VS Code falls back to the old approach.
