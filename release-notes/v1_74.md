...
* **[Hide Activity bar badges](#hide-badges-per-view-container)** - Simplify the editor UI by toggling Activity bar badges.
...
* **[Jupyter notebook "Just My Code" debugging](#just-my-code-for-notebook-cell-debugging)** - Avoid stepping into Python library code.
...
This improvements performance by allowing webview content to be cached in more cases. See the [previous release notes](https://code.visualstudio.com/updates/v1_73#_consistent-origin-for-webviews-and-webview-views) for more details and things to be aware of as an extension author.
...
* No longer depending (and bundling) node_modules that provided functionality that is readily available in node.js and the web (ex. removing uuid in favor of [Crypto.randomUUID()](https://developer.mozilla.org/docs/Web/API/Crypto/randomUUID)).
