...
Similar to how you were able to hide view containers by right-clicking the view container, we've now introduced the ability to hide the badges often indicating a number, icon, or progress on the container (displayed in the Activity bar, Panel, and Secondary Side bar).
...
* Extensions that provide `git blame` functionality now work in the incoming, current and base editor.
...
However, in workspaces that use scripts residing in nested sub-directories, developers could have a hard time setting up their tasks as various scripts seldom report file paths in a unified manner (for example, relative to the workspace's base directory).

To help alleviate this problem, there is a new file location method, named `search`. With this method, a recursive file system search is run to locate any captured path.
...
We are adding a kernel picker that shows the most recently used kernels at the top of the picker. This is an opt-in experiment that can be enabled by setting the `notebook.kernelPicker.type` setting to `mru`.
...
* Pull request checks status (continuous integration (CI) in progress, needs review, CI failed, etc.) is shown in the Pull Requests tree view.
...
Starting mid-November all VS Code extensions uploaded to the VS Marketplace are signed by the VS Marketplace - this process is called repository signing. On every extension install and update VS Code verifies the extension package signature. With signing and signature verification we can guarantee extension package integrity and authenticity. The signature verification is currently only done in VS Code Insiders - if you see any issues make sure to create an issue in the [vscode repository](https://github.com/microsoft/vscode/issues) and you can opt out of signature verification by setting `extensions.verifySignature` to `false`. We are working on enabling signature verification in VS Code Stable in the coming months.
...
Extensions can now use `<video>` tags in their README's. Videos are supported on both the VS Code Marketplace and on extension pages in VS Code. Compared to `.gif`s, video files are higher quality and often smaller too. You can even include audio!

Keep in mind that [only specific video and audio formats are supported in VS Code itself](https://code.visualstudio.com/api/extension-guides/webview#_supported-media-formats). At present, you must also use a full path to the `<video>` `src` and `poster` instead of a relative path.

### Comment thread additional actions

The new menu proposal `comments/commentThread/additionalActions` allows extensions to contribute always-visible buttons to comment threads.
...
No functionality was lost in this process and these extensions are now fetched and loaded faster due to the decreased size.

### Electron sandbox journey

You can learn about our progress toward enabling Electron's [sandbox](https://www.electronjs.org/docs/tutorial/sandbox) in the recent ["Migrating VS Code to Process Sandboxing"](https://code.visualstudio.com/blogs/2022/11/28/vscode-sandbox) blog post. This detailed article presents the timeline and describes the incremental changes made to support VS Code sandboxing.
...
The [Window Controls Overlay](https://wicg.github.io/window-controls-overlay) (WCO) has been re-enabled by default. It was previously enabled for a while in August, but then disabled again, because it was [rendering on the wrong side](https://github.com/microsoft/vscode/issues/159813) for users using right-to-left system languages, covering up the **File** and **Edit** menu items.
