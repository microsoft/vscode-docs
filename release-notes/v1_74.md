...
[Remote Tunnels](/docs/remote/tunnels) is now available as a preview feature on VS Code stable. Remote tunnels allow you to securely access your machine with VS Code from any device, anywhere. To enable remote tunnel access, you can _either_:

- Choose "Turn on Remote Tunnel Access" from the Account menu or Command Palette,
- Run `code tunnel` from a machine with VS Code installed and on the PATH, or
...
To find out more, check out our [blog post announcing the feature](/blogs/2022/12/07/remote-event-better) or the [remote tunnel documentation](/docs/remote/tunnels).
...
Previously, if an extension declared that it contributed a command `mycommand` in its package.json file for example, it also needed to list the corresponding `onCommand:mycommand` activation event in the `activationEvents` field of the package.json for the command to successfully execute. This was tedious and error-prone and could lead to large package.json files.
...
Extension authors can now safely remove these inferred activation events from your extensions. If you do so, please update your VS Code engine requirement to ensure that your extension is not treated as compatible with earlier versions of VS Code:
...
Several clarifications and a new property was added to the [Debug Adapter Protocol](https://microsoft.github.io/debug-adapter-protocol).

* The `DataBreakpointInfo` request has a new optional `frameId` property. Clients can use this flag to specify which stackframe contains a `name`d variable.
