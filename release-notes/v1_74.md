...
Previously, if an extension declared that it contributes a command `mycommand` in its package.json file for example, it also needed to list the corresponding `onCommand:mycommand` activation event in the `activationEvents` field of the package.json for the command to successfully execute. This was tedious and error-prone and could lead to large package.json files.

VS Code will now automatically populate activation events for the following extension contributions:

| Contribution type | Inferred activation event |
| --- | --- |
| `commands` | `onCommand` |
...
Extension authors can now safely remove inferred activation events from your extensions. If you do so, please update your VS Code engine requirement to ensure that your extension is not treated as compatible with earlier versions of VS Code:
