...
#### Merge editor not dirty on open anymore
...
We no longer disable the old inline conflict decorators, so that the merge editor and inline experience can be used together. When the new merge editor opens, you can get the previous experience with a single click in the editor toolbar:
...
A file that has a conflict or becomes conflicted will now automatically show an **Open in Merge Editor** button to transition between the text and merge editors.
...
We started exploring alternative diffing algorithms to improve the quality of conflicts. This new experimental diffing algorithm can be enabled with the following setting:
...
In `v1.68` we had adopted the Window Control Overlay feature support from Electron behind an experimental flag `window.experimental.windowControlsOverlay.enabled` and was off by default. We have now resolved the remaining issues around this feature affecting VSCode,
allowing us to enable it by default for all users on Windows.
...
[The VS Code build task is reconnected to on window reload](images/1_71/task-reconnection.mp4)
...
And if you missed the VS Code [livestreams](https:/code.visualstudio.com/livestreams) on TypeScript, have a look at two recent sessions with [Matt Pocock](https://twitter.com/mattpocockuk).
...
Users can now paste screenshots or image files into Markdown cells inside their notebooks. Currently only supported for the `image/png` mime type. To use the feature, add/enable the following settings:
...
<video autoplay loop muted playsinline controls>
  <source src="images/1_71/continue-working-on-codespace.mp4" type="video/mp4">
</video>
...
Tried of accidentally breaking links or images in your Markdown when you move or rename files? Try the new `markdown.experimental.updateLinksOnFileMove.enabled` setting!
...
<video autoplay loop muted playsinline controls src="images/1_71/md-link-update.mp4"></video>

This new behavior can be enabled using the `markdown.experimental.updateLinksOnFileMove.enabled` setting. You can control the types of files it is enabled for using `markdown.experimental.updateLinksOnFileMove.externalFileGlobs`.

ive this new feature a try and be sure to share feedback!
...
Microsoft [ended support for Windows 7](https://docs.microsoft.com/lifecycle/products/windows-7) on January, 2020. However, Electron continued to support Windows 7 by patching the `libuv` library. With the Electron 19 update, the `libuv` patch [no longer works](https://github.com/electron/electron/issues/35219) and we recommend users update to a newer Windows version in order to use the VS Code desktop version. VS Code will no longer provide updates on Windows 7. You can learn more about upgrading Windows at [support.microsoft.com](https://support.microsoft.com/en-us/windows/windows-7-support-ended-on-january-14-2020-b75d4580-2cc7-895a-2c9c-1466d9a53962).
