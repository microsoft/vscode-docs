...
* **[Jupyter notebook image pasting](#notebook-image-pasting)** - Paste and preview image files in notebook Markdown cells.
...
![A notebook playing an mp4 video file](images/1_71/codecs.png)
...
The Windows Controls Overlay feature lets VS Code users on Windows 11 select [Snap Layouts](https://docs.microsoft.com/windows/apps/desktop/modernize/apply-snap-layout-menu) to place VS Code on their desktop.

![Windows snap layout control in the right of the VS Code title bar](images/1_71/windows-snap-layout.png)
...
Sticky scroll shows the current scope at the top of the view port. The feature is no longer experimental and can be enabled through the settings with `editor.stickyScroll.enable`. Further improvements are:
...
![Sticky scroll Ctrl/Cmd click](images/1_71/sticky-scroll-ctrlclick.gif)
...
![The new Code Action control](images/1_71/code-action-widget.png)
...
The terminal now supports smooth scrolling, which will animate scrolling for a short period of time to help you orient yourself after scrolling, similar to the editor and lists. To enable smooth scrolling set:
...
In August we tried passing the focus and selection of the view's tree for commands contributed to the `view/title` menu of a tree view. For this release we have reverted the change as it caused problems for some extensions. We want to bring the change back for September when we plan to release it. If this will break an extension or if this is otherwise undesirable behavior, please leave a comment over in https://github.com/microsoft/vscode/issues/42903.
