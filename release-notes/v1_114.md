---
Order: 123
TOCTitle: Insiders
PageTitle: "Visual Studio Code 1.114 (Insiders)"
MetaDescription: Learn what's new in Visual Studio Code 1.114 (Insiders)
MetaSocialImage: 1_114/vscode-insiders-header.webp
Date: 2026-04-01
DownloadVersion: 1.114.0
Milestone: 1.114.0
ProductEdition: Insiders
---
# Visual Studio Code 1.114 <!-- %IF INSIDERS % (Insiders) %ENDIF % -->

_Last updated: March 26, 2026_

Welcome to the 1.114 <!-- %IF INSIDERS % Insiders %ENDIF % --> release of Visual Studio Code.

<!-- %IF INSIDERS %
These release notes cover the Insiders build of VS Code and continue to evolve as new features are added.

You can still track our progress in the [Commit log](https://github.com/Microsoft/vscode/commits/main) and our list of [Closed issues](https://github.com/Microsoft/vscode/issues?q=is%3Aissue%20is%3Aclosed%20milestone%3A1.114.1).
%ENDIF % -->

Happy Coding!

---

To try new features as soon as possible, [**download the nightly Insiders build**](https://code.visualstudio.com/insiders), which includes the latest updates as soon as they are available.

<!-- %IF IN_PRODUCT %
You can view the release notes online at [code.visualstudio.com/updates](https://code.visualstudio.com/updates).
%ENDIF % -->

---

<!-- TOC
<div class="toc-nav-layout">
  <nav id="toc-nav">
    <div>In this update</div>
    <ul>
      <li><a href="#march-26-2026">March 26, 2026</a></li>
      <li><a href="#march-25-2026">March 25, 2026</a></li>
      <li><a href="#march-23-2026">March 23, 2026</a></li>
    </ul>
  </nav>
  <div class="notes-main">
Navigation End -->

## March 26, 2026

* Visual Studio Code now ships with TypeScript 6.0. For more details, see the [TypeScript 6.0 announcement](https://devblogs.microsoft.com/typescript/announcing-typescript-6-0/). [#304483](https://github.com/microsoft/vscode/issues/304483)

* Persist chat permissions level and restore it when reopening chat sessions. Add docs link to provide additional context. [#302813](https://github.com/microsoft/vscode/issues/302813)

## March 25, 2026

* The Artifacts panel in chat now includes a "Clear" action for removing stale artifacts in longer conversations. [#303784](https://github.com/microsoft/vscode/issues/303784)

## March 23, 2026

* Pinned chat sessions show a pin icon indicator in the sessions list, making it easier to distinguish pinned from unpinned sessions. [#304046](https://github.com/microsoft/vscode/issues/304046)

* A new `${taskVar:name}` variable lets you capture dynamic values from a task's problem matcher output and use them in launch configurations. For example, capture a server URL from task output and reference it with `${taskVar:serverUrl}`. [#303361](https://github.com/microsoft/vscode/issues/303361)

---

We really appreciate people trying our new features as soon as they are ready, so check back here often and learn what's new.

<a id="scroll-to-top" role="button" title="Scroll to top" aria-label="scroll to top" href="#"><span class="icon"></span></a>
<link rel="stylesheet" type="text/css" href="css/inproduct_releasenotes.css"/>
