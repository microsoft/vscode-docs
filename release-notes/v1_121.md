...
Copilot's inline suggestions always align with the selection of the suggest widget. This is very useful, as you can quickly press Tab twice to accept the suggestion as well as the ghost text coming from Copilot. But we've found that when starting to type, in many cases the widget pops up and selects the very first (alphabetical) available global symbol that starts with the typed character. This is rarely the text that actually gets typed and it also forces Copilot to give suggestions that have that prefix, thus making the experience more noisy.

We've decided to change the default setting for quick suggestions. If an inline completion provider is available (such as Copilot), then typing letters in the editor will no longer automatically trigger the suggest widget. In all other cases, such trigger characters, the suggest widget will pop up as before. You can revert to the old behavior by configuring:
