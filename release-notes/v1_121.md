...
When a chat terminal command reaches a password, passphrase, PIN, or verification-code prompt, VS Code now treats that as sensitive input instead of another prompt for the model to answer. In ask-style sessions, chat shows a confirmation dialog that can focus the terminal so you can enter the secret directly there.

This closes a risky gap where secret prompts could otherwise stall in the background or tempt the model into trying to answer them. In auto-approve flows, VS Code cancels the command and tells the model not to retry or request the secret, so sensitive input never gets routed through chat.
