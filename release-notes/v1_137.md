...
### GitHub Pull Requests extension (experimental)

There is initial support for the GitHub Pull Requests extension in Agents. When `setting(extensions.experimental.enableAgentsWindowCapability)` is enabled and GitHub Pull Requests is installed (it must be installed in the default profile of VS Code), you can open issues and pull requests from github.com links directly within the Agents window.

### Continue chats in a workspace

Chats are useful for conversations that don't initially belong to a project. If a conversation becomes project-specific, you can ask the Copilot agent to continue the task in a workspace instead of starting over in a new session.
...
> **Note**: This option is currently available only for chats that use the Copilot harness.

You can attach a workspace to the existing chat and continue the conversation as a workspace session:
...
When an agent coordinates work across multiple sessions or chats, sending a follow-up request no longer has to interrupt a target chat that is already working. The `send_message` session-management tool queues the request and starts it after the active turn finishes successfully.

Agents can queue multiple messages for a chat in the same session or another session. Queued messages run in the order they were sent, which makes cross-session orchestration more predictable without losing requests or disrupting work in progress.
