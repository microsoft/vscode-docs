...
**Setting**: `setting(markdown.experimental.richLinks.enabled)`

GitHub issue and pull request links in the Markdown editor show the current title and state, so you can understand each reference without opening it first. Select a rich link to open the corresponding GitHub page. The link updates when its issue or pull request state changes, without requiring you to reopen the Markdown file.

Rich links are enabled by default and don't require the workspace to have a Git remote. Disable `setting(markdown.experimental.richLinks.enabled)` to show regular Markdown links instead.
