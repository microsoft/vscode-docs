...
* Run agents while chat is closed ([Show more](#local-agent-sessions-remain-active-when-closed)).
...
* Easily reuse custom agents across environments ([Show more](#agent-tooling-reorganization)).
...
* `sideBySide`: always show the sessions to the side as long as the width allows it
...
* Inline chat is optimized for your current file ([Show more](#inline-chat-ux)).
* Manage your chat models ([Show more](#language-models-editor)).
* Approve URLs fetched in tasks ([Show more](#url-and-domain-auto-approval)).
...
* Terminal experience in chat is more accessible ([Show more](#rich-terminal-output-in-chat)).
...
* Entra ID is the default authetication for Azure models ([Show more](#azure-model-provider-entra-id-as-the-default-authentication)).
* The Chat view is more usable ([Show more](#chat-view-appearance-improvements)).
...
* The GitHub remote MCP server is built into Chat ([Show more](#github-mcp-server-provided-by-github-copilot-chat-preview)).
...
* Approve agent requests via your keyboard ([Show more](#keyboard-approval-for-chat-confirmations)).
...
* More easily open recent windows ([Show more](#more-support-to-indicate-opened-windows-in-pickers)).
