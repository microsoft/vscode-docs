...
We are happy to introduce a agent sessions viewer into the Chat view. This gives you an overview of all agentic sessions (running locally, in background, in the cloud or from 3rd party extensions) at once. Each session provides progress, indicates when changes are ready to be looked at and provide some basic principles to manage their visiblity (archiving, read state indicator).

<video src="images/1_107/agent-sessions.mp4" title="Showing agent sessions in Chat view" autoplay loop controls muted></video>

You can access session related actions from the sessions context menu. By default sessions will open in the sidebar, but you can decide to open in the editor or new window too.

![Sessions context menu](images/1_107/sessions-context.png)

Note: you can disable the sessions in the Chat view by configuring `setting(chat.viewSessions.enabled)`.

#### Compact agent sessions

When the Chat view is narrow, recent sessions appear for empty Chats to the top. You will see the most recent 3 sessions here that are not archived.

![Recent sessions in the chat view](images/1_107/recent-sessions.png)

Click on the link to show all sessions to get an overview over all sessions with the ability to search and filter.
...
Once the chat view is wide enough (for example maximised), agent sessions present in a sidebar next to the Chat, allowing you to go over Chat sessions one by one without loosing context.

![All sessions in the chat view](images/1_107/all-sessions.png)

You can filter sessions by provider or state and this filter is persisted.
