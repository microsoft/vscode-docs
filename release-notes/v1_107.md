...
You can refer to the `setting(chat.viewSessions.orientation)` to decide on a fixed location for the agent sessions view if you dislike the automated appearance based on the size of the sidebar:
* `auto`: this is the default and will show the sessions side by side if the width allows
* `stacked`: only show the sessions above empty chats
* `sideBySide`: only show the sessions to the side if the width allows
