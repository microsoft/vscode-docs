...
![Screen showing the Git Worktree dropdown when creating a background agent.](images/1_107/background_worktree.png)
...
### Grep Search tool can search ignored files
The `#grep_search` tool now support searching in ignored files/folders specified by `files.exclude` or `search.exclude` settings, for example: the node_modules folder. Also it now returns hints to the chat agents about the ignored files/folders when there are no results, allowing chat agents to turn around and enable searching in those ignored files/folders.
