...
Last iteration, we improved the `setting(chat.agent.thinkingStyle)` setting which enabled displaying thinking tokens in chat.

Now, the experimental setting, `setting(chat.agent.thinking.collapsedTools)`, has been adjusted to include non-reasoning chat elements inside the dropdown, making successive tool calls collapsible by default.
...
![A screenshow showing multiple tool calls collapsed in a collapsible section](images/1_107/collapsed-tools.png)
