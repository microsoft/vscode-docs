...
### Continue in
Continuing a local chat with a background or cloud-based agent has been improved this iteration. The new 'Continue in' menu makes it easy to handoff to a cloud or background agent.  A continued chat will carry over the current chat context to the agent and then archives the original session after delegation.

the 'Continue in' menu can be accessed from the chat view:

![The 'continue in' button from the chat widget](images/1_107/continue-in-chatwidget.png)

As well as from an untitled prompt file:

![The 'continue in' button from the chat widget](images/1_107/continue-in-promptfile.png)

And lastly from Plan mode:

![The 'continue in' button from the chat widget](images/1_107/continue-in-planmode.png)

### GitHub Organization and Enterprise Agents

**Setting**: `setting(github.copilot.chat.customAgents.showOrganizationAndEnterpriseAgents)`

You can now access custom agents created at the organization level for your GitHub account. This experimental feature enables you to use organization-specific agents alongside your personal agents in GitHub Copilot Chat.

To enable this feature, set `setting(github.copilot.chat.customAgents.showOrganizationAndEnterpriseAgents)` to `true`. Once enabled, custom agents created by your organization will appear in your agent list, allowing you to leverage organization-specific agents tailored to your team's workflows and requirements.
...
### Agent tooling reorganization for GitHub compatibility

We've reorganized the agent tooling structure to enable better compatibility with GitHub custom agents. Agent files can now be used more easily in both VS Code and GitHub environments without requiring separate configurations.

As part of this change, we've renamed certain existing tool references and the toolsets they belong to. Existing tool references in your agent files will continue to work, but you'll see a quick action that allows you to rename them to the latest recommended format. This will ensure that your agent configurations follow the current best practices and maintain compatibility across platforms.

### Background Agents

#### Git Worktrees

Background Agents (previously called CLI Agents) can now run inside their own isolated [Git worktrees](https://git-scm.com/docs/git-worktree). You can choose between running the agent in the current workspace or in a dedicated worktree. If you select a worktree, the agent will automatically create a worktree and run the agent there — keeping any changes separate from your main workspace. This enables agents to operate multiple agents continuously in the background, without interfering with your active development flow.

![Git Worktree in Background Agents](images/1_107/background_worktree.png)

#### File Changes Tracking

As part of the improvements in Background Agents, we have also enhanced the experience to make it easier to track the modified files and have better control over what changes you want to be included in your session. This includes a new action for user to directly apply the changes from a worktree directly into their workspace.

![File Changes in Worktree Background Agents](images/1_107/filechanges.png)

### Remote Agents

#### Copilot Cloud Agents

We have made some improvements to the integration of Copilot Cloud Agents in VS Code so that you can review and interact with all your sessions when you have an empty VS Code window, allowing you to coordinate sessions cross-repository.

#### Expanded Context Attachments
...
![Additional Background Agent Attachments](images/1_107/background_attachments.png)

### Experimental support for Custom Agents
...
When you open a chat in the sidebar, a new title control appears to the top showing you the title of the Chat as well as giving you a quick way to get back to an empty Chat:

<video src="images/1_107/chat-title.mp4" title="Showing chat titles in action" autoplay loop controls muted></video>

This new chat title is configurable behind a new setting `setting(chat.viewTitle.enabled)`.

### Hide the Chat welcome banner in sidebar

A new setting `setting(chat.viewWelcome.enabled)` allows to hide the icon and welcome text in sidebar chat when no Chat is opened giving you a calmer Chat experience.

![Screenshot of empty chat view.](images/1_107/chat-welcome.png)

### Setting to not restore Chats in sidebar after restart

A new setting `setting(chat.restoreLastPanelSession)` allows to not restore the previous chat session into the sidebar after restart or when opening a different folder. By default we will restore the session. If you prefer an empty Chat experience when you restart or open a workspace, consider to change this setting.

### Diffs for edits to sensitive files

If the model attempts to edit a sensitive file, we'll now show the diff of the edits that would be made, rather than simply showing the raw edit edit.

### Local chat sessions continue running in background

Previously, when closing a local chat session, a running agent request would have been cancelled. Now, the local agent will continue running in the background, even when not open in a chat editor or view. You will be able to see the status of the running agent in the sessions list.
...
We added an indicator for when a workspace is opened in a window to the picker:

![Screenshot of recently opened workspaces.](images/1_107/pickers.png)

The currently active window is indicated slightly different from other opened windows to make that distinction clearer. Entries that are not opened in any window have no icon.

The indicator of which window is active has also been applied to the window picker.

### macOS: Mouse swipe to navigate

We added a new setting `setting(workbench.editor.swipeToNavigate)` that allows you to navigate between editors using 3-finger-swipe gesture with the trackpad on macOS. Swiping left or right will navigate across recently used editors in any editor group.
...
### Stashes in the Source Control Repositories view

This milestone we continued to expand the list of repository artifacts shown in the Source Control Repositoties view by adding a new "Stashes" node. Under this node you can see the complete list of stashes, view, apply, and pop each stash. The context menu also contains an action to drop each stash. You can enable the experimental repository explorer by setting the `setting(scm.repositories.selectionMode:single)` and `setting(scm.repositories.explorer:true)` settings. Please give it a try and let us know what other repository artifacts you would like to see in the repositories explorer.
...
### Control auto approval for chat tools

A new setting `setting(chat.tools.eligibleForAutoApproval)` has been added to require explicit approval for specific tools (e.g. `runTask`). Organizations can enforce this behavior via enterprise policy.

### Agent mode disabled by policy

When an enterprise policy disables agent mode, the mode picker now better communicates why agent mode is not available.

![Mode picker shows agent mode unavailable due to enterprise policy](images/1_107/agent-mode-disabled-by-policy.png)
