...
The `#textSearch` tool now support searching in ignored files/folders specified by `files.exclude` or `search.exclude` settings or `.gitignore` files, such as the `node_modules` folder. Also, it now returns hints to the agent about the ignored files/folders when there are no results, allowing agents to turn around and enable searching in those ignored files/folders.
...
When you receive a next edit suggestion that is outside the current viewport, it can be difficult to know what the suggestion is without scrolling away from your current position. We have improved this experience by rendering a preview of the next edit suggestion where your cursor is currently located. This helps reduce the impact on your flow when reviewing suggestions.
