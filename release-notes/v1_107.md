...
### Next rename suggestion

Next rename suggestion predicts a symbol rename alongside a normal textual next edit suggestion. When predicted, an additional indication is shown together with the normal textual edit. The symbol rename can then be executed using `Shift+Tab`. In the video below the users first renames the property `a` to `width`. The next rename suggestion then predicts to rename `b` to `height` as well as renaming the two parameters `a` and `b` accordingly. Next rename suggestion works best when it predicts related renames to other symbols.

<video src="images/1_107/next-rename-suggestion.mp4" title="Next rename suggestion" autoplay loop controls muted></video>

Please note that this feature is currently rolled out to our user base using an experiment and that it is only available for TypeScript right now.
