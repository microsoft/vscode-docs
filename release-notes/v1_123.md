...
#### Retry network-dependent terminal commands inside the sandbox

**Setting**: `setting(chat.agent.sandbox.retryWithAllowNetworkRequests)`

When a terminal command run by Copilot Chat using local agent requires access to domains that are not configured as allowed domains, it gets auto retried inside the sandbox with unrestricted network access before falling back to unsandboxed execution. This allows network-dependent operations such as git fetch to complete while keeping filesystem protections in place.

### Rubber duck agent (Preview)

**Setting**: `setting(chat.agentHost.rubberDuck.enabled)`

In Copilot CLI (local) sessions, you can now get a second opinion from the rubber duck agent, a built-in critic that provides feedback on your plans, code, and tests to help you catch issues early and improve the quality of your work.

To provide an independent perspective and reduce shared blind spots, it runs on a different language model than the one driving your session. The rubber duck agent has read-only access to your codebase and never edits files or runs commands that change your environment.

The rubber duck agent automatically provides feedback when useful and summarizes the critique in the timeline. You can also request a review explicitly by typing `/rubber-duck` followed by a prompt, for example:
...
For more details, see the [rubber duck agent documentation](https://code.visualstudio.com/docs/agents/agent-types/copilot-cli#_get-a-second-opinion-with-the-rubber-duck-agent).

> **Note**: The rubber duck agent is currently only available when you use a Claude or GPT language model.
...
Note that this delay does not apply to extensions from trusted publishers such as Microsoft, GitHub, and OpenAI. These extensions continue to update immediately.
