...
According to the rules of the settings, if a path is considered to be readonly, you cannot modify it from the explorer (e.g. delete it) and you will get a readonly text or notebook editor.

<video src="images/1_79/readonly-mode.mp4" controls title="Readonly mode"></video>

In addition, new commands have been added to change readonly state for the current session only:
