...
Notebooks now support CodeActions being run upon save. CodeActions can be specified under the `notebook.codeActionsOnSave` setting. Extension authors can define providers using the standard typings for cell level codeactions, or leverage the new `notebook.` prefix to define CodeActions that manage the entire notebook. [This](https://github.com/Yoyokrazy/clean-nb-imports-ext) sample extension, currently being worked on, can be referenced to learn how extensions can leverage this new setting.
