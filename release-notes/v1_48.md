...
We added an "Open Search Editor" command to open an existing search editor if one exists, otherwise create a new one. The preexisting command "Open new Search Editor" has been renamed to "New Search Editor", and will always create a new search editor.
...
There's a new setting `scm.alwaysShowRepositories` which makes the Source Control view always show the repository rows, even if only a single repository is open:
...
All of the text file encodings of the desktop version of VS Code are now supported when running in a browser as well.
...
The ability to copy values to your clipboard and paste them either inside the editor or into whatever other source you wish has been added.

![Copy and Paste](images/1_48/hex-copy-paste.gif)

**Find and Replace Support**

The hex editor now comes with a similar find and replace widget to that found within VS Code. It supports searching hexadecimal fields using wildcards (for example `FF ?? DD`) and regex searching on the decoded text section.
...
We improved the experience of drag and drop in Notebook. Dragging a collapsed markdown cell will move all nested cells inside the folding region.

![drag grouped cells](images/1_48/notebook-dnd-grouped-cells.gif)

**Editor reopen with**

You can now reopen the notebook document in text editor or any other available editor types directly from the context menu of the editor title
...
![cell collapse](images/1_48/cell-collapse.gif)
...
The [Language Support for Java](https://marketplace.visualstudio.com/items?itemName=redhat.java) extension now supports a lightweight mode, when you want to quickly review and navigate your Java source files without creating or opening a Java project. You'll still get powerful language features such as code navigation, outlining, and syntax checking for your source code as well as the JDK.
