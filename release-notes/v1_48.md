...
* **[New submenu layouts](#extensions-overflow-menu-cleanup)** - Refactored overflow menus for Extensions and Source Control.
* **[Settings Sync to Stable](#settings-sync)** - Settings Sync is now available for preview in stable.
* **[Debug Browser support](#debug-open-link-command)** - Start debugging any browser via website URL.
...
A new **Debug: Open Link** command has been added to quickly debug any URL. Previously, to debug a browser, you had to install the Debugger for Chrome extension and write a `launch.json` config file to debug a page. This command allows you to debug any URL without needing any additional launch configurations.
