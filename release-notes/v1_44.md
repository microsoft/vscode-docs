...
The following screencast shows the new progress UI for a (simulated) long running operation in Mock Debug (the only supporting debug extension as of today). We expect that other debug extensions will adopt the progress support soon.

![debug progress](images/1_44/debug-progress.gif)

### Code completion in the debug console now supports to control the selection

In the last milestone we've [added "selection control"](https://github.com/microsoft/debug-adapter-protocol/issues/87) to the "completion" request of the debug adapter protocol.
With this a debug extension can adjust the selection (or insertion point) after a completion item has been inserted.
In this milestone VS Code now fully supports the protocol addition.
...
### New "clipboard" context value for the "evaluate" request

DAP clients (frontends) are using the `evaluate` request when copying variable and expression values to the clipboard. To help debug adapters with better detecting this situation, a new value `clipboard` has been added to the value set for the `context` argument that is passed to the `evaluate` request.
To ensure backward compatibility, a client is only allowed to pass the new value if the debug adapter returns a `supportsClipboardContext` capability.
