...
* **[Better file Quick Open](#quick-open-rewrite)** - Navigate smoothly from files to symbols, filter files by folder name.
* **[Extension Pack display](#improved-extension-pack-display)** - Quickly see the contents of Extension Packs.
...
* **[New Python tutorials](#python)** - New tutorials for creating Python containers and building Data Science models.
...
You could use the debug terminal (**Debug: Create JavaScript Debug Terminal**) to automatically debug any Node.js process. Now, we also handle links to start debugging your browser whenever you `Cmd/Ctrl+Click` on links, no configuration needed.
...
When it's available in the Call Stack, you can now reference the function's `$returnValue` in the Debug Console and Watch expressions.
...
Certain views now support Welcome content contributed from extensions. Refer to the [original issue](https://github.com/microsoft/vscode/issues/89080) for more information. For details about how to use this API with extension provided views see the [tree view documentation.](/api/extension-guides/tree-view#welcome-content)
...
### New Extensions view context menu when clause context

We've added a new `extension` `when` clause context to the extension's context menu, which contains the extension's identifier.
