...
The HTML, Razor and Handlebar language support has been refactored to be a regular VS Code extensions which use a language server implementing the [language server protocol](https://github.com/Microsoft/language-server-protocol).

**Note:** Following language features were not ported during this refactoring but will be added back soon:
...
We haved added support for publishing and installing so called 'Extension Packs'. An Extension Pack is a set of extensions that are commonly used together. For example, it is now possible to create an Extension Pack for 'PHP Development' that includes a PHP formatter and the PHP debugger extension. 
