...
We added a new command to let you quickly move between open instances of VS Code.  Simply open the command pallette **TODO add Command** and type **Switch Window**, a picker showing all the opened VS Code Windows (instances) will pop up and you can quickly move from one to the other without leaving the keyboard and right from within VS Code.
...
We also made a few tweeks so that VS Code also now detects image file changes on disk so you no longer see stale old versions of an image.
...
The randomization for the experiments is done once per stable/insiders release stream. For existing users, you may notice command hints displayed in the empty editor window.  We really want to improve the discovery of key product features so expect to see us tweek these hints as time goes on.
...
Do you like to have your code formatted each time you save.  We certainly do :)  We now now support "Format On Save" which will automatically pick up relevant installed formatter extensions and format the whole document on each save. Enable "Format On Save" by setting `"editor.formatOnSave": true`
...
It can be a pain to change the file association for a given item over and over, and in the past to persist this it was a pretty mainual process.  Now when changing the Language Mode of a file, you can now pick **Configure File Association for...** and your choice will be stored to the [USer] settings file and remembered. You no longer have to manually open the file and type the change in there.
...
VS Code now has better handling of UTF-16 surrogate pairs and validates all edits such that it doesn't break up the surrogate pairs.  This is particularly important if you want to add things like Emojis into your code.
...
As you can see we recommend that anybody working on the VS Code code base is using the tslint and eslint extensions. In addition, to VS Code core we recommend to use the Chrome debugger extension.
...
The Yeoman generator for VS Code extensions has been updated to use TypeScript version 2.0.x. If you start a new VS Code extension simply install yeoman and the generator using `npm install -g yo generator-code` and run it using `yo code`. If you have an existing extension using TypeScript 1.8.x you can migrate it to TypeScript 2.0.3 using the following steps:

- open the `package.json` file and do the following changes:
  - change the TypeScript developer dependeny from `"typescript": "x.x.x"` to `"typescript": "^2.0.3"`.
  - add a developer dependency for the node typings using `"@types/node": "^6.0.40"`.
...
- open the `tsconfig.json` file and exchange the line `"noLib": true` with `"lib": [ "es6" ]` and the `"target": "es5"` with `"target": "es6"`. The file should look something like this:
...
The Yeoman generator has also been updated to make use of the new TypeScript 2.0.x features to help write JavaScript. If you want to migrate an extensing extension to use TypeScript 2.0.3 for JavaScript authoring do the following: 

- open the `package.json` file and do the following changes:
  - add a TypeScript developer dependeny `"typescript": "^2.0.3"`.
  - add a developer dependency for the node typings using `"@types/node": "^6.0.40"`
  - if your extension has Mocha tests change the developer dependeny to at least mocha version 2.3.3.
  - add a developer dependency for the Mocha typing using `"@types/mocha": "^2.2.32"`
- we also recomment to use ESLint for linting JavaScript code. So add the folloing entry as a developer dependency as well: `"eslint": "^3.6.0"`. In addition we recommend to install the [ESLint extensions](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint)
...
- open the `jsconfig.json` file and exchange the line `"noLib": true` with `"lib": [ "es6" ]` and the `"target": "es5"` with `"target": "es6"`. The file should look something like this:
...
* [@Eklavya](https://github.com/eklavyamirani)
  * Fixed: Markdown syntax highlighting to support alternative header styles [PR #11066](https://github.com/Microsoft/vscode/pull/11066)
