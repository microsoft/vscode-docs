...
* **[Format on Save](#format-on-save)** - Formatters can now hook into the file save event.
...
* **[Search term history](#search-term-history)** - Past search terms are available in the Search box.
...
* **[Workspace recommendations](#workspace-extension-recommendations)** - Your workspace can now provide suggestions for the extensions other members of the team should use.
* **[API for Settings](#new-apis)** - It's now possible to change settings via an API. This allows new options like persistent [Auto Save](#auto-save-menu-item), and [File Associations](#file-associations-configuration).
...
* **[*PREVIEW*  TS/JS Grammar](#preview-typescriptjavascript-grammar)** - A new colorizer for JS/TS with over 200 fixes. We will include this in the next release.

There are many other updates in this release as well, read on for the complete list...
...
* **[Extension Authoring](#extension-authoring)** - Adopting TypeScript 2.0 for JavaScript or TypeScript, New APIs (onWillSaveTextDocument, WorkSpaceConfiguration, Terminal processID, Create Terminal, Close terminal), Updates for Debug Extension Authoring (language neutral JSON schema, External Terminal support, Improved Initial Configuration contribution, breakpoint hit count).
* **[Miscellaneous](#miscellaneous)** - New location for dirty indicator in Tabs mode, Electron 1.3.7 update, Bundled extension support, Corrupt install notification, Improvements in issue reporting.
* **[New Commands](#new-commands)** - A quick summary of new commands this release.
* **[Notable Changes](#notable-changes)** - On top of all the features in the release, several notable issues are resolved.
...
* **[Download Links](#download-links)** - Release specific download links.
* **[Thank You](#thank-you)** - Community contributions to the code base.
...
Each release new commands become available, this is a quick summary list.
