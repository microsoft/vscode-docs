...
As soon as an icon theme is enabled, we now start to show icons in many more places throughout the UI. This includes most trees that show file icons (e.g. problems view, search view, opened editors) as well as results in the quick picker and editors.

A new setting `workbench.editor.showIcons` allows to control if icons should show up for editors.

![Icons](images/1_6/icons.png)

### Electron update

In this release we decided a major update of the Electron framework. We now run with version 1.3.5 which brings a big version increment to the Chrome rendering engine (from 49 to 52) and node.js (from 5.10.0 to 6.3.0). This also means that all extensions can expect the full node.js 6.3.0 runtime when running in this VS Code release. For a full list of changes in node.js please refer to https://github.com/nodejs/node/blob/master/CHANGELOG.md

### New command to quickly switch windows

A new command `Switch Window` was added that opens a picker showing all the opened windows and provides a fast way to switch to a specific window right from VS Code.

### Image dimensions in status bar

When you open a binary file in VS Code, some extra meta information is now displayed in the status bar. For all binary files we show the size of the file and for images we also show the image dimensions.

![Status](images/1_6/status.png)

You will also be happy to hear that icons refresh when they are changed on disk so you no longer see stale old versions of an image.

### Automatic configuration of auto save and file associations

We added support to make changes to the configuration files programmatically and this is exposed in some places in the workbench:
...
Using any of the methods `showInformationMessage`, `showWarningMessage` ,`showErrorMessage` you could always pass in a set of strings to show as actions next to the message. Previously the order of messages was reversed such as the action you pass in first shows last. We fixed this weird behaviour in this release and so please check your extension for usage of this API to see if you are impacted. We think that the new behaviour is more natural and thus did not add code that would keep the previous behaviour.
...
One change in this update can have a negative impact on your extension, so we want to call out early enough to make you aware: `fs.realpath` (and `fs.realpathSync`) got reimplemented in node.js 6.x and this brings behavioural changes on Windows. Best to describe with a table:

Assume that on Windows you have a path of `C:\Path\to\file.txt`. Note how the drive letter is upper case and the first folder is also capital case (`Path`). Assume you are using `fs.realpath` to normalize any path, here are the results depending on the node.js version: 
...
As you can see, node.js preserves the path that is being passed in and does not conversion of casing. In node.js 6.3.0, the casing of the path is returned as it is reported on Windows, thus you see an uppercase drive letter and the real casing of the first folder (`Path`).

If you are making assumptions about paths in your extension and have a need to compare paths, please keep this semantic change in mind. We encourage that you test your extension on Windows with VS Code to see if you run into issues. 
