...
The editor now supports *format on save*. It will automatically pick up installed formatters form extensions and format the whole document on save. Note, to keep cursors and selections stable we don't format when saving after delay. Enable format on save with `"editor.formatOnSave": true`

## Languages

### TypeScript

VS Code 1.6 ships with TypeScript 2.0.3 in the box. TypeScript 2.0.3 contains lots of new features, like strict null checking or modular libiray dependencies to only name a few. A complete list of the new features can be found [here](https://github.com/Microsoft/TypeScript/wiki/What%27s-new-in-TypeScript#typescript-20).

Besides new feature TypeScript 2.0.3 contains many bug fixes. This might result in additional compile errors when you first compile your project with TypeScript 2.0.3. Do the following to go back to TypeScript 1.8.10 in your workspace:

- change directory into your workspace folders
- run `npm install typescript@1.8.10` to install TypeScript version 1.8.10
- start VS Code. You will get promted whether you want to use the workspace version (1.8.10) or the bundled version (2.0.3) of TypeScript. Select workspace version and confirm the setting change. 
...
- The acquisition of Type Declaration files (typings) has been simplified. You can use `npm install --save @types/lodash` to install the typings.
...
* We added an `onWillSaveTextDocument`-event that is fired before a document is saved to disk. It allows to change the document enabling features like *remove unused imports on save*, *insert final newline*, etc.

* There is now `WorkspaceConfiguration#update` which allows extensions to add/update/remove configuration options. This should ease configuring the editor, think of configuring a linter from a warning via a code action.
