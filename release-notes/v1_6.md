...
This release has some big infrastructure changes for Visual Studio Code. We've updated to the latest Electron shell (version 1.3.7) and now include TypeScript 2.0.  We added programmatic access to user and workspace settings and you'll see this is action in several places in the UI (**File** > **Auto Save**, **Configure File Associations**). And we're continuing to improve Extension management (**Configure Workspace Recommended Extensions**) support for installing a **Extension Packs**). All of this work puts VS Code in great shape for us and our extension partners moving forward.
...
We often track down problems that are caused by an installed extension. To help us with this situation we have enriched the 'Help>Report Issues' action to seed the issue description with the list of installed extensions. 

Please use this action when reporting issues, then we have more context information to track down the issue.

### macOS Sierra work around
...
The hover widget went through a few UI improvement changes, aligning it more
with VS Code's Intellisense experience:

![hover](images/1_6/hover.png)

### Search History

You can now navigate to your previous search terms in global search view and in editor find widget.

* `kbstyle(Alt+Up)` naviagtes backward in the history. Corresponding commands are `search.history.showPrevious` and `find.history.showPrevious`
* `kbstyle(Alt+Down)` naviagtes forward in the history. Corresponding commands are `search.history.showNext` and `find.history.showNext`.
...
The HTML, Razor and Handlebar language support has been refactored to be regular VS Code extensions which use a language server implementing the [language server protocol](https://github.com/Microsoft/language-server-protocol).

**Note:** Following features are not ported back during this refactoring, which will be added back soon.
...
Often you have a set of extension that makes working with a particular workspace more productive. We now support that you can define which extensions are recommended for a workspace. The set of recommendations is defined in a file `extensions.json` which is located in the `.vscode` folder. In this way the recommendation can easily be shared inside a team. 

You can create this file using the `Extensions: Configure Workspace Recommended Extensions`. For example, this is the `extensions.json` file that we use for the [vscode workspace](https://github.com/Microsoft/vscode/blob/master/.vscode/extensions.json):
```json
{
	"recommendations": [
		"eg2.tslint",
...
```
An extension is identified using its publisher name and the extension identifier. You can see the name on the extension's detail page.
![Extension identifier](images/1_6/extension-identifier.png).

VS Code prompts a user to install the recommended extensions when a workspace is opened for the first time or the user can always request the list later as shown below.

![Show Recommendations](images/1_6/recommendations.png)

### Extension Packs

We haved added support for publishing and installing so called 'extension packs'. An extension pack is a set of extensions that are 
commonly used together. For example, it is now possible to create an extension pack for 'PHP Development' that includes a PHP formatter and the PHP debugger extension. 

An extension pack is represented as an extension that depends on other extensions. This dependency can be expressed using 
the `extensionDependencies` attribute inside the `package.json` file. For example, here is an extension pack for PHP that includes: debugger, language service, formatter:
...
bundled extensions development from VS Code's core development.
