...
By default, a user's cursor will be positioned at the bottom of the terminal Accessible View; to preserve the position instead, you can set `terminal.integrated.accessibleViewPreserveCursorPosition` to `true`.
...
<video src="images/1_83/chat-inline-stream.mp4" controls title="Inline Chat Making Progressive Edits"></video>
...
This iteration, we built a new way to interact with Copilot Chat: agents. Agents are like experts who have a speciality that they can help you with, and you can talk to them in the chat by mentioning them with the `@` symbol. Currently, there are two agents:
...
Each agent also supports a few slash commands, and the slash commands that you may have used before should now be used with an agent. For example, `/explain` is now `@workspace /explain`. But as a shortcut, you can also just type `/explain` and the chat will automatically add the `@workspace` agent for you.
