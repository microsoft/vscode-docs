...
Initial support for code highlighting across multiple documents has been added via the setting **Editor: Multi Document Occurrences** (`editor.multiDocumentOccurrencesHighlight`). This initial implementation features only textual occurrences, with support for semantic highlighting coming in the future.
...
`@workspace` uses a meta prompt to determine the information to collect from the workspace to help answer your question. One approach used by the meta prompt is to look back at your conversation history to resolve ambiguous words/phrases in a question. For example, if you ask `@workspace What does it do?`, the meta prompt will now consider the history to figure out what `it` actually is and what information to collect to answer the question. The meta prompt also uses a wide set of terms, including more synomyms, to generate a list of potentially relevant terms.
...
There is also a new button **Show output** to open the Test Logs more easily from the Test Explorer view when errors on test discovery occur.
...
[FastAPI](https://fastapi.tiangolo.com/) is a modern and fast web framework for building Python APIs, and it's become more and more popular thanks to its simplicity and performance.
...
The [Tensorboard](https://www.tensorflow.org/tensorboard) functionality has now been moved out of the Python extension into the standalone [Tensorboard](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.tensorboard) extension.
...
Support for language servers in WASM/WASI got added to the experimental [wasm-wasi-core](https://marketplace.visualstudio.com/items?itemName=ms-vscode.wasm-wasi-core) extension. An extension show casing a simple language server written in Rust and compiled to WASM can be found in the [vscode-wasm repo](https://github.com/microsoft/vscode-wasm/blob/d4fe7c2f4bec260bb97f4f83cec299e146faa985/testbeds/lsp-rust). It depends on the [language server crate](https://docs.rs/lsp-server/latest/lsp_server/) maintained by the [Rust analyzer team](https://github.com/rust-lang/rust-analyzer).

## Extension authoring

### Improved test runner

We now provide a command-line runner and extension for VS Code to make running tests for extensions easier. Extensions using the new approach can now be run in VS Code's testing UI. While some migration is required, this generally only takes a few minutes. See [our documentation](https://code.visualstudio.com/api/working-with-extensions/testing-extension) for more information.
...
File icon themes authors can now define name specific icons for root folders using the new properties `rootFolderNames` and `rootFolderNamesExpanded`. See the [File Icon Theme guide](https://code.visualstudio.com/api/extension-guides/file-icon-theme) for more information.
