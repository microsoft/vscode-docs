...
We introduced a method to maximize an editor group. This will hide all other groups and adds a button to the maximized group, allowing the user to restore the previous layout. When changing the setting `workbench.editor.doubleClickTabToToggleEditorGroupSizes` to `maximize`, users can double click on an editor tab to maximize & unamximize the editor group.
