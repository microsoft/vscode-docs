...
Generated imports are now always put to the top of the file or below existing import blocks. This is supported for the most common programming languages.
...
Copilot can now offer CLI command suggestions via the `kb(inlineChat.start)` keybinding when the terminal is focused. This will bring up Quick Chat with `@workspace /terminal` pre-filled:
