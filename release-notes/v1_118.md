...
**Last-two-messages breakpoint strategy.** In long agent sessions, older turns eventually fall out of the cacheable window. We now anchor cache breakpoints on **System prompt, the Tools list, and the two most recent messages**. This is currently available behind the `setting(github.copilot.chat.anthropic.cacheBreakpoints.lastTwoMessages)` setting.
