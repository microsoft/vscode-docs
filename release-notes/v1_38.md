...
We continue to fix [accessibility issues](https://github.com/microsoft/vscode/issues?q=is%3Aissue+label%3Aaccessibility+milestone%3A%22August+2019%22+is%3Aclosed) in this milestone.
Highlights are that the Windows Magnifier Tool can now follow the cursor in the editor.
And  word navigation is now complimentary to what NVDA and VoiceOver expect, thus words are properly announced when navigating using word navigation in the editor.
...
From the variables view now it is possible to set data breakpoints that will get hit when the value of the underlying variable changes.
Just like other breakpoints, data breakpoints can be disabled / enabled and removed in the breakpoints view.

Please note that data breakpoints are currently only supported by our Mock Debug extension and that other debug extensions will adopt this soon.
Only debug extensions that use a runtime that supports data breakpoints can support this, like C++ and C# for example.

![data breakpoints](images/1_38/data-breakpoints.png)

### Call Stack view improvements

We have done some improvments in the call stack view, most notably:
...
* Previously when there was a new debug session we would always focus, now we only focus a session automatically if it breaks.
* Debug session elements only have the twistie rendered when there are threads for that session
