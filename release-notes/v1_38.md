...
### Settings editor validation for array-of-string settings

The settings editor now displays error for array-of-string items if the current value does not satisfy the specified `minItems`, `maxItems`, `item.enum` or `item.pattern` schema.

![String of array error valdiation](images/1_38/string-of-array-error-validation.png)

### Preserve Case in Search/Replace

In 1.37 we added a "preserve case" option to the editor's find/replace widget. The same option is now available as the "AB" icon in search.

There is also a new case preservation mode for hypen-separated words.
...
You can now customize how many visible lines around the cursor should be displayed while moving cursor towards viewport boundaries by setting `editor.cursorSurroundingLines`. In Vim this feature is called `scrollOff`.
...
We've added new actions for copying or reverting deleted content in inline diff editor. When hovering on deleted content in the inline diff editor, you can now
...
Decorations on search results in the minimap will now appear for both searches within files and global search.
...
We thank MDN documentation team for their effort in curating [mdn-data](https://github.com/mdn/data) / [mdn-browser-compat-data](https://github.com/mdn/browser-compat-data) and making MDN resources easily accessible by VS Code.
...
When you add an import or apply a refactoring in JavaScript or Typescript code, we now try to infer whether or not we should include semicolons from existing code in the file:
...
We now expose commands for navigating through search results in Find Widget from Editor, Integrated Terminal, Extension Details View and Webviews.
...
`kb(editor.action.webvieweditor.findNext)`|Find Previous Result in Webview|`editor.action.webvieweditor.findNext`
