...
The new engine provides more flexibility for the workbench layout in the future. For now, this manifests itself with the ability to hide the editor area. There is a new command available with the grid layout, **Toggle Editor Area**. This will hide the editor area and allow the panel to take over completely. The **Toggle Maximized Panel** command has the same effect now. This means that a maximized panel is now truly maximized without the tiny gap previously left of the editor.
