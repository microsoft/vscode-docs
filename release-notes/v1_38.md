...
In the 1.37 release, we added a **Preserve Case** option to the editor's Find/Replace control. The same option is now available as the **AB** icon in search.
...
We have improved the process of creating top-level files and folders in the Explorer. There are now three ways to create a file or folder in the top level of the File Explorer:
...
There is a new setting, `explorer.incrementalNaming`, to control file naming, which can have the values `simple` or `smart`.

* `simple` - Appends the word "copy" at the end of the duplicated name potentially followed by a number. The current VS Code stable behavior.
...
The minimap (code overview) now shows search results for both searches within a file and global search.
...
Is your code getting a little ahead of itself? The new 'add missing await' quick fix help you `await` values that you may have overlooked:

![Detecting missing calls to await in JavaScript using @ts-check and quick fixing them](images/1_38/js-await-quickfix.gif)

This quick fix is available in TypeScript and in JavaScript code that has [type checking enabled](https://code.visualstudio.com/Docs/languages/javascript#_type-checking).
...
![branch name](images/1_38/branch-name.png)

### Git: Allow to sort branches alphabetically

There's a new `git.branchSortOrder` setting which allows to change the order of branches when checking out to a branch with the `Git: Checkout to...` command.
...
There have been numerous improvements to the Screencast Mode, via PR contributions from the community, namely:

- There's now a limit on the length of the screencast keybinding label
- The keybinding label vertical position can now be ajusted via the `screencastMode.verticalOffset` setting
...
- `html.experimental.customData` setting is deprecated and superseded by `html.customData` setting.
- `css.experimental.customData` setting is deprecated and superseded by `css.customData` setting.
- `contributes.html.experimental.customData` Contribution Point is deprecated and superseded by `contributes.html.customData` Contribution Point.
- `contributes.css.experimental.customData` Contribution Point is deprecated and superseded by `contributes.css.customData` Contribution Point.
...
We now expose commands for navigating through search results in Find control from Editor, Integrated Terminal, Extension Details view, and Webviews.
