...
* **[Optional chaining support in JS/TS](#optional-chaining-support-for-javascript-and-typescript)** - Use the `?.` operator to simplify working with nested structures.
...
* **[Visual Studio Online](#visual-studio-online)** - Create and connect to cloud-based development environments.
...
The example above uses the Node.js [`fs.stat` API](https://nodejs.org/api/fs.html#fs_fs_stat_path_options_callback) to get information about a file. But there's a bug! `.isDirectory` is a actually function, not a property! This means that `handleDirectory` will be called on every file, not just directories as intended.
...
The call hierarchy API is generally available and language extension authors are busy implementing it, so we have added a Call Hierarchy view. This complements the **Peek Call Hierarchy** feature and shares the same view as **Find All References** and **Find All Implementations**. Once a language extension supports the call hierarchy API, you be able to select **Show Call Hierarchy** from the context menu or the Command Palette to see the view:
...
* Remote - Containers: Easier to try out sample repositories (for example https://github.com/microsoft/vscode-remote-try-python) in a container.
* Remote - Containers: Improved performance when creating a container and more configuration options in the `devContainer.json` file.
