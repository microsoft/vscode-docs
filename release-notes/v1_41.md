...
<!-- TODO mjbvz: image of extract interface -->

### Semicolon formatter options for JavaScript and TypeScript

The new `javascript.format.semicolons` and `typescript.format.semicolons` formatting settings let you control how the formatter handls semicolons in JavaScript and TypeScript files. Valid settings values are:

- `ignore` — The default. Does not add or remove semicolons.
- `insert` — Insets semicolons at statement ends.
- `remove` — Remove unnessisary semicolons.
...
The code uses the node.js [`fs.stat` api](https://nodejs.org/api/fs.html#fs_fs_stat_path_options_callback) to get information about a file, but the programmar has forgotten that `.isDirectory` is a function, not a property! This means that `handleDirectory` will be called for every file, not just directories.

![VS Code can alert when you forget to call a function ](images/1_41/js-missing-function-call.png)

Note that you will only see this diagnostic when in strict mode.
...
VS Code encourages extensions to implement refactoring functionaly using [Code Actions](https://github.com/microsoft/vscode-extension-samples/tree/master/code-actions-sample). However it is difficult to discover which refactorings an extension supports and to understand why a given refactoring is not available for a specific selection. The `CodeAction.disabled` api proposal aims to  help extensions make their refactorings more discoverable.
...
<!-- TODO mjbvz: image of error message -->
