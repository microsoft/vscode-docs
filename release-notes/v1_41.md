...
We have added new proposed API for a [semantic tokens provider](https://github.com/microsoft/vscode/issues/86415). A semantic tokens provider can enrich existing TextMate grammars syntactical highlighting and provide semantic tokens which will take priority over the TextMate ones.

This new API is focused on efficient data encoding, especially in terms of memory usage and incremental updates. If you are interested in this topic, we would like to get your feedback regarding the current proposal and we would love to encourage experimentation and some early implementations.
