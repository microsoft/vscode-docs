...
The HTML language support now follow id (#) html links.

### JSON

To avoid performance issues with large files, the JSON language supports has a upper limit of folding regions and document symbols (for outline, breadcrumb) it computes. By the default the limit it is 5000, but you can now change hat limit with the setting `json.maxItemsComputed`.
