...
VS Code used to do nothing when selecting **Go to Definition** while already at the definition. With this release, alternative commands can be executed instead. For instance, **Go to Definition** can be mapped to run **Go to References**.

This can be customized via the `editor.gotoLocation.alternativeDefinitionCommand` setting. For example, you can make the alternative of **Go to Definition** be **Go to Declaration** and vice versa. You can leave the setting empty if you prefer the old behavior.
...
In contrast, **Go to Implementations** is about navigation and gets you to an implementation quickly. The video below (using the `"editor.gotoLocation.multipleImplementations": "goto"` setting) navigates to all implementations of the `ICommandService` interface by repeatedly pressing `kb(goToNextReference)`:

![Go to Implementations](images/1_41/goto-impl.gif)

Theme: GitHub Sharp, Font: FiraCode

Last, but not least and just like the **Find All References** command, there is now **Find All Implementations**. It presents the implementations as a stable list to the side of the editor. This can be used as a TODO list, for example when performing a refactoring that affects all implementations of an interface.
