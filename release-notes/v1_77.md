...
It is now possible to focus on the hover control and scroll horizontally and vertically with the `up`, `down`, `home`, `end`, `page up` and `page down` keys. The keyboard shortcuts to focus the hover are by default the same as those to show the hover at the primary cursor position.
