...
As more and more extensions are creating custom views and the majority of them are contributed to the Explorer, we noticed the Explorer was getting cluttered. To scale up, VS Code now provides a new `Test` view container and also allows you to extend the Activity Bar by contributing new view containers.
