...
* **[Middle mouse button column selection](#column-selection-using-middle-mouse-button)** - Use the middle mouse button for fast column selection.
...
* **[Workbench](#workbench)** - Copy search results, Git clone improvements, VS Code process explorer.
...
The Git extension also adopted the new proposed API to handle system-wide URIs and invokes Git clone on specific URIs. Here's an example URI for VS Code Insiders which clones `https://github.com/Microsoft/vscode-vsce.git`:
...
In this release, with data sourced from [Mozilla Developer Network](https://github.com/mdn/data), VS Code now identifies 87 new CSS properties. If you can find a property on MDN, VS Code should recognize it.
...
The JavaScript and TypeScript organize imports feature is now out of preview. Run **Organize Imports** (`kb(typescript.organizeImports)`) to quickly remove unused imports and sort the remaining imports in your JavaScript and TypeScript source code.
