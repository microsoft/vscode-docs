...
A command is registered to show each registered view container. For example, **View: Show Package Explorer**. You can also find all view containers listed when you run the command **View: Open View...**.
...
Extensions can now serve files and folders from arbitrary sources, like ftp-servers, and VS Code will handle them just like regular files. For that, we have defined the `FileSystemProvider` interface which extension associate with a URI scheme, like `ftp`. With that, the editor can discover and manage files and folders, for example, create new files or folders, delete them, read and write them.

![Files and Folders from a filesystem provider](images/1_23/fsp.png)

VS Code can also read and modify configuration files (`settings.json`, `tasks.json`, `launch.json`) served by the FileSystem Providers.
