...
The new `editor.codeActionsOnSave` setting lets you configure a set of code actions that are run when a file is saved. For JavaScript, TypeScript, and other extensions that contribute an organize imports code action, you can enable organize imports on save by [setting](https://code.visualstudio.com/docs/getstarted/settings):
