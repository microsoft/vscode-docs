...
A command is registered for each registered view container to show it. Example: `View: Show Package Explorer`. You can also find all view containers listed when you run the command `View: Open View...`.
...
VS Code also handles configuration (`settings`, `tasks` & `launch`) files served by the FileSystem Providers and customises accordingly.
