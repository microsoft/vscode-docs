...
As more and more extensions are creating custom views and the majority of them are contributed to the Explorer, we noticed the Activity Bar was getting cluttered. To scale up, VS Code now provides a new `Test` view container and also allows you to extend the Activity Bar by contributing new view containers.
...
You can now group the custom view contributions into a new view container in the Activity Bar using the contribution point `viewsContainers`.
