...
The setting `editor.experimentalEditContextEnabled` is now enabled by default on stable. This means that the input of the editor will now be powered by the EditContext API. Please see the [MDN docs](https://developer.mozilla.org/en-US/docs/Web/API/EditContext_API) for more detail on the API.
