...
The NodeJS extension host is now updated to v22 from v20 as part of our [runtime update](#electron-35-update). This updates brings is support for [`navigator` global object](https://github.com/nodejs/node/commit/b40f0c30743aaecd57071f7be305df43e1083817) in the desktop and remote extension hosts. If your extension relied on the presence of `navigator` to detect web environment then this would break today. We recommend migrating the detection logic to rely on `process.versions.node` to confirm desktop and remote extension host environments.
