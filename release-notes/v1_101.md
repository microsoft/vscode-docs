...
You can now specify the menu style used for the menubar and context menus on Windows and Linux, and for context menus on macOS. The `setting(window.menuStyle)` setting lets you choose whether menus are rendered as `native` (by the OS), `custom` (by VS Code), or `inherit` (which matches the style of the title bar as set by `setting(window.titleBarStyle)`). This allows for using a native title bar with a custom menu bar and context menus.
...
The setting `setting(github.copilot.chat.notebook.followCellExecution.enabled)` has been introduced, allowing users to enable or disable the follow mode for agent cell execution in Jupyter Notebooks. When enabled, the Notebook view will automatically scroll to the cell that is currently being executed by the agent.

Once the agent has used the run cell tool, the Notebook toolbar will be updated with a pin icon indicating the state of the follow mode. Users can toggle the behavior mid agent response without changing the base setting value, allowing them to follow the work of the agent in real-time, and toggle it off when they choose to review a specific portion of code while the agent continues to iterate. When they wish to follow again, they can simple toggle the mode, and they will join at the next execution.

<video src="images/1_101/notebook-follow-mode.mp4" title="Video that shows copilot executing cells in a notebook with follow mode enabled. When the cell is run, the notebook scrolls to reveal it." autoplay loop controls muted></video>
