...
In this example, I use the copilot to generate a prompt saved using the [Gistpad MCP server](https://github.com/lostintangent/gistpad-mcp), and then use it generate a changelog entry:

<video src="images/1_101/mcp-prompts.mp4" autoplay loop controls muted></video>

### MCP: Support for Resources

VS Code's Model Context Protocol support now includes resource support. This includes support for resource templates is available in several places:

1. Resources returned from MCP tool calls are available to the model and can be saved in chat, either via a save button or by dragging the resource until the file Explorer view.
...
* [draft spec](https://modelcontextprotocol.io/specification/draft/basic/authorization) where the MCP server behaves as a resource server (which is expected to be finalized any day now).
...
and which account that server should use (via the gear button in the previous quick pick):
...
Remember, you can use the `MCP: Add Server...` command to add MCP servers. This is the same entrypoint for servers with authentication.
...
Sampling support is still preliminary and we plan to expand improve it in future iterations.
...
* `watch` is a file glob pattern. When any of the watched files change, the MCP server will be restarted.
* `debug` allows you to set up a debugger with the MCP server. Currently we only support debugging Node.js and Python servers launched with `node` and `python` respectively.
...
### Fetch tool confirmation update

The Fetch tool has more descriptive text to warn about potential prompt injection:

![fetch with warning about prompt injection](images/1_101/fetch-warning.png)

### Customize more built-in tools

It's now possible to enable/disable all built-in tools in agent mode or your custom mode. For example, disable `editFiles` to disallow agent mode to edit files directly, or `runCommands` for running terminal commands.

In agent mode, click the "Configure Tools" button to open the tool picker, and select your desired set of tools.
...
### Settings editor AI search suggestions (Preview)

**Setting**: `setting(workbench.settings.showAISearchToggle:true)`

This iteration, we added a toggle to the Settings editor that kicks off an AI search to find semantically similar results instead of results that are based on string matching. For example, AI search finds the `editor.fontSize` setting  when you search "increase text size". However, not every query reveals such accurate results, and we welcome feedback on when a natural language query did not find an expected setting.

For the next iteration, we are also considering removing the toggle and changing the experimental setting to one that controls when to directly append the slower AI search results to the end of the list.
...
We have now implemented shell environment discovery for PowerShell on Windows. This means that VS Code will now inherit any environment configured in the PowerShell profiles, such as the PATH updates Node.js installed through various version managers.
...
#### Improvements to long running agent workflows

The agent has access to an internal Notebook Summary tool to help keep the agent on track with an accurate context. That summary will also be included when summarizing the conversation history when the context gets too large to keep the agent going through complex operations.

#### Cell preview in run confirmation

A snippet of the code is shown from a notebook cell when the agent requests confirmation to run that cell. The cell links in the chat now also directly navigate to cells in the notebook.
...
Extensions can now publish collections of MCP servers. This allows you to bundle MCP servers with your extension or build extensions that dynamically discover MCP servers from other sources. Learn more on our [MCP extension development guide](/api/extension-guides/mcp) or by checking out the [MCP extension sample](https://github.com/microsoft/vscode-extension-samples/blob/main/mcp-extension-sample).
...
Currently only leveraged in [MCP authentication](#mcp-support-for-auth), this API proposal allows your `AuthenticationProvider` to declare the authorization servers that are associated with it.
...
and [in the registation](https://github.com/microsoft/vscode/blob/c9f19f8ad20aa88b1d924c8f7ff8d6b9f6269be8/extensions/microsoft-authentication/src/extensionV2.ts#L78-L88):
```ts
...
Then, when a caller passes in an authorization server url when it asks for auth, that will be passed down to both the `getSessions` and `createSession` functions via the `AuthenticationProviderSessionOptions` that are already present.

As mentioned above, this is currently used in MCP support where we receive the authorization server url to authenticate with from the MCP server. That url is then mapped to an auth provider, or if none exists, an auth provider is dynamically created for that auth server.
