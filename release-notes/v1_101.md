...
You can now specify the menu style that is used for the menu bar and context menus on Windows and Linux, and for context menus on macOS. The `setting(window.menuStyle)` setting lets you choose whether menus are rendered as `native` (by the OS), `custom` (by VS Code), or `inherit` (which matches the style of the title bar as set by `setting(window.titleBarStyle)`). This allows for using a native title bar with a custom menu bar and context menus.
...
NES is enabled for all VS Code Insiders users, and it will progressively be enabled by default for all Stable users as well. You can enable NES yourself via its setting at any time.
