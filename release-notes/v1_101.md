...
This iteration, we're refreshing the UI of the chat. User messages will now be more distinct and an options to undo requests is more visible in each request. Attachments from the chat input box are also more navigable.

![A video of the new chat UI/UX where a request is removed to undo edits since that point. ](images/1_101/new-chat-ui-ux.mp4)

### Implicit Context Updates

Implicit context will no longer be automatically included in requests. Instead, suggested context can be explicitely added to include the entire file as context. Hints are also quietly added in each user request, including the current open file, selection, or open notebook.
