...
Configuring tasks and problem matchers can be tricky. Use the `Fix with Copilot` action offered when there are errors in your task configuration to address them quickly and efficiently.
...
### Gracefully shutdown terminal and task processes on macOS and Linux

We’ve added an opt-in setting, `setting(terminal.integrated.killGracefully)`, to cleanly shut down terminal processes—including child processes. This helps prevent orphaned processes, free up ports, and reduce the risk of resource leaks. The setting applies to macOS and Linux; Windows already handles this behavior by default.
