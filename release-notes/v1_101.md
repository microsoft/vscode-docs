...
The setting `editor.experimentalEditContextEnabled` is now enabled by default on stable. This means that the input of the editor will now be powered by the EditContext API. This fixes numerous bugs especially in relation to the IME experience and generally going forward will pave the way for a more versatile and robust input experience within the editor. If interested, please see the [MDN docs](https://developer.mozilla.org/en-US/docs/Web/API/EditContext_API) for more detail on the API.
