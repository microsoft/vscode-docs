...
This milestone, we added a toggle to the Settings editor that starts an AI search to find semantically similar results, instead of results that are based on string matching. For example, AI search finds the `editor.fontSize` setting  when you search for "increase text size". However, not every query reveals such accurate results, and we welcome feedback on when a natural language query did not find an expected setting.
