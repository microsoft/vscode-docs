...
![custom chat mode editos](images/1_101/custom-chat-mode-editor.png)
...
Accepting next edit suggestions is now more seamless. Once you accept a suggestion, you can continue accepting subsequent suggestions with a single `Tab` press, as long as you haven't started typing again. If you type, you'll need to press `Tab` to move the cursor to the next suggestion before accepting it.
...
VSCE now scans for secrets when packaging your extension. If any potential secrets (such as API keys, tokens, credentials, or environment variable files like `.env`) are detected in your source files, VSCE will display an error during the packaging process. This helps you avoid accidentally publishing sensitive information to the Marketplace. Review and address any error before publishing your extension.

If you need to bypass specific checks, you can use the `--allow-package-secrets <secret_type>` or `--allow-package-env-file` flags when running VSCE. These flags let you configure which secret or environment file checks can be skipped during packaging.
