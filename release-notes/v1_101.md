...
This iteration, we added a toggle to the Settings editor that kicks off an AI search to find semantically similar results instead of results that are based on string matching. This search mode only runs when you switch on the toggle.

For the next iteration, we are considering removing the toggle and changing the setting to one that controls when to directly append the slower AI search results to the end of the list.
