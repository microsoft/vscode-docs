...
Implicit context, such as the contents of the current file, is no longer automatically included in chat requests. Instead, we offer the current file and its content as a suggested context item. Just select the context item to add or remove it from context.

In addition, we are adding hints in each chat request, including the current open file, selection, or open notebook. You might notice that the language model uses these hints to get additional context, if needed.
...
This milestone, we added a toggle to the Settings editor that starts an AI search to find semantically similar results, instead of results that are based on string matching. For example, AI search finds the `editor.fontSize` setting when you search for "increase text size". We are in the process of identifying and fixing some of the less accurate search results, and we welcome feedback on when a natural language query did not find an expected setting.
...
Adressing a popular feature request, starting with this milestone, clicking on an item in the Source Control Graph view will reveal the resources of the history item. Users can toggle the view mode of the resources (list vs. tree) from the `...` menu. Users can still open all resources of a history item in the multi-file diff editor using the "Open Changes" inline action. Clicking on a particular resource opens the diff editor, while clicking on the "Open File" inline action open the file at the particular version.

### Source Control History Item Chat Context

Users can now add a source control history item as context to a chat request. This can be done either using the "Add Context" action, then choosing "Source Control...", and picking a particular history item or by right clicking on a history item in the Source Control Graph view and using "Copilot, Add History Item to Chat" context menu action.
