...
This milestone, we added a toggle to the Settings editor that starts an AI search with Copilot to find semantically similar results instead of results that are based on string matching. For example, the AI search finds the `editor.fontSize` setting when you search for "increase text size".
