...
* **[Pull Request extension](#pull-request-extension)** - Review, comment, and verify GitHub PRs within VS Code.
...
* **[Built-in Loaded Scripts view](#loaded-scripts-view-now-built-in)** - Loaded Scripts Explorer now available for debug extensions.
...
The [GitHub Pull Request extension](https://marketplace.visualstudio.com/items/github.vscode-pull-request-github) is now available as a preview. This extension adds a review flow to VS Code, allowing you to view, comment on, and verify pull requests without leaving the editor.

![GitHub Pull Request](images/1_27/github_pull_requests.gif)

From the extension, you can browse pull requests by category, viewing all of the associated file changes and an overview for each pull request. From the overview, you can add comments, approve or request changes on the pull request, and check out the changes. By checking out the pull request, the changes can be reviewed in the full context of the surrounding source code, with the power to navigate and debug as you would with any code.

## Editor

### Auto closing & surrounding characters

When typing certain brackets like `{`, `[` or `(` - depending on the programming language -, they will get auto-closed by default when they are followed by specific characters that typically cannot start an expression, like `;:.,=}])>`. This behaviour can be customized by tweaking `editor.autoClosingBrackets`. Similarly, it is possible to customize the behaviour of typing certain quotes like `"`, `'` or `` ` `` with `editor.autoClosingQuotes`.

When having a selection and typing one of these characters, the selection will be surrounded by brackets or quotes. It is now possible to tweak this behaviour separately via `editor.autoSurround`.
...
### Go to Definition for CSS links

Go to Definition for `@import` and `url()` links in CSS, SCSS and Less.
...
Previously, when you are building Language Server extensions using `vscode-languageclient`, you can specify `[langId].trace.server` to send [LSP logs](https://code.visualstudio.com/docs/extensions/example-language-server#_logging-support-for-language-server) into a VS Code output channel.

The new `[langId].trace.server` could output log in a machine-readable [JSON format](https://github.com/Microsoft/language-server-protocol-inspector#log-format):
...
This opens up interesting use-cases, such as streaming LSP logs into a [LSP Inspector](https://marketplace.visualstudio.com/items?itemName=octref.lsp-inspector-webview) to visualize the behavior of a Language Server:

![lsp-streaming](https://user-images.githubusercontent.com/4033249/45078686-7a22e600-b0a5-11e8-9c75-0d0dc3ec8256.gif)
