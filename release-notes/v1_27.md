...
It's not possible to set keybindings to only work under specific operating systems using `isLinux`, `isMac` and `isWindows` within the keybinding's `when` clause:
...
For more than a year the Loaded Scripts Explorer in the Debug viewlet was contributed by the Node.js debug extensions and was not available to other debuggers. In this release we've "promoted" the Loaded Scripts view to become a built-in and first class debugger feature. With this all debug extensions that have the notion of "scripts" will be able to support access to their scripts in a dynamically updated view.

In addition we've tried to improve some shortcomings of the old Loaded Scripts Explorer:
