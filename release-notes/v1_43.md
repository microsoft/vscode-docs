...
* **[Minimap sizing options](#minimap-size)** - New modes to fit or fill the minimap vertical layout.
* **[Column selection mode](#column-selection-mode)** - Column mode toggle lets you quickly select blocks of text.
* **[Display whitespace on selection](#render-whitespace-on-selection)** - See whitespace characters only in selected text.
...
* **[JS/TS Call Hierarchy view](#call-hierarchy-support-for-javascript-and-typescript)** - See all calls to or from a function and drill into callers of callers.
...
Below you can see the tab and space whitespace characters in the selected region:
...
>**Note**: File operations, like renaming a file, are not yet supported, so for example renaming a class name in Java (where the file name gets renamed as well) is not yet supported in the cross-file undo.
...
* Remote - WSL: See the recent [Using Docker in WSL 2](https://code.visualstudio.com/blogs/2020/03/02/docker-in-wsl2) blog post.
...
- Simplifying creation of custom editor for text files by introducing `CustomTextEditor`. This custom editor uses VS Code's normal `TextDocument` as its data model, so that you don't have to implement `save`, `backup`, and other operations yourself. This also makes it easy to have live synchronization between a custom editor for a resource and a text editor for the same resource.

- Introduction of `CustomDocument` to store for model that a binary `CustomEditor` works with. There is one `CustomDocument` per resource, but a user may open multiple custom editors on that resource.

Please continue to test and share feedback out our [custom editor API proposal](https://github.com/microsoft/vscode/issues/77131). Although the specifics of the API may still change, we believe the current proposal reflects what we are hoping to finalize soon.
...
We are tracking an issue from **Windows** users with high dynamic range (HDR) displays in [issue #88491](https://github.com/microsoft/vscode/issues/88491) where VS Code looks dull and unsuable. This is an issue with all Chromium-based applications on 78.0.3904.130 and the issue is fixed in more recent version of Chromium 81. VS Code will get this fix once we upgrade to Electron 9 and there are technical reasons why the fix couldn't be backported to Chromium 78. In the meantime, we have a temporary workaround for this issue:
...
We added this default runtime argument so that we can override Chromium's default color correction to sRGB and instead use the display color space on macOS to avoid problems like [issue #84154](https://github.com/microsoft/vscode/issues/84154) but this doesn't work for HDR displays.
...
VS Code uses [Azure Pipelines](https://azure.microsoft.com/services/devops/pipelines/) for its continuous integration builds and we recently improved the build performance considerably. You can learn more about this in the [Improving CI Build Times
