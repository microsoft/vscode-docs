...
When undoing a rename or a code action that affects multiple files, VS Code will now allow to undo the change in all the affected files. A prompt will be presented when undoing a cross file edit:

![Undo across files prompt](images/1_43/undo-across-files-prompt.png)

> File operations, like renaming a file, are not yet supported, so for example renaming a class name in Java (where the file name gets renamed as well) is not yet supported in the cross-file undo.

### Minimap size

There is now a new setting, `editor.minimap.size`, which controls the way in which the minimap deals with vertical space:
* when set to `proportional` (the default), each line gets a certain amount of vertical space in the minimap, depending on `editor.minimap.scale`. For example, on a high-DPI monitor, with a configured scale of `1` (the default), each line gets 4px in the minimap. So for a file of 100 lines, the height of the lines rendered in the minimap is 400px, and for a file of 1000 lines, it is 4000px. That means that for large files, the minimap will also scroll, as the current file is scrolled.
...
There are now new colors available for customizing the minimap and its slider. One of the new colors is `minimap.background` and when the color is defined to be transparent, the minimap will be rendered transparently:

![Transparent minimap](images/1_43/transparent-minimap.png)

### Column Selection mode

There is now a new global toggle, accessible via the `Selection > Column Selection Mode` menu. Once this mode is entered, the status bar will have a new entry to reflect that this mode is active, and the mouse gestures and the arrow keys will create a column selection:
...
The existing `editor.rulers` has been extended to allow defining rulers that have custom colors. In the following screenshot, there is a magenta ruler at column 80 and a red one at column 90:

![Colored rulers](images/1_43/colored-rulers.png)

### Editor top and bottom padding

It is now possible to reserve some whitespace at the top or bottom of the editor using `editor.padding.top` and `editor.padding.bottom`.

### Scrolling predominant axis

When trying to scroll vertically using a modern touchpad, it oftentimes happens that the file scrolls horizontally as well. There is now a new setting, `editor.scrollPredominantAxis` (which is set to true by default) which blocks "diagonal" scrolling. This enforces that scrolling gestures either end up scrolling vertically or horizontally (based on the highest absolute scroll delta), but never diagonally.

### Shift-Insert on Linux

With 1.42, Shift-Insert was modified to paste from the selection clipboard. We have received a lot of feedback that this was breaking existing muscle-memory and we have decide to roll back and have Shift-Insert paste from the regular clipboard, as before.
...
### "Run" is now more prominent in the UI

User studies have revealed that new users still have difficulties finding ways to run their programs. One reason being that the existing "Debugging" functionality is not something that they can easily relate to "Running" a program. For that reason we have tried to help those users by making "Run" more prominent in the UI.

- The main menu **Debug** has now become the **Run** menu.
...
We have been working the last couple of months to support synchronizing VS Code preferences (settings) across machines (popular feature request [#2743](https://github.com/microsoft/vscode/issues/2743)). In this milestone, we are happy to announce that this feature is ready for preview from our next [1.44.0-insider](https://code.visualstudio.com/insiders/) release. You can now have your preferences synchronized in all your VS Code installs across all your machines. You can also bootstrap VS Code with your personal preferences on a fresh machine with minimal effort. Following video demonstrates this:
...
**Note:** Settings Sync uses a Microsoft account to store your VS Code preferences for synchronization and therefore you would need an account to use this. Refer to the [Settings Sync documentation](https://aka.ms/vscode-settings-sync-help) for more information and help.
...
We've now added paging support (via _Load more_ above), and added a relative timestamp over on the right. Extensions can also contribute menu items to the Timeline view as well as to individual timeline items. In the screenshot above, you can see that the built-in Git extension is contributing a few new commands: _Copy Commit ID_ and _Copy Commit Message_.
...
Work started on version 3.16 of the Language Server Protocol. Proposed support got add for the following features:

- insert / replace edits for completion items
- structural codes on diagnostics (e.g. link support)
