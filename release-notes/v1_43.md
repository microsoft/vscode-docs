...
Below you can see a search for the word 'SearchEditor' with two lines of text before and after the match for context:
...
Visual Studio Code for macOS is now notarized by Apple, so you should no longer see the following dialog:
...
Below you can see that the operation is still running until the **Cancel** button is selected:
...
Here you can see the size is changed from `proportional` to `fit`, and then in a smaller file, from `fit` to `fill`:
...
As you can see, you can also disable Column Selection mode from the Status bar.
...
Here you can see the tab and space whitespace characters in the selected region:
...
Here you can see the editor with a top padding of 50 pixels:
...
Here you can see the remote `colorTheme` setting value is chosen with the **Accept Remote** action:
...
**Note:** Settings Sync uses a Microsoft account to store your VS Code preferences for synchronization and therefore you need an account to use this feature. For more information, see the [Settings Sync documentation](https://aka.ms/vscode-settings-sync-help).
