...
There are now new colors for customizing the minimap and its slider. One of the new colors is `minimap.background` and when the color is defined to be transparent (setting the #RRGGBBAA Alpha channel to 0), the minimap will be rendered transparently:
...
There is now a new global toggle, accessible via the **Selection** > **Column Selection Mode** menu item. Once this mode is entered, the Status bar will have a new entry to reflect that this mode is active, and the mouse gestures and the arrow keys will create a column selection:
