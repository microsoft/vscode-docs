...
After a few iterations of stabalization in in [Insiders](https://code.visualstudio.com/insiders/), we are shipping the ability to view workspace search results in a full-sized editor tab, complete with syntax highlighting and optional lines of surrounding context. This had been a [highly requested feature](https://github.com/microsoft/vscode/issues/23931) for some time.
...
In a search editor, results can be navigated to using "Go to Definition" actions, such as `kb(editor.action.revealDefinition)` to open the source location in the current editor group, or `kb(editor.action.revealToSide)` to open the location in an editor to the side. Additionally, double clicking can optionally open the source location, configurable with the `search.searchEditor.doubleClickBehaviour` setting.
...
**Note**
You can try out the experimental [`Search Editor: Apply Changes`](https://github.com/JacksonKearl/vscode-search-editor-apply-changes) extension to syncronize edits you make in a search editor back to source files:

![Search Editor: Apply Changes](images/1_43/search-editor-apply.gif)
...
VS Code now bundles TypeScript 3.8.3. This [major update](https://devblogs.microsoft.com/typescript/announcing-typescript-3-8/) brings support for new language features including [ECMAScript private fields](https://devblogs.microsoft.com/typescript/announcing-typescript-3-8/#ecmascript-private-fields), [top level await expressions](https://devblogs.microsoft.com/typescript/announcing-typescript-3-8/#top-level-await), and the [`export * as ns` syntax](https://devblogs.microsoft.com/typescript/announcing-typescript-3-8/#export-star-as-namespace-syntax), along with many tooling features and improvements. The update also fixes a number of important bugs.
...
You can use the new `convert to template string` refactoring for JavaScript and TypeScript to quickly convert string concatenations to [template strings](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals):
...
When associating file names to schemas you can now also use exclusion patterns. Exclusion patterns start with '!'. To match, at least one pattern needs to match and the last matching pattern must not an exclusion pattern.
```
	"json.schemas": [
		{
			"fileMatch": [
...
### New Setting `debug.console.historySuggestions`
It is now possible to turn off history suggestions in the debug console via a new `debug.console.historySuggestions` setting. By default history suggestions are **on**.
...
The debugger can now pretty-print files, especially useful when dealing with minifed sources. It will show a prompt when you step into or open a file that looks minified, and you can also trigger pretty-printing manually via the **Debug: Pretty print for debugging** command.
...
We now support launching the [new Edge browser](https://www.microsoft.com/en-us/edge), just change your debug type from `chrome` or `pwa-chrome` to `pwa-msedge` to try it out. It supports all the same configuration settings as `chrome` does.

With this comes support for the [WebView2](https://docs.microsoft.com/en-us/microsoft-edge/hosting/webview2) control in desktop Windows applications. Take a look at our [webview demo](https://github.com/microsoft/vscode-js-debug/tree/master/demos/webview) to see how ot set this up.
...
Here's an example of using `.disabled` in our [CodeAction sample extension](https://github.com/microsoft/vscode-extension-samples/tree/master/code-actions-sample).
...
**Please note:** starting with this release VS Code supports this DAP enhancement.
...
**Please note:** VS Code does not yet support this DAP enhancement. It is planned for the next release.

#### Proposal: Progress Events

We made some progress on a proposal for adding _progress events_ to the Debug Adapter Protocol. If you are interested you can contribute to the discussion on this [DAP feature request](https://github.com/microsoft/debug-adapter-protocol/issues/92).
...
Enabling strict mode was a long journey that everyone on the core VS Code team and may community members contribute to. By doing this work incrementally however, we were able to benefits from all this hard work from day one. Going forward, strict mode should encourage coding best practices, make refactoring safer, and ultimately let us ship faster and with fewer bugs!
