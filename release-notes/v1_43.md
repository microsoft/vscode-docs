...
We are tracking an issue from **Windows** users with HDR displays https://github.com/microsoft/vscode/issues/88491 that VS Code looks dull and unsuable. This is an issue with all chromium based applications on 78.0.3904.130 and the issue is fixed in more recent version of Chromium 81, VS Code will get this fix once we upgrade to Electron 9 and there are technical reasons on why the fix couldn't be backported to Chromium 78. While the upgrade might take some time, we have a temporary workaround for this issue.
