...
#### Copy paste file incrementor change

Previously when copy pasting files and folders that were duplicates inside the VS Code Explorer, we would increment the name of the pasted file. The way we were doing the increment led to some unexpected results with our users.
...
![minimap_search](images/1_36/minimap_search.png)

## Integrated Terminal

### Preview: Launch terminals with clean environments

The terminal in VS Code has always acted a little differently to normal terminals, particularly on Linux and macOS. That's because the environment was always inherited from VS Code's window (instance) and then VS Code/Electron-related environment variables were removed from it, whereas a normal terminal would typically be launched from the Dock/Start menu and use the system environment. This could cause issues depending on what you were doing, for example often broke Python virtual environments because of how they use the `$PATH` variable.

There's a new option `terminal.integrated.inheritEnv` that when `false` will tell the terminal to not user VS Code's environment, instead it will do the following:

* **Linux**: Fetches and uses the environment of the parent process of VS Code's "main process".
* **macOS**: Pulls a handful of important environment variables off the current environment and includes only them. Eventually we would like macOS to behave the same as Linux but ran into some issues with fetching environments.
* **Windows**: Currently this setting does not affect Windows.

The main visible result of setting this to `false` is that `$SHLVL` (shell level) should now be 1 and `$PATH` should not include duplicate paths, provided your launch scripts don't intentionally do that.

The default value for `terminal.integrated.inheritEnv` is `true`, which is the previous behavior, but we will probably switch this to `false` in the future.

### Better default shell selector

Windows has had the **Select Default Shell** command for some time and last release it was added to the dropdown menu. This is now also available on macOS and Linux and exposes the shells registered on the system by reading the `/etc/shells` file.
...
Features such as IntelliSense require that the TypeScript server process the entire JavaScript or TypeScript project before it can return any results, and this can take a bit of time for larger projects. While this processing is happening the server is not able to handle any other requests, including requests for simple features like code folding that only require a basic semantic understanding of the current file. You may have run into this issue if you've ever seen it take a moment before code folding or the document outline become available.

In order to let you start working with your code faster, we've added a new experimental option that lets VS Code talk to two TypeScript servers: one that only handles simple syntax-based operations, and a complete one that handles project processing, intellisense, error reporting, and other advanced language features. To enable this behavior, just set `"typescript.experimental.useSeparateSyntaxServer": true`. This setting requires using TypeScript 3.4 or newer in your workspace.

## Debugging

### Jump to Cursor

We have added a new debug command **Jump to Cursor**, which lets the user move the execution point to a certain location without executing any of the code in between. This new command will be shown in the editor context menu and Command Palette while debugging and only for debuggers that support this capability. Currently this command is supported by the C# extension, but other debug extensions should follow.
...
With the [Remote - WSL](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl) extension universal WSL support has arrived in VS Code. Now all extensions can be used in WSL in a seamless way.
...
Here are the simple steps for how to debug an existing project that uses the `useWSL` flag:
...
The new [JavaScript and TypeScript nightly extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-next) enables the nightly build of TypeScript (typescript@next) as VS Code's built-in TypeScript version used to power JavaScript and TypeScript IntelliSense.
...
If you are working on a web project in a remote window, you can use the Chrome debug extension to debug it in a local Chrome window. Just install the extension, start your dev server on the remote, forward the server's port, and start your launch config. See the extension's [README](https://github.com/microsoft/vscode-chrome-debug#usage-with-remote-vs-code-extensions) for details.
...
### vscode.env.openExternal and Webview Port mapping now forward 127.0.0.1

The `vscode.env.openExternal` API opens an uri using the default external application. When `openExternal` is called by a remote extension on local uri—such as `http://localhost:8080` for example— VS Code automatically opens a tunnel that connects a port on the local machine to the opened port on the remote machine. This automatic tunneling previously only
was enabled for `localhost` uri but it is now enabled for `127.0.0.1` as well.

Additionally, the [`Webview`](https://code.visualstudio.com/api/extension-guides/webview) port mapping API now also handles `127.0.0.1` in addition to `localhost` uris.
...
![warning-color-updated](images/1_36/warning-color.png)
...
Comments API now adds support for displaying and managing user reactions on comments. When `Comment.reactions` is present, reactions will be rendered under the comment body.

If an extension registers reaction handler by `CommentController.reactionHandler`, users will be able to respond the existing reactions or use the reactions picker to create new reactions.

![comment-reactions](images/1_36/comment-reaction.png)
...
Since the `terminal.integrated.shell.<platform>` settings now default to `null`, that can no longer be used to detect the type of shell being used. To solve this there is a new proposed API `vscode.env.shell` that will return the default shell that is used for the terminal:
...
* [Chase Adams (@chaseadamsio)](https://github.com/chaseadamsio): Support themeing of Quick Input / Quick Open  [PR #74041](https://github.com/microsoft/vscode/pull/74041)
