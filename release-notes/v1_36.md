...
We have added a new debug command **Jump to Cursor**, which lets you move program execution to a new location without executing any of the code in between. The new command is available in the editor context menu and Command Palette while debugging and only for debuggers that support this capability. Currently this command is supported by the C# extension, but other debug extensions should soon follow.
