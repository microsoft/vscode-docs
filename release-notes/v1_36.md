...
The terminal in VS Code has always acted a little differently to normal terminals, particularly on Linux and macOS. That's because the environment way always inherited from VS Code's window and then VS Code/Electron-related environment variables were removed from it, whereas a normal terminal would typically be launched from the dock/start menu and use the system environment. This could cause issues depending on what you were doing, for example often broke Python virtual environments because of how they use the `$PATH`.
...
The main visible result of settingt his to `false` is that `$SHLVL` (shell level) should now be 1 and `$PATH` should not include duplicate paths, provided your launch scripts don't intentionally do that.
...
Windows has had the Select Default Shell command for some time and last release it was added to the dropdown menu. This is now also available on macOS and Linux and exposed the shells that have been registered on the system by reading the `/etc/shells` file.
...
Consequently, we are now planning to remove the now obsolete WSL support we've introduced into VS Code's Node.js debug extensions two years ago. As a first step toward this goal, we are deprecating the `useWSL` debug configuration attribute. Starting with this milestone the attribute will show up with a squigglly when opening a launch.json in the editor. In addition a notification appears when a debug session is launched that contains a `useWSL`.

Here are the simple steps for how to debug an existing project that uses the `useWSL` flag:
- make sure that you have the "Remote-WSL" extensison installed
- open the folder of your project in VS Code
- reopen the project in WSL by using the "Remote-WSL: Reopen folder in WSL" action.
- press F5
...
Unfortunately we have we have to disable macOS simple fullscreen support (`"window.nativeFullScreen": false`) on macOS for one release due to a late issue that was found (https://github.com/microsoft/vscode/issues/75054). We expect to include a fix to our insider builds early next month.
