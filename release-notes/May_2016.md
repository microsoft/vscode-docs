...
Thanks to [PR #5423](https://github.com/Microsoft/vscode/pull/5423), there is a new setting `editor.trimAutoWhitespace`, which is on by default. This setting causes the editor to track the automatic whitespace it inserted and remove it if it no longer is needed (typing somewhere else). This helps keep your source code nice and tidy!

![](images/May/trimAutoWhitespace.gif)

### Select all find matches

Thanks to [PR #5715](https://github.com/Microsoft/vscode/pull/5715), when searching for something, by pressing `kb(editor.action.selectAllMatches)` you can now immediately select all the find results:
...
Thanks to community Pull Requests ([see below](/updates/May_2016.md#thank-you)), there is now support for:
