...
The python Interactive Window is now fully integrated with the `hot exit` feature and will restore the editor state between vs code reloads. The `interactiveWindow.restore` setting no longer had any effect and has been removed. Without `hot exit` enabled, a prompt will show upon closing vs code to optionally save as a `.ipynb` file.
