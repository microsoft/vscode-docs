...
`Open Accessible View` `kb(editor.action.accessibleView)` allows screen reader users to inspect the content of elements character by character, line by line. Try this when focusing a hover or panel chat response.

### Accessibility help improvements

`Open Accessibility Help` `kb(editor.action.accessibilityHelp)` takes into account the current context and applies to the editor, terminal, notebook, panel chat, and inline chat features.
...
The toolbar continues to work with all three label strategies: Always, Never, and Dynamic. This setting can be changed via setting ID: `notebook.globalToolbarShowLabel`.
...
>**Note**: To get access to the chat view, inline chat, and slash commands (for example `/explane`, `/fix`), you'll need to sign up for the [GitHub Copilot chat waitlist](https://github.com/github-copilot/chat_waitlist_signup/join).

#### Create workspace

You can ask Copilot to create workspaces for popular project types with the `/createWorkspace` slash command.

TODO@bhavyaus include image

This is a preview only feature and can be enabled by configuring `github.copilot.advanced` setting as follows:
...
You can ask Copilot to create Jupyter notebooks based on your requirements with the `/createNotebook` slash command.

TODO@rebornix include videos for outline generation and notebook generation
...
When typing in the new diff editor, changes are heuristically applied immediately and trigger an asynchronous diff computation after some delay.
In the old diff editor, changes were not applied immediately, causing visible flickering when typing.
...
#### Preview: Move Detection

This experimental feature detects moved code blocks within a file.
Use `diffEditor.experimental.showMoves` to turn it on.
At the moment, only very simple moves are detected, but we plan to improve this feature in the future.
Once a moved code block is selected, source and target locations are aligned and a diff between them is shown.
