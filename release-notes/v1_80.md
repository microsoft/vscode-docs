...
Support for new link formats are supported, including links that need to scan up in order to find the file and links that have a range (line to line or character to character).
...
We have introduced preview-only slash commands in the chat view to help you generate code and search for text in your workspace
