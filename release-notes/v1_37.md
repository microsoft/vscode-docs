...
We've updated the our button toggle active state in the Find widget so that you can now easily tell when you are focused on an active toggle. This appears in the Search viewlet, Find and Replace, and Terminal Find.

With this update, we've introduced a new theme color,  `inputOption.activeBackground`, to control the background color of the toggle active state. We've also set the default for `inputOption.activeBorder` to transparent so that we only use the border for focus.
...
Previously searching within the terminal would start from the top of the terminal's viewport and search down, when it reached the bottom it would wrap around. The new behavior aligns with several other terminal emulators and searches from the very bottom of the buffer and search upwards, leading to a much less disorienting experience.
...
Some other accessibility improvements were also made:
...
- Screen readers will now announce chatacters when typed and removed, there are some cases where this will break but the majority of cases are covered.
...
Previously when the terminal launched with a bad configuration it would just say the process exited with some error code. Better error messages that guide the user to the solution are now provided for several common scenarios.
...
The proposed `vscode.env.shell` API is now stable. This should be used to get the default shell for the terminal over the `terminal.integrated.shell.<platform>` setting.
...
The proposed TerminalRenderer API introduced last year has gone through a major change and is now deprecated in favor of "Extension Terminals". The new API aligns much more with how the terminal actually works, the extension now implements a `Pseudoterminal` object which is very similar to a *nix-style pty. Here's the current proposal:
...
- An extension terminal is now done in much the same way as regular terminals, just using a different options object. This also goes away with the confusing access of the `Terminal` object via `TerminalRenderer.terminal`.
- The idea of an OS process is gone and with it are exit code, the `Pseudoterminal` can simply close to close the terminal and all error display/notifications are driven by the extension.
...
A new color, `inputOption.activeBackground`, allows you to control the background color of activated options in input fields. We've also set the default for `inputOption.activeBorder` to transparent so that we only use the border for focus.
