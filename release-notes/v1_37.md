...
Previously, you had to enable the setting `npm.enableScriptExplorer` to make the NPM Scripts Explorer visible. This was difficult to discover. Now, the NPM Scripts explorer is shown collapsed, when there is a `package.json` file at the root of the workspace. If you want to hide the explorer, you can right-click on its title and select **Hide** from the context menu.
