...
### Settings UI support for array-of-string settings

The Settings UI now allows editing array-of-string settings:

![Settings UI for array-of-string settings](images/1_37/settings-ui-array-of-string.png)

You can enable this support by [contributing a configuration](https://code.visualstudio.com/api/references/contribution-points#contributes.configuration) with `type` and `items` keys:
...
VS Code now resolves `@import './foo'` to one of these locations if the resolved file exists on disk. Note that if a link ends with `/`, it would only resolve to either `/index.scss` or `/_index.scss`. If a link ends with suffix such as `.css` or `.scss`, VS Code would not try the alternative file locations.
