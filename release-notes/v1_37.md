...
* **[Icon refresh](#new-product-icons)** - New modern icons across all of VS Code.
...
* **[Reveal search result in File Explorer](#reveal-search-result-in-the-explorer)** - Quickly jump to file locations in the Explorer.
* **[Show whitespace in selection](#show-whitespace-in-selection)** - See whitespace characters just for selected text.
* **[Find and Replace preserves casing](#preserve-case-in-find-and-replace)** - Replace text without loosing casing.
* **[Terminal search improvements](#search-UX-improvements)** - More intuitive bottom-up search order for the terminal.
* **[Better SCSS @import navigation](#improved-navigation-for-SCSS-import)** - VS Code now searches more locations for SCSS files.
* **[Remote Development tutorials](#remote-development-tutorials)** - New step-by-step tutorials for working over SSH and in WSL.
...
Previously, you had to enable the setting `npm.enableScriptExplorer` to make the NPM Scripts Explorer visible and this was difficult to discover. Now, the NPM Scripts explorer is shown collapsed, when there is a `package.json` file at the root of the workspace. When you have no `package.json` at the root in your workspace, you can still use the setting to enable the NPM Scripts Explorer. If you want to hide the explorer, you can right-click on its title and select **Hide** from the context menu.
...
You can now preserve case when doing replacement in editor's Find widget. The feature is turned on when the Preserve Case option (**AB** button) is turned on in the editor's Replace input box.

![preserve case button](images/1_37/preserve-case-button.png)

Note that currently VS Code only supports preserve **Full Upper Case**, **Full Lower Case**, and **Title Case**.
...
With this update, we've introduced a new theme color, `inputOption.activeBackground`, to control the background color of the toggle active state. This color re-uses `focusBorder` so that it works better with themes by default. We've also set the default for `inputOption.activeBorder` to transparent so that the border is only used for focus.
...
CSS completion now offers detailed documentation for 754 less commonly used CSS property values.
...
`resolveTask` has existed for a long time, but was never implemented. Now, task providers that implement `resolveTask` can take advantage of the performance benefit that it provides. `resolveTask` is called after `provideTasks`, so if your extension has a setting that turns off `provideTasks` (like the built-in gulp extension), but the user still references one of your tasks in `tasks.json` then `resolveTask` will be called so that your extension can provide that single task.

### TypeScript detected tasks warning

Last release the TypeScript task auto detection changed from using OS specific path separators in the `tsconfig` path to a more cross-platform friendly `/`. We now show a warning if we see `\\` in a tsconfig path.
...
A new variable **${relativeFileDirname}** resolves to the workspace folder relative path of the current open file's directory. The variable can be used in task and debug configurations (`task.json` and `launch.json`).
...
The diagnostic in the screen capture above is created by the `validate` function shown above, this line adds the deprecation tag: `diag.tags = [vscode.DiagnosticTag.Deprecated];`
...
We hope to stabilize this and remove the deprecated terminal renderers relatively quickly, likely in v1.39.

### vscode.CustomExecution2

This is the evolution of `vscode.CustomExecution`. It uses the new and improved `vscode.Pseudoterminal` to allow task providers to create tasks that written with code in the extension instead of just executing a script or process.
