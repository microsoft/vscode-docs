...
VS Code now supports Portable Mode, which enables all data created and maintained by VS Code to live near itself, so it can be moved around across environments. Portable Mode is supported on the ZIP download for Windows and Linux, as well as the regular Application download for macOS. [Click here](/docs/editor/portable) to learn more.
...
Users have complained that the hover shown in the editor can get in the way. The editor hover can now be customized with three additional settings. Use `editor.hover.enabled` to turn off or on the editor hover, use `editor.hover.delay` to customize the time until the hover is shown and use `editor.hover.sticky` to change if the hover should remain visible when moving the mouse over it.
...
Here is, for example, how you can bind them:
...
### Update import paths on folder rename for JavaScript and TypeScript
...
The built-in Markdown support uses the new `DocumentSymbol` API to make sure markdown headers are properly nested in the outline panel.

![The outline panel for a markdown document](images/1_25/md-outline.png)

### New CSS Pseudo Selectors and Pseudo Elements from MDN

In April release, we started [sourcing data](https://code.visualstudio.com/updates/v1_23#_improved-css-support-for-new-properties) from Mozilla Developer Network for up-to-date CSS/SCSS/Less support. In this release, we added the following pseudo selectors / elements:
...
People who are using CSS pre-processors may use custom at-rules that are compiled to valid CSS at build time. Previously, VS Code cannot handle parse or handle such at-rules. In this release, we have better support for such at-rules, so that they:

- Receive correct syntax highlighting as they still abide by CSS syntax.
- No longer generate errors in incorrect place.
- Generate correct errors that can be controlled by `css.lint.unknownAtRules`.

![CSS Unkown At-Rules](images/1_25/css-unknown-atrules.png)
...
![New Settings Editor](images/1_25/settings-editor.gif)

Some of the new changes compared to last month are:

- A "table of contents" that organizes settings into new categories
- A cleaned up and refreshed layout for the setting rows
- The "Reset" button has been removed - editing a setting to its default value will remove it from the settings.json

Also, try the `"workbench.settings.settingsSearchTocBehavior"` setting to see three different options for the "table of contents" while searching settings, and let us know which option you prefer.
...
We recommend extensions to you based on the files you have opened and your workspace environment. We understand that not every recommendation might be appropriate for your project. Now, you can dismiss such recommendations.

To dismiss a recommendation, click on the extension item to open the Details pane and then press the **Ignore Recommendation** button. Ignored recommendations will not be recommended to you from this point onward.

![Dismiss Recommendations](images/1_25/dismiss.gif)

### Manage workspace recommendations

You could already configure a set of extensions to be recommended to the users of your workspace using the [workspace recommendations](/docs/editor/extension-gallery.md#workspace-recommendations). Managing these recommendations is now much easier. Click on any extension in the Extensions view to open the Details pane. You can then use the new commands below to add recommendations or black-list certain extensions that you don't want recommended for the users of your workspace.
...
Windows users now have a new **preview** Setup package available, which does not require Administrator priviledges to install. This setup package also provides a smoother background update experience. This feature is only distributed to Insiders for now, as we're aiming to get more feedback during July in order to provide the best experience for our stable user base.

Here are the download links for the Insider user setup packages:

- [Windows 64 bit](https://vscode-update.azurewebsites.net/latest/win32-x64-user/insider)
- [Windows 32 bit](https://vscode-update.azurewebsites.net/latest/win32-ia32-user/insider)
...
Language Servers that use [vscode-languageclient](https://github.com/Microsoft/vscode-languageserver-node) can specify a setting `[langId].trace.server` to receive logging support. Although the generated log is valuable for understanding the LSP communication between VS Code and Language Servers, the log can be long and hard to extract useful information from due to the chatty nature of Language Servers.

In this release, we built a LSP Inspector that could help you understand the LSP logs: https://microsoft.github.io/language-server-protocol/inspector/

In the inspector, you can:

- Easily scan through the requests, responses and notifications between Language Client / Server.
- Filter through the massive log by search or pre-defined filters to locate useful information.

![LSP Inspector](images/1_25/lsp-inspector.gif)

The Inspector can be useful for debugging Language Extensions. When a language feature such as completion is not working as expected, there can be two causes:

- The Language Server sends back wrong `CompletionList` response
- VS Code does not handle the `CompletionList` response correctly

The first type of issue belongs to the Language Extension author, and the second type of issue belongs to VS Code. For this release, we were able to diagnose a [JSON completion bug](https://github.com/Microsoft/vscode/issues/52999) by:
- Collecting logs from JSON Language Server in 1.24 and 1.25
- Comparing the logs using the Inspector
- Observing no difference in the completion responses between 1.24 and 1.25
- Identifying the cause of the issue as an updated handling of TextMate snippets in completion response

If you are authoring Language Extensions, we encourage you to ask users to attach logs to their bug reports and use the Inspector to facilitate your Language Extension debugging process. Please let us know any issues or feature-requests at: https://github.com/Microsoft/language-server-protocol-inspector

Finally, the updated [Language Server Guide](/docs/extensions/example-language-server#_logging-support-for-language-server) includes a section on the logging support and the functionalities of the inspector.

### Language Server: End to End Testing

A section on End to End Testing has been added to the [Language Server Guide](/docs/extensions/example-language-server#_testing-the-language-server).

Compared to unit-testing parts of Language Servers, End to End tests open a workspace in VS Code, launch the Language Extension in the Extension Host and make assertions against the real behaviors of the Language Extension. This makes mocking workspace and VS Code state easy and assert against real VS Code behaviors.

### Language Server: Refreshed Guide

The [Language Server Guide](/docs/extensions/example-language-server) has received a refresh. Major improvements include:
...
- Updated illustration of LSP and Language Server.
...
`DiagnosticTag.Unnecessary` indicates that a diagnostic is for unreferenced or unreachable code. Any code marked with `DiagnosticTag.Unnecessary` will be rendered faded out. The amount of fading is controlled by the `"editorUnnecessaryCode.opacity"` theme color. For example, `"editorUnnecessaryCode.opacity": "#000000c0"` will render the code with 75% opacity. For high contrast themes, use the `"editorUnnecessaryCode.border"` theme color to underline unnecessary code instead of fading it out.

### WebviewPanel.active and WebviewPanel.visible

Webview panels have two new properties:
...
The new webview persistence APIs are documented on the [webview extension authoring page](https://code.visualstudio.com/docs/extensions/webview). The [example webview extension](https://github.com/Microsoft/vscode-extension-samples/tree/master/webview-sample) also demonstrates how to use these new APIs.3

## Language Packs: Minimal Translations

We will now recommend users your language pack in the same translation language you authored for. Following is an example for recommending Simplified Chinese language pack in Simplified Chinese and English languages.

![Recommendation](images/1_25/lang-pack-recommendation.png)

This requires you to translate following minimal translations in your language pack.
...
This additional information is used by VS Code to improve the user experiance for `go to definition`.

To use `DefinitionLink`, a `DefinitionProvider` must currently implement a `provideDefinition2` method. Once the `DefinitionLink` API is stabalized, we plan to remove `provideDefinition2` and update the normal `DefinitionProvider.provideDefinition` method to also allow returning `DefinitionLink`.
