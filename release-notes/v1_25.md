...
To support flexible layouts, editor groups can now show up as empty. By default, closing the last editor of an editor group will also close the group itself, but you can change this with the new setting `workbench.editor.closeEmptyGroups: false`. You can find a set of predefined editor layouts from the new "Editor Layout" menu under the "View" entry:
