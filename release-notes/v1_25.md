...
Editors in the workbench can now be arranged freely both vertically and horizontally and there is no longer a limit of just showing 3 editors side by side at the same time:

![Grid Editor Layout](images/1_25/grid-layout.gif)

To support flexible layouts, editor groups can now show up as empty. By default, closing the last editor of an editor group will also close the group itself, but you can change this with the new setting `workbench.editor.closeEmptyGroups: false`.

![Grid Empty](images/1_25/grid-empty.png)

You can find a set of predefined editor layouts from the new "Editor Layout" menu under the "View" entry:

![Grid Editor Layout Menu](images/1_25/grid-layout-menu.png)

Editors that open to the side (e.g. by clicking the "Split Editor" action in the editor toolbar) will by default open to the right hand side of the active editor. If you prefer to open editors below the active one, configure the new setting `workbench.editor.openSideBySideDirection: down`.

There are a lot of [new commands](#new-commands) introduced to use the grid editor layout with keyboard alone, but if you prefer to use the mouse, using drag and drop is a fast way to split the editor into any direction:

![Grid Editor Drag and Drop](images/1_25/grid-dnd.gif)

Since editor groups are now unlimited and can be arranged in many ways, the "Open Editors" view is no longer talking about a "Left", "Center", or "Right" editor group. Instead, editor groups are now showing up with numbers according to their creation time. Thus, the last editor group created will always be at the end of the list. We found this to be a good model that minimizes the number of times the list would change by moving editor groups around in the layout:

![Grid Open Editors View](images/1_25/grid-open-editors.png)

Please refer to [Extension Authoring](#extension-authoring) for the impact on extensions or themes and to [New Commands](#new-commands) for a list of new commands that we now support as part of introducing this feature.

**Pro Tip**: If you press and hold the `Alt`-key while hovering over the action to split an editor, it will offer to split to the other direction. This is a fast way to split either to the right or to the bottom.
...
If no `ViewColumn` is provided when opening a text editor or webview , we now default to open in the currently active editor group, instead of opening it in `ViewColumn.One`. If you want to preserve the previous behaviour, just pass in `ViewColumn.One` when opening.

### New Theme colors

There are two new theme colors for the background color of empty editor groups - a feature that came in through the work of supporting a grid editor layout:
...
The `layoutEditorGroups` command allows to set an editor group layout the way you like and assign it a keybinding. The layout is described as object with an initial (optional) orientation (`0` = horizontal, `1` = vertical) and an array of editor `groups` within. Each editor group can have a `size` and another array of editor `groups` that will be layed out orthogonal to the orientation. If editor group sizes are provided, their sum must be `1` to be applied per row or column. For example:
