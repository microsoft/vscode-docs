...
The Outline-feature has left the preview-stage and is now enabled by default. Find it as separate section in the file explorer. Once expanded it will will show the symbol tree of the currently active editor.

The outline tree allows for different 'Sort By' modes, optional cursor tracking, and supports the usual open gestures. It also features an input box which finds or filters symbols as you type. Last, errors and warnings are shown in the outline tree telling at a glance where the trouble is.

![Outline](images/1_25/outline.png)

The outline comes with a couple of configuration settings which allow you to enable/disable icons and that allow you to control if and how errors and warnings are shown.

Last, there is also [new API](#document-symbols) for **extensions authors**.
...
Naviating history in various input boxes is made more natural and consistent across VS Code. You can use <kbd>UpArrow</kbd> (`history.showPrevious`) and <kbd>DownArrow</kbd> (`history.showNext`) keys to navigate in input boxes in Search view, filter box in Problems view and find widget in Editor, Terminal and Web view.

**Note:** In order to get this simplicty and consistency, we have changed the default keybindings for navigating among search input boxes from <kbd>UpArrow</kbd> and <kbd>DownArrow</kbd> to  <kbd>Ctrl+UpArrow</kbd> (or <kbd>Cmd+UpArrow</kbd> on macOS) and <kbd>Ctrl+DownArrow</kbd> (or <kbd>Cmd+DownArrow</kbd> on macOS) keys respectively. You can always go back to previous behaviour by updating `kb(search.focus.previousInputBox)`, `kb(search.focus.nextInputBox)` and `kb(search.action.focusSearchFromResults)` keybindings.
...
Language smarts often know what suggestions are best and with this version of VS Code they can let us know. There is new [API](#Preselected-Completion-Items) for preselected suggestions and when present, the editor will select not the first but the preselected suggestion.

![suggestion selection](images/1_25/suggest_preselect.png)

### Snippt Placeholder Transformations

Snippets finally support placeholder transformations. Those are like variable transformations but more dynamic. A transformation is applied when switching from one placeholder to the next and this is the grammar rule for them: __${__*int*__/__*regex*__/__*format_string*__/__*opts*__}__.

A sample, that replaces "Hello World" with its German counterpart can be seen below:
...
Refer to the [snippet grammar](https://code.visualstudio.com/docs/editor/userdefinedsnippets#_grammar) for more details and how to unleash the full potential of transformations. Last but not least, special thanks to [@go2sh](https://github.com/go2sh) for implementing this!

## Terminal

### New parser

The terminal's parser was re-written from the ground up for enhanced ANSI compliance and boosting the rate at which incoming data is processed by approximately 30%. This came in as an upstream community contribution to [xterm.js](https://github.com/xtermjs/xterm.js/pull/1399).

### Experimental: Dynamic texture atlas

An experimental setting has been added to enable the new "dynamic texture atlas" in the terminal. This changes the caching strategy used by the terminal to store character glyphs. Instead of being a fixed set of glyphs on the default background, glyphs are now added to the texture atlas as they are required and regardless of the background being used. This should reduce the first terminal start up time, reduce rendering time of characters not using the default background, as well as improving overall rendering performance.
...
Extension that provide completion items can now marks them as 'preselected'. That is useful when many completions exist but when some are most likely to be selected. A sample for this is assiging known variable types but this also enables extensions to favor more commonly used completions, like favor `console.log` over `console.dirxml`.
...
Note that you cannot publish an extension that uses proposed API. That's because we are likely breaking it with the next release and we never-ever want to break extensions.
...
We are proposing to events that fire before and after a rename occurs. They are called `onWillRenameFile` and `onDidRenameFile` and they allow react or participate in the rename.
