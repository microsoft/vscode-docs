...
### Custom Titlebar and Menus for Windows/Linux

Windows and Linux users can now use the `window.titleBarStyle` setting value `custom` thanks to the work by [@rianadon](https://github.com/rianadon)! On top of this, we've also implemented a new menubar to match. The setting is not yet enabled by default but can be done so with the previously mentioned setting.

With the setting enabled for Windows and Linux, the old menubar will be replaced with a new menubar that will match your light or dark theme. The new menubar also comes with some enhancements such as top-level keyboard navigation with the arrow keys. Lastly, the menus throughout the product will also match the new menubar.

![Custom Titlebar and Menubar](images/1_25/custom_title.png)

We will continue to improve the new menus and titlebar as well as address issues tagged with the [`workbench-title`](https://github.com/Microsoft/vscode/labels/workbench-title) or [`workbench-menu`](https://github.com/Microsoft/vscode/labels/workbench-menu) labels.

## Extensions

### New default view for Extensions viewlet

Until now, the default view for the Extensions viewlet contained a section for Installed extensions and another for recommended extensions. Since the presence of disabled extensions in the former can get noisy, we now provide a new default where the Installed (but not disabled) extensions are shown first, followed by a section for recommneded extensions and in the end a collapsed section for disabled extensions.

You can always re-order, collapse/expand or hide any of these sections.

![debug toolbar](images/1_25/defaultExtensionsView.png)

### Dismiss extension recommendations

We recommend extensions to you based on the files you have opened and your workspace environment. We understand that not every recommendation might work in your favor. Now, you can dismiss such recommendations.

Just click on the extension to open the details in the editor pane. Then press the `Ignore Recommendation` button. Such dismissed recommendations will not be recommended to you from this point onward.

### Manage workspace recommendations

You could already configure a set of extensions to be recommended to the users of your workspace using the [workspace recommendations](/docs/editor/extension-gallery.md#workspace-recommendations). Managing these recommendations is now much easier. Click on any extension in the extensions view to open the details in the editor pane. Then use any of the below new commands to add recommendations or black-list certain extensions that you dont want recommended for the users of your workspace.
