...
Semantic search powered by Copilot is now enabled by default in the search view (`setting(github.copilot.chat.search.semanticTextResults:true)`). You can use the `kb(cmd+i)` shortcut to trigger a semantic search, which will show you the most relevant results based on your query, on top of the regular search results.
...
You can also reference the semantic search results in your chat by using the `#searchResults` tool. This allows you to ask Copilot to summarize or explain the results, or even generate code based on them.
...
For the past several months, we've had a "Chat" view for chatting with Copilot, and a "Copilot Edits" view for editing files with Copilot. This month, we aim to streamline the whole experience by merging the two views into one Chat view. In the Chat view, you'll see a dropdown with three modes:

![chat mode picker](images/1_99/chat-modes.png)

- **[Ask](/docs/copilot/chat/chat-ask-mode.md)**: This is the same as the previous Chat view. Chat with Copilot, using any model. Use `@` to chat with chat participants from Copilot or from installed [extensions](https://marketplace.visualstudio.com/search?term=chat-participant&target=VSCode&category=All%20categories&sortBy=Relevance). Use `#` to attach any kind of context manually.
- **[Agent](/docs/copilot/chat/chat-agent-mode.md)**: Use Copilot with a set of tools that let it autonomously collect context, run terminal commands, or take other actions to complete a task. Agent mode is enabled for all [VS Code Insiders](https://code.visualstudio.com/insiders/) users, and we are rolling it out to more and more users in VS Code Stable.
- **[Edit](/docs/copilot/chat/copilot-edits.md)**: In Edit mode, Copilot can make directed edits to multiple files. Attach `#codebase` to let it find the files to edit automatically. But it won't run terminal commands or do anything else automatically.
...
Besides hopefully making Chat simpler to use, this unification enables a few new features for editing in Copilot:

- **Switching modes in the middle of a conversation**: For example, you might start brainstorming an app idea in Ask mode, then switch to Agent mode to execute the plan. Tip: press `kb(workbench.action.chat.toggleAgentMode)` to change modes quickly.
- **Edit sessions in history**: Use the "Show Chats" command (clock button at the top of the Chat view) to restore past edit sessions and keep working on them.
- **Editing in chat editors and new windows**: Click "Open Chat in New Editor/New Window" to pop out your chat from the sidebar into a new chat editor or window. Chat has supported this for a long time, but now you can run your edit/agent sessions from an editor pane or a separate window as well.
...
As part of aligning our chat modes, we're making a change to the way Edit mode works. The new Edit mode works similarly to Agent mode, letting the model call a tool to make edits to files. An upside to this alignment is letting you switch seamlessly between all 3 modes, and a huge simplification to how these modes work under the hood.

A downside is that this means that the new mode will only work with the same reduced set of models that Agent mode works with- namely, models that support tool calling and have been tested to be sure that we can have a good experience when tools are involved. You may notice models like `o3-mini` and `Claude 3.7 (Thinking)` missing from the list in Edit mode. If you'd like to keep using those models to edit, you can disable `setting(chat.edits2.enabled)` to get the old Edit mode back, you'll just be asked to clear the session when switching modes. We've learned that prompting to get consistent results across different models is harder when using tools, but we are working on getting these models lit up for Edit (and Agent) modes.

This setting will be enabled gradually for users in VS Code Stable.

### Model Context Protocol Server Support

This release supports [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) servers in Agent mode. Once configured in VS Code, MCP servers provide tools that can be called by language models in chat to interact with other systems, such as databases, cloud platforms, search engines, or any 3rd party API.

MCP servers can be configured under the `mcp` section in your user, remote, or `.code-workspace` settings, or in `.vscode/mcp.json` in your workspace. You can use the **MCP: Add Server** command to quickly set up an MCP server from a command line invocation, or use an AI-assisted setup from an MCP server published to Docker, npm, or PyPI.

When MCP servers are available, there is a refresh action shown in chat that can be used to start the servers and discover their tools. The tools provided by each server are cached, and afterwards they're started on-demand.
...
You can see the list of MCP servers and their current status using the **MCP: List Servers** command, and pick the tools available for chat to use using the **Select Tools** button in Agent mode chat.

You can read more about how to install and use MCP servers in [our documentation](/docs/copilot/chat/mcp-servers.md).

### VS Code Extension Tools in Agent Mode

Several months ago, we finalized our extension API for [language model tools](/api/extension-guides/tools.md#create-a-language-model-tool) contributed by VS Code extensions. Now, you can use these tools in agent mode. Any tool contributed to this API which sets `toolReferenceName` and `canBeReferencedInPrompt` in its contribution will be automatically available to the model in Agent mode. Similarly to tools from MCP servers, you can enable and disable these with the **Select Tools** button in Agent mode. See our [extension guide](/api/extension-guides/tools.md#create-a-language-model-tool) to build your own!

### Remember Tool Approvals

You can now remember tool approvals on a session, workspace, or application levels.

![](./images/1_99/chat-tool-approval.png)
...
In Agent mode, this tool will be picked up automatically but you can also reference it explicitly via `#fetch` along with the URL you are looking to fetch.
...
* Currently, JavaScript is disabled in this browser window. The tool will not be able to acquire much context if the website depends entirely on JavaScript to render content. This is a limition we are considering changing and likely will change to allow JavaScript.
...
With the setting `setting(chat.tools.autoApprove:true)` turned on, VS Code will not confirm with you when a language model wishes to run tools. Bear in mind that, with this setting on, you will not have an opportunity to cancel potentially destructive actions a model wants to take.
...
Notebook support in Copilot Edits (including _agent_ mode) is now available in the stable release. This was a feature that was added last month as a preview feature in [VS Code Insiders](https://code.visualstudio.com/insiders).
...
VS Code now provides a dedicated tool for creating new Jupyter notebooks directly from Copilot. This tool will plan and create a new notebook based on your query.

The new notebook tool is available in several places throughout VS Code:

- In Copilot Edits (note that you need to enable the improved editing mode with the setting `setting(chat.edits2.enabled:true)`. More info can be found here: [Improved Edit mode](#improved-edit-mode))
- In Agent mode of Copilot Edits.
- Via the `/newNotebook` slash command in Copilot Chat or the Ask mode if using the new [Unified Chat views](#unified-chat-views)
...
Use the diff toolbars to iterate through copilot's changes that were made across cells to review each one.
...
#### Undoing copilot edits

When focused on a cell container, the undo command will undo the full set of changes made by copilot at the notebook level.
...
This iteration, we've finalized our terminal shell API, allowing extensions to see user's current shell type in their Terminal.
Subscribing to event `onDidChangeTerminalState` will allow you to see the changes of user's shell type in the Terminal.
For example, shell could change from zsh to bash.
List of all the shells that are identifiable are currently listed [here](https://github.com/microsoft/vscode/blob/99e3ae5586a74ab1c554b6a2a50bb9eb3a4ff7fd/src/vscode-dts/vscode.d.ts#L7740-L7750)
...
#### Experimental: faster and more reliabile diagnostic experience with Pylance
...
There's a new `setting(python.analysis.nodeArguments)` setting which allows you to pass custom arguments directly to Node.js when using `setting(python.analysis.nodeExecutable)`. By default, it is set to `"--max-old-space-size=8192"`, but you can modify it to suit your needs (e.g. to allocate more memory to Node.js when working with large workspaces).

Additionally, when setting `setting(python.analysis.nodeExecutable)` to `auto`, Pylance will now automatically download Node.js.
