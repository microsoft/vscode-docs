...
* **Code editing**
  * Next Edit Suggestions is now generally availability ([more...](#next-edit-suggestions-general-availability)).
...
Use the new notebook tool agent mode or edit mode (make sure to enable the improved edit mode with `setting(chat.edits2.enabled:true)`). If you're using ask mode, type `/newNotebook` in the chat prompt to create a new notebook.
