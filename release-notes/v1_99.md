...
IntelliSense now supports subcommands for the `code` CLI. For instance, typing `code tunnel` shows available subcommands like `help`, `kill`, and `prune`, each with descriptive info.
