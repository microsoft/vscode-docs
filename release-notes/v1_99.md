...
### Semantic text search improvements

**Setting**: `setting(github.copilot.chat.search.semanticTextResults:true)`

Semantic text search powered by Copilot is now enabled by default in the Search view. Use the `kb(search.action.searchWithAI)` keyboard shortcut to trigger a semantic search, which shows you the most relevant results based on your query, on top of the regular search results.
...
### Next Edit Suggestions UX improvements

This update includes improvements to the visual representation of Next Edit Suggestions (NES). The changes make suggestions more compact, less interfering with surrounding code, and easier to read at a glance. The update also include changes to the gutter indicator, making sure all suggestions are more easily noticeable to the user. NES can be enabled with the setting `setting(github.copilot.nextEditSuggestions.enabled:true)`.

<video src="images/1_99/next-edit-suggestion.mp4" title="Copilot suggesting edits based on the recent changes due by the user." autoplay loop controls muted></video>
...
### Auto-approve tools (Experimental)

**Setting**: `setting(chat.tools.autoApprove:true)`

When you enable the auto-approve setting, VS Code will not ask for confirmation when a language model wishes to run tools. Bear in mind that with this setting enabled, you will not have the opportunity to cancel potentially destructive actions a model wants to take.
...
The `.github/copilot-instructions.md` file now behaves like any other reusable `.prompt.md` file, with support for nested link resolution and enhanced language features. Furthermore, any `.prompt.md` file can now be referenced in Copilot configuration and is handled appropriately.
