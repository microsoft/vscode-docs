...
In January, we introduced terminal shell integration as a preview feature. We have made improvements to its stability and expanded its support since then. TODO@Daniel

#### Terminal command decorations

Terminal command decorations have been added to shells that support shell integration.

A command decoration visually breaks up the buffer and provides access to information and actions which leverage shell integration capabilities.
...
Clicking on the decoration displays actions such as `Re-run command` and `Copy Output` in a context menu.
