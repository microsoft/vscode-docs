...
The list of recently opened local files and folders is now functional when you access https://vscode.dev/ or https://insiders.vscode.dev/ with a browser that supports the [web file system access APIs](https://developer.mozilla.org/en-US/docs/Web/API/File_System_Access_API). Selecting a local file will open it as editor and selecting a local folder will update the explorer to show its contents. You may get asked by the browser to confirm local file access before the contents are revealed.
