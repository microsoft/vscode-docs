...
You can read more about TypeScript 4.6 on the [TypeScript blog](https://devblogs.microsoft.com/typescript/announcing-typescript-4-6-rc).
...
![Detecting a re-declaration of a const variable in a JS file](./images/1_65/js-syntax-errors.png)
...
More of the JavaScript and TypeScript settings can now be configured as [language specific settings](https://code.visualstudio.com/docs/getstarted/settings#_language-specific-editor-settings). This is useful if you want different settings for `javascript` vs `javascriptreact` for example.
...
The proposed `baseUri` property on `MarkdownString` lets you specify a base uri that relative links and images in the markdown are resolved relative to. The behaves much like the [`<base>` element in html](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/base).
