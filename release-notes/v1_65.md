...
A new audio cue command **Help: List Audio Cues** lists all available audio cues, lets you play each audio cue as you move through the list, and review which cues are currently enabled. All audio cues but `lineHasWarning` are enabled by default for screen reader users.
...
You can now drag and drop a Problem, Search, or Reference result into the editor that will open the file and revealing the respective Problem, Search result, or Reference position. This can be useful if you want to start a new editor group or drop a result into an existing editor group.
