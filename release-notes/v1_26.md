...
* **[Breadcrumbs](#breadcrumbs)** - Breadcrumb navigation lets you jump to files anywhere in your workspace.
* **[Problems view Quick Fixes](#problems-view-quick-fixes)** - Apply code fixes while reviewing warnings and errors.
...
* **[Add all missing imports](#add-all-missing-imports-Quick-Fix)** - Single action to add all missing imports in a file.
* **[JSX tag completion](#jsx-tag-completion)** - Work with JSX tags just like you do in HTML.
* **[Better JS/TS error reporting](#better-error-reporting)** - Concise, clear messages with relevant source code links.
* **[Custom extensions search](#intellisense-for-extension-search)** - Filter Marketplace queries to find the extensions you want.
* **[Extension Pack improvements](#extension-packs-revisited)** - Manage Extension Packs as a single unit or by individual extension.
* **[Preview: Settings editor](#new-settings-editor)** - Rich settings description display, TOC tracks scrolling.
...
**Note:** Light bulb indicator is shown only if the extension contributing Quick Fixes provides [Diagnostic](https://github.com/Microsoft/vscode/blob/release/1.26/src/vs/vscode.d.ts#L2029) information associated with the Quick Fix.
...
As a current user of the system-wide setup, you will be prompted to switch to the user setup, which we recommend using from now on. New users will be directed towards using it by default via our downloads page.
...
The main, window and shared process each have their own log channel where you can now view the telemetry events sent by VS Code when the log level is set to "Trace".
Data common to all telemetry events will not be visible here for the sake of brevity. The complete payload of each telemetry event is logged in a separate log file named
`telemetry.log` which you can access via the `Developer: Open Log File...` command.

### Offline mode

Some users do not want any outgoing network requests from VS Code unless they specifically invoke features that need to do so.
To that affect, we have added a few new settings that can now be used to turn off features like the automatic checking for extension updates, downloading data to run
A/B experiments and fetching of online data in one of the built-in extensions to provide auto-completions.
Below is the complete list of settings that you can now use to control VS Code features that make network requests.
...
This release makes it easier to search for extensions by adding IntelliSense autocompletions to the extension search field. This will help you refine your extension searches to filter results based on things like category and install state; or sort results by name, rating, or install count.
...
* `breadcrumb.foreground`: Color of focused breadcrumb items.
...
If you are a Python developer, have a look at [IntelliCode for VS Code](https://docs.microsoft.com/visualstudio/intellicode/intellicode-visual-studio-code). The [IntelliCode](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode) extension provides a set of AI-assisted capabilities for IntelliSense in Python, such as inferring the most relevant auto-completions based on the current source code context.
...
* [Michael Crane (@mrcrane)](https://github.com/mrcrane): Fix race condition when killing Chrome on Windows when disconnecting [PR #703](https://github.com/Microsoft/vscode-chrome-debug/pull/703)
* [Todd Chaffee (@tchaffee)](https://github.com/tchaffee): Clearer documentation about where to change the webpack devtools option. [PR #694](https://github.com/Microsoft/vscode-chrome-debug/pull/694)
...
* **Chinese (Simplified):** Joel Yang,Yusong,Liu Dingming 刘丁明,hackereric,gzponline.
* **Chinese (Traditional):** Duran Hsieh, Alan Tsai, Kevin Yang, MartinLau, Ben Tsai 蔡和恩, Alan Liu.
* **French:** Antoine Griffard, William Thibodeau, Adrien Clerbois, Alain BUFERNE.
* **German:** Carsten Siemens, Carsten Kneip, Ettore Atalan, AlCalzone, Levin Rickert.
* **Italian:** Alessandro Alpi, Andrea Dottor, Emilie Rollandin, Aldo Donetti.
* **Japanese:** Satoshi Kajiura, Yuichi Nukiyama, Shunya Tajima, Nakamura Yuta.
* **Korean:** Helloyunho.
* **Russian:** Michel Ace.
...
* **English (United Kingdom):** Swotboy2000.
* **Finnish:** Petri Niinimäki, Make.
* **Greek:** Ioannis Paraskevopoulos, Theodore Tsirpanis.
* **Hungarian:** Dóczi Dominik.
* **Indonesian:** Laurensius Dede Suhardiman, Riwut Libinuko, Bagus Ilman.
* **Norwegian:** andreona, Daniel Bjørnbakk.
* **Polish:** Sebastian Baran.
* **Portuguese (Brazil):** Otacilio Saraiva Maia Neto, Pedro Sereno, Danilo Dantas, Vitor Dutra Freire, Fabio Correia.
* **Portuguese (Portugal):** Daniel Correia, Pavlo Zakharuk, António Santos, João Mata, Diogo Barros, Danilo Dantas.
* **Romanian:** Cornel Cocioaba, Dan Ichim, Bogdan Mateescu.
