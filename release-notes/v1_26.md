...
We spent time this milestone improving the custom menus on Windows and Linux when the custom title bar is enabled. While still in preview, you can try it out by setting `window.titleBarStyle` to `custom` in your `settings.json`.
...
The terminal access APIs are now stable. They allow extensions access to access `Terminal` objects, regardless of whether the extension created them or not.
