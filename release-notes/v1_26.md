...
If you are a current user of the system-wide Windows setup, you will be prompted to switch to the user setup, which we recommend using from now on. New users will be directed towards using it by default via our downloads page.
...
2. If there are no breakpoints (or problems) in that shutdown code, the debuggee and the debug session will terminate. The user will not not see any difference in behavior to previous versions of VS Code.
