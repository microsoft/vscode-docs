...
Previously, dragging and dropping a file into the explorer would automatically open it in the editor window as well. In some cases this is undesirable. A new setting <a href="vscode://settings/explorer.autoOpenDroppedFile" codesetting="true">`explorer.autoOpenDroppedFile`</a> enables you to toggle this behavior. By default, the file continues to be opened when dragged and dropped, but when set to `false`, this behavior is disabled.

## Editor

### Using cursor as anchor when editor height changes

Previously, when the editor height changed, for example when a code lense is added, the editor would try to keep the topmost line at the same position. This could result in the editor jumping to make space for the code lense.

Now, when the cursor is in the view port, the editor tries to keep the line where the cursor is positioned stable, instead of the topmost line. This new behavior reduces the editor jumping in various cases, for example when using Go to Definition.
...
This feature kicks in whenever you copy and paste text with links between two Markdown files. If there are links that can be updated, you will see the paste widget after pasting. Simply select `Paste and Update Pasted Links` and VS Code fixes up the links for you. This feature can be disabled entirely by setting <a href="vscode://settings/markdown.updateLinksOnPaste" codesetting="false">`markdown.updateLinksOnPaste`</a> to `false`.
