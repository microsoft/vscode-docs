...
VS Code can now help you move sections of text between Markdown documents by updating links in any copy and pasted text. With this feature, VS Code will fix all relative path links, reference links, and all images/videos with relative paths so that they work in the new document:
...
This feature kicks in whenever you copy and paste text with links between two Markdown files. If there are links that can be updated, you will see the paste widget after pasting. Simply select `Paste and Update Pasted Links` and VS Code will fix up the links for you. This feature can be disabled entirely by setting `"markdown.updateLinksOnPaste": false`.

### Paste and drop files to insert `url()` in css

Need to use and image in your css? Now you can quickly insert a `url()` reference just by dragging and dropping or copy and pasting the image file into the css editor:
...
### Improvements to `/new`
The `/new` slash command has been updated to support quick file generation. You can also provide additional context, such as `#selection`, for Copilot to use during file or project creation. Additionally, `/new` has been enhanced to allow saving the generated files and folders in existing workspaces.

### `/runCommand` enabled by default

A new slash command on the `@vscode` participant, `/runCommand`, enables you to search for and execute a core VS Code command. It can be configured with <a href="vscode://settings/github.copilot.chat.runCommand.enabled" codesetting="true">`github.copilot.chat.runCommand.enabled`</a>.
...
* Revert pull requests using the "Revert" button in the pull requet description of merged PRs
...
The new `terminal.integrated.suggest.runOnEnter` setting allows configuring the enter behavior to run the command if certain conditions are met. The following values are available:
...
To start using preview builds of TypeScript 5.6, just install the [TypeScript Nightly extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-next). Please share feedback and let us know if you run into any bugs with TypeScript 5.6.
...
### Move Markdown language server to own Repository

The language server that powers VS Code's built-in Markdown IntelliSense has been moved into [its own repository](https://github.com/microsoft/vscode-markdown-languageserver). Previously this project was published from a subfolder of the main VS Code repository. This makes it easier to contribute to the project.
