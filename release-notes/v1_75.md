...
Every extension uploaded to the VS Marketplace starting from November is code signed by the VS Marketplace. Once a user installs a signed extension through VS Code's extensions view, VS Code will verify the signature and thus prove this extension is indeed coming from the VS Marketplace and that the extension package has not been modified. If the signature verification fails, VS Code will not install the extension.

VS Marketplace is in the process of signing all existing extensions (including extension that have not been updated since November). Once this process is done, in a couple of months, VS Code will require that all extensions coming from the VS Marketplace are signed. This requirement will guarantee the integrity of every pacakge coming from the VS Marketplace, thus improving the overall security of our extension ecosystem.
