...
## Find widget improvement

* Command: Go to X of Y in Editor Find
...
We can now use all editor line number options when `notebook.lineNumbers` is turned on. For example, with `"notebook.lineNumbers": "on"` and `"editor.lineNumbers": "relative"`, we can get relative line numbers in cell editor.
