...
#### [Go to Recent Directory](https://code.visualstudio.com/docs/terminal/shell-integration#_go-to-recent-directory) - Ctrl+G
...
Since `Ctrl+G` is now used to Go to Recent Directory, this is the new way to send Ctrl+G directly to the shell.

#### [Run Recent Command](https://code.visualstudio.com/docs/terminal/shell-integration#_run-recent-command) - Ctrl+Alt+R

This opens a quick pick with recent commands that have been run, modeled after most shell's reverse index search (`Ctrl+R`) but in a more accessible and more functional package. It supports pinning and fuzzy matching.
...
Because reverse index search isn't very accessible to screen readers, when accessibility mode is on `Ctrl+R` will trigger [Run Recent Command](https://code.visualstudio.com/docs/terminal/shell-integration#_run-recent-command) and `Ctrl+Alt+R` will send `Ctrl+R` to the shell.
...
Windows shares `Ctrl+C` between copying a selection and sending `SIGINT` to the shell, depending on whether there is a selection. A common annoyance because of this is that `Ctrl+C` may or may not send `SIGINT`, for example if you click and make an accidental one character selection. To help mitigate this, `Ctrl+C` to copy the selection will now also clear the selection, so pressing `Ctrl+C` twice will reliably send `SIGINT` either 1 or 2 times.
...
We now pull Quick Fixes from the `[General]` and `[cmd-not-found]` feedback providers, the Quick Fix dialog can be opened by clicking on the lightbulb or via `Ctrl/Cmd+.`.
...
If a user launches VS Code via terminal with a conda or virtual environment already activated, the [Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python) extension now detects that and then either auto-selects the environment itself, or asks the user if they would like to make that environment their selected one; depending on the environment.
...
We've also added a new [AI Tools](https://code.visualstudio.com/docs/editor/artificial-intelligence) topic to the VS Code docs that will help you get started with Copilot.
