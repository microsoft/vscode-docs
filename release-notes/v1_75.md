...
We are happy to anounce that the **Profiles** feature is now available by default in VS Code. A Profile is comprised of settings, keyboard shortcuts, extensions, state, tasks, and snippets. You can customize VS Code for different development environments like Machine Learning, Data Science, or for multiple programming language classrooms like Python, Java or for personal modes like Work or Demo and save them as Profiles. You can open multiple workspaces (folders) with different profiles simultaneously based on the projects you are working on or the classroom you are attending or when you are presenting.

Following image demonstrates a folder opened with **Work** profile that is customised for work mode.
...
Following video demonstrates how to export a profile using GitHub gist and share it with others. Others can preview the profile in VS Code for the Web and import it to their local VS Code instance.
