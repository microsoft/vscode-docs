...
### Workspace trust
This milestone we have focused on fixing issues and reacting to customer feedback.

In order to help users discover the user settings that can be used to customized the feature, we have added a `Configure your settings` actions into the header of the Workspace Trust editor.

![Workspace trust editor](images/1_58/workspace-trust-settings.png)

Speaking of settings to configure the Workspace Trust feature, you'll notice a new setting to configure when the banner appears to notify indicate the current window is in restricted mode. Like the `security.workspace.trust.startupPrompt` setting, the new `security.workspace.trust.banner` allows you to completely disable the banner after you've become familiar with the Workspace Trust feature and don't need any indication of which of your windows are in restricted mode. By default, this banner shows once per workspace and remains until it is dismmissed.

You can learn more about the rationale and development of the [Workspace Trust](https://code.visualstudio.com/docs/editor/workspace-trust) feature in the recent [Workspace Trust blog post](https://code.visualstudio.com/blogs/2021/07/06/workspace-trust). The post explains how Restricted Mode helps protect you from unintended, and potentially malicious, code execution when working with unfamiliar source code and has tips to set up your development machines so you can work both quickly and safely.
...
Added troubleshoot view that contains all settings sync logs and last sync states to settings sync views. You can access this view by running the command **Settings Sync: Show Synced Data** and enabling it from **Views** submenu like below:
...
We added a new setting, `search.maxResults`, which lets you set the maximum number of text search results. The default is 20,000, up from 10,000 previously. You may see some slowness when loading a very large number of search results.
...
The new `terminal.integrated.showLinkHover` setting allows disabling link hovers in the terminal. This can be useful if you find them distracting, for example.

## Editor

There are now new settings to customize the editor's scrollbars:

* Control their visibility using `editor.scrollbar.horizontal` and `editor.scrollbar.vertical`.
* Change their thickness with `editor.scrollbar.horizontalScrollbarSize` and `editor.scrollbar.verticalScrollbarSize`.
* Determine whether clicking scrolls by page or jumps to the click position using `editor.scrollbar.scrollByPage`.
...
The focus this iteration for the [GitHub Pull Requests and Issues](https://marketplace.visualstudio.com/items?itemName=GitHub.vscode-pull-request-github) extension was on fixing bugs. Checkout the [changelog](https://github.com/microsoft/vscode-pull-request-github/blob/main/CHANGELOG.md#0280) to see the highlights.
...
Terminal profiles can now be contributed by extensions, this will cause them to show up in in the profile picker:
...
To contribute terminal profiles, you will need to do 3 things, first add the contribution to your `package.json`:
...
Finally, register the terminal profile provider that will return the set of options to create the terminal with. These can use either a standard process-based `TerminalOptions` or the custom `ExtensionTerminalOptions`:
...
The new `Pseudoterminal.onDidChangeName` allows changing the name of terminals created with `ExtensionTerminalOptions`.
...
There is a new `Memento.keys()` API to enumerate the set of saved `Memento` keys. This can make it easier to deal with previously saved data and be helpful if you need to migrate saved data.
...
The march to finalization of testing APIs continues. This iteration, we refactored some of the testing APIs to better match the style of other APIs in the extension host. To track the finalization process and hear about changes to the APIs, follow [issue #122208](https://github.com/microsoft/vscode/issues/122208).

The primary change we forsee this month are changes to how "run configurations" are provided. To track or weigh in on the progress of these runner APIs, follow [issue #127096](https://github.com/microsoft/vscode/issues/127096).
...
![Inline Suggestion Demo](images/1_58/inline-suggestions.gif)
