...
* you can change their visibility using `editor.scrollbar.horizontal` and `editor.scrollbar.vertical`
* you can change their thickness using `editor.scrollbar.horizontalScrollbarSize` and `editor.scrollbar.verticalScrollbarSize`
* you can change if clicking scrolls by page or jumps to the click position using `editor.scrollbar.scrollByPage`.
...
The inline suggestions API allows extensions to provide inline suggestions that are decoupled from the suggestion widget.
An inline suggestion is rendered as if it was already accepted, but with a gray color. Users can cycle through suggestions and accept them with *tab*.
