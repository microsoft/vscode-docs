...
We've added a new `Memento.keys()` API to enumerate the set of saved `Memento` keys. This can make it easier to deal with previously saved data and can be helpful if you need to migrate saved data.
...
In this iteration we largely focused on bug fixes and some performance improvements, as well as better integration with the GitHub Pull Requests and Issues extension. Additionally, we added the ability to remove a remote repository from the _Remote Repositories_ view in the _Remote Explorer_, and better support for opening `\blob` urls with line ranges.
