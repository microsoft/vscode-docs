...
When debugging is started without a `launch.json` file, VS Code looks at the active editor and based on the language mode of the active editor decides what debug extension to use. However, for some languages, there are multiple fitting debug extensions, and in that case, VS Code would prompt you to choose. To make starting debugging smoother, VS Code now remembers this choice per active file, so that the next time you start debugging, the session starts without any prompts.
