...
Finally register the terminal profile provider which will return the set of options to create the terminal with. These can use either a standard process-based `TerminalOptions` or the custom `ExtensionTerminalOptions`:
...
The march to finalization of testing APIs continues. This iteration, we refactored some of the testing APIs to better match the style of other APIs in the extension host. To track the finalization process and hear about changes to the APIs, follow: https://github.com/microsoft/vscode/issues/122208.

The primary change we forsee this month are changes to how "run configurations" are provided. To track or weigh in on the progress of these runner APIs, follow: https://github.com/microsoft/vscode/issues/127096.
