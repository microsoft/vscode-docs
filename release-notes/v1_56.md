...
Tasks 2.0.0 has been available and working well for over three years, and tasks 0.1.0 has been deprecated in that time. In light of our [node.js-free renderer](https://github.com/microsoft/vscode/issues/92164) goal, tasks 0.1.0 has been deleted rather than brought into the Node.js-free world. When you open a folder that contains 0.1.0 tasks, they will be automatically upgraded to version 2.0.0 for you.
...
Based on [heuristics](https://github.com/microsoft/vscode/issues/57590#issuecomment-813616178), VS Code will show a notification that suggests opening the breakpoint diagnostic tool if we detect the user may be having trouble getting breakpoints to bind.
...
VS Code uses [KaTaX](https://katex.org) for rendering the equations. There are two ways to embed a math equation into Markdown:
...
### Updates to Welcome Page Walkthroughs
...
Once you enter a URL or choose a repository or pull request, VS Code will open a new workspace for that repository. The remote status indicator, on the left side of the status bar, shows the connected remote provider name, e.g. GitHub, for the remote repository.
...
* Search — full-text search requires a pre-built index for exact text matching, otherwise it will fallback to GitHub's fuzzy main-branch only native search.
...
Other extensions do the same. They serve content from ftp-servers, from cloud storage or from databases seamlessly provide these as files to the user in VS Code.
