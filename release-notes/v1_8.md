...
The activity bar is home to all viewlets that we ship with (e.g. files explorer, search, debug). Many people use it to quickly switch between viewlets and to see activity that is going on inside a viewlet (e.g. the number of outgoing files on the git viewlet).

In this release we improved the activity bar with several new features. First of all, it is now possible to reorder viewlets using drag and drop. The order is kept and restored between restarts for all your windows. Second, you can now remove entries from the activity bar using the context menu on either the activity bar itself or the particular viewlet to remove. 

Removed viewlets will still show up in the activity bar when opened but their icon will remove once you switch to another view. This very much simulates the well known UX of an application dock where you can pin applications to always show them or unpin them if you want the application icon to be removed once the application quits.

![Scalable Activity Bar](images/1_8/viewlet.gif) 

Finally, if the window gets small enough so that all viewlet icons can not show, a new overflow menu was added to show the viewlets in a dropdown:

![Scalable Activity Bar](images/1_8/overflow.png) 

### View Picker

A new picker is added that shows viewlets, panels, output channels and terminals and allows for easy opening of those. You can open it via the **Open View** command or by typing `view` followed by a `space` into quick open.

![View Picker](images/1_8/view-picker.gif) 

On Windows and Mac you can press **Ctrl+Q** to bring up the picker. On Linux you can reassign the keybinding if you want (**Ctrl+Q** is assigned to quitting on Linux). Pressing **Ctrl+Q** and holding the **Ctrl** key while pressing the **Q** allows you to jump to an entry from the list and open it after you release the keys.

### Mac: Custom Themed Title

On macOS we are now drawing our own custom title instead of the native one to allow for the theme color to be picked up. This behavior can be changed with the new `window.titleBarStyle` setting. The new default is `custom` and `native` can be set to go back to the old behavior after a restart.

![Custom Title](images/1_8/custom-title.png) 

### Hide the Activity Bar

A new setting `workbench.activityBar.visible` lets you hide the activity bar to the side of the viewlet area. 
