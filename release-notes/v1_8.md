...
Multiple target debugging was introduced in the previous milestone however now we have productise it. This feature helps with the following scenarios:
...
To enable 'multi target debugging' start debugging as always but now once you are debugging it is possible to choose an aditional launch configuration and start it simultaneously. The individual sessions show up as top level elements in the CALL STACK view and the debug toolbar shows a drop down with all the active sessions.
...
![Debug Actions Widget](images/1_8/debug-actions-widget.png)


#### Compounds
An alternative way to start multi target debugging is to create a new `compounds` configuration with an array typed attribute `configurations`. Under that attribute list the names of other launch configurations that should be launched in parallel when this compound gets launched. For example:
...
```json
{
    ...
    "compounds": [
        {
            "name": "Compound",
            "configurations": ["Launch First", "Launch Second"]
        }
    ]
}
```

### `skipFiles`, AKA "Just My Code", AKA "blackbox scripts"
We have added a feature to avoid code that you don't want to step through, which can be enabled with the `skipFiles` setting in your launch config. It's available in the `node`, `node2`, and [Chrome](https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome) debug adapters. It takes an array of glob patterns for script paths to skip, e.g. `"skipFiles": ["node_modules/**/*.js"]`.
* Similarly to `smartStep`, if you step into a skipped file, you won't stop there - you will stop on the next executed line that is not in a skipped file.
...
We have improved the experience of adding new configurations in 'launch.json'. Apart from the `Add Configuration` button in the lower right corner it is possible to manually get snippet suggestions by invoking intelli-sense.
...
### Breakpoints Pane Polish
We have restructured and simplified how we display breakpoints in the breakpoints pane.

![Breakpoints](images/1_8/breakpoints.png)

### Debug Start Dropdown
We have polished the look and feel of the debug start action inside the debug viewlet. We have brought closer together the start debug and the select debug configuration.

![DebugStart](images/1_8/debug-start.png)

### `node2` features: `.scripts` and `sourceMapPathOverrides`
When debugging a Node app, you may have seen breakpoints turn gray with the message "Source map problem?". The new `node2` debug adapter has two new features which make it easier to understand these issues. Typing `.scripts` in the debug console during a debugging session will cause the adapter to print out all the information it has about loaded scripts and their sourcemaps. And the `sourceMapPathOverrides` option in the launch config allows you to rewrite paths in your sourcemap file to enable sourcemaps in cases where the sourcemap file doesn't  include enough information for the debug adapter to find the correct source files on disk. See more information about both of these features [in the `node2` README](https://github.com/Microsoft/vscode-node-debug2#the-scripts-command).

### Proxy debugging note
If you are using ES6 Proxy in your Node app, you may notice the debug adapter crash. This is due to a bug in Node/V8's deprecated debugging protocol. As a workaround, we recommend using our debug adapter that uses their new debugging protocol, which you can opt-in to by setting `"type": "node2"` in your `launch.json` config. It's supported for Node v6.3+. This issue is tracked in [Microsoft/vscode#12749](https://github.com/Microsoft/vscode/issues/12749).
