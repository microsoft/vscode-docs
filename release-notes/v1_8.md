...
We have added a feature to avoid code that you don't want to step through, which can be enabled with the `skipFiles` setting in your launch config. It's available in the `node`, `node2`, and [Chrome](https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome) debuggers. It takes an array of glob patterns for script paths to skip, e.g. `"skipFiles": ["node_modules/**/*.js"]`.
