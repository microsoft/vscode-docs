...
Welcome to the 1.111 release of Visual Studio Code, **the first of our weekly Stable releases**! This release further enhances the agent experience with autopilot permissions, agent-scoped hooks, easier agent debugging and troubleshooting, and more.
...
The new permissions picker in the Chat view lets you control how much autonomy the agent has during a session. Select a permission level from the permissions dropdown in the chat input area.
...
The permission level applies to the current chat session. You can change it at any time during a session by selecting a different level from the permissions picker.

### Autopilot (Preview)

Autopilot is enabled by default in Insiders. You can also try it in Stable by enabling `setting(chat.autopilot.enabled)`.
...
VS Code supports [localizing strings in a extension's `package.json`](https://github.com/microsoft/vscode-l10n?tab=readme-ov-file#packagenlsjson). This iteration we added a few basic IntelliSense features that makes working with these localized strings easier.

To start, you can use `Go to Definition` on a localized string in a package json to either jump to or peek at the string's definition in the `package.nls.json` file:

![Peeking at the definition of a localized strings](images/1_111/nls-string-def.png)

You can also use `Find all References` on a localized string in either the `package.nls.json` or `package.json` to find the place it's used:
