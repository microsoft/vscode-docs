...
We’ve also improved the user experience when using the find widget. When scrolled to the top of the file explorer, additional space is created at the top, ensuring the widget doesn’t obstruct your search results.

![The find widget is rendered above the first file or folder in the explorer when scrolled to the top](images/1_96/explorer_find_empty_space.png)
...
This iteration we have added overtype mode which allows to replace text instead of inserting on type. This mode can be toggled with the command `View: Toggle Overtype/Insert Mode`. While in overtype mode, a status bar item is shown indicating `OVR`. It is also possible to change the cursor style while in overtype mode using the setting `setting(editor.overtypeCursorStyle)`. As well as that, there is a setting `setting(editor.overtypeOnPaste)` which determines whether pasting in overtype mode should overtype or insert. By default this setting is false.
...
Notebooks now have the **Run Cells in Section** action exposed to the cell toolbar of markdown cells. If the markdown cell has a header, all cells contained within the section and children sections will be executed. If there is no header, this will execute all cells in the surrounding section if possible.
...
This iteration we have made it possible to view expanded/contracted information from the TS server. The extension uses the Expandable Hover API to show + and - markers in the editor hover which upon click display more or less information. The experimental setting can be enabled using `setting(typescript.experimental.expandableHover)`. For this setting to work, one must be on TypeScript version 5.8 or above. You can change the TypeScript version using `TypeScript: Select TypeScript Version...`.
...
To try this out, copy some code and paste it in Inline Chat, Quick Chat, or the Chat view. Select the paste widget that shows up and select `Pasted Code Attachment.` Alternatively, you can set the `setting(editor.pasteAs.preferences)` setting:
