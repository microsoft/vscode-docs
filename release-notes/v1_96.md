...
This version of copilot introduces a new `copilot-debug` terminal command to help you debug your programs using VS Code. You can use it by prefixing the command you would normally run with `copilot-debug`. For example, if you normally run your program using the command `python foo.py`, you would run `copilot-debug python foo.py`.

<video src="images/1_96/copilot-debug.mp4" title="copilot-debug command" autoplay loop controls muted></video>

After your program exits, you'll be given options to rerun your program or view, save, or regenerate the VS Code [launch configuration](/docs/editor//debugging.md#launch-configurations) used to debug your program.

![](./images/1_96/copilot-debug.png)
_Theme: [Codesong](https://marketplace.visualstudio.com/items?itemName=connor4312.codesong) (preview on [vscode.dev](https://vscode.dev/editor/theme/connor4312.codesong))_

#### Tasks Support

Copilot's Debugging features, including `copilot-debug` and the `/startDebugging` intent, will now generate `preLaunchTask`s as needed for code that needs a compilation step before debugging. This is often the case for compiled languages such as Rust and C++.
...
### Editor Find History Persistence

The Find widget now can persist the search history across sessions. The search history is stored per workspace and can be disabled via the `setting(editor.find.history)` setting.
...
### Multi Cursor: Select All Find Occurrences
<!-- @Yoyokrazy -->

### Run Cells in Section from Markdown Cells
<!-- @Yoyokrazy -->
...
Test failure messages have been reworked to be both more eye-catching and less obtrusive, particularly with busy scenarios such as in diffs from SCM or Copilot Edits. Clicking on the failure message still opens the peek with complete details about the failure.

![Image of new test error messages](./images/1_96/test-errors.png)
...
It's been reworked to include a drop-down menu that allows turning continuous run on or off individually per-profile. Clicking the indicator will toggle the last used set of run profiles on or off.
...
We've published an npm package, `@vscode/chat-extension-utils`, that aims to make it as easy as possible to get a chat participant up and running. It takes over several things that you would otherwise have to do yourself so that your chat participant can be implemented in a just a few lines of code, as well as a collection of useful, high-quality elements to use with [@vscode/prompt-tsx](https://github.com/microsoft/vscode-prompt-tsx).

You can view full documentation in [its readme](https://github.com/microsoft/vscode-chat-extension-utils/blob/main/README.md) and see it in action in the [sample chat extension](https://github.com/microsoft/vscode-extension-samples/tree/main/chat-sample). Our new [LanguageModelTool API docs](https://code.visualstudio.com/api/extension-guides/tools) also describe how to use it.

### Attributable Coverage API

The test coverage APIs now allow extensions to provide coverage information on per-test basis. To implement it, populate the the `includesTests?: TestItem[]` property on the `FileCoverage` to indicate which tests executed code in that file, and implement `TestRunProfile.loadDetailedCoverageForTest` to provide statement and declaration coverage.

See the [Attributable Coverage section above](#attributable-coverage) for an example of what this looks like for users.

### Contribution Mechanism for the JavaScript Debug Terminal

The JavaScript debugger in this release has added a mechanism for other extensions to participate in the creation of JavaScript Debug Terminals. This allows frameworks, or runtimes aside from Node.js, to enable debugging in the same familiar place. Please refer to its documentation [here](https://github.com/microsoft/vscode-js-debug/blob/main/EXTENSION_AUTHORS.md#extension-api).

## Preview Features

### Pasting Code as an Attachment in Copilot Chat

You can now paste code as an attachments in chat, which will include file information and corresponding line numbers. This will only work with code coming from the current workspace.

To try this out, copy some code and paste it in inline chat, quick chat, or panel chat. Click on the paste widget that shows up and select `Pasted Code Attachment.` Alternatively, you can set the [editor paste as preferences](https://github.com/microsoft/vscode-docs/blob/vnext/release-notes/v1_96.md#preferred-edit-type-for-pasting-files) with the following:
