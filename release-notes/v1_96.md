...
The global `fetch` function now comes with proxy support enabled (`setting(http.fetchAdditionalSupport)`). This is simiar to the `https` module which already had proxy support.
