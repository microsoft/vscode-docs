...
This milestone we have added support for custom instructions when generating a commit message. You can use the `setting(github.copilot.chat.commitMessageGeneration.instructions)` setting to either specify the custom instructions or specify a file from your workspace that contains the custom instructions. These instructions will be appended to the prompt that is used to generate the commit message.

### Inline Chat

We tweaked the inline chat UI, its progress reporting is more subtle, during streaming squiggles are disabled, and detected commands are rendered more nicely.
...
Please give this experimental feature a try and let us know what you think.
