...
### Experimental terminal completions for `zsh` and `bash`
...
![The command, code, is typed on the terminal, which shows sugggestions. Then -- is typed and options are provided, --locale is selected. Completions are requested with ctrl+space and all locales are shown. e is typed and the list is filtered to en and es.](images/1_96/terminal-completions.gif)
...
### Microsoft Account now using MSAL (with WAM support on Windows)

In order to ensure a strong security baseine for Microsoft authentication, we've adopted the [Microsoft Authentication Library](https://github.com/AzureAD/microsoft-authentication-library-for-js) in the Microsoft Account extension.
...
For `InputBox` you have been able to set the "value selection" which allows you to programatically select part or all of the input. This iteration we added proposed API to allow value selection in a QuickPick.
