...
We added a notebook toolbar in the editor, in which you easily access actions like insert cell, run all cells or switch kernels. The toolbar can be disabled by setting `notebook.globalToolbar`.

![Notebook toolbar](images/1_57/notebook-toolbar.png)

**Output toolbar**

The "Clear Cell Outputs" action and the button to pick a different output renderer or mimetype are now combined into a single `...` menu next to cell outputs. This can be disabled by setting `notebook.consolidatedOutputButton`

![Notebook output toolbar](images/1_57/notebook-output-toolbar.png)

**Focused cell highlight on gutter**

We now support rendering focused cell highlight in the gutter area of a cell, which helps identify which cell is focused when it's long.

![Notebook focus indicator on gutter](images/1_57/notebook-focus-indicator.png)

**Show folding icon on mouse over**

The folding icon on markdown cells are now visible on mouse over other than visible all the time. You can customize the behavior by setting `notebook.showFoldingControls`.


![Notebooh show focus icon on mouse over](images/1_57/notebook-show-folding-icon-on-mouse-over.gif)

The full list of notebook layout settings we implemented this month are:
...
* `notebook.editorOptionsCustomizations`: This setting lets the user to customize the cell editor settings in the notebook.
...
`@link` is support in hovers, suggestions, and signature help. We have also updated `vscode.d.ts` to use `@link`.
...
This is probably most useful when using <kbd>ctrl</kbd>/<kbd>cmd</kbd> click to navigate through your code.
...
This is useful if you want to add more explicit typings. It's also can save time when you want to extract the return type to a named type/interface, or if you need to modify a function's return type.
