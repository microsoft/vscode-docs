...
![Drag and drop a terminal tab](images/1_57/tabs-drag-and-drop.gif)

Dragging a tab in a terminal group into the empty area will remove it from the group (result is the same as unsplit, also available via the context menu).

![Unsplit a terminal tab](images/1_57/tabs-unsplit.gif)

Dragging a tab into the main terminal area allows joining a group (also available via the context menu).

![Joining a terminal tab to an existing tab group](images/1_57/tabs-joining.gif)

There are now more ways to split a terminal:

- On hover, select the inline split button.
- Right-click the context menu and select the **Split Terminal** option (`kb(workbench.action.terminal.splitInstance)`).
- `kbstyle(Alt)` click a tab, the **+** button, or the single tab on the terminal panel, as shown below.
