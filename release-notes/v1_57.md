...
### Refreshed Welcome Page

Over the the past couple iterations we have been working on refreshing the welcome page for VS Code, and we're happy to announce that starting this iteration our new welcome page ("Getting Started") is enabled by default for all users. In this new expereince, both the core of VS Code and installed extensions can contribute "walkthroughs" to the welcome page, which are designed to help you with getting started in either VS Code itself or with a new extension. For example, the C++ extension can contribute a walkthrough to help you get set up with a compiler and various other platform-specific tooling.
...
Additionally, once you've completed (or dismissed) all walkthroughs, we shift around the contents to double the number of recents workspaces accessible with one click.
...
The **Remote Repositories** extension was introduced as a [built-in preview feature in 1.56](https://code.visualstudio.com/updates/v1_56#_remote-repositories-remotehub) and is now publically available, but it is no longer built-in to VS Code.
...
In this iteration, while we were mainly focused on bug fixing and polishing the extension up for release, we did add a few new features. You can now see your remote repositories in the _Remote Explorer_ view, to quickly open or see and compare any uncommitted changes you may have. Additionally, you can apply changes, from the same repository, to the current remote repository &mdash; allowing you to quickly and easily move uncommitted changes from one branch to another.
...
### Notebook layout customizations

We added several notebook layout settings this month to allow users to customize the notebook editor for their workflow. The customizations which are turned on by default are:
...
The **Clear Cell Outputs** action and the button to pick a different output renderer or mimetype are now combined into a single **...** menu next to cell outputs. This can be disabled by setting `notebook.consolidatedOutputButton` to `false`

![Notebook output toolbar](images/1_57/notebook-output-toolbar.png)

**Focused cell highlight on gutter**

VS Code now supports rendering focused cell highlight in the gutter area of a cell, which helps identify which cell is focused when it's long.

![Notebook focus indicator on gutter](images/1_57/notebook-focus-indicator.png)

**Show folding icon on mouse over**

The folding icon on Markdown cells are now visible on mouse over rather than visible all the time. You can customize the behavior by setting `notebook.showFoldingControls` to either `mouseover` (default) or `always`.
...
We now support showing a simple context menu inside webviews, such as the markdown preview, release notes, and extension pages:
...
There is a new setting `editor.suggest.showDeprecated` - it defaults to
on" but when turned "off" suggest will not show items that are marked as deprecated.
...
The JavaScript debugger integrates with the Edge Developer tools to provide a DOM, style, and network inspector for browser debugging, accessible via the new "Inspect" icon in the debug toolbar.
...
If you aren't already using [Edge](https://aka.ms/getedge) for browser debugging, you can switch to it be replacing `"type": "chrome"` or `"type": "pwa-chrome"` with `"type": "pwa-msedge"` in your `launch.json`. This is also supported in [Blazor](https://dotnet.microsoft.com/apps/aspnet/web-apps/blazor) applications by setting `"browser": "edge"` in its launch config.
...
We now support [JSDoc @link tags](https://jsdoc.app/tags-inline-link.html) in JavaScript and TypeScript comments. These let you create clickable links to a symbol in your documentation:
...
The **Infer function return** refactoring for TypeScript adds explicit return type annotations to functions:

![The infer return type refactoring in action](images/1_57/ts-infer-return.gif)
...
We now prompt you if you have TypeScript server logging enabled for a prolonged period of time:
...
In previous versions of VS Code, sending typed arrays to or from a webview has a few quirks:
...
Some time ago, VS Code added support for hierarchal debug sessions, but no information about the hierarchy was exposed in the extension APIs. To address this, there is a new property on the `DebugSession` interface which references the parent session, if any. A session's parent will never change.
...
When working on extensions that use proposed APIs, it's possible that VS Code a new Insiders build is released with breaking changes. In order to provide a more seamless transition for users, you can now target Insiders versions precisely with a date tag. For example, setting `engines.vscode` to `^1.56.0-20210428` will target any VS Code 1.56 (or newer) build which was created on or after 0:00 UTC, April 28th, 2020. This allows you to safely release post-dated extension updates before an upcoming Insiders version is released.
...
When a `TreeItem` has a trusted markdown tooltip, that tooltip can contain commands of the format `[this is a link](command:workbench.action.quickOpenView)`. This follows the same format as all the other places where command links are supported in markdown.
...
We intended to finalize the first parts of the testing APIs this iteration, but it was deferred due to the number of other in-flight features. While there were no breaking changes to the testing APIs this iteration, we continued to make progress on its editor experience. For example, the peek view gained a split view which allows you to browser the results of current and past test runs.

![Image of a peek view with a references-style tree shown on the right side of a split view](images/1_57/testing-peek.png)
*Theme: [Codesong Theme](https://marketplace.visualstudio.com/items?itemName=connor4312.codesong)*

Testing gained a new set of keybindings as chords starting with `kb(Ctrl+;)`, the peek view features better support for markdown messages, and many bugs were fixed.

If you use the [Test Explorer UI](https://marketplace.visualstudio.com/items?itemName=hbenl.vscode-test-explorer) extension for tests, you can toggle on the setting `testExplorer.useNativeTesting` to switch to the new, native UI. This iteration, we plan to increase socialization on this setting and begin adoption with partner extension teams.
...
VS Code's language packs got migrated to a new translation pipeline to support a standardized process.
...
There is a new topic on how to use [PyTorch](https://pytorch.org) within VS Code. In the [PyTorch support in VS Code](https://code.visualstudio.com/docs/python/pytorch-support) article, you'll learn how Jupyter Notebooks can be extended with PyTorch and [TensorFlow](https://www.tensorflow.org) data types for machine learning.
