...
We have added an experimental variable view to the debug panel that can be used to view variables from a notebook kernel if the extension provides them. This can be enabled with the setting `"notebook.experimental.variablesView": true`.

<video src="images/1_86/notebook-variable-viewer.mp4" title="Notebook variable tree view" autoplay loop controls muted></video>
...
Triggered breakpoints can be set by right-clicking on the glyph margin, selecting **Add Triggered Breakpoint...**, and then choosing which breakpoint enables this one.
...
After you paste, you can always switch to paste the content in a different way using the paste widget.
...
* Easier navigation from Quick Search to the search viewlet. Simply select the button beside the input or a result to transfer the results to the search viewlet.
...
Also improvements for the `vscode.commands.openIssueReporter` command, which can now contribute additional extension data and modify the URL for where the GitHub issue will be directed to.
