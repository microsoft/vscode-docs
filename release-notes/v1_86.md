...
A zoom level indicator is shown in the Status Bar when the custom zoom level of a window does not match the `window.zoomLevel` setting value. Click the Status Bar indicator to find controls to change zoom levels, reset the zoom level, or quickly go to the related settings.
...
A new `files.autoSaveWhenNoErrors` setting lets you to disable Auto Save if there are error markers in the file when Auto Save would normally save the editor.

This can be very useful when you have external tools watching for file changes, where you want to avoid that these tools act on a changed files that contains errors.
...
A new setting `window.systemColorTheme` lets you explicitly override the system color theme that applies to native elements in VS Code, such as the menu or dialogs on macOS.

By default (`default`), the color theme will match what is configured in the OS, but there are other options:
...
The Output panel now has an **Open Output in New Window** action in the panel title menu to view the Output channel in a separate window.
...
This milestone we have added a new setting, `scm.inputMinLineCount`, that can be used to control the initial height of the commit input field. The setting that controls the maximum size of the commit input field has been renamed from `scm.inputMaxLines` to `scm.inputMinLineCount` so that it matches the naming convention of the other similar settings.

User can now use some language specific editor settings for further customize the commit input field. Language specific editor settings can be used to define editor rules, as well as disable word wrapping:
...
To help better manage workspaces with large number of repositories, we have added a new command to the repository context menu - "Close Other Repositories". We have also added multi-selection support for both the "Close Repository" and "Close Other Repositories" commands so users can close multiple repositories at once, or close all repositories except for a few.

The Source Control Repositories view can get busy with all the actions that are contributed by extensions. To help with this, we have enabled the ability to hide contributed actions by right clicking on the action and choosing "Hide". The hidden actions are moved into the "..." menu. All actions can be hidden except the "Checkout" and "Sync" actions.

### Incoming/Outgoing changes improvements

We continue to polish the incoming/outgoing changes section in the "Source Control" view. This milestone we made changes to that the "Incoming" changes node is only being shown if the current branch has a tracking remote branch. We have added `Fetch`, `Pull` actions to the incoming changes node, and `Push` action to the outgoing changes node. We have added actions to view each individual commit, and all the incoming/outgoing changes that leverages the multi-file diff editor. Users can also use a new setting, `scm.showChangesSummary`, to hide the "All Changes" entry.
...
To address a long standing feature request we have added the capability to merge tags. A new command has been added to the command palette, `Git: Merge...` that replaces the `Git: Merge Branch...` command. Invoking the `Git: Merge...` command will display a quick pick widget that will now list both branches as well as tags.

### Commit signing using SSH keys

This milestone we have added the capability to sign commits using an SSH key with a passphrase. When signing a commit with the SSH key, VS Code, will display a quick input widget at the top of the screen to prompt for the passphrase. VS Code does not cache the passphrase and will prompt for it each time a commit is signed.
...
<video src="images/1_86/notebook-variable-viewer.mp4" title="Notebook variable tree view in the Run and Debug view." autoplay loop controls muted></video>
...
You can now set breakpoints that are automatically enabled once another breakpoint is hit. For example, this can be very useful when diagnosing failure cases in code that happen only after a certain precondition.
...
<video src="../docs/editor/images/debugging/debug-triggered-breakpoint.mp4" title="Add a triggered breakpoint that depends on another breakpoint to be hit." autoplay loop controls muted></video>
...
The previous Less grammar had been archived. VS Code now uses the grammar from [Better-Less](https://github.com/radium-v/Better-Less) for Less syntax highlighting.
...
* Release the keys to stop recording. Your request is sent to Copilot.
...
In addition to "hold to speak", we are experimenting with a lighter alternative UI for this functionality. Activate Quick Voice by pressing `kbstyle(Cmd+K)` or `kbstyle(Ctrl+K)` and then holding `I`. While holder `I`, a more lightweight recording UI is shown. Upon release, the request is sent.
...
If possible, the full contents of the file will be included. If that is too big to fit into the context window, an outline of the file will be included that includes functions and their descriptions without implementations. If the outline is also too big, then the file won't be part of the prompt.
...
We now support the beta release of TypeScript 5.4. Check out the [TypeScript 5.4 iteration plan](https://github.com/microsoft/TypeScript/issues/56948) for details on this release. Highlights include:
...
* Easier navigation from Quick Search to the Search view. Simply select the button beside the input or a result to transfer the results to the search viewlet.
...
This iteration, test coverage's API and in-editor experience has reached feature-complete state. We encourage extension authors to try it out and provide feedback prior to its forecasted finalization in the VS Code 1.87 release.
...
When registering a `FileSystemProvider` with `registerFileSystemProvider`, the `options` can be set to mark the entire file system as readonly. Now, if the `isReadonly` option is set to a `MarkdownString` then that message will be shown in the editor when the user tries to edit the file a file in the filesystem.

![Example of readonly message showing](images/1_86/readonly-message.png)
