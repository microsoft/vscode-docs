...
A new setting `window.zoomPerWindow` is enabled by default and configures the zoom commands ("Zoom In", "Zoom Out", "Zoom Reset") to apply to the active window and not all opened windows. Previously using these commands would not only apply it to all opened windows but also write it as setting to the `window.zoomLevel` property.
...
As you can see from this example, only the active view zoom level changes, but not from the other window:
...
This can be very useful when you have external tools watching for file changes where you want to avoid these tools to act on a state with errors in the file.
