...
The existing setting `setting(workbench.secondarySideBar.defaultVisibility)` adds two more options that allow to open the Chat view maximised by default. You can pick `maximizedInWorkspace` to open Chat maximized when opening a new workspace or `maximized` when opening any new workspace including empty windows.

![Chat view maximised.](images/1_103/max-chat.png)

### Confirm to exit with a pending chat session

There is now a dialog showing when you try to quit VS Code or close its window when a chat response is in progress:

![Screenshot of confirmation to exit with running chat.](images/1_103/confirm-chat-exit.png)

This helps to prevent accidentally closing a workspace where an agentic session is actively changing files or responding to your request.

### Notification toast when confirmation is needed

When `setting(chat.notifyWindowOnConfirmation)` is enabled, we now leverage the OS native notification system to show a toast when confirmation is needed:

![Screenshot of toast for confirmation of a chat agent.](images/1_103/chat-toast.png)

We plan to improve this experience in the future to allow for displaying more information and allowing to approve directly from the toast. For now, clicking the toast will focus the window where the confirmation originated.
...
The context menu on editors has been cleaned up to group split and move related options into a sub-menu:

![New split and move tab context menu](images/1_103/editor-menu.png)
