...
![new tool picker using a quick tree](images/1_103/tool-picker-quick-tree.png)

Notable features:
* Expand/Collapse tool sets, MCP servers, extension contributed tools, etc
...
![Inline and block equations in a chat response](images/1_103/chat-math.png)
...
Math rendering can be enabled using `setting(chat.math.enabled)`. Currently it is off by default but if testing goes well we hope to enable it in future releases.

### Azure Dev Ops repos remote index support

The [`#codebase` tool](../docs/copilot/reference/workspace-context.md) now supports remote indexes for workspaces that are linked to Azure Dev Ops repos. This allows `#codebase` to search for relevant snippets almost instantly without any initialization. This even works for larger repos with tens of thousands of indexable files. Previously this feature only worked with GitHub linked repos.

Remote indexes will be used automatically when working in a workspace that is linked to Azure Dev Ops through git. Just make sure you are also logged into your VS Code with the Microsoft account you use to access the Azure Dev ops repos.

We're gradually rolling out support for this feature on the services side so not every organization may be able to use it initially. We hope to turn it on for as many organizations as possible if it looks like the rollout is going well.
...
Terminal inline chat now better detects your active shell, even when working within subshells (e.g., launching Python or Node from PowerShell or zsh). This dynamic shell detection improves the accuracy of inline chat responses by providing more relevant command suggestions for your current shell type.
![Terminal inline chat showing node specific suggestions](images/1_103/hello_node.png)

### Notebook Inline Chat with Agent Tools

The new experimental setting setting(inlineChat.notebookAgent:true) will allow the inline chat widget to use the full suite of notebook agent tools to allow for additional capabilities like running cells and installing packages into the kernel. This also currently requires enabling the setting for inline chat v2: setting(inlineChat.enableV2:true)
...
In this release of VS Code, we have enabled an experimental tool-calling mode when the number of tools exceeds the limit (currently 128.) Tools will be automatically grouped and the model is given the ability to activate and call groups of tools. This behavior, including the threshold, is configurable via the setting `setting(github.copilot.chat.virtualTools.threshold)`.

### Chat Checkpoints

We've introduced checkpoints that allow you to restore different states of your chat conversations. You can easily revert edits and go back to certain points in your chat conversation. Clicking on a checkpoint will revert workspace changes and the chat history to that point. After restoring a checkpoint, you can redo that action as well!

<video src="images/1_103/chat-checkpoints.mp4" title="Creating and managing chat checkpoints" autoplay loop controls muted></video>
...
<video src="images/1_103/chat-previous-edits.mp4" title="Inline edits being applied directly in the editor" autoplay loop controls muted></video>

You can still control this behavior using the `setting(chat.editRequests)` setting if you prefer editing via the toolbar hovers above each request.

## MCP

### MCP Autostart and Trust Changes

MCP servers can automatically be started when updated or changed with the setting `setting(chat.mcp.autostart:newAndOutdated)`. With this setting, you will no longer see the blue "refresh" icon beside the tool picker in chat, and servers will be started when the first message is sent in chat. You can also change this setting within the icon's tooltip and see which servers will be started:

![modal saying that DCR is not supported but you can provider client credentials manually](images/1_103/mcp-refresh-tip.png)

We will also now prompt you the first time a server from your workspace is run after being updated or changed. Giving trust to these servers is particularly important with autostart turned on to running undesirable commands unknowingly.

### Client credentials flow for remote MCP servers

The ideal flow for a remote MCP server that wants to support authentication is to use an auth provider that supports Dynamic Client Registration (DCR). This allows the client (VS Code) to register itself with that auth provider so the auth flow is seemless.

However, not every auth provider supports DCR so to accommodate those that do not, we have introduced a client credentials flow that allows you to supply your own client id & (optionally) client secret that will be used when taking you through the auth provider's auth flow. Here's what that looks like:

* Step 1: VS Code detects that DCR can't be used, and asks if you want to do the client credentials flow:

![modal saying that DCR is not supported but you can provider client credentials manually](images/1_103/mcp-auth-no-dcr1.png)

  > NOTE: At this point, you would go to the auth provider's website and manually create an application registration. There you will put in the redirect uris mentioned in the modal - a very important step.
...
![an input box to provide the client id](images/1_103/mcp-auth-no-dcr2.png)

* Step 3: Then you'll put in the client secret if you have one, and hit ENTER (leave blank if you don't have one)

![an input box to provide the optional client secret](images/1_103/mcp-auth-no-dcr3.png)
...
Since the addition of remote MCP authentication, there has been a command available in Command Palette called `Authentication: Remove Dynamic Authentication Providers` which allows you to remove client credentials (client id & if available, a client secret) and all account information associated with that provider.

We've now exposed this command in the Account menu. You can find it inside of an MCP server account:

![manage dynamic auth option in an account's submenu](images/1_103/mcp-remove-dynamic-auth1.png)

or at the root of the menu if you don't have any MCP server accounts yet:

![manage dynamic auth option in the root of account menu](images/1_103/mcp-remove-dynamic-auth2.png)
...
You can now view, create, delete, and open worktrees in a new or current window by using commands available from the Command Palette or Source Control view.
...
![Terminal suggest completion items with lsp now shows documentations](images/1_103/lsp_documentation.mp4)
...
We now support shell integration for Python when using version 3.13 or later. When enabled, PyREPL will be automatically disabled to ensure compatibility. Users may disable shell integration if they prefer to continue using PyREPL.
![Python Shell Integration](images/1_103/python_shellIntegration.png)

### TypeScript 5.9

VS Code now includes TypeScript 5.9.2. This major update brings new a few new language improvements including [support for import defer](https://devblogs.microsoft.com/typescript/announcing-typescript-5-9/#support-for-import-defer), along with tooling improvements such as [improved docs for many DOM apis](https://devblogs.microsoft.com/typescript/announcing-typescript-5-9/#support-for-import-defer).
...
When you hover over a symbol in JavaScript or TypeScript, VS Code tries to show the most useful IntelliSense type information about that symbol. Types can be very complex, so one challenge for us has been trying to find the right balance between showing enough details to be useful while also not showing so much info that it becomes overwhelming. It's hard to come up with a good one size fits all approach to this, and the level of type detail you want may change depending on what you are working on.

That's why this iteration we've added new UI that gives you more control over how types are shown in hovers. When you hover over a symbol, now you can click the little `+` on the left side of the hover widget to expand the interfaces and complex types in the hover into their components. For example, you can use this to see the properties of an interface directly in the hover:

<video src="images/1_103/ts-expandable-hovers.mp4" title="Expanding interface types" autoplay loop controls muted></video>

Hovers can be expanded multiple times, which will recursively expand types from previous expansion. If you ever expand too much, just click the `-` to go back to the previous level. Also keep in mind that not every type is going to be expandable and that we still have to have some limits on just how much expansion we can support. Please [let us know](https://github.com/microsoft/vscode/issues/new?template=bug_report.md) if there are any cases where expandable hovers aren't working how you'd like.
...
Enable the `setting(chat.agentSessionsViewLocation)` setting to try this experimental feature. When set to `view`, the **Chat Sessions** view is shown in the VS Code Side Bar. When set to `showChatsMenu`, chat sessions are shown alongside local chat in history.
...
#### Installing dependencies in Virtual Environments created using uv

We now support installing required dependencies when run Jupyter Notebooks against a Virtual Environment created using [uv](https://docs.astral.sh/uv/pip/environments/).

<video src="images/1_103/uv-install-ipykernel.mp4" title="Install IPyKernel in uv Virtual Environment" autoplay loop controls muted></video>
...
### Custom webview based widget in chat responses

The chat output renderer api lets extensions take chat responses beyond text and images. With it, your extension can use a [webview](../api/extension-capabilities/extending-workbench.md) to render arbitrary html content in the chat output. Example use cases include: custom visualizations, inline previews, and even interactive controls.
...
![A mermaid diagram](images/1_103/chat-output-1.png)

Again the neat thing is not that VS Code can render mermaid diagrams, but that this rendering can be contributed entirely by extensions. With it, you can iterate on the custom outputs in chat:

![Asking chat to generate a modified version of the first diagram](images/1_103/chat-output-2.png)
...
The new chat session provider API proposal allows extensions to integrate their own chat backend into VS Code's native chat UI. Using it, your extension can open a new chat session, populate the history for that session, and respond to new user prompts.

This API is still in early stages and is likely to change. However we're already using to power the new [Github coding agent session flow](#chat-sessions-experimental), which loads chats from GitHub and allows you to chat with an agent that is controlled entirely by Github.
...
NOTE: This change is dependant on a change to anything that provides an alternative implementation of a Secret Storage, notably https://vscode.dev, which has adopted the new API, and https://gitub.dev, which will adopt the new API soon. In an environment where it is not supported, this API will throw.
