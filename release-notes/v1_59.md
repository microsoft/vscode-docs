...
### New setting to share view state across editor groups

A new setting `workbench.editor.sharedViewState` was added to configure how editor view state (e.g. the scroll position in the editor) is shared across editor groups.

By default this setting is disabled to preserve todays behaviour: if you open an editor to the side and close that editor group later only to open the editor again to the side, no view state is restored because you are opening a new editor group.

However, when you enable this setting, the most recent editor view state is preserved across all editor groups and will be used unless more specific view state is found for the editor group.
