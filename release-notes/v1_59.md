...
Improved Extensions view when resizing. In the following gif, you can see that the extensions view with defeault width shows all details (Previously icon, ratings & install count were not shown). As it shrinks, smaller extension icon is shown and when its width reduces further, icon and ratings are hidden.
