...
* **[Remote - Containers devcontainer CLI](#remote-containers-devcontainer-cli)** - Command line interface for working with development containers.
...
Improved Extensions view on resize. In the following short video, the Extensions view with default width shows all details (previously icon, ratings & install count were not shown). As it shrinks, a smaller extension icon is shown, and when its width reduces further, the icon and ratings are hidden.
...
The Extension pane Details tab now shows categories, resources links, and other information such as extension release and update date. Selecting a category will display other extensions in that category in the Extensions view.
...
The Settings editor also now supports multiline string settings, where the value is rendered in a multiline textarea instead of a single-line input box:
...
A new setting `explorer.copyRelativePathSeparator` allows to explicitly set the path separator to use when invoking the **Copy Relate Path** action. The available options are:
...
The [Remote - Containers](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers) extension, for working with Docker containers from within VS Code, now includes a `devcontainer` command line interface to let you easily open a folder within a dev container (`devcontainer open`) or build a dev container image (`devcontainer build`).
...
Extensions that contribute a way to create a new file editor, for instance notebooks or custom editors, can now contribute commands to the new `file/newFile` menu contribution point. This menu can be accessed from the **New File...** item on the Welcome page or in the File menu.
...
Note that object settings must have `additionalProperties` set to either `false` or an object to be supported in the Settings editor. Otherwise, the Settings editor will interpret the setting as a complex setting that can take on any shape, and direct the user to the settings JSON file.

### Multiline string settings

To add support for a multiline string setting in the Settings editor, add `"editPresentation": "multilineText"` as a key-value pair to the string setting.
Changing a string setting to a multiline one will cause the Settings editor to render the setting value in a multiline textarea instead of a single-line inputbox.
...
A new next version of the [Language Server Protocol](https://microsoft.github.io/language-server-protocol), together with the corresponding npm modules, has been published. The version contains a new proposal for the completion item label details that is in line with the latest changes in VS Code itself.
...
We had always been running a suite of smoke tests for each build that is triggered that literally opening VSCode (both desktop and web) and executing a bunch of UI elements to ensure proper functionality. This milestone we invested more into this infrastructure allowing us to remove any manual smoke testing from our endgame plans.

Specifically: smoke tests now run on all platforms (macOS, Linux and now also Windows). On top of that, one of our more involved smoke test suites that specifically tests for regressions between the last released stable version of VSCode and the new release has been automated to run on each commit.

Finally, thanks to the awesome [Playwright](https://github.com/microsoft/playwright) library that we use for automated web smoke testing, we enabled their tracing capabilities which allows us to replay failing smoke tests using their [trace viewer](https://playwright.dev/docs/next/trace-viewer/) tool.
