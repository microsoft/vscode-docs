...
The [Mock Debug extension](https://github.com/microsoft/vscode-mock-debug) has been updated to "exercise" these features.
...
A new next version of the language server protocol, together with the corresponding npm modules, has been published. The version contains a new proposal for the completion item label details that is in line with the latest changes in VS Code itself.
