...
At this point, you can open an Untitled file and paste in a chunk of code into your editor. Here's an example:
...
The setting also allows you to provide language overrides that can be used to specify language modes you don't want to be automatically switched off from. Given this example:
...
We've separated the code that interacts with the ML model into its own library released as an npm package which lives over in [the vscode-languagedetection repo](https://github.com/Microsoft/vscode-languagedetection).
