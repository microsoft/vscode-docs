...
We have redesigned the new-session input in the Agents window to make it clearer and easier to start delegated work. The updated experience brings the prompt, model selection, workspace selection, and other session controls together in a more streamlined layout.
...
The Agents window is a place you might leave open all day, so it no longer has to be a flat panel color. Chat can now have a decorative background: either a theme-aware pattern of built-in VS Code icons, or an image of your own.
...
Agents can now resolve workspaces by their project name in addition to absolute paths and workspace URIs. Session tools also preserve project URIs and all working directories for multi-root workspaces.
...
Copilot and Claude agent sessions in the editor window Chat view now support [multi-root workspaces](https://code.visualstudio.com/docs/editing/workspaces/multi-root-workspaces) (experimental).
...
### Layout density for the editor window

**Settings**: `setting(workbench.experimental.modernUI)`, `setting(window.density.layout)`

When `setting(workbench.experimental.modernUI)` is enabled, the editor window now has a **Layout Density** setting. This setting lets you choose between two layout densities for the editor window:
...
Word wrapping now takes the visual width of inline decorations into account, including color decorators, inlay hint spacing, inline progress indicators, and breakpoint placeholders. This keeps wrapped lines within the editor viewport instead of allowing decorations to push text offscreen.
