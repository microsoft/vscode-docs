...
Two iterations ago, we introduced the new Emmet model which gave Emmet abbreviation expansions in suggestion/auto-completion list as well as multi cursor support for most of the Emmet commands. This model was opt-in until now. In this milestone, we are switching over to the new model on for all users. Please read more about it on the [Emmet 2.0 blog post](/blogs/2017/08/07/emmet-2.0.md)
