...
You can now quickly switch active terminals and rename them using the quick picker. Either use the command `Terminal: Switch Active Terminal` or type in `term␣` in quick open.

![Switch Terminals](images/1_15/terminal_quickpick.png)

### Window Terminal Session Name Changes

Terminal titles in Windows now change when launching a process from the initial shell. For example, running `cmd.exe` inside powershell will now update the title to cmd.
