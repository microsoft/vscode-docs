...
Only `resource` specific settings are applied at Folder level and `workbench` specific settings are not. Default settings editor on the left shows only those settings that can be applied at Folder level. If `workbench` specific settings exist in Folder settings, then they are shown dimmed with an info icon.

![Unsupported Setting](images/1_15/unsupported_setting_info.png)

Refer to **[Settings classification](#settings-classification)** to know about workbench and resource specific settings.
