...
In the august release we will stop supporting variable substitution like `${workspaceRoot}` for the `program` and `runtime` contributions in the `package.json` of extensions. In the july release we will continue supporting this substitution to give time for extensions to migrate.
