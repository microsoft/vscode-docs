...
* **[Release Notes search](#search-in-markdown-preview)** - Full search for Release Notes, Markdown preview, and Extensions.
* **[Customize Side Bar views](#managing-views)** - Rearrange/remove sections from Explorer, Debug and Extensions views.
* **[Quick terminal switching](#terminal-switching)** - Faster navigation between your active terminals.
...
* **[Preview: Color picker](#preview-color-picker)** - Quickly create colors in your CSS, SASS, and Less files.
* **[Preview: Multi-root workspaces](#preview-multi-root-workspaces)** - Use "Workspaces" to organize multiple projects (Insiders build).
...
* **[Workbench](#workbench)** - Customizable Explorer sort order, prefilled Quick Open actions.
* **[Integrated Terminal](#integrated-terminal)** - Session environment variables, delete word keyboard shortcuts.
* **[Debugging](#node-debugging)** - New Node.js Loaded Script Explorer.
...
Two iterations ago, we introduced the new Emmet model which users could opt in for. This gave Emmet abbreviation expansions in suggestion/auto-completion list as well as multi cursor support for most of the Emmet commands.

In this milestone, we are switching over to this new model for all users. Please read more about it on the [Emmet 2.0 blog post](/blogs/2017/08/07/emmet-2.0.md)
...
Thanks to [Cody Hoover (@hoovercj)](https://github.com/hoovercj), you can now customize the syntax highlighting colors in the user settings.
![Token Color Customization](images/1_15/token_color_customization.png)

The new color will be applied on top of the current theme. It will applied live, no reloading of VSCode is required.

A preconfigured list of syntax tokens ('comments', 'strings', ...) is available for the most common constructs. If you want more, you can do so by directly specifying TextMate theme color rules.
