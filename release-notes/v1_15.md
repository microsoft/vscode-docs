...
The context menu item to open a folder in a terminal now defaults to open the folder in the integrated terminal. You can configure this to point at the externla terminal using the `terminal.explorerKind` setting.
...
The terminal now has default keys for deleting words in the terminal:
...
These word by sending keystrokes to the shell, as a result they don't work on some Windows shells.
