...
Two releases ago, we introduced the new Emmet model and users could opt in to try it out. The new implementation showed Emmet abbreviation expansions in the suggestion/auto-completion list and added multi-cursor support for most of the Emmet commands.

In this release, we are switching over to this new model for all users. Please read more about it on the [Emmet 2.0 blog post](/blogs/2017/08/07/emmet-2.0.md)
...
The markdown preview now blocks http images by default. A popup is shown in the preview window when content has been blocked. You can change the security settings of markdown previews in the current workspace by clicking on this popup or using the `Markdown: Change Preview Security Cettings` command.
