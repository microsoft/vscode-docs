...
### Auto Indent
In this milestone, we've changed the default value for `editor.autoIndent` from `false` to `true`. So if the language you are working on has proper indentation rules (e.g. TypeScript, JSON, CSS), the indentation can be adjusted when you type, move lines and paste into code block with inproper indentation.
