...
Visual Studio Code 64 bit builds are now available on Windows for the Stable release! They come in the usual Setup Installer format as well as a ZIP archive. Below are the download links:
...
**Note:** If you are using the Setup Installer, you'll need to uninstall the 32 bit version before you install the 64 bit build. All your settings and extensions will be preserved when you upgrade to the 64 bit version.
...
* The root folder where the setting is defined becomes a de facto "master" folder, but not all multi-root scenarios have a master-details relationship.
...
Extension authors can now extend the markdown preview. The new API offers [three new contribution points](docs/extensionAPI/api-markdown.md):
