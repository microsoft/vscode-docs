...
Two releases ago, we introduced the opt-in feature of showing Emmet abbreviations in the suggestion/auto-completion list along with mutli cursor support for most of the [Emmet actions](https://docs.emmet.io/actions/).

In this release, we enable this feature to all users by default.

With Emmet abbreviations showing up in suggestion/auto-completion list and not being triggerd using the `kbstyle(Tab)` key solves 2 major issues with the old Emmet

* Unexpected Emmet expansions occurred when the user wanted to just add an indent using the `kbstyle(Tab) key.
* Items from the suggestion list got inserted on using the `kbstyle(Tab) key, when the user was expecting the Emmet abbreviation to be expanded.

#### Custom Emmet Snippets

If you have ever used custom Emmet snippets, then you will have to modify them slightly to make them work in the new Emmet.

* Custom Emmet Snippets in HTML should be a valid abbreviation. For example: use `ul>li` instead of `<ul><li></li></ul>`
* Custom Emmet Snippets in CSS should have the name fuzzy matchable with the snippet value.

Read more about the new Emmet in [Emmet 2.0 blog post](/blogs/2017/08/07/emmet-2.0.md).
...
We added two new problem matchers to parse the output of the [lessc](https://www.npmjs.com/package/less) and [node-sass](https://www.npmjs.com/package/node-sass) compilers to detect and display problems from within VS Code. They are named `$lessc` and `$node-sass`.
...
Above you can see the [VS Code Markdown Mermaid extension](https://github.com/mjbvz/vscode-markdown-mermaid) adding scripts to create [mermaid](http://knsv.github.io/mermaid/index.html) diagrams and flowcharts.
...
Extension authors can now extend the Markdown preview. The new API offers [three new contribution points](https://code.visualstudio.com/docs/extensionAPI/api-markdown.md) for styles, scripts and markdown-it extensions.
...
We've added a [Community](https://code.visualstudio.com/community) page to let you quickly engage through Twitter, Stack Overflow, and GitHub. There are also links to community created content about VS Code hosted on such sites as PluralSight, Scotch.io, and LinkedIn Learning.
...
* [@cristianhosu](https://github.com/cristianhosu): #28617 - implemented according to comments in PR [PR #28952](https://github.com/Microsoft/vscode/pull/28952)
