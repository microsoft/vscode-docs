...
* **/slash commands for background agents**
    ...
    **Update 1.109.5**: [Prompt files](https://code.visualstudio.com/docs/copilot/customization/reusable-prompts), [hooks](#agent-hooks-preview), and [skills](#use-skills-as-slash-commands) are now available to background agents as slash commands in chat.
...
* `user-invocable`: controls whether the agent can be selected from the agents dropdown in chat. Set to `false` to create agents that are only accessible programmatically or as [subagents](https://code.visualstudio.com/docs/copilot/agents/subagents).
...
* **Edit Mode** is deprecated as of VS Code version 1.109. Users may temporarily re-enable Edit Mode via VS Code setting `setting(chat.editMode.hidden)`. This setting will remain supported through version 1.125. Beginning with version 1.125, Edit Mode will be fully removed and can no longer be enabled via settings.

### Upcoming deprecations

* **Edit Mode** was deprecated as of VS Code version 1.109. Users can temporarily re-enable Edit Mode via policy xyz, which can be managed through Intune. This policy will remain supported for one more release cycle (through version 1.112). Beginning with version 1.112, Edit Mode will be fully removed and can no longer be enabled via policy.
