...
* [Chat UX](#chat-ux) - chat feels just better and snappier with faster streaming, improved reasoning results, and a revamped editor inline chat
...
* [Agent Security & Trust](#agent-security--trust) - feel confident running terminal commands with sandboxing and effective auto-approval rules

* [Workbench & productivity](#workbench--productivity) - test your apps without leaving the editor with the new integrated browser
...
* [Coding & Editor](#coding--editor) - several impactful improvements to make everyday coding smoother

* [Extensions & API](#extensions--api) - new capabilities for extension authors to build richer experiences
...
<div class="toc-nav-layout">
  <nav id="toc-nav">
    ...
    <ul>
      ...
      <li><a href="#agent-security--trust">Agent Security & Trust</a></li>
      <li><a href="#terminal-enhancements">Terminal enhancements</a></li>
      <li><a href="#coding--editor">Coding & editor</a></li>
      <li><a href="#workbench--productivity">Workbench & productivity</a></li>
...
We added support for a search subagent that runs in an isolated agent loop, enabling it to iteratively refine searches, try multiple queries, and explore different parts of your workspace.
...
* The new `awaitTerminal` tool gives the agent the ability to wait for background terminals to complete, this also requires the `timeout` property. Provided the model leverages this tool, the hope is it should prevent the `echo "successful"` and `sleep n` calls that would previously be used to wait for background commands to finish.
...
A smoother, more capable terminal. This release adds Kitty keyboard protocol support for better key handling in terminal apps, gives you control over which commands appear in sticky scroll, and introduces options for safely using the terminal in restricted workspaces.
