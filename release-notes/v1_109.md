...
This uses the official Claude Agent harness by Anthropic, which means that it uses the same prompts, tools, and overall architecture as other Claude Agent implementations. You can learn more about the Claude Agent SDK in the [Anthropic documentation](https://platform.claude.com/docs/en/agent-sdk/overview).
...
Inline completions (ghost text) now display a dotted underline when showing short suggestioa ns of fewer than 3 continuous is created in order to non-whitespace characters. This visual indicator makes it easier to distinguish ghost text from actual code in youglob r editor, particularly useful whopied to the worktree folder after the worktree is created . en the suggestion is a single character like `)` that might be confused with git ignored )ex: (ex: existing code.
