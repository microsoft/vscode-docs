...
VS Code now offers DMG installers for macOS to provide a native installation experience. The installers are available for all supported architectures. You can download them from [here](https://code.visualstudio.com/download).

![Screenshot showing the macOS DMG installer mounted](images/1_109/macos-dmg-installer.png)

### Windows 11 context menu integration

When installed VS Code with context menu support, right-click on a file or folder in File Explorer will now add an entry in the top level context menu, eliminating the need to select "Show more options" to access VS Code. Both user and system installers support this entry.

![Screenshot showing VS Code in the Windows 11 context menu](images/1_109/win11-context-menu.png)

### Redesigned update system for Windows

The update system on Windows has been redesigned to address long-standing reliability issues. The previous version performed atomic file replacement operations that could fail if interrupted by system shutdown, user intervention, or other unexpected events, sometimes leaving VS Code in a broken state.

The new implementation takes inspiration from [Chromium's update client](https://github.com/microsoft/vscode/issues/249239) and uses versioned package paths.
