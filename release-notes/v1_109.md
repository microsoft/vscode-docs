...
<div class="toc-nav-layout">
  <nav id="toc-nav">
    ...
    <ul>
      ...
      <li><a href="#browser-preview">Browser (Preview)</a></li>
...
Tool call outputs now stream more quickly into chat providing the user with a better sense of progress. Rather than showing "Working..." during a long tool call, users will now see progress such as "Creating File 20 lines...".
...
The chat input now includes a context window widget that displays the promt token usage broken down by category to help better understand how Copilot is using the available context window.

![A partially filled pie chart with a rectangular widget above it showing a breakdwon of token usage in various categories such as system and user context](images/1_109/context-window-widget.png)
...
VS Code has long included the Simple Browser for opening basic web pages, such as localhost sites during development. However, because it relied on iframes, users encountered limitations — signing into websites wasn't possible, and common sites like Google, GitHub, and Stack Overflow couldn't be opened.
...
You can also configure the integrated browser to replace the Simple Browser using the `setting(simpleBrowser.useIntegratedBrowser)` setting, or to be used by the Live Preview extension using the `setting(livePreview.useIntegratedBrowser)` setting.
...
The installation layout on Windows has been redesigned to address long-standing reliability issues regarding in-app updates. Most notably, the previous version performed atomic file replacement operations that could fail if interrupted by system shutdown, user intervention, or other unexpected events, sometimes leaving VS Code in a broken state. The new implementation takes inspiration from [Chromium's update client](https://github.com/microsoft/vscode/issues/249239) and uses versioned package paths.
