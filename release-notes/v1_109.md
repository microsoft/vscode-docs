...
By default, editor inline chat now always uses the default model of the selected vendor. This ensures extensions can provide a specific model that's tailored for Inline Chat. Users who want to use a different model can use the `setting(inlineChat.defaultModel)` setting to control this behavior.

### Inline chat UX Preview

We continue to revamp the inline chat experience and have two preview features: an affordance that makes it easier to trigger inline chat, and a contextual rendering that's lighter and easier to use. Control both features with these settings: `setting(inlineChat.affordance)` and `setting(inlineChat.renderMode)`.
