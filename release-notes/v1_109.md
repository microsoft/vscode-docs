...
This tool is especially useful in combination with the Plan agent, where the agent asks questions to validate architectural assumptions, clarify requirements, or let you choose between different technical approaches.
...
As models become more capable, they provide increasingly detailed reasoning about their thought process. To help you follow along without getting overwhelmed, we've improved the reasoning UX in chat:

* **Thinking blocks style**: choose between a detailed or compact display with the `setting(chat.thinking.style)` setting.

* **Terminal tool calls**: control whether terminal tool calls are shown within thinking blocks with the `setting(chat.agent.thinking.terminalTools)` setting.

* **Visual enhancements**: various visual improvements to make following model activity more intuitive.

    * Show large thinking content in a scrollable container
    * Show contextual spinner text based on the most recent tool call or reasoning block
    * Show shimmer animations to indicate activity
    * Show tool icons in chain of thought
...
This iteration, we've made numerous performance improvements which should make chat, especially large chats, more fluent and reliable.
