...
We've added a new session type picker in the chat input area that makes it easy to switch between agent types and create new sessions. The picker displays the current agent type (Local, Background, or Cloud) and lets you quickly switch between them without navigating to the `+` menu in the Chat view header.

The session type picker includes:

* **Agent type selection**: switch between Local, Background, and Cloud agent types directly from the chat input. Each option includes detailed tooltips explaining the agent type and its capabilities.

  ![Screenshot showing the session type picker dropdown in the chat input area with Local, Background, and Cloud options.](images/1_109/session-type-picker.png)

* **New session button**: quickly create a new session of the selected agent type without leaving the input area.
* **Continue session in a different place**: change the session type in the dropdown to continue your current session in a different environment. For example, start a local session to plan your task, then switch to Background or Cloud to let the agent work autonomously while you continue with other work.
...
A new command `workbench.action.chat.newLocalChat` is available to quickly create a new local agent session. This is useful when you're viewing a cloud or background session and want to quickly start a new local session without navigating through menus. Bind this command to a keyboard shortcut for even faster access.
...
![Claude agent next to Local, Background & Cloud in the session target picker](images/1_109/claude-agent.png)

This uses the official Claude Agent harness by Anthropic and that means it uses the same prompts, tools, and overall architecture as other Claude Agent implementations. You can learn more about the Claude Agent SDK in the [Anthropic documentation](https://platform.claude.com/docs/en/agent-sdk/overview).
...
* `user-invokable`: Controls whether users can select and invoke the agent directly from the UI. Set to `false` to create agents that are only accessible programmatically or as subagents.
* `disable-model-invocation`: When set to `true`, prevents the agent from being invoked as a subagent by other agents. This is useful for agents that should only be triggered explicitly by users.
...
![A mermaid diagram in a chat response](images/1_109/chat-mermaid.png)
...
We'll be rolling out external indexing gradually over the next few weeks. Note that any workspaces that are hosted on GitHub already support remote indexing and are do not require an more expensive call to build up the index on first request.
...
The **Checkout** button in the chat changes now shows even when GitHub Pull Requests is not installed. When clicked, it installs GitHub Pull Requests if needed, and then hands off to it to perform the checkout.

### Subagents

We made a few improvements to subagents this month.

![2 active subagents](<images/1_109/subagents.png>)

Multiple subagents can now actually run in parallel. This can significantly speed up workflows that can be split into independent tasks.

The UX for running subagents is improved as well. By default we show the name of the custom agent (if enabled with the setting `chat.customAgentInSubagent.enabled`), the task it's working on, and whichever tool is currently in use. This section can be expanded by clicking on it to show all the work that the subagent is doing, including the full initial prompt it was provided with and the result it returned.
...
<video src="images/1_109/integrated-browser.mp4" title="Video demonstrating the integrated browser." autoplay loop controls muted></video>
...
![Preview of the new experimental VS Code Dark theme.](images/1_109/vscode-dark-experimental-theme.webp)

**VS Code Light (Experimental)**:

![Preview of the new experimental VS Code Light theme.](images/1_109/vscode-light-experimental-theme.webp)
...
We have removed support for winpty from the terminal. [ConPTY](https://devblogs.microsoft.com/commandline/windows-command-line-introducing-the-windows-pseudo-console-conpty/) has been available since Windows 10 version 1809 (Fall 2018) and is now required for the integrated terminal on Windows. Users on older Windows versions before ConPTY will need to upgrade to continue using the terminal.
...
If a new update becomes available while the previous update is still pending to be applied, VS Code will now invalidate the previous update and proceed with applying the new one, instead of forcing the user to restart the application twice.
