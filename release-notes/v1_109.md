...
* **Preserved agent type on navigation**: when using the back arrow to return from a session, VS Code now returns you to a new session of the same agent type. For example, clicking back from a Cloud agent session brings you to a new Cloud agent session, rather than switching to a Local agent session.
