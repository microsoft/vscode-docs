...
### Reasoning and terminal tool calls
...
* **Thinking blocks style**:

    * Choose between a detailed or compact display (`setting(chat.thinking.style)`).

* **Terminal tool calls**:

    * Show terminal tool calls inside thinking blocks (`setting(chat.agent.thinking.terminalTools)`).
    * Automatically expand failing tool calls (`setting(chat.tools.autoExpandFailures)`).
...
* Multi-folder workspace and empty workspace improvements

    When in a multi-folder workspace window or empty workspace window, you can now select the folder to be used for the cloud agents.
...
* Multi-folder workspace improvements

    When in a multi-folder workspace, you can now select the folder to be used in background agents.
