...
We're continuing to reimplement [Emmet Preferences](https://docs.emmet.io/customization/preferences/) after the switch to  [Emmet 2.0](https://code.visualstudio.com/blogs/2017/08/07/emmet-2.0). In addition to the preferences we brought back [last release](https://code.visualstudio.com/updates/v1_16#_emmet-preferences), we've added :
...
The format for the `filter.commentAfter` preference is different and simpler in Emmet 2.0. For example, instead of the older format `"\n<!-- /<%= attr('id', '#') %><%= attr('class', '.') %> -->"`, you will use `"\n<!-- /[#ID][.CLASS] -->"`.
