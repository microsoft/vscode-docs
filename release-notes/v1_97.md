...
You can add `#codebase` to your query to allow Copilot Edits to discover relevant files for your task. Previously, `#codebase` exclusively performed semantic search. We've added experimental support for discovering relevant files using additional tools like file and text search, Git repository state, and directory reads.
...
In chat, any codeblock can be be applied to to a file in the workspace using the 'Apply to Editor' action in the toolbar of the code block.
...
- If the code block is for a not yet existing file, you will be prompted where to create the file: At a file path suggested by Copilot, in a untitled editor or in the currently active editor.
- When to changes are computed and applied, the same flow and UI as for Copilot edits are used. You can review, improve or discard each change individually.
...
When you ask Copilot a question about the code in your current project using `@workspace` or `#codebase`, we use an index to quickly and accurately search your codebase for relevant code snippets to include as context. This index can either be a [remote index managed by GitHub](../docs/copilot/workspace-context.md#remote-index), [a locally stored index](../docs/copilot/workspace-context.md#local-index), or [a basic index](../docs/copilot/workspace-context.md#basic-index) used as a fallback for large projects that can't use a remote index.

This iteration we've added a new workspace index status item to the language status indicator that shows both the type of index currently being used by Copilot, along with any relevant information about this index, such as the number of files being reindexed. To see this, just click on the `{}` in the status bar:

![The Copilot workspace index status item](images/1_97/copilot-workspace-status.png)

Check out the [Copilot workspace docs](../docs/copilot/workspace-context.md#managing-the-workspace-index) for more info about the types of workspaces indices and how you can switch between them.

### New command to build a remote index

[Remote workspaces indexes](../docs/copilot/workspace-context.md#remote-index) are managed by GitHub. A remote index can provide high quality results quickly, even for large projects. They also only have to be built once per GitHub project, instead of once per user.

Given all these advantages, this iteration we added a few new ways to upgrade a project to a remote index:

- Run the new `GitHub Copilot: Build remote index` command.

- Click the `Build Index` button in the [Workspace index status UI](#workspace-index-status-ui). This will only show up if your project is eligible for remote indexing.

- Click the `Build Index` button in the first `@workspace` response you see. Again this will only show up if your project is eligible and will also only show once per workspace.
...
- Improved tracking and handling of locally changed files when using a [remote index](../docs/copilot/workspace-context.md#remote-index).

- Background updating of changed files in the [local index](../docs/copilot/workspace-context.md#local-index) so that `@workspace` questions don't have to wait for them to be updated.

- Optimization of the [basic index](../docs/copilot/workspace-context.md#basic-index) for large projects.

### Git changes context variable

When writing queries for Chat or Edits, you can now reference files that have been modified in your Git source control view using the `#changes` context variable.

### Agent mode (Experimental)

We've been working on a new mode for Copilot Edits. When in "agent mode", Copilot will automatically search your workspace for relevant context, edit files, check them for errors, and run terminal commands (with your permission) to complete a task end-to-end.

TODO screenshot

You can switch between the Edits mode that we've had for a few months and Agent mode with the dropdown in the Copilot Edits view. To see the dropdown, enable the setting: `setting(chat.agent.enabled)`. You can use agent mode in VS Code Insiders today, but we will be rolling it out to VS Code Stable users gradually. If the setting doesn't show up for you, then it isn't enabled for you yet.

![agent mode setting](images/1_97/agent-setting.png)

In agent mode, Copilot will run autonomously, but it can only edit files within your current workspace. When it wants to run a terminal command, it will show you the command and wait for you to review it and click "Continue" before proceeding.
...
Invoking the **Show Accessibility Help** command while the Source Control viewlet is focused will open the Source Control Accessibility Help dialog which provides important information for screen reader users. The dialog contains the summary of the current source control state, and general information about the views and how to navigate them.
...
This milestone we continued to polish the feature to display git blame information in the editor and in the status bar. We have also improved the hover that is shown when users hover over the editor decoration or the status bar item. The git blame status bar item is now enabled by default, and users can disable it using the setting(git.blame.statusBarItem.enabled:false) setting. Users can enable the git blame editor decoration using the setting(git.blame.editorDecoration.enabled:true) setting. We have also added commands to easily toggle the git blame information using the command palette or using keybindings: **Git: Toggle Git Blame Editor Decoration**, and **Git: Toggle Git Blame Status Bar Item**.

### GitHub repositories

For repositories that are hosted on GitHub, we have added a new command, "Open on GitHub", to the timeline context menu and hover, the source control graph context menu and hover, and the git blame editor decoration and status bar item hover. GitHub issue and pull request references are rendered as links in the timeline, source control graph, and git blame editor decoration and status bar item hovers so that they can be easily opened in a browser. Last but not least, we also added the GitHub avatar in the timeline, source control graph, and git blame editor decoration and status bar item hovers. Users can disable the rendering of the GitHub avatar using the setting(github.showAvatar:false) setting.

### Source Control Graph actions

We have expanded the functionality of the Source Control Graph view by adding actions scoped to a history item reference (ex: branch tag, etc.) to the context menu. The first set of actions that we added are **Checkout**, **Delete Branch**, and **DeleteTag** that allows users to easily checkout a branch/tag, delete a branch, and delete a tag directly from the Source Control Graph view. We are planning to add more actions in the upcoming milestones.
...
- Ligatures will now temporarily be disabled while the cursor or the selection is within the ligature.
- `setting(terminal.integrated.fontLigatures.featureSettings)` is available to set ligature sets and variants. This is passed on to the [`font-feature-settings`](https://developer.mozilla.org/en-US/docs/Web/CSS/font-feature-settings) CSS property behind the scenes.
- `setting(terminal.integrated.fontLigatures.fallbackLigatures)` is available to set ligature sequences manually when the library we use to parse ligatures is not supported.

### Support for ConEmu's progress escape sequences

The `ESC ] 9 ; 4` escape sequence pioneered by ConEmu that reports progress in the terminal is now supported. This is used by some CLI tools such as `winget` to report progress. Progress can be viewed in terminal tabs by adding `${progress}` to either `setting(terminal.integrated.tabs.title)` or `setting(terminal.integrated.tabs.description)`. This will typically show as a progress spinner or a percentage.

### Sticky scroll shows when the command is truncated
...
The new `setting(terminal.integrated.hideOnLastClosed)` has been added that allows configuring whether the panel will be closed when the last terminal has been closed. Along with this the experience when there is no terminal open has been improved.
