...
### Trusting Publisher for Extensions

When installing an extension from a publisher for the first time you will see a dialog to help you assess the trustworthiness of the extension publisher. This feature helps ensure that you only install extensions from trusted sources, enhancing the security of your development environment.

![Trust Publisher Dialog](images/1_97/trust-publisher-dialog.png)

When installing an extension pack or an extension with dependencies, trusting the publisher will implicitly trust the publishers of the extensions that get installed along with it. For existing users, the publishers of currently installed extensions will be automatically trusted upon updating to this release. You can manage the trusted extension publishers using the command `Extensions: Manage Trusted Extensions Publishers`. This command allows you to reset or revoke trust for publishers you have previously trusted.

![Manage Trusted Publishers Dialog](images/1_97/manage-trusted-publishers.png)

**Note**: When no VS Code window is open and you install an extension from the CLI (e.g., `code-insiders --install-extension pub.name`), the extension will be installed, but publisher will not be added to the trusted list.

For more information, you can visit the [learn more](https://aka.ms/vscode-extension-security) link.

### Output View Filtering

You can now filter the content in the Output view, enhancing your ability to manage and analyze logs, especially when dealing with large volumes of log data.

![Output View Filtering](images/1_97/output-view-filtering.png)

- **Filter by Log Level:** Filter logs based on their severity level (e.g., Error, Warning, Info). This helps you focus on the most critical issues first.
...
You can now view multiple logs in a single compound output view, making it easier to analyze related logs together. Create custom compound logs by combining multiple log sources using the action **Create Compound Log...** in the overflow menu of the Output view. You can also open compound logs in an editor or an auxiliary window for flexible monitoring and analysis. This feature improves your ability to diagnose issues by providing a consolidated view of related logs. Note that these added compound log views are not persisted.
...
Support for `@outdated` and `@recentlyUpdated` options has been added to the input suggestions in the Extensions view.
