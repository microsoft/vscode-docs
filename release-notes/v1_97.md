...
### Copilot Vision Preview in VS Code Insiders (Preview)

The nightly release of [VS Code Insiders](https://code.visualstudio.com/insiders/) and the pre-release of GitHub Copilot Chat introduces end-to-end vision support in Copilot Chat, allowing users to attach images and interact with them in prompts.

Users can now attach images using several methods. You can drag and drop images from your operating system or directly from VS Code's Explorer. Pasting images from the clipboard is also supported, along with capturing and attaching a screenshot of the VS Code window using the "Screenshot Window" option. Warnings will be shown if the model currently does not have the capabilitiy to handle the image - supported models include `GPT 4o`, `Gemini 2.0 Flash`, and `Claude 3.5 Sonnet`. Currently, the only supported types are `JPEG/JPG`, `PNG`, `GIF`, and `WEBP`. 

![Attaching an image to a chat request](images/1_97/image-attachments.gif)
...
When writing queries for Chat or Edits, you can now reference files that have been modified in your Git source control view by using the `#changes` context variable. For example, you could prompt for `summarize the #changes in my workspace`.
...
When you're getting code blocks returned in a Copilot response, you might not want to hear them read out in a text-to-speech session.

A new setting `setting(accessibility.voice.ignoreCodeBlocks)` lets you skip code blocks from text-to-speech sessions.
...
![Quick actions in the comment editor](images/1_97/quick-actions-in-comment.gif)
...
![Git blame information status bar item hover](images/1_97/scm-git-blame.png)
...
![Source control history item hove](images/1_97/scm-graph-hover.png)
...
The notebook editor now supports showing inline values after cell execution with the setting `setting(notebook.inlineValues)`. When enabled, after a successful cell execution, inline values are displayed according to either the results of a registered `InlineValueProvider` or a generic fallback approach. The fallback approach will match values found in the kernel against the cell document via simple regex matching, so it is recommended to use a provider from a language extension to ensure more accurate results.

![Custom font for rendered Markdown cell](images/1_97/nb-inline-values.png)
...
![Custom font for rendered Markdown cell](images/1_97/markdown-cell-font-family.png)
...
The new `${columnNumber}` variable can be used in `tasks.json` and `launch.json` to refer to the column number of cursor.
...
We've moved off of the old, unmaintained, Ruby grammar from `textmate/ruby.tmbundle` and now get our Ruby grammar from `Shopify/ruby-lsp`.
...
* All non-outdated comments will show in the Comments view when you open the description of an un-checked out pull request.
...
The comments API support making and showing file level comments. File-level comments show at the top of the file, before the first line. They are not attached to a specific line or range in the file. To show a file level comment, simply set the `range` of the comment to `undefined`. To support leaving file level comments from your commenting range provider, set the `enableFileComments` property on your `CommentingRangeProvider` to `true`.
