...
Given all these advantages, we have added a few new ways to upgrade a project to a remote index:
...
A warning is shown if the selected model currently does not have the capability to handle images. The supported models include `GPT 4o`, `Gemini 2.0 Flash`, and `Claude 3.5 Sonnet`. Currently, the only supported types are `JPEG/JPG`, `PNG`, `GIF`, and `WEBP`.
