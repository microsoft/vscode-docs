...
You can now see a dialog to check the trustworthiness of the publisher before installing an extension from a publisher for the first time. Microsoft and GitHub publishers are trusted by default. This feature helps ensure that you only install extensions from trusted sources, enhancing the security of your development environment.
...
**Note**: When no VS Code window is open and you install an extension from the CLI (e.g., `code-insiders --install-extension pub.name`), the publisher will not be trusted.
