...
The notebook editor now supports showing inline values after cell execution with the setting `setting(notebook.inlineValues)`. When enabled, after a successful cell execution, inline values will be displayed according to either the results of a registered `InlineValueProvider` or a generic fallback approach. The fallback approach will match values found in the kernel against the cell document via simple regex matching, so it is recommended to use a provider from a language extension to ensure more accurate results.
