...
In the following example, changing the variable name triggers an edit suggestion at another location in the file. As you use the `kbstyle(Tab)` key to navigate and accept the suggestion, the arrow in the gutter indicates the relative position of the next suggestion.
...
Depending on the size and type of edit that Copilot suggests, the rendering of the suggestion dynamically change from side-by-side to below the current line. Configure the `setting(editor.inlineSuggest.edits.renderSideBySide:never)` setting to always render suggestions below the current line.
...
### OpenAI o3-mini model availability

OpenAI’s latest model, o3-mini, is now available in GitHub Copilot, bringing OpenAI’s newest reasoning model to your coding workflow.

This cutting-edge model is rolling out gradually and will be available to GitHub Copilot Pro, Business, and Enterprise users today via the model picker in Visual Studio Code and github.com chat. Learn more about the o3-mini model availability in the [GitHub blog post](https://github.blog/changelog/2025-01-31-openai-o3-mini-now-available-in-github-copilot-and-github-models-public-preview/).
