...
Key updates:
- Improved widget styling and configuration to match the editor's suggest widget.
- A configurable status bar (`terminal.integrated.suggest.showStatusBar`) displays suggestion-related info.
- Enhanced argument knowledge for commands, including:
  - `code`, `code-insiders`
  - `cd`, `ls`, `rm`, `echo`
  - `mkdir`, `rmdir`, `touch`
- Provided the path for the command or resource as a detail
- Added directory navigation support such as `.`, `..`, and `../../`.
