...
In version `1.53` we updated our build images to consume newer Electron versions causing a change in CXXABI requirements for native modules which broke our rpm packages in some distros https://github.com/microsoft/vscode/issues/115784. With update to Electron v13 we are now able to build our native modules with the same compiler toolchain as the runtime, specifically we now use clang and statically link the libcxx from the runtime. This allows to bypass the CXXABI needs for a small increase in binary size ~10MB and bring back support for these distros. Thanks for your patience and help with testing the insiders.
