...
Please note that debug extensions have to opt into this feature in order to make the "Set Action" available. For now, JS-debug, C# and C++ extensions support this, but we expect other debug extensions to follow soon.

### Minor UI improvements

* We now detect links for values also in the VARIABLES view, WATCH view and the Debug Hover. When a variable's value is a link, this will make it easier to follow that link by simply clicking on it.
...
* If you want to avoid accidentally closing the window during a debug session, you can enable the new setting `debug.confirmOnExit`. With this you will be prompted to confirm that you want to stop the debug session when closing the window.
![confirm quit while debugging](images/1_60/debug-confirm-exit.png)
* We have introduced a new setting `debug.console.acceptSuggestionOnEnter` that controls whether suggestions should be accepted on `Enter` in the Debug Console. `Enter` is also used to evaluate whatever is typed in the Debug Console, so this should allow users to better control what happens when pressing `Enter`.
...
VS Code will emit `onNotebook:{type}` and `onNotebook:*` events when a notebook is opened in the workspace, notebook extensions can be activated by listening to this event. Previously VS Code will wait for all extensions listening to these two events prior to file opening but now it will only wait for the serializer for the specific notebook to be registered. For example, when users open an ipynb file in a fresh workspace, the builtin ipynb serialzier extension will be activated and then the file will be opened immediately. At the same time, VS Code emits activation events to all other extensions, e.g., Jupyter, .NET Interactive or Julia extensions. The activation of other extensions will not slow down the file opening and ensure a smooth and fast notebook file opening.
...
The most significant new tooling feature in TypeScript 4.4 is inlay hint support. Inlay hints add additional inline information to code to help you understand what the code does.
...
This can help you understand the meaning of each argument at a glance, which is especially helpful for functions that take boolean flags or have paramters that are easy to mix up.
...
**Variable type inlay hints** (`javascript.inlayHints.variableTypes.enabled` and `typescript.inlayHints.variableTypes.enabled`) show the types of variables that don't have an explicit type annotations:

![Variable type inlay hints](images/1_60/ts-inlay-var-types.png)

**Property type inlay hints** (`javascript.inlayHints.propertyDeclarationTypes.enabled` and `typescript.inlayHints.propertyDeclarationTypes.enabled`) show the type of class properties that don't have an explicit type annotation:

![Property type inlay hints](images/1_60/ts-inlay-property-types.png)

**Parameter type hints** (`javascript.inlayHints.parameterTypes.enabled` and `typescriptscript.inlayHints.parameterTypes.enabled`) show the types of implicitly typed parameters:

![Parameter type inlay hints](images/1_60/ts-inlay-parameter-types.png)

**Return type inlay hints** (`javascript.inlayHints.functionLikeReturnTypes.enabled` and `typescript.inlayHints.functionLikeReturnTypes.enabled`) show the return types of functions that don't have an explicit type annotation:

![Return type inlay hints](images/1_60/ts-inlay-return-type.png)

We plan to continue improving our inlay hints going forward so be sure to [share any feedback](https://github.com/microsoft/vscode/issues/new/choose) you have on the new feature!
...
* `auto` — The default. Spawn both a full server and a lighter weight server dedicated to syntax operations. The syntax server is used to speed up syntax operations such as code folding and provide IntelliSense while projects are loading.
...
We now support the **Run By Line** feature in Jupyter notebooks. This is a simple debug mode that lets you execute the code of a cell line by line. To try it out, just make sure that you have ipykernel v6+ installed in your selected kernel, then select the **Run By Line** button.

![RBL in Jupyter notebook](images/1_60/jupyter-rbl.gif)

**Debugging**

We also have experimental support for debugging Jupyter notebooks using the full debug features of VS Code. To try it out, make sure you have ipykernel v6+ installed in yor selected kernel, set `"jupyter.experimental.debugging": true`, set a breakpoint, and select the **Debug Cell** command.

![debugging jupyter notebook](images/1_60/jupyter-debug.gif)

**Jupyter Keymap**

We extracted the Jupyter classic keybindings to a separate [extension](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-keymap), which you can disable if you prefer to use other styles of [keybindings](https://marketplace.visualstudio.com/search?term=%40tag%3Anotebook-keymap&target=VSCode&category=All%20categories&sortBy=Relevance) from the marketplace.

### Python

**Revamped testing interface**

Thanks to the [new testing API](https://code.visualstudio.com/api/extension-guides/testing), the Python extension has revamped its testing features.  You can now enjoy a more stable experience for test discovery, navigation and status rendering, as well as new features such as search tests, run a selection of tests, rerun last executed test, and more!
...
The new `workspace.isTrusted` property on the context object passed to [notebook renderers](https://github.com/microsoft/vscode-extension-samples/tree/main/notebook-renderer-sample) lets you check if the current workspace is trusted or not:
...
### Honor DAP's "showUser" property on error response from "launch" request
...
For other DAP requests, there is no change. If `showUser` is `true`, the error will be shown using a notification. Otherwise, nothing will be shown to the user.
