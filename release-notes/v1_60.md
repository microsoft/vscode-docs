...
We added this feature because we found that some new users of VS Code didn't know how to set the language mode and often times those users didn't know that they needed to do this in the first place in order to get VS Code's rich feature set (language colorization and extension recommendations). Getting to that point is pivotal for these users in their VS Code journey and automatic language detection helps those users discover what VS Code is capable of.

Along side of that, we also love our power users who use untitled text editors as a scratch pad. You have been manually setting the language mode for years and now and with automatic language detection, we hope to take out one more step and streamline your workflow more.
