...
A new API for extensions is proposed so that also other languages can provide their syntax aware folding providers.
...
* Expanding Emmet abbreviations in large css/scss/less files is much faster now. This is achieved by parsing only a small content around the cursor instead of the complete file when trying to determine whether the current location is a valid location for expanding Emmet abbreviations.
* Emmet abbreviations in stylesheets are fuzzy matched with [pre-defined snippets](https://github.com/emmetio/snippets/blob/v0.2.9/css.json) to give you the closest matched result. You can now control the accuracy of this fuzzy matching by changing the below setting which controls the minimum score a match needs to have to be considered.
  ```json
  "emmet.preferences": {
      "css.fuzzySearchMinScore": 0.3
  }
  ```
* When using either the `Emmet: Wrap with Abbreviation` or `Emmet: Wrap Individual Lines with Abbreviation` command, you can now see the preview of what the wrapped text would look like.

  ![Wrap text with abrbeviation preview](images/1_22/wrap.gif)
...
The boxes for include and exclude patterns in the search viewlet have been combined into a single box. The functionality is exactly the same, except that an exclude pattern must be prefixed with `!`. For example, `!*.js` will skip searching files that have a .js extension.
...
In addition to HTML, Path completion is now availble for CSS too:
...
The API is in proposed state and can be found [here](https://github.com/Microsoft/vscode/blob/master/src/vs/vscode.proposed.d.ts#L653)
...
* We now have over 800 members in the Transifex [VS Code project](https://aka.ms/vscodeloc) team. About 100 active contributors every month. 
* With community contribution, we have successfully released monthly updates for 9 core languages to international users. 
* Plus 4 languages fully localized by community members released as language pack VS Code extension. More languages are catching up. 
