...
\* *Scroll one line up/down was moved to `kbstyle(Cmd+Alt+PgUp)`/`kbstyle(Cmd+Alt+PgDown)`*
...
### Natural scrolling in alt buffer
...
### Refocus terminal when using VS Code as a git editor

When using VS Code as a git editor and git is triggered from the integrated terminal, after finishing the message focus will now be sent back to the terminal.
