...
### Explorer shows Errors & Warnings

Not exactly new, but now final and enabled by default. Errors and warnings in a file are now shown as decoration in the explorer.

![Errors in Explorer](images/1_22/explorer-errors.png)

In the sample pictured above, two files have errors ('action.ts' and 'arrays.ts') and the file 'cache.ts' as three warnings.

## Editor

### Diagnostics and References Navigation

With this release navigating errors and warnings via F8 (F1 > Go to Next Problem) takes all reported errors into account. Instead of looping inside one file it will take you to the next file with errors and back.

![Visit All Diagnostics](images/1_22/f8-navigation.gif)

Similar to this is a new command that allows to navigate to reference search results. Trigger 'Find References' on a symbol and now use F4 and Shift+F4 to visit each reference. This is great to make edits in all places that reference a symbol because your fingers don't need to leave the keyboard anymore.
...
The extension API allows to create hint-diagnostics. Those aren't errors or warnings but suggestions how something could be made better, e.g by running a certain refactoring.

We now render such diagnostics with an ellipsis under the start of the word in question.
...
VS Code allows to run formatters just before saving a file to disk. For that, speed matters because a slow extension would slow down saving. Because we don't like slow saving VS Code is very strict and cancels a format-on-save-request after 750ms. For some extensions, esp. when dealing with large files, that's not enough and we have added a new setting `"editor.formatOnSaveTimeout"` which allows to configure the timeout. For you, to see what's going on, there is now a little progress indication in the statusbar.

![Save Participant](images/1_22/save_part.gif)

## Even more Snippet variables

There are new snippet variables for referring to the current data and time. In addition to numbers, e.g. 28/3/2018 you can now use names like '28 of March'. The new variables are: `${CURRENT_DAY_NAME}`, `${CURRENT_DAY_NAME_SHORT}`, `${CURRENT_MONTH_NAME}`, and `${CURRENT_MONTH_NAME_SHORT}`.
