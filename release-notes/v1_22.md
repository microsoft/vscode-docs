...
This will escape the spaces in the argument using the shell's escape character which is ````` under PowerShell and `\`under bash. If the shell used doesn't support character escaping the strong quotes are used. Besides escaping the following values are supported:
