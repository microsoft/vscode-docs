...
* **[Log Points](#log-points)** - Log diagnostic messages without changing code or breaking on your debug session.
...
This milestone includes our first work towards **Log Points**. A Log Point is a variant of a breakpoint that does not "break" into the debugger but instead logs a message to the console. Log points are especially useful for debugging production servers which cannot be stopped.
