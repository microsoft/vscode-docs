...
Tasks have been updated to give usersbetter control over how arguments and the command itself are quoted when executed in a shell like bash or PowerShell. Tasks of type `shell` now support providing the command and its argument separately. Below is an example of a task that list the directory of a folder named `folder with spaces` (observe the space in the name).
...
There are now new task APIs to query all existing tasks in the system, to execute tasks programmatically, to terminate a running task, and to listen on task start and end events. The API is in proposed state and can be found [here](https://github.com/Microsoft/vscode/blob/master/src/vs/vscode.proposed.d.ts#L653)
