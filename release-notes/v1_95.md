...
Open the "Visual Outline" view to dynamically generate diagrams from the active editor.  Break out into chat for finer control.
...
Data Analysis for Copilot empowers people in data science field. From cleaning up their data in user's .csv file to performing higher level of data analysis by leveraging different statistics measures, graphs, and predictive models, the @data agent helps user make more advanced and informed decisions by offering tailored insights and interactivity for data tasks. The extension contributes a tool where the LLM can ask it to execute Python code via using Pyodide and get the result of the relevant Python code execution. It is also able to smartly re-try for better or more appropriate execution results in case of error or failure. We also allow exporting to .ipynb Jupyter notebook file if the user decides to move forward with the code provided
