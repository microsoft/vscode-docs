...
![Example of input being detected and responded to](images/1_104/prompt-input-demo.mp4)
...
![Example of agent running the VS Code - Build task](images/1_104/build-task.mp4)
...
To combat these misconceptions and to further protect our users, there is now a deservedly scary-looking warning the first time global auto approve attempts to be used, so the user can easily back out and disable the setting. The setting has also been changed to the clearer `setting(chat.tools.global.autoApprove)` without any automatic migration, so all users (accidental or intentional) need to go and explicitly set it again.
...
Extensions are now able to register view containers in the secondary side bar, in addition to the previously available Activity Bar and Panel. This is currently behind the `contribSecondarySideBar` proposed API, and we are hoping to make this available to all extensions soon.
