...
To help improve the readability of chat messages, we have added two new settings that allow users to customize the font family (`setting(chat.fontFamily)`) and font size (`setting(chat.fontSize)`) of the chat messages. Please note that content displayed in a list does not yet honour these settings but this is something that we are working on fixing in the upcoming releases.
...
### Select tools in toolsets

The tool picker now show the tools that are part of each tool set. You can individually enable or disable each tool.

The tools picker is available via the `Configure Tools...` button in the chat view.

![Tool Picker showing tools inside toosets](images/1_104/tools_in_toolsets.png)

### Use Custom Chat Mode in Prompt Files

Prompt files support custom modes as well.

![use a custom mode in prompt file](images/1_104/custom_modes_in_prompt_files.png)

Prompt files are can be run with the 'Play' button or as slash commands.
The mode defines the system message.

### Automatically add AGENTS.md files

AGENTS.md is now automatically picked up as context for chat requests. ,when found workspace root(s). See https://agents.md/ for more information on AGENTS.md.
...
This iteration, the changed files list has been reworked with a few quality-of-life features. First, the list is now collapsed by default. While collapsed, the total number of changed files and the lines added or removed are displayed in the header. Second, when a user stages/commits a file using the source control view, the change will automatically be accepted. Third, when a user keeps or accepts a suggested change, the file is now removed from the list to conserve space. Lastly, changes _per file_ are now shown next to each list item.
...
- Sidebar content collapses on narrow windows
