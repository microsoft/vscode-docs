...
This iteration, the changed files list has been reworked with a few quality of life features. First, the list is now collapsed by default. While collapsed, the total number of changed files will be displayed in the header. Second, if a user `keeps` or accepts a change, the file is now removed from the list to conserve space. Lastly, changes _per file_ are now shown next to each list item.
