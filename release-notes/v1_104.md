...
This iteration, the changed files list has been reworked with a few quality-of-life features. First, the list is now collapsed by default. While collapsed, the total number of changed files and the lines added or removed are displayed in the header. Second, when a user keeps or accepts a suggested change, the file is now removed from the list to conserve space. Lastly, changes _per file_ are now shown next to each list item.
