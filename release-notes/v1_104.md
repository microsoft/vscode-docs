...
### Improved Changed Files UX

This iteration, the changed files list has been reworked with a few quality of life features. First, the list is now collapsed by default. While collapsed, the total number of changed files and the lines added or removed will be displayed in the header. Second, if a user `keeps` or accepts a change, the file is now removed from the list to conserve space. Lastly, changes _per file_ are now shown next to each list item.

We hope that these changes will improve your experience when working in Agent mode!
...
We're continuing to evolve the Chat Sessions experience to help developers collaborate more effectively with coding agents.
...
We continued expanding on ways to delegate local tasks in VS Code to coding agent.
...
#### Delegate from sidebar chat

 **Setting**: `setting(githubPullRequests.codingAgent.uiIntegration)`

Additional context, including file references, are now forwarded to GitHub coding agent when the 'Delegate to coding agent' action is performed. This allows you to precisely plan out a task for coding agent to complete.
...
We are experiment with improving the quality of next edit suggestions for Notebooks. Currently the langauge model has access to contents of the active cell when generating NES. However with the `setting(github.copilot.chat.notebook.enhancedNextEditSuggestions.enabled)` setting enabled, the language model will have access to the entire notebook, allowing it to generate more accurate and higher-quality NES.
