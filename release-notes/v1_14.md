...
* **[Preview: Multi-root workspaces](#preview-multi-root-workspaces)** - Open multiple projects in the same editor (available in Insiders build).
...
* **[Emmet abbreviation enhancements](#emmet-abbreviation-improvements)** - Add Emmet to any language. Now with multiple cursor support.
...
* **[Extension Authoring](#extension-authoring)** - SVG rendering constraint, custom views visibility, new Debug API.
...
* `none` - to never restore any workspaces
* `one` - to restore the last active workspace (default)
* `folders` - to only restore opened folders
* `all` - to restore all workspaces.
...
The new `"editor.minimap.showSlider"` setting can be configured to `"always"` to always render the minimap slider. There is now also a subtle indicator of the horizontal scroll position within the slider. The slider geometry computations received a lot of improvements too (see the Notable Changes section):
...
We published a new version of the [Monaco Editor](https://microsoft.github.io/monaco-editor/), bringing it up to date with VS Code, and we have made numerous accessibility improvements.
...
* [VSTS Build Status](https://marketplace.visualstudio.com/items?itemName=jeppeandersen.vstsbuildstatus): We introduced ability to group several build definitions into single for build status bar indicator (pull request [#12](https://github.com/jlandersen/vscode-vsts-build-status/pull/12)).
