...
### Selection reimplemented

The way selection works in the terminal has been completely reimplemented by moving away from regular web selection to a model where it's managed and rendered by the terminal itself. This fixes [a lot of issues](https://github.com/sourcelair/xterm.js/pull/670), here are the major call outs:
...
The terminal now has basic find functionality which can be triggered with `kb(workbench.action.terminal.focusFindWidget)`.

(image)

If you want ctrl+f to go to the shell istead of launching the find widget on Linux and Windows, you will need to remove the keybinding like so:
...
Terminal sessions can now be renamed using the `workbench.action.terminal.rename` command. The new name will be what shows in the terminal selection dropdown.
...
Following good news of Brazilian Portuguese language last month, Hungarian and Turkish catch up this month. Congratulate and thank the Hungarian, Turkish, Brazilian Portuguese community localization teams! Special thanks to Tar Dániel and Adem Coşkuner who localized majority of 3500 strings of their language. Due to efforts of community members, localization is now completed in Transifex and the translations have been integrated into the [Insiders](https://code.visualstudio.com/insiders) build for testing. Depending on validation progress, we hope to soon integrate the translations into the stable builds for customers need these languages.
