...
Rendering user provided SVG images within VS Code has been a concern for a while. We therefore put new rules for extensions in place that makes it safer for users to browse the extension marketplace within VS Code:

- the icon provided in `package.json` may not be a SVG
- the badges provided in the `package.json` may not be SVGs if they are not from trusted badge providers
- image urls in `README.md` and `CHANGELOG.md` need to resolve to `https` urls
- images in `README.md` and `CHANGELOG.md` may not be SVGs if they are not from trusted badge providers
- urls with the following prefixes identify trusted badge providers
...
To make life easier for you, we show linting errors on your extension's `package.json`, `README.md` and `CHANGELOG.md` files if we see that they are not yet compliant with those constraints. Also, the `vsce` command line, performs compliance checks and prevents you from uploading a new version of your extensions if there are problems.
...
Debug adapters now have to opt-into the delayed loading of stack frames by specifying a newly introduced capability `supportsDelayedStackTraceLoading`. More about delayed loading of stack frames can be found [here](https://github.com/Microsoft/vscode/issues/25605).
