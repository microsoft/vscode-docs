...
### Multi Root Workspace Preview

We were quite busy during this sprint to start the development work on "[Multi Root Workspaces](https://github.com/Microsoft/vscode/issues/396)" in VS Code (refer to our [previous release notes](https://code.visualstudio.com/updates/v1_13#_multiroot-workspaces) for the design decisions we made last sprint). The outcome of this work is what we call a "Minimum Viable Product" (MVP) to enable selfhosting on multiple workspaces. The features outlined below are currently only available from our insider builds and not stable. We think that we will need another iteration before making this feature available for all users. To try them out, grab our insiders build from [https://code.visualstudio.com/insiders](https://code.visualstudio.com/insiders).

#### Explorer

The files explorer is now capable of showing multiple root folders in one tree. You can easily add root folders from the File menu or context menu:
...
The files explorer should work and behave as before. You can move files between root folders and use any of the typical file operation actions provided in the context menu and the explorer view. Settings like `files.exclude` will be supported for each root folder if configured and across all folders if configured as global user setting. 

#### Search

The fulltext search and files picker take the additional root folders into account. When searching, results from all of the root folders will show up in the search results view:

![Multi Root Search](images/1_14/search-multi-root.png)

To distinguish results with the same file name across root folders, the name of the root folder is used as prefix to the relative path in the results list. Settings like `search.exclude` will be supported for each root folder if configured and across all folders if configured as global user setting. 

![Multi Root Quick Open](images/1_14/multi-root-quickopen.png)

#### Settings

With multiple root folders in one workspace, it is possible to have multiple `.vscode` folders in each root folder defining some settings that should apply for the folder. A subset of settings is supported in this environment, specifically:
* `editor.*`
* `files.exclude`
...
Other settings are currently not supported on a per-folder basis. We will adopt more settings in the future to be supported on this level but some settings will not be supported on a folder level (e.g. windows settings).

#### Proposed Multi-Root-API

Having multiple folders open in the editor requires more powerfull API. From the single-root days there is the `rootPath`-API and for compatibility that will remain. Still, we have new and more powerful API in the pipeline. The current *proposed* API is shown below and enables you to inspect all workspace folders and sends an event when those change.
...
*Note* that the API is still proposed and that you need to enable it per extension. In the `package.json`-file of your extension, add the following line: 
```json
"enableProposedApi": true,
```
Also, copy the [`vscode.proposed.d.ts`](https://github.com/Microsoft/vscode/blob/master/src/vs/vscode.proposed.d.ts)-file from our repository to your project. That enables type-checking and IntelliSense. 

Keep in mind that the API is still proposed. That means it might still be refined and that breaking changes are likely to happen. Because of that we don't support to publish extensions that use proposed-api. The good side of this is that extension writers, the customers of the extension-API, can help us define how the final API should look and what it should support. Don't be shy with feedback!


#### Upcoming Features

There is still a lot of work ahead of us to make the multi-root experience pleasant. These areas will get special attention in the next sprint:
* SCM UI for multiple root folders
