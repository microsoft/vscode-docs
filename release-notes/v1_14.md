...
### "attach" support for all Node.js versions
...
The new `markdown.preview.breaks` setting controls the rendering of line breaks in the markdown preview. By default, single line breaks in markdown paragraphs and ignored:
...
- `emmet.includeLanguages`
   ...
   Get `Emmet: Expand Abbreviation` and `Emmet: Wrap with Abbreviation` to work in the language of your choice by providing a mapping in this setting. Use language ids for both sides of the mapping.
   For example:
    ```json
    "emmet.includeLanguages": {
...
With this release, you can have auto indentation adjustment while tying, moving lines and pasting content in TypeScript, JavaScript, HTML, CSS, JSON or languages who have indentation rules by setting `editor.autoIndent` to `true`. The indentation rules for now may not be mature so please let us know if you run into any issue.

#### Indentation Rules

We made some improvements to the indentation rules in 1.9. With this release, language extension authors can put the indentation rules to `language-configuration.json` file instead of registering them in extension activation phase, like below
...
TypeScript, JavaScript, CSS, HTML and JSON have built-in indentation rules. We didn't ship indentation rules for other languages yet but if you use an extension which defines rules for a language you use, you can see below features if you turn on `editor.autoIndent`.

#### Type

For C-like languages, we already handle brackets nicely, indent next line when users type an open bracket and adjust the indentation when users type a matching close bracket. But for languages who use keywords to wrap blocks, we don't have handle for that. With the new auto indent support, now this is possible by setting indentation rules for that language.

![auto-indent-on-type](images/1_14/auto-indent-on-type.gif)

#### Move lines

Image you are moving the content out of a nested block or moving the content from one block to another, you may want the editor to help adjust the indentation level automically then you don't need to adjust them yourself later on.

![auto-indent-on-move-lines](images/1_14/auto-indent-on-move-lines.gif)

#### Paste
Pasting is similar to moving lines, we will adjust the indentation for you based on the lines before the pasted area.
...
Lists such as the suggestion widget, exception list and the **Quick Open** drop-down now set `aria-setsize` and `aria-posinset`. These lists are virtualized and so without these [aria](http://www.w3.org/TR/wai-aria/) properties, the screen reader could not correctly determine the number of entries.
...
The [Probot](https://github.com/probot/probot) we have deployed to assist in our issue management now uses a machine learning algorithm to guess the feature area of new issues. When the algorithm estimates a sufficently high probability of being correct the bot adds the area label and (in some cases) directly assigns a developer on the team:

![VSCodeBot labels an new issue with the 'emmet' label](images/1_14/bot-labels-emmet.png)

For a few days after a new release has come out the bot labeles new issues filed for that release with the `new release` label. This helps us spot issues for regressions introduced in the release more quickly. We remove the label after a few days:
