...
A number of performance improvements have been made to the startup of Jupyter kernels, specificially `Python`. Users who have previously opened a notebook would see a 2x speedup when starting a new kernel, even after re-loading VS Code. Restarting `Python` kernels should now be even faster.
