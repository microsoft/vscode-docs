...
You can now configure language specific editor settings for multiple languages at once. Following example shows how you can customise settings for `javascript` and `typescript` languages together:
...
### Better <kbd>Escape</kbd> handling in lists

Pressing <kbd>Escape</kbd> on lists and trees will now shrink the selection to a single element before clearing it.

### Screencast: Command names

Thanks to [@PEZ (Peter Strömberg)](https://github.com/PEZ), you can now choose to show command names as you trigger them via keyboard shortcuts, in screencast mode. Use the `screencastMode.keyboardShortcutsFormat` command to configure this behavior.

### Pre-Release Extensions

As VS Code now supports extensions to offer pre-releases, you can now opt-in to install them and use the latest cutting edge features from extensions.

VS Code shows an additional **Install Pre-Release Version** option in the extension **Install** drop-down menu for installing the pre-release version of the extension.

![Pre-Release install dropwdown](images/1_63/extensions-pre-release-install.png)

*Theme: [GitHub Light Theme](https://marketplace.visualstudio.com/items?itemName=GitHub.github-vscode-theme)*

Once installed, you can know that you are using the pre-release version of the extension using following indicators

![Pre-Release Indication](images/1_63/extensions-pre-release-install-indicators.png)

*Theme: [GitHub Light Theme](https://marketplace.visualstudio.com/items?itemName=GitHub.github-vscode-theme)*

VS Code also indicates you about the availability of the pre-release of an extension that you have already installed so that you can switch to it.

![Pre-Release Indication](images/1_63/extensions-pre-release-indicators.png)

*Theme: [GitHub Light Theme](https://marketplace.visualstudio.com/items?itemName=GitHub.github-vscode-theme)*

If you have installed the pre-release version of an extension and have extensions auto update feature enabled, then VS Code will automatically updates to the new pre-release version whenever available.

## Source Control

### Ignore trim whitespace in quick diff

You can now configure the SCM quick diff feature to ignore trim whitespace, using the `scm.diffDecorationsIgnoreTrimWhitespace` setting.
...
![Unicode Highlighting Example: Invisible characters are highlighted](./images/1_63/unicode-highlighting-invisible.png)

In addition, characters that can be confused with ASCII characters are also highlighted:

![Unicode Highlighting Example: Invisible characters are highlighted](./images/1_63/unicode-highlighting-confusable.png)

Read [this article](https://certitude.consulting/blog/en/invisible-backdoor/) or [this article](https://www.trojansource.codes/) for how invisible or confusable Unicode characters can be used in Unicode spoofing attacks.
...
To be on the safe side, the [restricted mode](https://code.visualstudio.com/docs/editor/workspace-trust#_restricted-mode) of the workspace trust should be used to review source code, as *all* non-ASCII characters are highlighted in untrusted workspaces.

The settings `editor.unicodeHighlight.invisibleCharacters`,
`editor.unicodeHighlight.ambiguousCharacters` or
`editor.unicodeHighlight.nonBasicASCII` can be set to `false` to disable the corresponding feature.
...
VS Code now ships with TypeScript 4.5. This update brings a number of new language features, along with a number of new tooling improvements and bug fixes.
...
Method signature completions let you quick override methods and implement interface methods. With this feature, when you are inside a class body suggestions for methods — such as those from a super class — insert the entire signature of the method when accepted:
...
This feature requests TypeScript 4.5 in the workspace and can be enabled/disable using: `typescript.suggest.classMemberSnippets.enabled` and `javascript.suggest.classMemberSnippets.enabled`.
...
TypeScript 4.2 added native support for semantic highlighting. On older versions, semantic highlighting was instead implemented as a TypeScript language plugin provided by VS Code. Removing the plugin reduces the amount of code we have to maintain and ship.
...
The built-in markdown preview now more intelligently updates itself as you type. Instead of replacing the entire markdown preview document for every update, the preview now uses dom diffing to only replace the elements in the preview document that have changed.

This reduces the jittering that would sometime occur while typing.
...
![Emmet Remove Tag Demo](images/1_63/emmet-remove-tag.gif)
...
The Python extension now has limited support for a workspace which is not trusted (see [workspace trust](https://code.visualstudio.com/docs/editor/workspace-trust#_extensions)) or is on a virtual file system (for eg. if a remote Github repository is opened). Only Partial IntelliSense is provided in such cases and any other feature of the extension is not available:

* Hover, same file completion, completion for builtins like print are available but extension is limited to working only with your currently opened files.
* For untrusted workspaces, only Pylance is supported.
* For virtual workspaces, only Jedi & Pylance is supported.

The language item in status bar is updated to indicate such scenarios:

![Limited support for Python](images/1_63/python-limited-support.png)
...
In this milestone, we added a new `replace` api to the `OutputChannel` object to replace the content in the output channel.
...
More information about supporting pre-release extensions can be found in the [Pre-Release Extensions](https://code.visualstudio.com/api/working-with-extensions/publishing-extension#pre-release-extensions) topic.
...
A new next version of the [Language Server Protocol](https://microsoft.github.io/language-server-protocol), together with the corresponding npm modules, has been published. Besides of [various minor improvements](https://microsoft.github.io/language-server-protocol/specifications/specification-3-17/#version_3_17_0) the new version contains a proposed implementation for inline values.

## Debug Adapter Protocol

### New 'important' category for the 'Output' event
The `Output` event of the Debug Adapter Protocol is used to send the stdout and stderr streams of the debuggee as well as informational messages from the debugger to the debug console. That stream of messages can be a lot to process and critical things can go unnoticed. For this reason a new category `important` has been added to the `Output` event which can be used by a debug adapter for extra critical one-off messages that need to stand out and not get overlooked by the user.
The `important` category is a hint for the client to show important information in a highly visible UI, for example as a popup notification. Since this category is a hint, clients might ignore the hint and assume the `default` category (i.e. `console` ).
...
- "continue" requests resume all threads.
...
In addition some debug adapters have a need for an "alternative semantic" where `step` or `continue`  operate only on the current thread but keep all other threads in the suspended state.
For this "alternative behavior" a new optional `singleThread` property has been added to all execution control requests. A corresponding new capability `supportsSingleThreadExecutionRequests` must be used by a debug adapter to indicate to the client that the execution control requests support the `singleThread` property.

## Tasks

### `automationProfile` setting

The old settings `terminal.integrated.automationShell.*` have been deprecated an new `terminal.integrated.automationProfile.*` settings have been added. This allows for greater freedom in specifying properties of terminals used for tasks, including shell, icon, color, and shell argument.
...
![NPM Scripts view](images/1_63/npm-scripts-view.png)
...
You can now open an Azure Repos URL with the `<org>.visualstudio.com` format in https://vscode.dev by prefixing the URL with https://vscode.dev.
...
* [138302](https://github.com/microsoft/vscode/issues/138517) vscode.dev can't switch branches in private repositories
* [135838](https://github.com/microsoft/vscode/issues/135838) I cannot save a files when there is another file with longer name
