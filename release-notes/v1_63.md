...
Also, characters that can be confused with ASCII characters are highlighted by default:

![Unicode Highlighting Example: Invisible characters are highlighted](./images/1_63/unicode-highlighting-confusable.png)

Note that this feature does not detect all possible unicode spoofing attacks, as this depends on your font and locale settings. Also, confusable-detection is done heuristically.
To be on the safe side, use the [restricted mode](https://code.visualstudio.com/docs/editor/workspace-trust#_restricted-mode) of the workspace trust feature to review code.
In untrusted workspaces, *all* non-ASCII characters are highlighted.

You can set any of the settings  `editor.unicodeHighlight.invisibleCharacters`,
`editor.unicodeHighlight.ambiguousCharacters` or
`editor.unicodeHighlight.nonBasicASCII` to `false` to disable the corresponding feature.
