...
Quick Open (`kbstyle(Ctrl+P)` on Linux and Windows, `kbstyle(Cmd+P)` on Mac) applies the exclusions from the `.gitignore` files when enabled with the `"search.useIgnoreFiles"` setting.

## Integrated Terminal

### Better Windows support

Windows support in the terminal has been improved:

- Deeply nested processes are now properly correctly cleaned up after exiting the terminal session, this happened most frequently with servers launched from node.
