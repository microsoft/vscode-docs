...
You can now review your source code changes right within the standard editor, if the active Source Control of your workspace provides such a feature. This is already enabled for all git repositories out of the box.
...
When staging a file which is flagged as conflicted due to a merge or rebase, Code will check for the standard merge markers (eg `<<<<<<<`) and alert you when you forget to delete them if you are about to stage the file.

![Move confirmation](images/1_18/merge.png)

### Git: Improved Integration

Recently, there was a [patch to Git](https://public-inbox.org/git/20170921043214.pyhdsrpy4omy54rm@sigill.intra.peff.net/) which enables tools like Code to use `git status` without interfering with other concurrent git commands. Code is now prepared for that upcoming Git version by using the right environment context (`GIT_OPTIONAL_LOCKS`) when spawning Git.
...
Along with the new SCM inline change review, a new contributable menu is now available as API: `scm/change/title`. It allows you to contribute commands to the header of an inline change review view.
