...
Welcome to the October 2017 release of Visual Studio Code. There are a number of significant updates in this version in fact we have managed to cross off a number of the most popular outstanding feature requests - we hope you like it - highlights include:
...
* **[Vertical panels](#vertical-panels)** - Now you can dock the Terminal, Output, Error List or Debug console to the right of your editor.
* **[Git indicators in the Explorer](#git-status-in-file-explorer)** - See pending changes in the File Explorer.
* **[Inline pending change review](#inline-change-review)** - View pending changes and perform partial commits in the editor.
* **[Several performance improvements](#TODO:windows-start-up)** - Improved windows start-up and editor performance tweaks.
...
* **[Install @types for TypeScript](#install-types-quick-fix-for-typescript)** - A new Quick Fix simplifies adding missing @types files to your project.
* **[Improved extension recommendations](#recommended-extensions-badge)** - Recommendations are now highlighted in extension searches.
...
* **[Source Control](#source-control)** - Inline pending change view, explorer indicators.
...
Support for multi-root workspaces is now enabled by default in the Stable release.  This was our [#1 feature request](https://github.com/Microsoft/vscode/issues/396) from you all - it's been a while coming but it's now here.

![Multi Root Workspaces](images/1_18/multiroot.gif)

TODO: I wish this did not have the distro in it... I have messaged Ben re: this

Please refer to our [documentation](https://code.visualstudio.com/docs/editor/multi-root-workspaces) for a full explanation of all the features that come with it - spanning improvements to settings, our SCM experience, Tasks, Debugging and more.
...
Crossing off another [very popular feature request](https://github.com/Microsoft/vscode/issues/178) we added in support for the Explorer to shows modified, added, conflicting, and ignored files in a different color and with a badge.
...
TODO: not a great image due to theme and also the Git output - I would use the terminal - pri 2
...
The VS Code logo for Stable release is now blue again. You can read all about our icon journey in this [blog post](https://code.visualstudio.com/blogs/2017/10/24/theicon).
...
TODO: Merge into a performance section....
We'd been investigating VS Code start up times on Windows and, thanks to a variety of improvements, cold start launch on Windows has been reduced by 5 seconds.
...
TODO: need to crop image background - looks bad on black.
...
Due to [popular demand](https://github.com/Microsoft/vscode/issues/8869), the **Add Selection to Next Find Match** (`kb(editor.action.addSelectionToNextFindMatch)`) action has been modified to behave differently when you start adding multiple cursors with no selected text. In this case, the action will temporarily disassociate with the Find widget search options and will always search for whole words and case sensitive matches.

### Navigate to next / previous symbolic highlight

Thanks to [PR #35740](https://github.com/Microsoft/vscode/pull/35740) from [Krzysztof Cieślak](https://github.com/Krzysztof-Cieslak), it is now possible to navigate between symbolic highlights using `kbstyle(F7)` or `kbstyle(Shift+F7)` in languages where there is a semantic highlight provider.

TODO: what does this mean?  A symbolic highlight?
...
We have the ability to set custom Environment Values for the terminal already - but some users would love to clear some existing Environment Variables for there integrated terminal sessions - it is now possible to do this by assigning them to null in the `terminal.integrated.env.<platform>` settings:
...
TODO: Should this all be in workbench - I think so???
...
TODO: image to big but Pri2

### Single view combining workspace and general recommendations

The two views for [workspace](https://code.visualstudio.com/docs/editor/extension-gallery#_workspace-recommended-extensions) and general extension recommendations are now combined into a single view with the two kinds of recommendations showing up in a split view. Use the command **Extensions: Show Recommended Extensions** to see this view.

There is a handy **Install All Workspace Recommendations** command on the **WORKSPACE RECOMMENDATIONS** title bar to install all the extensions that are recommended by other users of your workspace.
