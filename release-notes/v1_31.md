...
With this release, we bring you hot reload when you install extensions in VS Code, this is one of the most upvoted feature requests. You are no longer required to reload (restart) VS Code when you install or enable an extension. For example:

* Rich language features are applied immediately after installing a progamming language extension.

![Extensions hot reload](images/1_31/hot-reload-language-extension.gif)

* Views are shown immediately after installing an extension contributing views.

![Extensions hot reload](images/1_31/hot-reload-docker-extension.gif)

Hot reload also works when you uninstall or disable an extension if it is not activated.

**Note:** There are certain extensions that do not support hot reload because of external contributions (contributing to the extension points of other extensions). It requires adoption from the external contribution point owners by listening to the [`extensions.onDidChange`](#extensions-change-event) event.
...
Pressing Ctrl+A (Cmd+A) in a tree will now increment the tree's selection in a hierarchical fashion.

![Type Filter](images/1_31/select-all.gif)

#### Customizable Indentation

You can now customize the node identation of all trees across the workbench using the **Workbench > Tree: Indent** (`workbench.tree.indent`) setting.
...
There is a new **Screencast Mode** in VS Code which you can use for demonstration purposes.
...
We have adopted our new tree widget in the File Explorer. The new tree widget supports several new tree features, read the [New Tree Widget](#New%20Tree%20Widget) section for more details.
...
### Semantic selection for HTML, CSS/LESS/SCSS and JSON

Semantic selection is now available for HTML, CSS/LESS/SCSS and JSON:

![html-css-selection](images/1_31/html-css-selection.gif)
...
We have observed that several extensions use node modules like `opn` or `open` to open an url in a browser. Given the desire for this function we have added new API: `vscode.env.openExternal`. It expects an url and can be used to open website-links, mail-links, or application-url-handler. Also, file-urls are accepted which allows to open them in their default app, like a pdf-file. We encourage extension authors to switch from using node modules to this new API.
...
A new event `extensions.onDidChange` was added which fires when the `extensions.all` array changes. This can happen when extensions are installed, uninstalled, enabled, or disabled. See [Hot reload of extensions](#hot-reload).
...
When developping a color theme extension, you can see changes to colors applied live in the `Extension Development Host` window.
