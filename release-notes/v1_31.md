...
In the example below the user follows the recommendation to install the `vetur` extension after opening a `.vue` file. Notice that the language features (such as syntax highlighting, errors) are available immediately after installing the recommended extension.
