...
With this release, we bring you hot reload when you install extensions in VS Code. You are no longer required to reload (restart) VS Code when you install or enable an extension. It means that you can do following without reloading the window:
...
* Install the Pull Request extension and create a PR.
...
**Note:** There are certain extensions which do not support hot reload because of external contributions (contributing to the extension points of other extensions). It requires adoption from the external contribution point owners by listening to the [`extensions.onDidChange`](#extensions-change-event) event.
