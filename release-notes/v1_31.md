...
We added more entries to the **Go** actions menu and put separators to group actions logically:
...
We have worked on the implementation of **Expand Selection** and **Shrink Selection**. Not only are we in the process of adding an API so that language servers can specify selections but we are also improving the default implementation as well.
