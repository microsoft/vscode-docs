...
This is one of the most upvoted feature requests and we bring it to you in this release. You are no longer required to reload (restart) VS Code when you install or enable an extension. For example:
...
* We introduced GraphQL gradually into the project so the description view now presents more detailed information of a pull request and the overall performance of the extension is improved as well.
* You can now choose *Create Merge Commit*, *Squash and Merge* or *Rebase and Merge* when merging a pull request.
* Performance improvement. We reduced both the API calls to GitHub and git operations when you browse pull requests, file changes and checkout a pull request, the extension responds faster, especially on Windows.
