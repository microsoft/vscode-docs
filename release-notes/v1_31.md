...
Welcome to the January 2019 2018 release of Visual Studio Code. There are a number of significant updates in this version that we hope you will like, some of the key highlights include:

* **[No reload on extension install](#no-reload)** - No interruptions when you install new extensions.
* **[Tree UI improvements](#new-tree-widget)** - Improved Explorer navigation and filtering, horizontal scrolling.
* **[Main menu updates](#improved-go-menu)** - Redesigned Go actions menu, Edit menu Cut command.
...
* **[Command-based user input variables](#custom-command-user-input-variable)** - Custom user input for task and debug configuration.
...
* **[Workbench](#workbench)** - Zen Mode hides line numbers, new Screencast mode for demos.
* **[Editor](#editor)** - Smart selection improvements, tune Reference CodeLens behavior.
* **[Integrated Terminal](#integrated-terminal)** - ConPTY support on Windows, Find improvements.
* **[Extension Authoring](#extension-authoring)** - New Octicons available, open resource action.
...
Being able to install extensions without forcing a reload (restart) of VS Code is one of the most upvoted feature requests and we bring it to you in this release. You are no longer required to reload VS Code when you install or enable an extension. For example:

* Rich language features (such as syntax highlighting) are available immediately after installing a programming language extension.
...
We have pushed our widget game to the next level: a new tree widget was created to address performance issues and enable us to provide more features in several workbench areas. The new tree widget was created through composition over our high-performance list widget. We intend to write a separate blog post about this engineering work and the performance improvements. For now, let's focus on the goodies instead.

**Note:** The new tree was adopted in the File Explorer, all Debug trees, Search, and Peek References. All features that follow apply to most of those feature areas, unless explicitly disabled by us.

#### Improved keyboard navigation

There are now three different types of keyboard navigation in trees: `simple`, `highlight` and `filter`. In both `highlight` and `filter`, typing in a tree will display a widget at the top of the tree which indicates that you are now navigating the tree:

![Type Filter](images/1_31/type-filter.gif)

This widget can be used to switch between the `highlight` and `filter` modes. You can configure a keybinding to execute the `list.toggleFilterOnType` command if you'd like a keyboard shortcut for switching between modes. The **Workbench > List: Keyboard Navigation** (`workbench.list.keyboardNavigation`) setting sets the default mode, including the `simple` mode in which typing the first few characters of a tree element simply focuses that element.
...
**Note:** These keyboard navigation modes currently only work on _resolved_ tree nodes. For example, in the File Explorer, if a folder was never expanded, the tree won't look for its children. We are considering several options to make this experience better.

#### Hierarchical Select All

Pressing `Ctrl+A` (`Cmd+A` on macOS) in a tree will now increment the tree's selection in a hierarchical fashion.

![Type Filter](images/1_31/select-all.gif)
...
We added more navigation actions to the **Go** menu to make them more easy to discover:
...
There is a new **Screencast Mode** in VS Code which you can use for demonstration purposes (**Developer: Toggle Screencast Mode**).

![Go menu](images/1_31/screencast.gif)

## Editor

### Smart Selections

We have worked on improving the implementation of **Expand Selection** and **Shrink Selection** from the **Selections** menu. We are in the process of adding an API so that language servers can specify selection steps based on the their semantics knowledge. In addition, we have also improved the default implementation as well.
...
![html-css-json-selection](images/1_31/html-css-json-selection.gif)
...
### Custom command user input variable
...
You can now configure tasks to show output in a split terminal panel instead of creating a new one. A task configuraton can use the `group` attribute in the `presentation` section to define where the task's output should be shown. 
...
This milestone, we are shipping with one such experience to demonstrate something that can be achieved more easily with the new grid layout, toggling the editor area. With a new command `Toggle Editor Area`, the code editors can be hidden enabling the user to create a VS Code terminal window as shown below.

![Terminal only window](images/1_31/panel-only.png)

And if you open a file from the terminal using `code file.txt`, it will pop open an editor just as you would expect. While this area is still a work in progress and there are known issues, we encourage you to check it out and file issues. You can keep up to date with issues regarding this work with [this query](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-layout+label%3Abug).
...
A new event `extensions.onDidChange` was added which fires when the `extensions.all` array changes. This can happen when extensions are installed, uninstalled, enabled, or disabled. See [No reload](#no-reload) section.
...
**Note** extension authors that use the `opn` or `open` module should switch to this new API.
...
**Note** extension authores that use a custom location file system location to store state across workspaces should switch to this new API.
...
When developing a color theme extension, you can see changes to colors applied live in the `Extension Development Host` window.

![live theme editing](images/1_31/live-theme-editing.gif)

Same works for file icon themes.
...
The new [TypeScript TSLint Plugin](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-tslint-plugin) extension now support auto fix on save using `source.fixAll` code actions.
...
We recommend that all users of the old TSLint extension migrate to the new [TypeScript TSLint Plugin](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-tslint-plugin) extension.
...
* We introduced [GraphQL](https://graphql.org/) gradually into the project so the description view now presents more detailed information of a pull request and the overall performance of the extension is improved as well.
...
There are almost 1400 members in the MLCP [Cloud + AI Localization](https://github.com/Microsoft/Localization/wiki) community with over 100 active contributors every month. We appreciate your contributions, either by providing new translations, voting on translations, or suggesting process improvements.
