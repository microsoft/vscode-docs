...
* **[Open terminal in custom working directory](#open-new-terminals-with-custom-working-directories)** - Bind keyboard shortcuts to specific working directories.
...
* **[CSS property completions include semicolons](#css-property-completion-with-semicolon)** - Semicolons added automatically as you enter CSS properties.
...
* **[Remote Explorer updates](#remote-development-preview)** - Now displays WSL distros and repository containers.
...
The Source Control view has been updated to use the latest tree widget. You can now toggle between a tree and a list view with the **Toggle View Mode** button on the Source Control title bar. You can change the default view using the `scm.defaultViewMode` setting. You also now benefit from the automatic keyboard navigation and filtering of the updated tree widget by simply starting to type in the view.

Since the tree will render folders, the Git extension now also contributes commands to folders, for example **Stage Changes**, which will be applied to all files within a folder.
...
In the image below, you can see how the narrower `UI` typeface looks in the menu bar.
...
* Remote Explorer now supports the Remote - WSL extension and lets you easily connect to installed Linux distros.
...
[Java 13](http://jdk.java.net/13/) is out (GA) and VS Code is ready for it. The latest [Java Extension](https://marketplace.visualstudio.com/items?itemName=redhat.java) now supports Java 13. Developers using Java 12 with preview features will need to upgrade to JDK 13 to keep working with them.
