...
On Windows, we have switched the UI typeface for Japanese display from `Meiryo` to `Yu Gothic UI` and `Meiryo UI`.

The `UI` typefaces has narrower Japanese kana to save horizontal space. You can use the Japanese UI by installing the [Japanese Language Pack](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ja) and set `Display Language` to `ja`.
