...
Previously, there was no way to review changes that would have beeen applied by the refactoring. To improve the refactoring experience, we have added a refactoring preview code action option that can be accessed from the Command Palette. The **Refactor with Preview...** command works just like the **Refactor...** command, but will open up a refactor preview pane in the workbench. Users can select the refactor previews in the workbench to open up a larger view of the refactoring preview in the editor. This iteration primarily focused on just allowing the code changes from refactorings to be previewed, with future features and a custom Code Action menu control to follow.
