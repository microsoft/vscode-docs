...
* **[Toggle Light/Dark themes](#toggle-between-lightdark-themes)** - Quickly switch between preferred light and dark themes.
* **[Terminal shell integration](#shell-integration)** - Support for run recent command and go to recent directory.
...
* **[JavaScript sourcemap toggling](#sourcemap-toggling)** - Switch to debugging compiled rather than source code.
* **[Color theme tester](#theme-tester-for-built-in-themes)** - Use vscode.dev to preview color themes.
...
You toggle Do Not Disturb mode by opening the Notification center (select the bell icon in the right of the Status bar) and clicking the crossed out bell icon. Do Not Disturb mode is indicated in the Status bar by the crossed out bell icon.
...
Enable the setting to turn on the feature:
...
The decorations can be interacted with to give some contextual actions like rerunning the command:
...
A few iterations ago, we added decorations to the terminal buffer and overview ruler to use shell integration capabilities and improve navigation of the terminal. Decorations now also mark points of interest for tasks, which can be jumped to with the command navigation feature (`Ctrl/Cmd+Up`, `Ctrl/Cmd+Down`).
...
This milestone we merged a long standing community contribution that lets you use a full text editor to author commit messages, whenever no message is provided in the commit input box. To take advantage of this new flow, click the **Commit** button without providing a commit message.
...
You can also access a list of Debug Consoles from within the existing view menu. Type `view ` (with a trailing space) in the Quick Open menu and scroll down to the **Debug Console** panel header. You can see a list of Debug Consoles here too.
...
Previously, there was no way to review changes that would have been applied by a refactoring. To improve the refactoring experience, there is now a refactoring preview code action that can be accessed from the Command Palette. The **Refactor with Preview...** command works just like the **Refactor...** command, but will open up a refactor preview pane in the workbench. Users can select the refactor previews in the workbench to open up a larger view of the refactoring preview in the editor. This iteration primarily focused on previewing code changes from refactorings, with future features and a custom Code Action menu control to follow.
...
This iteration, we've improved our support for these angle bracket links so that they are correctly highlighted and clickable in the editor.
...
### Theme tester for built-in themes
...
* Linting support in notebook cells for all supported ESLint languages.
...
Other notable `IPyWidget` fixes include displaying of complex and nested (including interactive) outputs within the [Jupyter Output Widget](https://ipywidgets.readthedocs.io/en/latest/examples/Output%20Widget.html).
...
We've enabled experimental support for using WebUSB, WebSerial, and WebHID from web extensions.

To access these apis, first your extension needs to request access to the desired devices by running on these commands:

- `workbench.experimental.requestUsbDevice`
- `workbench.experimental.requestSerialPort`
- `workbench.experimental.requestHidDevice`

These commands will prompt the user to select a device. Each command takes a [list of device filters](https://developer.mozilla.org/en-US/docs/Web/API/USB/requestDevice) as its argument. For example:
...
After this call completes, your extension can use the normal web APIs to access any devices that the user granted access to:
...
All of the normal web device APIs should work, with the expectation of those such as [`requestDevice`](https://developer.mozilla.org/en-US/docs/Web/API/USB/requestDevice) that need show UI elements to the user.

At the moment this functionality only works on web and support is limited to Chrome and Edge. Also keep in mind that this feature is still experimental, so we may iterate on its design. Please give it a try and share your feedback!
