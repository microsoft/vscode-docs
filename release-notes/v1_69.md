...
VS Code now has support for [Windows Registry-based Group Policy](https://docs.microsoft.com/en-us/previous-versions/windows/desktop/policy/implementing-registry-based-policy). Namely, a new `UpdateMode` policy is now supported, which overrides the `update.mode` setting. Refer to [the documentation](/docs/setup/enterprise.md#group-policy-on-windows) to learn more.
...
* [@benmcmorran (Ben McMorran)](https://github.com/benmcmorran): Clarify pre-release to release update semantics [PR #5399](https://github.com/microsoft/vscode-docs/pull/5399)
