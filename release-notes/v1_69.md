...
A new command allows to toggle between your preferred light and dark themes: **Preferences: Toggle between Light/Dark Themes**. When in a light theme, it swiches to a dark theme and vice versa. When in a high contrast theme, the command sticks to high contrast and switches to the preferred high contrast scheme of the opposite color scheme.
The preferred themes are defined by the settings `workbench.preferredDarkColorTheme`, `workbench.preferredLightColorTheme`, `workbench.preferredHighContrastColorTheme` and `workbench.preferredHighContrastLightColorTheme`
...
The setting `html.format.endWithNewline` has been removed in favour of `files.insertFinalNewline`.

`files.insertFinalNewline` applies to all langauges. In case you want different settings per language, you can use a language scope:
```json
{
    "[html]": {
...
The theme tester is a route in [vscode.dev](https://vscode.dev) that can be used to showcase and try a color theme without installing it.
It can now also be used with built in themes:
