...
- Preview the position when using `Terminal: Navigate Accessible Buffer kb(workbench.action.terminal.navigateAccessibleBuffer)` before accepting a command to go to
...
### Diff editor audio cue experience improvements
...
The drop selector widget appears whenever you drop content and there is more than one possible way it could be inserted. You can open the widget by clicking on it or using `kb(editor.changeDropType)`. The widget goes away as soon as you start typing or move the cursor outside of the inserted text. You can also fully disable this widget using `"editor.dropIntoEditor.showDropSelector": "never"`.
...
The [Code Spell Checker](https://marketplace.visualstudio.com/items?itemName=streetsidesoftware.code-spell-checker) extension for example adds spelling fixes to the SCM input. Extensions can contribute additional fixes and code actions.

## Notebooks

### Drop image files into notebooks to create attachments

You can now drag and drop image files into notebook Markdown cells to create attachments. When you drop the image, use [the new drop widget](#drop-selector) to select `Insert Image as Attachment`:

![Using the drop selector in a notebook markdown cell](images/1_78/notebook-drop.png)

This will add the image to the notebook as an attachment instead of simply adding a link to the image:
...
This will insert a `<video>` tag pointing to the image. You can drag videos from VS Code's explorer or from your local operating system.
...
### Commit files to Git Large File Storage from github.dev and vscode.dev

Git LFS allows you to efficiently store large files which change frequently in Git repositories. github.dev and vscode.dev now support committing files to Git Large File Storage in repositories hosted on GitHub, enabling easy updates from your browser without needing to install the LFS extension for Git locally.

LFS commit support in github.dev and vscode.dev works out of the box when your repository already has a `.gitattributes` file in the root of your repository that specifies which file types should be stored with Git LFS. To set up your repository for Git LFS for the first time, please see https://git-lfs.com.
