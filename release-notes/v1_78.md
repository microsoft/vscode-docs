...
This new setting, like the `autoFormatStrings` setting mentioned earlier, is currently disabled by default. However, we plan to enable this behavior in the next release based on feedback. Eventually, we plan to remove this setting entirely.
...
The Jupyter extension now ships [platform-specific extensions](https://code.visualstudio.com/api/working-with-extensions/publishing-extension#platformspecific-extensions), each VSIX built for a specific platform (Windows 64 bit, Windows 32 bit, Linux x64, Alpine x64, macOS Intel, macOS Apple Silicon, etc.). As a result of this, the download size of the Jupyter extension for individual platforms is  smaller, resulting is faster download times and less disk space usage.
...
* All of the places where you can **Checkout default branch** now respect the Git setting `"git.pullBeforeCheckout"`.
...
Editor chat sessions are now integrated with the Quick Fixes. Select the light bulb for a squiggle for options to fix or explain with Copilot.
...
* Lastly, if your results yield no results, you can **Ask GitHub Copilot** which, for now, puts what's in your filter box in a new chat for Copilot to handle.
