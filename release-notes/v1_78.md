...
Previously, users of accessiblity mode had a different behavior of Command Palette and other quick pick experiences. In that experience, the first item of the quick pick wasn't selected in order to provide a fully accessible experience. This iteration, we've introduced new behavior that allowed us to have the best of both world: an accessible _and_ fast quick pick experience allowing you to hit ENTER right away.

> Note: one compromise with this is that if an item in the quick pick is selected, then you are not able to hear ARIA changes to the quick pick input box, an ARIA limitation. To hear these changes, you can hit <kbd>SHIFT</kbd>+<kbd>TAB</kbd> until no item of the list is selected.
...
>**Note**: To get access to the new chat-based Copilot features, you'll need to sign up for the [GitHub Copilot chat waitlist](https://github.com/github-copilot/chat_waitlist_signup/join). These features are only available in VS Code Insiders.
