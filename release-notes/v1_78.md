...
<video src="images/1_78/notebook-toggle-scrolling.mp4" controls title="Toggle notebook cell scrolling"></video>
...
#### Jupyter extension no longer installed by default with the Python extension

The Jupyter extension is no longer automatically installed alongside the Python extension by default. This change was made in response to [feedback](https://github.com/microsoft/vscode-python/issues/18073) from [Dev Containers](https://code.visualstudio.com/docs/devcontainers/containers) users who wanted a faster container creation process without the Jupyter extension installed by default.

If you have dev containers definitions that only list the Python extension and wish to continue using the Jupyter notebooks features in your containers, you can add the Jupyter extension ID to your `devcontainer.json` file:
...
Alternatively, you can also create a [Profile](https://code.visualstudio.com/docs/editor/profiles) with Python and Jupyter, as well as all of your other favorite extensions.

#### Better support for "Create environment" command with microvenv
...
We have released a new [Mypy extension](https://marketplace.visualstudio.com/items?itemName=ms-python.mypy-type-checker) that provides linting support for Python files using the [mypy](https://pypi.org/project/mypy/) type checker. If you have any issues or feature requests, you file an issue at the [vscode-mypy](https://github.com/microsoft/vscode-mypy) GitHub repository.

![Mypy errors reported in Problems window](images/1_78/mypy-extension.png)

_Theme: [HardHacker](https://marketplace.visualstudio.com/items?itemName=HardHacker.hard-hacker-theme
)_

#### Formatter extensions recommendation
...
In order to streamline the Python commands available when right-clicking on the editor, the **Run Python File in Terminal** and **Run Selection/Line in Python Terminal** commands have been moved to sub-menu items under the "Run Python" entry.
...
* [@burkeholland (Burke Holland)](https://github.com/burkeholland): initial commit [PR #6225](https://github.com/microsoft/vscode-docs/pull/6225)
