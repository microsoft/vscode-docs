...
Given the focus on shipping `vscode.dev` and not everybody in the team had cycles for clean-up, some of the issue clean-up will continue in November. After housekeeping, we have also addressed several feature requests and community [pull requests](#pull-requests).
...
Only a subset of extensions can run in the browser. Extensions that are purely declarative, such as most themes, snippets, or grammars, can run unmodified in the browser. Extensions that are running code need to be updated by the extension authors. We'd like to say [thank you](#web-extensions) to the extension authors that already have published their extensions as [web extensions](https://code.visualstudio.com/api/extension-guides/web-extensions).
...
If you are an extension authors you can reach out to [VS Code Marketplace](https://marketplace.visualstudio.com) for getting your domain verified.

### Copy multiple problems

You can now select and copy multiple problems in the Problems view.
...
Notebook cells can output multiple types of data (mimetypes) which can be rendered in different ways both by VS Code itself and extensions. Previously, VS Code had basic memoization of the selected renderer for a mimetype, and you could manually configure your preferred mimetypes using the `notebook.displayOrder` setting. However, it wasn't particularly smart about picking the best renderer for a mimetype, and preferred mimetypes had to be updated manually.
...
You can now filter the list of kernels displayed in the kernel picker either globally or on a workspace basis. This is valuable if you have a large number of kernels but usually only work with a subset of those..
...
* Windows Subsystem for Linux indicator lets know you whether you are using WSL 1 or WSL 2.
* Advanced container configuration videos covering how to [persist bash history](https://code.visualstudio.com/remote/advancedcontainers/persist-bash-history) and [working with monorepos](https://code.visualstudio.com/remote/advancedcontainers/change-default-source-mount).
...
This update support for the upcoming TypeScript 4.5 release. The [TypeScript 4.5 beta announcement post](https://devblogs.microsoft.com/typescript/announcing-typescript-4-5-beta) has more information about the new language and tooling features. Some tooling highlights:
...
Extension authors for enabling extensions that run code as [web extensions](https://code.visualstudio.com/api/extension-guides/web-extensions) (the list below is from November 2nd):
