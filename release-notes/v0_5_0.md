...
Order: 3
TOCTitle: v0.5.0
PageTitle: Visual Studio Code v0.5.0
...
This release is available now - see [how to update](/docs/supporting/howtoupdate) to get the bits.
...
VS Code launched with [snippet support](/docs/editingevolved#_snippets), and we've added some notable enhancements in this release.
...
We added a new debug pane for watch expressions. You are able to add, rename or remove watch expressions from the debug viewlet. Based on the focused stack frame, watch expressions are reevaluated whenever the debug target has stopped. It is also possible to add an expression to watch using the editor context menu.
