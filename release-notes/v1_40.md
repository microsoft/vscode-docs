...
During this housekeeping milestone, we addressed several feature requests and community [pull requests](#thank-you) that resulted in new features.
...
### Less notification toasts in Zen mode

While using Zen mode, notification toasts will no longer distract you unless they inform about errors. You can control this from the new `zenMode.silentNotifications` setting. You can always access all notifications from the status bar by clicking on the notification icon in the bottom right corner. 
...
Previuosly, VS Code would only highlight matching brackets when the cursor would be next to a bracket character. We have improved our bracket matching to always highlight enclosing brackets if they exist. We have also adjusted our **Go to Bracket** and **Select to Bracket** actions to use enclosing brackets:
...
It is now possible to have more fine grained control over the font features. When configuring `"editor.fontLigatures": true`, VS Code would turn on `liga` and `calt`. But some fonts have more settings, such as stylistic sets used by [Fira Code](https://github.com/tonsky/FiraCode/releases).
...
The above settings turn on font ligatures for all languages, in JavaScript files the 'Horizontal Bar' ('ss02') font feature setting, and in C++ further enables the 'Dotted zero' ('ss19') font feature setting. This makes the `>=` ligature render with a horizontal bar in both languages and the `0` render with a dot inside it in C++:

![Image showing highlighted enclosing brackets](images/1_40/font-feature-settings.png)
...
**Cursor Undo** has been renamed to **Soft Undo**. This action is very useful, for example when using `kbstyle(Ctrl+D)` / `kbstyle(Cmd+D)` and going one time too much, as it undoes only the last cursor state change. Additionally, we now have added **Soft Redo** which redoes the previously undone cursor state change.

### Diff Editor improvements

The built-in diff computation used to have a hard-coded timeout of 5 seconds, after which it would give up and return a non-minimal diff. It is now possible to configure this timeout using `"diffEditor.maxComputationTime"` and it can be set to `0` to always compute a minimal diff, irrespective of how long that would take. Additionally, the diff editor now renders a progress bar when the computation takes longer than one second.

A source of confusion for using the diff editor was when comparing files with trim whitespace only differences, when the diff editor was configured to ignore trim whitespace. This would result in SCMs showing the files as modified, while the diff editor would render no diffs. We now render a button when we detect this case to help you quickly show diffs in trim whitespace. This can be undone using the **Show/Ignore Trim Trailing Whitespace Difference** action that renders at the top of the diff editor:

![Animation showing a button that shows when comparing two files that contain only whitespace diffs but the diff editor is configured to ignore trim whitespace](images/1_40/diff-editor-show-whitespace-button.gif)
...
for controlling whether a hierarchy of debug sessions shares a single Debug Console or use individual consoles. This API has now become official so you can use it in extensions published on the Marketplace.
...
* [NotWearingPants (@NotWearingPants)](https://github.com/NotWearingPants)
