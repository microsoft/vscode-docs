...
We've introduced a new indicator for the active item in activity bar to make it stand out better and increase readability. We also slightly tweaked the inactive foreground colors for a stronger contrast with the active element.

![active activity bar indicator](images/1_40/activity-bar-indicator.gif)

You can control the active indicator via the new color token, `activityBar.activeBorder`. We also introduced an optional background color for the active element, `activityBar.activeBackground`, and when confgured can look like so:
...
There are new settings to configure what symbols the Outline view and Breadcrumbs navigation should show. The settings are called `outline.showVariables`, `outline.showFunctions`, etc. and `breadcrumbs.showVariables` respectively.
...
A new command has been added for better accessibility of the detailed hover widget traditionally triggered by hovering a symbol with the mouse and pressing the control key. Previously, only the command `Show Hover` was provided which is equivalent to hovering the mouse cursor over a symbol. Now, with `Show Control Hover`, the detailed hover information may be shown via the keyboard.
...
These icons appear in the outline, breadcrumb, and suggest widget.
...
You can also explictly set the icon colors for the problems panel and notifications:
