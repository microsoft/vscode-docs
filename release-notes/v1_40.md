...
When the setting `npm.enableRunFromFolder` is enabled, then the Explorer's context menu shows the command `Run NPM Script in Folder...` when a folder is selected. The command shows a quick pick list of the NPM scripts contained in this folder and you can select the script to be executed as a task.
