...
If `asExternalUri` is called with a `vscode.env.uriScheme` URI, then it returns a uri that - if opened in a browser (e.g. via `openExternal`) - will result in the registered URI handler being triggered.
