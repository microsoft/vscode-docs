...
When the conflict can be resolved without conflicts on a character level, the option `Accept Combination` is shown and can be used to automatically resolve the conflict (note that both symbols are new imported in a single import statement):
...
The default diff algorithm for the merge editor has been changed (to rollback, use `"mergeEditor.diffAlgorithm": "smart"`). The new diff algorithm is optimized for merge scenarios. In common cases, the number of hunks are minimized and insertion positions are optimized.
...
* Dev Container Features - Add new capabilities to a Dev Container by including prepackaged features.
