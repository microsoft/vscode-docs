...
The Pylance extension now has powerful faetures that can help you work more efficiently and effectively when writing and maintaining clear, concise and easy to understand tests with pytest:

- Support for completions, go to definition, find all references and symbol rename (F2) for pytest fixtures and parameterized arguments
...
![Showcasing new pytest intellisense features](images/1_76/pylance-pytest-intellisense.mp4)
...
### `tooltip`s for `QuickPickItem`s

This iteration we are introducing a new proposed API for a `tooltip` property for `QuickPickItem`s. This property allows you to specify a tooltip for each item in a quick pick. The tooltip will be displayed when the user hovers over the item or it can be toggled with <kbd>Ctrl</kbd>+<kbd>Space</kbd>.
