...
We have decided to simplify the debug start view and to only show one button. We removed the Run button since Run is just Debug without breakpoints, and for ignoring breakpoints there is still a main menu action "Run Without Debugging".

Based on user feedback we have decided to show the breakpoints view in the start view if you have at least one brekpoint set.
...
Now the debug console input uses the mode of the current active editor. Due to this debug console input now has coloring, indentation, auto closing of quotes and other language features.
...
We have introduced a setting `debug.console.closeOnEnd` that controls if the debug console should be automatically closed after debugging stopped.

#### Support for `console.table(...)`
In the debug console we now properly render expressions coming from `console.table(...)` calls.
...
In the debug console input we now suggest entries entered previously by the user. We use the text icon to distinguish them from the rest.

![debug console history](images/1_42/debug-console-history.png)


### Loaded Scripts View Improvements
Compact tree view feature - TODO@weinand
Searching/Filtering - TODO@weinand
