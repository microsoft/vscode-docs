...
For most product icons inside VS Code the [codicon](https://microsoft.github.io/vscode-codicons/dist/codicon.html) icon-font is used. Extensions can now re-use those icons in very simple and declarative ways:

* The `vscode.ThemeIcon`-type can now be instantiated with the name of a codicon e.g. `new vscode.ThemeIcon("zap")` so that it renders that icon, for instance in custom trees.
* The `vscode.MarkdownString`-type and now supports the `$(<name>)`-inline syntax, e.g `myMdString.appendMarkdown('Hello $(globe)');`. Note that to prevent false-positives this must be enabled with the `supportThemeIcons`-constructor argument.
* `commands` are defined in the [`package.json`](https://code.visualstudio.com/api/references/contribution-points#contributes.commands)-file of extensions and their icon can now also be a codicon. Again, use the inline-syntax, e.g `"icon": "$(zap)"`

![codicon hover](images/1_42/codicon-api-md.png)
*Theme: Light+*

The hover above has been is the result of rendering each and every codicon. The snippet below is all it takes but for readability not all codicon names are listed anymore.
...
Completions that have been triggered at the end of a word usually replace that word. However when completion are triggered inside words then its not clear if the word-end should be replaced or not. Usually there is two camps and depending on the preference of the extension author users get the one or the other behaviour. That's not ideal and now there is new API that allows to express what the insert and replace range for a completion item is. For that, we have added a new or-type to `CompletionItem#range`:
...
1. The range is `undefined` which means VS Code uses the current word range for *replacing* and the same range cropped at the cursor position for *inserting*.
2. The range is a simple `vscode.Range` tells VC Code that there is only one way of inserting or replacing.
3. The range is an object of *inserting* and *replacing* ranges which means VS Code can just use them as-is.

This information is used in combination with `editor.suggest.insertMode`-setting that allows users to define their preference. We ask extension authors to please adopt this API quickly becase we have plans to make insert- and replace-behaviour more visible in the next release.


### Refined CallHierarchyProvider

We have made a small tweak to [`CallHierarchyProvider#prepareCallHierarchy`](https://github.com/microsoft/vscode/blob/8486d683444d324eaaa53ec41e5be7b47fd47a6a/src/vs/vscode.d.ts#L4027) so that it now allows to return one or many `CallHierarchyItem`-elements. That `many` part is new and helps when starting call hierarchy on an element that, for instance, has multiple overloads.

### Creation options exposed on Terminals

`Terminal` has a new `creationOptions` property that can be used by extensions to help identify how it was created.
