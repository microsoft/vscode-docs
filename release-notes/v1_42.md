...
You can add keyboard shortcuts to quickly navigate in this picker without using the mouse. For example, below is a keybinding to so that `kbstyle(Ctrl+Tab)` navigates across editors of all groups:
...
Use `workbench.action.quickOpenPreviousRecentlyUsedEditorInGroup` for picker-based navigation.
