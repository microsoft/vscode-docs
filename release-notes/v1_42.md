...
### New setting for files.defaultLanguage to pick the language of the active editor

You can now configure `"files.defaultLanguage": "${activeEditorLanguage}"` instead of picking a specific mode. With this setting, new untitled editors will always inherit the language mode of the currently active editor if any.

### Changed `window.restoreWindows` settings default value to restore all windows
