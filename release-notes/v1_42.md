...
The input box in the Source Control view now uses the [Monaco Editor](https://github.com/Microsoft/monaco-editor). This brings in useful functionality such as multiple cursors and move/copy line commands.
...
When you are using a multi-root workspace, the "Collapse Folder" button in the file explorer now has a two-step behavior where the first click will collapse all folders within each top level, and the second click will collapse the top level folders. The search viewlet has already had the same behavior for awhile.

![two-step collapse button](images/1_42/2-step-collapse.gif)

## Accessibility

### Suggest widget improvements

The suggestions widget now no longer uses ARIA alerts and properly indicates to the screen reader that it is focused. Thus it should no longer happen that the screen readers gets in the way and that the screen reader is unaware of this.

### editor.accessibilityPageSize set to 160 when screen reader detected

The `editor.accessibilityPageSize` setting controls the number of lines in the editor that can be read out by a screen reader at once. However, there performance implications when using a large value. We have decided to automatically set the value to 160 lines when we detect a screen reader (and not to more due to an NVDA bug).

### Linux accessibility with Orca

We have started looking into making VS Code accessible on Linux with the Orca screen reader. However, since this work requires Electron 7, we expect to have more progress in the next stable release. In the meantime, for Linux accessibility you can use our Insiders version and provide feedback.
...
To further simplify starting with the Debug view, VS Code now only shows one **Run and Debug** button. The **Run Without Debugging** action (which is not supported by all debuggers), is still available under the Debug main menu.

Based on user feedback, we now show the Breakpoints pane in the start view, if you have at least one breakpoint set.

![Debug start pane](images/1_42/debug-start.png)

### Debug Console improvements

**Debug Console input**

The Debug Console input now uses the language mode of the current active editor. This means that the Debug Console input now supports syntax coloring, indentation, auto closing of quotes, and other language features.

![Debug Console input with syntax coloring](images/1_42/debug-console-mode.png)

**Input/output presentation**

We have tuned how we present input and output in the Debug Console to make it more distinguishable. VS Code displays an arrow only next to the input expression.
...
The Debug Console input box now automatically suggests previously entered entries. The text icon is used to distinguish these entries from the rest.

![Debug console history](images/1_42/debug-console-history.png)

### Loaded Scripts View improvements

The Loaded Scripts View has been updated to use VS Code's new tree view. It now renders single child folders in the same compact form as VS Code's File Explorer. In addition, it is now possible to search and expand the tree as you type.
...
Based on user feedback, we've update our debug icon in the Activity Bar to better represent "Run and Debug". And when debugging, we now show a number decoration in the debug Activity Bar. This number represents the count of active debug sessions.
...
Semantic highlighting support for TypeScript and JavaScript is in development and not yet enabled by default. But you can try it out by adding the following setting:

```json
"editor.semanticHighlighting.enabled": true
```

When enabled, you will see that some identifiers get a new color and style and are now highlighted according to their resolved type. The syntax (TextMate) highlighter classifies many tokens as `variables`. These now turn into namespaces, classes, parameters and so on.
...
We've been working on a new debugger for Node.js and Chrome. It's installed by default on Insiders, and can be installed [from the marketplace](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug-nightly) in VS Code Stable. You can start using it with your existing launch configurations by enabling the `debug.javascript.usePreview` setting. Some new features we added this month include:

#### JavaScript Debug Terminal

Automatically debug commands you run in the new JavaScript Debug Terminal:

![](images/1_42/js-debug-terminal.gif)

*Theme: Earthsong, Font: Fira Code*

#### Code Lens for `npm` Scripts

The new debugger provides a "Debug" code lens in your package.json's `scripts` section that can run and debug scripts without extra configuration:

![](images/1_42/js-debug-code-lens.png)

You can configure it via the `debug.javascript.codelens.npmScripts` setting.

---

...and more! Make sure to [file issues](https://github.com/microsoft/vscode-js-debug/issues/new) if you run into any problems.

### Moving views from the sidebar to the panel

Work is actively being done to make it easier to customize the layout of VS Code. This milestone, some progress was made on this front and you can test it out under the setting:

```json
"workbench.view.experimental.allowMovingToNewContainer": true
```

With this setting enabled, a selection of views will now have a context menu entry to move them between the sidebar and the panel. Today, this setting affects the Outline view in the Explorer and views that are contributed via extensions. You also cannot combine these views in the panel like you can in the sidebar.
...
This is only a preview of the features and many aspects of it are likely to change including the limitations mentioned above. Feel free to check out progress in our Insiders version.

**Note about search** With the new generalized method of moving views, the old search setting `search.location` and command **Search: Toggle Search View Position** (`search.action.toggleSearchViewPosition`) are being deprecated. Your setting should migrate automatically, but you will need to use the new generalized method to move search going forward. You do not need to enable the preview setting above in order to move search with the new context menu entry.

### Timeline view

In this milestone, we've made progress on the new Timeline view, and have an early preview to share. This is a unified view for visualizing time-series events (e.g. commits, saves, test runs, etc) for a resource (file, folder, etc). To enable the Timeline view, you must be using the [Insiders Edition](https://code.visualstudio.com/insiders/) and then add the following setting:
...
In this early preview, the Timeline view shows the Git commit history of the active document, which is currently limited to 32 items. Clicking on one of those commits, will open a diff of the changes introduced by that commit. Extensions will also be able to [contribute their own timeline sources](#timeline-providers), which will be shown in this unified timeline view. Eventually, you will also be able to select (filter) which sources you'd like to see in the view.

Stay tuned, we have much more in store for this new feature. You can follow along by subscribing to [#84297](https://github.com/microsoft/vscode/issues/84297) and by watching for issues tagged with the [`timeline`](https://github.com/Microsoft/vscode/labels/timeline) label. And if you have ideas on other types of information you'd like to see in this view, let us know!

### Search Editor

We've been continuing work on our new search editor, with the goal of this iteration being to allow the search editor to serve as the primary search experience. To that end, we have implemented a UI for configuring the search editor's query:
...
As well as several commands for interacting with search editors which you can bind to keyboard shortcuts of your choice:
- `search.action.openNewEditor`: Open a new search editor in the active editor group.
- `search.action.openInEditor`: Copy the current results from a search in the viewlet or panel into a new search editor.

For more information and further updates on the ongoing development of search editors see [Issue 23931](https://github.com/microsoft/vscode/issues/23931).
...
For more information refer to the [Configuration API](https://github.com/microsoft/vscode/blob/be0aca7188ec6a76e7c2379758c0fbc1e9c21f7b/src/vs/vscode.d.ts#L8753)

### Extensions view context menu

You can now contribute commands to the extension's context menu in Extensions view using the new menu location `extension/context`.
...
The Following predefined contexts are supported, which you can use in `when` condition of the command:

* `extensionStatus==installed`
* `isBuiltinExtension`

### New DebugConfigurationProvider hook for accessing substituted variables
...
On the other hand this approach makes it difficult to use the **resolved variable values** in the `resolveDebugConfiguration` hook, for example for validation or for implementing functionality that solely exists in the extension (and not in the debug adapter where the substituted values **are** available). See [issue #85206](https://github.com/microsoft/vscode/issues/85206) for an example. Some extensions have started to substitute variables on their own (which is often incomplete since it does not cover all cases).

From an end users perspective this is confusing: in some debug configuration attributes variables can be used (because they are processed in the debug adapter after substitution has occurred), in some attributes variables cannot be used (because they are processed in the extension where substitution has not yet occurred). [Issue #87450](https://github.com/microsoft/vscode/issues/87450) is a good example for the latter problem.
...
With the availability of this new API we are now asking all debug extension authors to review their current implementations of `DebugConfigurationProvider.resolveDebugConfiguration` carefully and to move all code that does not introduce new variables but would profit from being run on the substituted variables into the new method `resolveDebugConfigurationWithSubstitutedVariables`.

#### Support for implementing a debug adapter inside an extension

The existing debugging extension API supports to register a `DebugAdapterDescriptorFactory` for running a debug adapter:
...
Based on this interface an implementation of a debug adapter would start with this code:
...
Since the debug adapter protocol (available as a [node module `vscode-debugprotocol`](https://www.npmjs.com/package/vscode-debugprotocol)) is a compatible superset of the `vscode.DebugAdapter` interface, you can import the module if you need the full protocol and want to implement a debug adapter all by yourself.

A much simpler alternative is to (continue to) use our default implementation of a debug adapter available as a [node module `vscode-debugadapter`](https://www.npmjs.com/package/vscode-debugadapter). Starting with version 1.38.0-pre.4 the `DebugSession` (or `LoggingDebugSession`) introduced in this module is compatible with the interface `vscode.DebugAdapter` defined in the extension API.

With this an existing debug adapter implementation can be easily used like this:
...
For details please see [Mock debug](https://github.com/microsoft/vscode-mock-debug/blob/668fa6f5db95dbb76825d4eb670ab0d305050c3b/src/extension.ts#L144-L150).
...
If you are interested in adding semantic highlighting to your language, here's a summary of the concepts and the added APIs:
[Semantic Highlighting Overview](https://github.com/microsoft/vscode/wiki/Semantic-Highlighting-Overview)

### Authentication providers

Some extensions require authenticating to other services. This [proposal](https://github.com/microsoft/vscode/issues/88309) would make it possible for extensions to register authentication providers that can be accessed by other extensions, providing a common mechanism for performing authentication.

### Timeline providers

Extension will soon be able contribute their own sources into the new [Timeline view](#timeline-view). Interested in adding your own timeline source? Check out this early [proposal](https://github.com/microsoft/vscode/issues/84297) and provide any feedback you have.
