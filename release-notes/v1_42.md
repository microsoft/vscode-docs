...
* **[Preview features](#preview-features)** - Get first looks at the Timeline view, Search editor, semantic highlighting, and more.
...
Windows and macOS now support light and dark color schemes. A new setting (`window.autoDetectColorScheme`) has been added to listen to a change to the OS's color scheme and switch to a matching theme in VS Code accordingly.

To customize the themes that are taken when such a color schemes happens, you can preferred light and dark theme:
...
![Folding shift click](images/1_42/folding-shift-click.gif)
...
Tasks declared in `tasks.json` are now supported at the User Settings level. You can run the **Tasks: Open User Tasks** command to create user level tasks. These tasks will be available across all folders and workspaces. Only the `shell` and `process` task types are supported here.
