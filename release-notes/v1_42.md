...
Windows and MacOS now support light and dark color schemes. A new setting (`window.autoDetectColorScheme`) has been added to listen to a change to the OS's color scheme and switch to a matching theme in VSCode accordingly.
...
`tasks.json` is now supported at a User Settings level. You can run the **Tasks: Open User Tasks** command to create user level tasks. These tasks will be available across all folders and workspaces. Only the `shell` and `process` task types are supported here.
