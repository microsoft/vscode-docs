...
Semantic highlighting for TypeScript and JavaScript are in development but not yet enabled by default. You can try it out by adding the following setting:

`"editor.semanticHighlighting.enabled": true`

When enabled, you will see that some identifier get a new color and style and are now highlighted according to their resolved type. The syntax (TextMate) highlighter classifies many tokens as `variables`. These now turn into namespaces, classes, parameters and so on.
