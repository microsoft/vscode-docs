...
To customize the themes that are used when a color scheme changes, you can set the preferred light and dark theme:
...
By default, the editor computes word wrap breaks by assuming that all characters have the same width. This assumption allows the default algorithm to be quite fast, but its results are poor when using a non-monospace font, or when writing text in a script where characters vary in width. There is a new setting, `editor.wrappingStrategy`, that can be set to `advanced` to have the editor delegate word wrap break computation to the browser.
...
We have started looking into making VS Code accessible on Linux with the Orca screen reader. However, since this work requires Electron 7, we expect to have more progress in the next stable release. In the meantime, for Linux accessibility you can use our [Insiders](https://code.visualstudio.com/insiders) version and provide feedback.
...
Based on user feedback, we also show the Breakpoints pane in the starting view, if you have at least one breakpoint set.
...
Automatically debug commands you run in the new JavaScript Debug terminal, which you can open via the **Debug: Create JavaScript Debug Terminal** command:
...
**Attention:** In this release we have finished the effort to provide extension APIs for creating debug adapters. With this we are now announcing that we will finally **remove support for two deprecated attributes of the `debuggers` contribution point in the March release**:
- Attribute `adapterExecutableCommand`: please use the extension APIs `vscode.DebugAdapterDescriptorFactory` and a `vscode.DebugAdapterExecutable` instead. Mock-Debug [shows how to use the API](https://github.com/microsoft/vscode-mock-debug/blob/28b4fde41228b0aabe93eab9064992e276859405/src/extension.ts#L91-L116).
- Attribute `enableBreakpointsFor`: please use the `breakpoints` contribution point instead. The conversion needs to be done in the "package.json". If the current (deprecated) JSON looks like this:
  ```json
  "contributes": {
    "debuggers": [{
      "type": "csharp",
  ...
  ```
  the conversion should result in this:
  ```json
  "contributes": {
    "breakpoints": [
      { "language": "csharp" },
...
We have fixed various issues around mouse handling and dynamic CSS to allow that the Monaco Editor can now be hosted inside a [Shadow DOM](https://developer.mozilla.org/docs/Web/Web_Components/Using_shadow_DOM) API [shadow root](https://developer.mozilla.org/docs/Web/API/Element/shadowRoot).
