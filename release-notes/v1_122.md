...
* [GitHub Copilot moves to usage-based billing](#usage-based-billing-with-github-ai-credits): GitHub Copilot moves to usage-based billing with GitHub AI Credits, with updated usage visibility in VS Code.
...
<div class="toc-nav-layout">
  <nav id="toc-nav">
    ...
    <ul>
      <li><a href="#github-copilot">GitHub Copilot moves to usage based billing</a></li>
...
GitHub Copilot has moved from request-based billing to usage-based billing. Instead of counting premium requests, Copilot now uses **GitHub AI Credits** as the billing unit, where 1 AI credit = $0.01 USD.

Under the new model, each interaction consumes tokens (input, output, and cached), and the total cost is converted into AI credits based on the model used. More complex interactions and more capable models consume more credits, while lightweight models and simpler tasks use fewer. Code completions and next edit suggestions remain unlimited for all paid plans and are not billed in AI credits.

Learn more about the transition in the GitHub documentation for [usage-based billing for individuals](https://docs.github.com/en/copilot/concepts/billing/usage-based-billing-for-individuals) and [usage-based billing for organizations and enterprises](https://docs.github.com/en/copilot/concepts/billing/usage-based-billing-for-organizations-and-enterprises).

### Updated Copilot status dashboard

The Copilot status dashboard in the VS Code Status Bar now reflects usage-based billing. You can view your AI credits consumption so you can monitor your spending at a glance.

// TODO add screenshot

### Model costs in the model picker

The model picker now displays cost information to help you make informed model choices. Different models have different costs per token type, so choosing the right model for your task can help extend your usage allowance.

You can view all available models, their capabilities, context sizes, and billing details in the Language Models editor. Open it from the model picker by selecting **Manage Models**, or run the **Chat: Manage Language Models** command from the Command Palette.

### Per-turn credit cost in chat

Each chat response now shows the AI credits consumed for that turn. After a response completes, hover over the response header to see a breakdown that includes the model used, and the total AI credits cost for that interaction.

When your AI credits allowance is running low, VS Code shows a warning in the chat input area. You can also check your remaining balance at any time through the [Copilot status dashboard](#updated-copilot-status-dashboard) in the Status Bar.
...
VS Code now supports 1 million token context windows for compatible Anthropic and OpenAI models. This expanded context window enables you to work with significantly larger codebases and longer conversations without losing important context.

The expanded context window is available when using supported models, such as Claude Opus 4.7 and GPT-5.4. You can monitor how much of the context window is in use through the context window usage indicator in the chat input area.

> **Note**: Larger context windows consume more tokens per interaction, which increases AI credits usage under [usage-based billing](#github-copilot-moves-to-usage-based-billing). Use the context window indicator to stay aware of your token consumption.
