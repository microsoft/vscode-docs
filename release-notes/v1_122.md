...
Thanks to feedback from Insiders users, enabling terminal sandboxing via `setting(chat.agent.sandbox.enabled)` no longer has any effect when you use **Bypass Approvals** or **Autopilot** mode.

Previously, commands in these modes were attempted in the sandbox first. If the command failed with a non-zero exit code, it was automatically retried outside the sandbox. Since approvals were bypassed anyway, this did not provide a meaningful safety benefit and could make the behavior harder to reason about.

Sandboxing now only applies when you use **Default Approvals**, where it can provide a clearer balance between safety and usefulness.
...
Now in **Manage Language Models** view, supported provider groups expose targeted actions based on the provider schema: **Update API Key**, **Add Model**, **Rename Group**, and **Delete**. This makes common provider maintenance tasks faster while keeping you in the same flow.
