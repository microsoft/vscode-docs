...
Thanks to feedback from Insiders, enabling terminal sandboxing via chat.agent.sandbox.enabled no longer has any effect when using **Bypass Approvals** or **Autopilot** mode.
...
Sandboxing now only applies when using **Default Approvals**, where it can provide a clearer balance between safety and usefulness.
