...
If you use self-hosted, enterprise, or other compatible AI endpoints, you can now configure them directly in **VS Code Stable** with the Custom Endpoint provider. It lets you connect models that implement Chat Completions, Responses, or Messages APIs, so you can use chat with your own endpoint and API key.

To learn how to set it up, see [Add a custom endpoint model](https://code.visualstudio.com/docs/copilot/customization/language-models#add-a-custom-endpoint-model).

### BYOK works without a GitHub sign-in

Bring Your Own Key (BYOK) works without a GitHub sign-in, so you can use chat in air-gapped or restricted environments where signing in to GitHub is not an option. Configure a provider once, and chat, tools, and MCP servers operate against your own endpoint and API key.

To get started, run **Manage Language Models** from the Command Palette and add a provider such as Anthropic, Azure, Gemini, OpenAI, Ollama, OpenRouter, or a [custom endpoint](https://code.visualstudio.com/docs/copilot/customization/language-models#add-a-custom-endpoint-model). Once at least one BYOK model is configured, the Chat view becomes available even though you are signed out, and you can send messages right away.

Built-in tools and any configured MCP servers continue to work as they do with a Copilot subscription. Requests go directly to your provider. To verify, open the **Chat Debug View** with **Developer: Show Chat Debug View** and inspect the outgoing request.

> **Note**: Inline code completions and next edit suggestions (NES) still require a GitHub sign-in. BYOK powers chat, tools, and MCP servers only.
...
* Select **Configure** to open settings and pick a BYOK model for `setting(chat.utilityModel)` and `setting(chat.utilitySmallModel)`. This unlocks the full set of AI features.
* Dismiss the notification if you only need to chat. The utility-driven features remain inactive until you set a model.
...
### Add Screenshot to Chat in the Integrated Browser

The new "Add Screenshot to Chat" feature lets you attach a screenshot of the current browser viewport to the chat as context.
