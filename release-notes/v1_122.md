...
* **Local VS Code harness (Insiders only)**: We're continuing to iterate on the ability to use the local harness in the Agents window, including a new custom agent picker. This is an early, experimental feature available only in VS Code Insiders. To try it out, enable the `setting(sessions.chat.localAgent.enabled)` setting in Insiders.
