...
**Note:** you can also press and hold the `Shift` key while scrolling to get the opposite behavior (for example, you can switch to tabs even with this setting being turned off).

### Allow customize the window title separator

A new setting `window.titleSeparator` lets you change the separator that is used in the window title. By default a dash `-` is used.
...
In June 2019, we [split the `vscode` module into `@types/vscode` and `vscode-test`](https://code.visualstudio.com/updates/v1_36#_splitting-vscode-package-into-typesvscode-and-vscodetest) in light of the `event-stream` incident. Today, a [security alert for `minimist`](https://snyk.io/blog/prototype-pollution-minimist/) has caused security alerts for extensions that still depend on `vscode`, which depends on `mocha@5.2.0` and therefore `minimist@0.0.8`. Unfortunately, `mocha@5.2.0` no longer receive updates and upgrading to a new `mocha` version would break existing functionalities.
