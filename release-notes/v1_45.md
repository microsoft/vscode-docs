...
* Status Bar is now accessible and when focused, screen readers will read its content.
...
The Debug Quick Pick can be opened by typing 'debug' in the Quick Pick or by triggering the **Debug: Select and Start Debugging** command. Once a dynamic configuration provider is chosen, VS Code presents all the dynamic configurations available and a new debugging session is started once one is chosen.
