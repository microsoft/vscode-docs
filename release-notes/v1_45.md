...
The `editor.semanticTokenColorCustomizations` setting allows you to override the theme color values to customize the active theme.

Here is an example that adds semantic styling to all themes:
...
The setting above colors all constants with `#35166d` and makes all occurrences of symbols from a default library (for example `Math`, `setTimeout`) bold.

This example adds semantic styling to the Dark+ themes:
...
See the [Product Icon Themes](https://code.visualstudio.com/api/extension-guides/product-icon-theme) documentation for more details and have a look at the [Product Color Theme Sample](https://github.com/Microsoft/vscode-extension-samples/tree/master/product-icon-theme-sample)
