...
After focussing on housekeeping we have also addressed several feature requests and community [pull requests](#thank-you). Read on to learn about new features and settings.
...
Explorer now supports `Undo` and `Redo` for all file operations: delete, rename, copy, move, new file and new folder. Make sure the focus is in the Explorer and trigger the `Undo` or `Redo` commands and your last file operation will be undone or redone respectivly.

![Explorer Undo](images/1_52/explorer-undo.gif)

### Progress for long running operations in Explorer

We now show progress in the Explorer and Status bar for long running File operations (longer than 500ms). There is also an inital support for cancelling these long running operations. This should be particularly helpful when copying large folders or downloading resources from remote.
...
This should help when you have lots of files open and you want to match up files that are associated (e.g. hello.component.js and hello.component.html).
...
Now the workbench progress is accessible. We set the appropriate aria roles so the screen reader reads progress for all long running operations in VS Code. Most screen readers by default only announce progress if it is ongoing for more than 10 seconds.
...
Exception information area in the editor is now accessible. While debugging once an exception is hit, VS Code will automatically move focus to the Exception area which will make Screen Readers read the exception details and the stack frame.

It is now also possible to close the exception widget area using the `Esc` key or the X action in the top right of the Exception area.
...
We now show the currently debugged line in the overview ruler on the right of the editor. For this decoration we use the already present `editor.stackFrameHighlightBackground` color. In addition to this we also show the currently focussed debugged line in the overview ruler using the `editor.focusedStackFrameHighlightBackground` color (focussed debugged lines are non top stack frame locations which are manually selected in the CALL STACK view).


#### Automatic debug configurations improvements

 Previously, we introduced a feature so that extension authors can analyze the current project and offer debug configurations automatically. These debug configurations are dynamically created and so they do not show up in the `launch.json` configuration file. From this milestone we are also preserving recently used automatic debug configurations across VS Code reloads to make the whole experience smoother for the user. In a case that an automatic debug configuration is no longer valid after a reload, VS Code will ask the user to pick a new automatic configuration from the same provider once debugging is started.
...
Debug console now collapses identical output by showing the count of how many times it occurred. This feature should make it much easier to make sense of very repetitive program output.

![Debug Console collapse](images/1_52/repl-collapse.png)


### new Variable `fileWorkspaceFolder`
TODO@weinand
...
In this milestone, we added quite number of improvements to Extensions view and Extension editor.
...
- Extension editor now shows all actions an extension shows in Extensions view

![Extension Editor Actions](images/1_52/extension-editor-actions.png)

*Theme: [GitHub Light](https://marketplace.visualstudio.com/items?itemName=github.github-vscode-theme)*

- Feature Contributions tab in the extension editor shows Activation Events of an extension now

![Extension Activation Events](images/1_52/extension-activation-events.png)

*Theme: [GitHub Light](https://marketplace.visualstudio.com/items?itemName=github.github-vscode-theme)*

- You can now disable an extension and its dependents together. Following notification with **Disable All** action is shown when you disable an extension with dependents.

![Disable Extension](images/1_52/extension-disable.png)

*Theme: [GitHub Light](https://marketplace.visualstudio.com/items?itemName=github.github-vscode-theme)*

- You can add or remove an extension from workspace recommendations using **Add to Workspace Recommendations** or **Remove from Workspace Recommendations** actions available in extension's context menu in Extensions view

![Extension Add Recommendation](images/1_52/extension-add-recommendation.png)

*Theme: [GitHub Light](https://marketplace.visualstudio.com/items?itemName=github.github-vscode-theme)*

- Extension recommendation notifications now show the install dropdown action like in Extensions view when Settings Sync is enabled.

![Extension Recommendation Notifications](images/1_52/extension-recommendation-notification.png)

- Simplified configuring extension recommendations by deprecating `extensions.showRecommendationsOnlyOnDemand` setting.
  - Use `extensions.ignoreRecommendations` setting to control extension recommendation notifications
  - Use Extensions view's visibility actions to hide Recommended view by default.


### Keyboard Shortcuts Editor

Following improvements are added to the Keyboard Shortcuts Editor in this milestone

- You can configure keybinding of a command from Command Pallette.

![Configure Keybinding](images/1_52/configure-keybinding.gif)

*Theme: [GitHub Light](https://marketplace.visualstudio.com/items?itemName=github.github-vscode-theme)*

- You can add several keybindings to a command from Keyboard Shortcuts editor

![Add Keybinding](images/1_52/add-keybinding.png)

*Theme: [GitHub Light](https://marketplace.visualstudio.com/items?itemName=github.github-vscode-theme)*

- You can filter by particular column from search input:
...
- You can navigate search history in Keyboard Shortcuts editor using `kbstyle(Up)` and `kbstyle(Down)` arrows.
## IntelliSense

### Word-based suggestions from other documents

VS Code supports simple word-based suggestion. They are useful when a language service isn't available or when a language service cannot compute results, like when typing inside comments. With this release VS Code can be configured to suggest words from other open files as well. Use the `editor.wordBasedSuggestionsMode`-setting with `allDocuments` to suggest words from all open files, use `matchingDocuments` to suggest words from open files of the same language, and use `currentDocument` (default) to only suggest words from the current file.
...
Suggestions in VS Code have details that show in a separate view beside the suggestion. That details view can be turned off and then a preview of the details show inline with suggestion. This sometimes takes away too much space and can now be configured via: `editor.suggest.showInlineDetails`.

### TypeScript suggestions show path

TypeScript can provide completions that also add an import-statement. However, when there are multiple symbols with an equal name it is hard to to pick the right completion. This release makes this simpler because  paths of auto-import completions are shown with the label.
...
Snippets contributed by extensions can now be hidden from IntelliSense. This feature is available from the "Insert Snippet" picker: "Select F1 > Insert Snippet", find an extension snippet that you want to hide, and select the eye-icon on the right.

![Hide Extension Snippet](images/1_52/snippet-hide.png)

Hidden snippet aren't showing in IntelliSense but can still be used via the "Insert Snippet", Last, when using settings sync, then hidden snippets will be synchronized across your devives.


### Snippets without prefix

When authoring snippets you can now omit the prefix-property. Snippets with a prefix don't show in IntelliSense but can used via "Insert Snippet".
...
The true power of VS Code is its extensions: theme extensions add pretty colors and icons, language extensions provide IntelliSense and enable code navigation, debugger extensions enable you to drill into bugs. This list goes on and on but sometimes extensions misbehave and sometimes it isn't obvious if an issue is caused by an extension and if so by which extension. Until today you needed to disable all extensions and then one by one re-enable extensions to find a bad extension. This process is now getting easy with a new feature called "Extension Bisect". It uses the binary search algorithm to quickly identify an extension that causes trouble. In essense, it disables half your extensions and asks you to check for the issue you were seeing. If the issue is gone the bad extension must have been in the list of disabled extensions, otherwise in the list of enabled extensions. This process is now repated on the list which must contain the bad extension until only a single extension is left.


You can start extension bisect via "F1 > Start Extension Bisect". It then guide through the process of repeatedly disabling and re-enabling extensions. After each reload you will be prompted to confirm if the issue is still there.

![Start Extension Bisect](images/1_52/ext_bisect1.png)
Initial "Extension Bisect" prompt

![Extension Bisect Prompting](images/1_52/ext_bisect2.png)
"Extension Bisect" will repeatedly ask you to check if the issue is still there or not. You can always abort via "Stop Bisect" and when haveing dismissed the bisect notification you can continue via "F1 > Continue Extension Bisect".

![Extension Bisect Done](images/1_52/ext_bisect3.png)
When "Extension Bisect" is done you are asked to file an issue against the extension. You can also keep the extension disabled from here.
...
Explorer operations like create file, rename file or delete file raise events which allow participation, eg `onWillCreateFiles`, `onWillRenameFiles`, or `onWillDeleteFiles`. This release adds support for undoing such explorer operations and you should know that aforementioned events are not fired when undoing an operation.
...
A proposed for a logging API existed. That proposed has been abandoned around 1 year ago and has now been removed. Extensions can use the output channel API for logging or, when using a custom logger, use `ExtensionContext#logUri`.
