...
You can start extension bisect via the **Start Extension Bisect** command. It then guide through the process of repeatedly disabling and re-enabling extensions. After each reload you will be prompted to confirm if the issue is still there.
...
"Extension Bisect" will repeatedly ask you to check if the issue is still there or not. You can always abort via "Stop Bisect" and when having dismissed the bisect notification you can continue via the **Continue Extension Bisect** command.
...
* Try out the step-by-step Microsoft Learn module covering [Use a Docker container as a development environment](https://docs.microsoft.com/learn/modules/use-docker-container-dev-env-vs-code/).
* And check out the "Tabs versus Spaces" YouTube video discussing [VS Code Remote Development](https://youtu.be/XkLjxr9iQ-8).
