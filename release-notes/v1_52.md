...
* You can now define and change environment variables from a terminal before launching a second window of VS Code and these will properly overwrite the variables from the first launch, effectively allowing for per-window environment variables (for example this enables to change the Node.js version via a tool such as `nvm` per window before starting).
...
Modify terminal settings by selecting `Configure Terminal Settings` in the terminal dropdown
