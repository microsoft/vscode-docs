...
![Check out detached](images/1_52/checkout-detached.gif)
...
![Show command output on error](images/1_52/git-show-output.png)

### Git: New options in `git.api.getRemoteSources` command

The `git.api.getRemoteSources` git API command options now support an optional `branch?: boolean` property which will make Git prompt the user to pick a branch from a remote source, given there's support from the remote source provider.

Additionally, it further supports a `providerName?: string` options which allows the caller to bypass the remote source choice by the user and use a specific one directly.
