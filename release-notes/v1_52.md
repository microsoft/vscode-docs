...
Explorer now supports `Undo` and `Redo` for all file operations: delete, rename, copy, move, new file and new folder. Make sure the focus is in the Explorer and trigger the `Undo` or `Redo` commands and your last file operation will be undone or redone respectivly. Keep in mind that we have separate undo stacks for the editor and the explorer and we choose which one to undo based on focus.
