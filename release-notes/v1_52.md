...
Work has resumed on the product icon themes. [Product icon themes](https://code.visualstudio.com/api/extension-guides/product-icon-theme) is a proposed feature that make it possible to style VS Code with a cutsom set of icons. That works for all icons the built-in views as well as all extensions that use icons from the Codicon library. We plan to make product icon theme public in the next milestone.
