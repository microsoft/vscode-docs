...
* **Chat**
  ...
  * TODO: custom modes improvements like language model selection ([Show more]()).

* **MCP**
  * MCP support is now generally available in VS Code ([Show more](#mcp-support-in-vs-code-is-generally-available)).
  * Easily configure and manage MCP servers from the Extensions view ([Show more]()).
  * MCP servers as first-class resources in profiles and Settings Sync ([Show more]()).
  * Install popular servers from our [curated MCP list](https://code.visualstudio.com/mcp).
...
The new **MCP Servers** view in the Extensions panel includes welcome content that links directly to the [popular MCP servers from our curated list](https://code.visualstudio.com/mcp). Visit the website to explore available MCP servers and click on **Install** on any MCP server. This will automatically launch VS Code and open the MCP server editor that displays the server's readme and manifest information. You can review the server details and click **Install** to add the server to your VS Code instance. Once installed, MCP servers automatically appear in your Extensions view under the **MCP SERVERS - INSTALLED** section, and their tools become available in the chat view's tools quick pick. This makes it easy to verify that your MCP server is working correctly and access its capabilities immediately. Following video demonstrates this:

<video src="images/1_102/mcp-servers-discovery-install.mp4" title="MCP Server Discovery and Installation" autoplay loop controls muted></video>

### MCP Server management view

The new **MCP SERVERS - INSTALLED** view in the Extensions panel provides comprehensive management capabilities for your installed MCP servers. This dedicated view makes it easy to monitor, configure, and control your MCP servers.

![Screenshot showing the MCP Servers management view with installed servers](images/1_102/mcp-servers-installed-view.png)

The view displays each installed MCP server with useful information and provides several management actions through the context menu:
...
- **Profile-specific**: Each VS Code profile maintains its own set of MCP servers, allowing you to have different server configurations for different workflows or projects
...
VS Code provides comprehensive migration support for users upgrading from the previous MCP server configuration format. The migration system includes:
...
* MCP view gear menu:
    ![Disconnect account shown in mcp view gear menu](images/1_102/mcp-view-signout.png)

* MCP editor gear menu:
    ![Disconnect account shown in mcp editor gear menu](images/1_102/mcp-editor-signout.png)

* MCP quick pick:
    ![Disconnect account shown in mcp quick pick menu](images/1_102/mcp-qp-signout.png)
...
![The Learn More action appears in the terminal suggest control status bar](images/1_102/terminal-suggest-discoverability.png)
...
You can learn more about these features in the [Remote Development release notes](https://github.com/microsoft/vscode-docs/blob/main/remote-release-notes/v1_102.md).
