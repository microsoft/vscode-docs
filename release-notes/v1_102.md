...
There's been several improvements and bug fixes in this area.

#### Configure language model

New in this milestone you can now also set the language model to be use in a mode.

![Model code completion](images/1_102/prompt-file-model-code-completion.png)


#### Improved editing support

The editor for modes, prompts and instruction files now supports completions, validation and hovers for all supported properties

![Tools hover](images/1_102/tools-hover.png)

![Model dialignistics](images/1_102/prompt-file-diagnostics.png)

#### Gear menu in the chat view

The 'Configure Chat' action in the chat view toolbar lets you manage cutom modes as well as reusable instructions, prompts and tools sets:
![Configure Chat menu](images/1_102/chat-gear.png)

Selecting 'Mode' will let you see all currently installed custom modes. You can open, create new or delete modes.

#### Import modes via a `vscode` link

You can import a mode file from an external link:
...
This will prompt for a destination folder and a name for the mode and then import the mode file from the URL in the link.

The same mechanism is also available for prompt and instruction files.
...
### Load custum instruction files on demand

Instruction files can be used to describe coding practices and project requirements. Instruction can be manually or automatically included as context to chat requests
...
New in this release, the LM can load instruction on demand. Each request gets a list of all instruction files along with glob pattern and description. In this example, the LLM has not instructions for TypeScript file, so it asks for code style rules before creating a TypeScript file:

![Load instruction files on demand](images/1_102/instructions-loading-on-demand.png)
...
![Screenshot of the terminal, asking to run `cd C:\Github\Tyriar\xterm.js && echo hello` only runs `echo hello` when the current working directory already matches.](images/1_102/image.png)
...
![Screenshot showing the terminal suggest boosts main and master compared to other branch suggestions that start with 'ma'.](images/1_102/terminal-suggest-sorting.png)
