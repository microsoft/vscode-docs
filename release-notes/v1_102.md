...
The Copilot agent now understands which background terminals it has created and which tasks are actively running. The agent can read task output using the new `GetTaskOutput` tool. This helps prevent running duplicate tasks and improves workflow automation.
...
The `Learn More` action in the terminal suggest widget is now highlighted for the first 10 times or if the widget is shown for 10 seconds. This helps you discover how to configure or disable the suggest widget.
