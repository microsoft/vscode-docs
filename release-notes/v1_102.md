...
### MCP: Quick management of authentication

You are now able to Sign Out/Disconnect accounts from the MCP gear menu & quick picks.
...
"Disconnect" will be shown when the account is used by either other MCP servers or extensions while "Sign Out" will be shown when the account is only used by the MCP server. Sign out will completely remove the account from VS Code, while disconnect will only remove the access to the account from the MCP server.
