...
As part of the rollout of our new testing architecture, the test explorer panel now supports error-tolerant Pytest discovery. When an error that can be handled is thrown by Pytest during discovery, for example, an error from an unknown import, all other tests will still be discovered outside of the file with the error. This feature is only available on the testing rewrite which, as a reminder, we are rolling out under an experiment. It is currently active for 100% of vscode insiders and 25% of stable users. The rewrite will be adopted shortly but for now the setting to opt in and out still exists called `pythonTestAdapter`.
