...
We now have an accessible view for notifications, inline chat responses and Jupyter notebook output.
...
We have made performance improvements for large cell outputs that are streamed during execution. This will reduce the amount of rendering activity and currently only applies to scrollable cell outputs.
...
As part of the rollout of our new testing architecture, the test explorer panel now supports error-tolerant Pytest discovery. When an error that can be handled is thrown by Pytest during discovery, for example, an error from an unknown import, all other tests will still be discovered outside of the file with the error. This feature is only available on the testing rewrite which, as a reminder, we are rolling out under an experiment. The rewrite is currently active for 100% of insiders and 25% of stable users but will be universally adopted shortly. In the meantime the setting to opt in and out still exists called `pythonTestAdapter`.
