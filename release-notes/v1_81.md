...
In the new diff editor, the *Difference Review Pane* was renamed to *Accessible Diff Viewer*.
The accessible diff viewer adresses accessibility shortcomings of the diff editor and can be opened with F7 or the diff editor context menu.
With this rename, we also fixed some bugs and added some new features, such as the accessibility help or jumping to the modified editor from the viewer.
...
In the [1.76 update](https://code.visualstudio.com/updates/v1_76#_asynchronous-tokenization), we implemented asynchronous tokenization on a web-worker to improve the performance of the renderer process. Since then, we have fixed a couple of bugs caused by rare race conditions. With this release, we are starting to roll out this feature to stable users.
...
In this iteration, we continued to work on the new diff editor and plan to gradually roll it out to stable users.
You can try it out immediately by setting `"diffEditor.experimental.useVersion2": true`.
The new diff editor includes several new features and bug fixes.
...
### Inner-hunk alignment

The new diff editor now aligns text within hunks, making it much easier to review diffs where indentation has changed and lines have been inserted.

Before (without inner-hunk alignment and whitespace diffing enabled):

![Before](images/1_81/diff-editor-without-hunk-alignment.png)

After (with inner-hunk alignment):

![After](images/1_81/diff-editor-with-hunk-alignment.png)

### The new diff algorithm is now enabled by default

We have completed the roll-out of the new diff algorithm, which is now the default in VS Code.
You can read about some of the improvements in [past changelogs](https://code.visualstudio.com/updates/v1_78#_diff-algorithm-improvements).

### Diff noise reduction

In this update, we added some heuristics to the new diff algorithm to reduce the probability of matching unrelated words.

This is how it looked before (note how `this` matches on both sides, even though the matching is not helpful):

![Before](images/1_81/diff-algorithm-before.png)

This is how it looks with the improvements (note that there are much less unrelated matches):

![After](images/1_81/diff-algorithm-after.png)

### Switch Sides

When using the side-by-side view of the new diff editor, there is now a new command *Diff Editor: Switch Sides* that allows you to jump from one side to the other.
When switching sides, the cursor position and the current selection in the active editor are mapped to the other side as well.

### And there is more

Checkout the changelog of the [1.80 release](https://code.visualstudio.com/updates/v1_80#_new-diff-editor) for a more detailed list of what has changed in the new diff editor!
...
This release brings the first iteration of a Sticky Scroll widget for the notebook editor. This UI element will show notebook users the relevant markdown hearders to the cell or cell output that they are currently viewing. Clicking that Sticky "line" will focus that cell, allowing you to navigate the notebook quickly. This feature can be enabled via `notebook.stickyScroll.enabled` or the command palette.
...
Additionally, we have introduced an experimental setting, `chat.experimental.defaultMode`, to control what parts of chat are shown. It has 3 options:
...
We've made the following usability improvements to slash commands in panel and inline chat:
1. Slash commands now render as blocks and can be deleted with a single backspace in both inline and panel chat.
2. Accepting slash commands from the suggest widget such as `/clear` and `/help` will now also execute them, removing the need to explicitly submit the chat request after accepting a completion.
...
<video src="images/1_81/slash-commands.mp4" controls title="Usability improvements to slash commands"></video>
...
#### New debugger configuration for running files with arguments
...
Now when you want to debug (`kb(workbench.action.debug.start`) a Python file that requires command-line arguments, a prompt will appear allowing you to enter the desired arguments to pass to the Python file.
