...
We added an experimental `/setupTests` slash command that can recommend a testing framework for your workspace, provide steps to setup and configure it, and recommend a VS Code extension to provide [UI integration](/docs/editor/testing). `/setupTests` and extensions can be recommended when triggering `/tests` generation if it doesn't look like such an integration has been set up yet.
...
We've added specialized logic to help diagnose the reasons why unit tests fail. This logic is triggered in some scenarios in `/fix`, and can be explicitly called via the `/fixTestFailure`. It is on by default but can be disabled via the setting `setting(github.copilot.chat.fixTestFailure.enabled)`.

### Accept & Run in Notebook

When using Copilot to generate code in a notebook, you can now accept the response and run it directly from the Inline Chat toolbar.

<video src="images/1_94/notebook-accept-run.mp4" title="Title" autoplay loop controls muted></video>

### Attach variable in chat requests in Notebook

When using Copilot in Notebook, you can now attach variables from Jupyter kernel in your requests, via either `#kernelVariable` completions, or **Attach Context** (`kb(workbench.action.chat.attachContext)`) action from the Inline Chat widget. It gives you more precise control over the context of your requests in Jupyter Notebooks.
...
### Folders & Workspaces section in the Profiles Editor

In this milestone, we introduced a **Folders & Workspaces** section in the Profiles Editor. This section displays all folders and workspaces associated to a specific profile. It also includes actions to add or modify folders, and also to open the folder or workspace in a new window.

![Folders & Workspaces section](images/1_94/profiles-editor-folders-workspaces.png)

### Update extensions across all profiles

In this milestone, we introduced the ability to update extensions across all profiles. This is useful for users who have multiple profiles and want to keep their extensions versions in sync.
...
The notebook diff view now respects the setting `diffEditor.hideUnchangedRegions.enabled`. When enabled, unchanged code blocks are collapsed by default.

![](./images/1_94/notebook-unchanged-region.png)

### Notebook serialization in web worker

This release introduces an experimental feature that enables notebook serialization in a web worker. This can help reduce main thread blocking time in the Extension Host process when working with large notebooks. By default, this feature is disabled but can be enabled by setting `ipynb.experimental.serialization` to `true`.
...
VS Code supports new text styling capabilities from the Debug Adapter Protocol. This allows for data in the Variables view, Watch view, hovers, and Debug Console to be colorized via ANSI escape sequences.
...
#### Launch.json Autocomplete for Node Commands
...
Uh oh! Looks like we've forgotten to call `.test()` on the regular expression, meaning that the `if` conditional always evaluates to true. That's not what we wanted.
...
#### Setting for Shell Integration in Python Terminal REPL
The Python extension now includes a setting to opt in and out of PYTHONSTARTUP script, which runs before user types 'python' or any other way to launch Python REPL in Terminal. Opting in essentially allows users to use features from terminal shell integrations such as command decorations, re-run command, run recent commands, etc if they are in Mac or Linux. User can enable this via setting "python.terminal.shellIntegration.enabled": true in their settings.json.
