...
You can now easily attach additional files as context by dragging files or editor tabs from the workbench directly into chat. By holding `Shift`, you can drop a file into an inline chat instead of opening it in the editor.
...
### Refreshed Chat view user experience

We've refreshed the Chat view user experience with a clean new welcome view - you can view it when you start a new session by entering `/clear`.

![welcome view](images/1_94/chat-welcome.png)

We've also updated the layout of the chat input box to make it easier for your to find chat participants. There a several built-in chat participants and extensions can contribute their own chat participants. We've added a `@` button that brings up the list of chat participants that you've installed. You can also still find these by typing `/` or `@`, like you already could before.
...
We've introduced the command **Comments: Focus Comment on Current Line** that lets to quickly move to the comment control from the editor by using the keyboard.
...
We’ve improved the "Find" feature in the Explorer to make it easier to search for files in large projects. You can open the Find widget in the File Explorer using the `kb(list.find)` keybinding. While searching, you can switch between fuzzy matching and continuous matching for more flexible results. Some context menu actions will be temporarily disabled during searches. Stay tuned for more improvements coming soon!
...
Last milestone we have added the new **Source Control Graph** view to the **Source Control** viewlet. This milestone we have been working on expanding the functionality available in the newly added view as well as polishing the layout of the view.

#### Repository picker

When opening a folder/workspace that contains multiple repositories, the **Source Control Graph** view title will show a repository picker. By default the **Source Control Graph** view shows the active repository matching the source control status bar actions. Users can use the repository picker to "lock" the **Source Control Graph** view to a particular repository.

#### History Item Reference picker

This milestone we have added a new history item reference picker to the **Source Control Graph** view title. By default the history item reference picker is set to `Auto`, and the view renders the graph for the current history item reference, it's remote, and an optional base. Users can use the history item reference picker to filter the history items shown in the graph.

#### History Item Actions

This milestone we have expanded the list of actions that are available in the history item context menu. We have added actions to create a new branch/tag from a history item, cherry-pick a history item, and checkout detached a history item.

#### Settings

This milestone we have added a set of new settings to allow users to customize the **Source Control Graph** view:
  * `setting(scm.graph.badges)` - controls which badges are shown in the Source Control Graph view
  * `setting(scm.graph.pageOnScroll)` - controls whether the Source Control Graph view will load the next page of items when you scroll to the end of the list
  * `setting(scm.graph.pageSize)` - the number of items to show in the Source Control Graph view by default and when loading more items
...
Beginning of September 2024, we have [completed the switch](https://github.com/microsoft/vscode/issues/196795) from yarn to npm for package management in the [microsoft/vscode](https://github.com/microsoft/vscode) repo. This decision was based on the specific requirements for VS Code and center around these criteria:

1) Performance: we initially moved to yarn because of performance reasons and npm can now also meet our requirements for performance.
2) Security: we make our supply chain more secure by limiting exposure and reducing the number of tools we depend upon.
