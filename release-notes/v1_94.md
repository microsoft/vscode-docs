...
We are experimenting with a command center entry for chat. It provides quick access to all chat relavant commands, like starting chat or attaching context. You can enable this via `setting(chat.commandCenter.enabled)` but note that the command center itself needs to be enabled as well.
