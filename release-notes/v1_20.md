...
Read more on [how to control vendor prefix in emmet](/docs/editor/emmet.md#include-vendor-prefixes).
...
Other notable bug fixes in Emmet:
  ...
  * `Emmet: Expand Abbreviation` command doesnt expand abbreviation that contains unescaped single quotes. [Bug #38807](https://github.com/Microsoft/vscode/issues/38807)
...
MacOS has a global find clipboard that allows apps to share text for searching. It's already supported by many MacOS apps like Chrome, TextEdit, and Sublime, and is now enabled by default in the editor find widget. When you search for text in another app, you will notice that text automatically populated in the find widget.

You can use the `"editor.find.globalFindClipboard"` setting to disable it in the editor. It's also supported by the search viewlet, disabled by default, and can be enabled with the `"search.globalFindClipboard"` setting.
...
<BR>For node-debug we've added a mechanism that tracks all subprocesses of a debuggee and tries to automatically attach to those processes that are launched in debug mode. This feature simplifies debugging of programs that fork or spawn Node.js processes like programs based on the "cluster" node module.
...
* [Yitong Ding (@yitongding)](https://github.com/yitongding):  Add selectedText to variable substitution section [PR #1317](https://github.com/Microsoft/vscode-docs/pull/1317)
