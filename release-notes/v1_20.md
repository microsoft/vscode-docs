...
* Thanks to @ironyman, **OPEN EDITORS** view can now be resized once it reaches the maximum size specified by `explorer.openEditors.visible`. Due to this change, the setting `explorer.openEditors.dynamicHeight` is no longer supported.
...
Some places where you can contribute commands to have now provide more fine grained group identifiers to better control the location of commands:
