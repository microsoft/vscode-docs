...
Command `workbench.action.moveEditorToFirstGroup` is a duplication of an already available command `revealFileInOS`. Therefore we are deprecating the `workbench.action.moveEditorToFirstGroup` command in the January release and plan to drop it in February.
