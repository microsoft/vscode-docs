...
Zoom in and out by clicking, using the scroll wheel, or pinching on a track pad. The current zoom level is shown in the Status Bar. Click on the zoom Status Bar item to quickly switch zoom levels or reset the view.
...
This milestone we added new proposed extension API for two other areas. We plan to add these API to stable in a future milestone once we are confident enough with it. We welcome any feedback on how it works for your extension.

> **Note:** These API are still proposed, so in order to use it, you must opt into it by adding a `"enableProposedApi": true` to `package.json` and you'll have to copy the [`vscode.proposed.d.ts`](https://github.com/Microsoft/vscode/blob/master/src/vs/vscode.proposed.d.ts) into your extension project. Also be aware that you cannot publish an extension to the Marketplace that uses the `enableProposedApi` attribute.
...
Your extension can use the methods on `Logger` to write log messages with some log level, and the user can use the `Set Log Level` command to specify at which minimum level should log messages be recorded.

You can find the log files by running the `Open Logs Folder` command. That folder will contain a folder for each running instance of an extension that has written logs.
