...
As [announced](https://code.visualstudio.com/updates/v1_18#_debug-api-updates) in the previous release, we have deprecated the  `vscode.logToDebugConsole` command in favor or real debug API. Due to that we will remove support for the `vscode.logToDebugConsole` command in the next milestone.
