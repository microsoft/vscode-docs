...
In 1.19 [we introduced the "global find clipboard"](https://code.visualstudio.com/updates/v1_19#_global-find-clipboard-on-macos) which makes it easier to share search text across apps. This is now disabled by default, but can be enabled with the `"editor.find.globalFindClipboard"` setting. It's also now supported by the Search viewlet, disabled by default, and can be enabled with the `"search.globalFindClipboard"` setting.
