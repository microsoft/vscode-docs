...
The setting `problems.decorations.enabled` allow you to enable/disable the error/warning decorations.
...
With multi-root workspaces its quite common to have back-end and front-end projects as separate folders loaded in the same workspace. Now compound launch configurations are able to reference a launch configuration from another workspace folder. In case configurations do not have unique names a compound can reference them using the following syntax:
```json
configurations: [{
  "name": "Launch Program",
  "folder": "Server"
...
```
 ...
 We now support having workspace specific launch configurations. In the workspace configuration file, it is now possible ot have a `"launch"` section. Note that in the workspace configuration most of the variables need to be scoped per workspace folder since they are not belonging to any specific folder. More about substituting variables scoped per workspace folder is specified in the section [above](#Variable-subsitution-scoped-per-workspace-folder).
 Here's an example of a worspace configuration `"launch"` section which specifies one launch configuration and a compound which references launch configurations inside the workspace folders.
...
<BR>For node-debug we've added a mechanism that tracks all subprocesses of a debuggee and tries to automatically attach to those that are launched in debug mode. This feature simplifies debugging of programs that fork or spawn Node.js processes like done in programs that use the "cluster" node module.
...
**"Nvm" Support**
<BR>If you are using "nvm" (or "nvm-windows") to manage your Node.js versions it is now possible to specify a `runtimeVersion` attribute in a launch configuration for selecting a specific version of Node.js. In this version
...
**Replace 'adapterExecutableCommand' with real debug API**
TODO@AW

**Command `vscode.logToDebugConsole` will be removed**

As [announced](https://code.visualstudio.com/updates/v1_18#_debug-api-updates) in the previous release, we have deprecated the  `vscode.logToDebugConsole` command in favor of real debug API. Due to that we will remove support for the `vscode.logToDebugConsole` command as soon it is no longer used.

**Run DA inside extension**
TODO@AW
...
* [42050](https://github.com/Microsoft/vscode/issues/42050): Debug: replace notification about "errors in workspace" with a modal dialog
