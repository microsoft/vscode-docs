...
During this milestone the UX team worked on redesigning the notifications experience in VS Code. Some of the shortcomings of our current design are:
* notifications are always on top of the editors, we want to explore to allow for moving/resizing them
* once a notification is dismissed, there is no way to get back to it
* it is not easily possible to read a notification if its text exceeds the size of the notification
* multiple notifications with buttons are hard to handle and look out of place

![Notifications](images/1_20/notifications.png)

We plan to start development work on these designs in the February milestone. More details about the design can be found in this [GitHub issue](https://github.com/Microsoft/vscode/issues/22388#issuecomment-361915332) and we are happy for your feedback.
...
There is now the option for global snippets - that is snippets that aren't scoped to a single language but snippets that can target many or all kinds of files. Hit 'F1 > Configure User Snippets' and select 'New Global Snippets file'. That will open a `.code-snippets`-file which allow you to define new snippets, use the `scope`-attribute to list the languages that are targeted by a snippet. For instance, the snippet below shows the copyright-header we add to JS and TS files:
...
We didn't forget extensions-authors and when crafting snippet extensions you can also contribute global snippets. Simply use a `code-snippets`-file and omit the language property when contributing.

## More Snippet Variables

We have added new snippet variables to read your clipboard and to insert the current date and time. Use `CLIPBOARD` and for date and time combine any of these variables: `CURRENT_YEAR`, `CURRENT_YEAR_SHORT`, `CURRENT_MONTH`, `CURRENT_DATE`, `CURRENT_HOUR`, `CURRENT_MINUTE`, `CURRENT_SECOND`.

## Suggest Memory 2

We have refined how suggestions are memorized and we have added a new setting for this: `editor.suggestSelection` controls how suggestions are selected in the UI. You can make sure the top item is always selected (`first`), you can make it select an item you have inserted in the past (`recentlyUsed`), or you can selected items based on prefixes you have completed in the last (`recentlyUsedByPrefix`). A more complete explaintation and images(!) can be found here: https://github.com/Microsoft/vscode/issues/41060#issuecomment-360529596
...
### New Context Key Operator

Context keys allow users to control when keybindings are active. Also, extension authors customize menus with them. They are also referred to as `when`-clauses because they define *when* something is active or enabled. With this release a new operator for `when`-clauses is added. The expression `key =~ value` treats the right hand side as regular expression matches it against the left hand side. For instance, to contribute a context menu items for all docker-files one could write `resourceFileName =~ *docker*`.
