...
As a consequence, clicking with pressed `Ctrl/Cmd`-key will no longer open an editor to the side because this is now used to add more items to the multi-selection. A new setting `workbench.multiSelectModifier` allows to change back to the old behaviour by setting it to `"workbench.multiSelectModifier": "alt"`. This will enable to rather use the `Alt`-key to add items to the selection.
...
You can now set `"workbench.fontAliasing": "auto"` to control font aliasing depending on the DPI of the monitor that VS Code is showing on. If set to `auto`, will apply `default` or `antialiased` automatically based on the DPI of displays.

## Editor

### Save Large Files >256 MB

It is now possible to save files >256 MB. Previously an error was reported. This was achieved by changing the implementation of the code that stores the contents of the editor to disk: Instead of loading the entire content into memory, we leverage a snapshot of the content and stream it into the file in chunks of 64kb.
...
Accepting one of these suggestions automatically converts to use bracket accessor notation:

![]()

### Automatic member property suggestions
Tired of typing `this.` to access class properties in JavaScript and TypeScript? Now you can just start typing to see avalible members:
![]()

Acccept a member property suggestion, and VS Code automatically inserts the require `this.`:
...
JavaScript users will also see `?` for completions that come from `*.d.ts` typings packages.

### Quick fix all for JavaScript and TypeScript
Fix errors in a flash with new Quick fix all for JavaScript and TypeScript! Just move your cursor to fixable error such as an unused variable, and trigger quick fixes using the lightbulb or by pressing `kb(editor.action.quickFix)`. If one of the available quick fixes can be applied to multiple errors in the current file, you'll see a new `Quick fix all in file code action`

![Quick fix all in file](images/1_20/ts-fix-all-in-file-pre.png)

Accept and watch the errors disappear right before your eyes:
...
A new command `vscode.removeFromRecentlyOpened` was added that allows to remove entries from the "Recently Opened" list in the File menu.
