...
### List/Tree: new setting to use double-click to open
...
**Note:** the setting is supported for most trees/lists in VS Code, but for some we left the default behaviour where we think single-click still makes more sense. Happy to hear feedback around this if you have any!
...
### New API to add, remove and change workspace folders

New proposed API was added to change workspace folders in the currently opened workspace:
...
This method is powerful enough to:
* remove existing workspace folders (by providing the index of which folder to delete via `start` and the number of folders to remove from there via `deleteCount`)
* add new workspace folders to a specific index (by providing the index where to add to via `start`, leaving `deleteCount` to `0` and providing the workspace folders to add as arguments)
* update existing folders, e.g. move them or rename them (by removing existing ones first and then adding them again)

We plan to make this API stable in a future milestone once we are confident enough with it and welcome any feedback how it works for your extension.
