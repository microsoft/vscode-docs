...
With multi-root workspaces its quite common to have back-end and front-end projects as separate folders loaded in the same workspace. Now compound launch configurations are able to reference another workspace launch configuration. As specified in the section [above](#Variable-subsitution-scoped-per-workspace-folder) we now support subsituting variables scoped to a workspace folder.
