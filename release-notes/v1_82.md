...
The Settings editor now allows natural language search with extensions such as [GitHub Copilot Chat](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot-chat) that implement the [RelatedInformation](#addition-of-relatedinformation-and-removal-of-semanticsimilarity) API.
...
The performance "DOM renderer", which is used when GPU acceleration is disabled, has been significantly improved thanks to [a rewrite of the component](https://github.com/xtermjs/xterm.js/pull/4605). The rewrite focused on reducing the number of DOM elements used and the savings scale much better the larger the terminal is.
...
During this milestone, we explored providing voice-to-text capabilities in VS Code.
...
This new capability is not yet ready for users to try out, but we plan to provide some early access over the coming months. At that time, we will also share more details about the underlying Node.js module for the actual voice-to-text transcription service.
...
Currently we have implemented this in GitHub Copilot for commands, in the Command Palette, and settings, in the Settings editor. We plan on expanding the API to include smart results for symbols search and file text search.
