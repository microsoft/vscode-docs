...
### New setting to control how pinned editors close from keyboard or mouse
...
We are able to benefit from all the rich capabilities the browser provides to us for processing audio input from the microphone. And we can leverage our [`MessagePort`](https://developer.mozilla.org/en-US/docs/Web/API/MessagePort) based architecture for the audio transcription:
* an [`AudioWorklet`](https://developer.mozilla.org/en-US/docs/Web/API/AudioWorklet) captures raw uncompressed audio from the microphone in a separate thread
* the data is sent to our shared process via direct `MessagePort` communication
* a new native node.js module converts the audio to text
* the text is sent back to the UI

This new capability is not yet ready for users to try out, but we plan to provide some early access soon. We will then also share more details about the underlying node.js module for the actual voice-to-text service.
