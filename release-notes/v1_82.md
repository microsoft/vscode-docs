...
The terminal now has a `terminal.integrated.focusAfterRun` setting so that users can specify if the terminal should be focused when **Terminal: Run Selected Text In Active Terminal** is invoked.
...
Dimmed text will now have _half_ the contrast requirements. While this does mean the text may no longer meet the minimum contrast ratio, it's now definitely differentiatable from regular text which is more important.
...
![Typing any filter will load in the results from the whole buffer](./images/1_82/terminal-links-after-filter.png)

Notice that `CodeQL.yml` was not included in the initial results becuase it was not in the viewport.
