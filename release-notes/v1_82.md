...
The command `JSON: Sort Document` has been extended to a source action with ID `source.sort.json`. It is now possible to sort JSON files on save by adding `source.sort.json` to the `editor.codeActionsOnSave` setting.
