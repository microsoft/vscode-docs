...
The Status bar already provides many themable colors for its items. There are now more colors to this set to allow for theming hover foreground and background colors:
...
The performance "DOM renderer" that is used when GPU acceleration is disabled, has been significantly improved thanks to [a rewrite of the component](https://github.com/xtermjs/xterm.js/pull/4605). The rewrite focused on reducing the number of DOM elements used and the savings scale much better the larger the terminal is.

When testing a typical render call on a terminal with 117 columns and 36 rows, it took ~10 ms before and ~2 ms after. Increasing the terminal size to 300x100 on the specific machine shows a render taking ~25-35 ms before and ~4-5 ms after.
...
* Remote - Tunnels connection reliability improvements.
* Install Docker in WSL command.
...
The image above shows dim unfocused enabled with opacity set to 0.5.
...
Here's an example usage that listens in to any successful `git push` command and triggers a refresh in the extension:
...
A simple API to access the terminal selection is proposed:
...
This will likely be a corresponding change event before it's finalized and the API name may change to make it clearer that it's a simple string and not `Range`-based like `TextEditor.selection`.
...
In this milestone, we are promoting the Electron 25 update to our stable users. This update comes with Chromium `114.0.5735.289` and Node.js `18.15.0`. We want to thank everyone involved with self-hosting on Insiders builds and providing early feedback.
...
* Certain [Mesa](https://mesa3d.org) version updates cause broken shader compilation leading to artifacts in application UI. The issue is tracked in [issue #190437](https://github.com/microsoft/vscode/issues/190437), which also contains the link to the Chromium bug report. You can identify this issue by running with `--verbose` and looking for the following line `ERROR:shared_context_state.cc(81)] Skia shader compilation error` in the logs. If you are affected by this issue, the current workaround is to delete the GPU cache located at `~/.config/Code/GPUCache`.
