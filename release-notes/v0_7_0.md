...
Order: 2
TOCTitle: v0.7.0
PageTitle: Visual Studio Code v0.7.0
MetaDescription: See what is new in Visual Studio Code 0.7.0
---

# VS Code v0.7.10 (Windows Only)
...
* [Editor](/Docs/editor/codebasics) - An introduction to VS Code itself from editing all the way to debugging and tasks
* [Languages](/Docs/languages/overview) - Walkthroughs on how to get the best out of VS Code when using specific languages e.g. JavaScript
* [Runtimes](/Docs/runtimes/nodejs) - Walkthoughs on how to use VS Code with specific runtimes e.g. Node.js
...
* Included the contents of the `settings.json` file in the [Customization topic](/Docs/customization/overview)
* Separated out [key bindings](/Docs/editor/keybindings) into their own topic area
...
>**Note:** These improvements require version 0.12.x of node.js (these improvments are not yet supported for io.js).
...
VS Code now supports variables in the `launch.json` file in the same way as in `tasks.json` (see [Tasks](//Docs/editor/tasks#_variables-in-tasksjson)).
...
## Tasks
We did a significant review of the [Tasks documentation](/Docs/editor/tasks) with this update and have included several examples of using tasks.

We also now support automatic detection of tasks in a `gruntfile`. This works the same as our support for the Gulp and Jake task runners (see [Tasks](/Docs/editor/tasks#_autodetecting-gulp-grunt-and-jake-tasks)).



## Markdown
We added a full overview of [how to use VS Code with Markdown files](/Docs/languages/markdown).
...
## JavaScript
In our [JavaScript documentation](/Docs/languages/javascript) we've added topics covering:
...
## HTML
We improved our HTML formatter in this release and updated the [HTML documentation](/Docs/languages/html).


## JSON
We improved the JSON formatter, specifically formatting with comments and formatting ranges.

In the [JSON documentation](/Docs/languages/json) we now include examples of:
...
## CSS, Less and Sass
We updated our [CSS, Less and Sass documentation](/Docs/languages/CSS) to provide examples of how to work with CSS in VS Code.
...
## Dockerfile
We added a new document that covers our [Docker](/Docs/languages/dockerfile) support.
