...
Welcome to the October 2019 release of Visual Studio Code. As announced in the [October iteration plan](https://github.com/microsoft/vscode/issues/108473), we focused on housekeeping GitHub issues and pull requests as documented in our issue grooming guide.
...
VS Code now supports installing an extension VSIX file from the Explorer by right clicking on a VSIX file and choosing the the `Install Extension VSIX` context menu item.

### Input blur command

A new internal `workbench.action.blur` command is now available, which removes focus from any focuseable input. You can assign a keyboard shortcut for this command, in the Keyboard Shortcuts Preferences.
...
We now render emoji shortcodes, e.g. `:smile:`, in the Timeline view.

![Timeline with emoji](images/1_51/timeline.png)
...
**Note**: this setting may not cover all cases. Browsers may still decide to close a tab or window even without confirmation.
...
For more details and for feedback please use [issue 108929](https://github.com/microsoft/vscode/issues/108929).


### Password APIs

As part of continued work on [Authentication Proivders](https://github.com/microsoft/vscode/issues/88309), we have introduced API for storing and retrieving sensitive information. Internally, this acts as a wrapper of the [keytar](https://github.com/atom/node-keytar) library that VS Code uses for storing secrets.
