...
This addresses a **[feature request](https://github.com/microsoft/vscode/issues/26952)** to navigate SCM commit history. Press the `kb(UpArrow)` and `kb(DownArrow)` to display the prior and next commits, respectively. To move directly to the first and last position of the input box, press `kb(Alt)` in conjunction with the corresponding arrow key. Build off of past commit messages and look back in the history without losing your drafted message.
