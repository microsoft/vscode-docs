...
A lot of work when into the suggest widget. First and foremost: it can now be resized! Simply drag the sides and corners.

![Resizable Suggest](images/1_51/suggest-drag.gif)

_Theme: [GitHub Light](https://marketplace.visualstudio.com/items?itemName=github.github-vscode-theme)_, _Font_: FiraCode

The size of the suggestions list will be remembered and restored across session. Details size is only remembered per session because their sizes vary often. Also, the `editor.suggest.maxVisibleSuggestions` has become obsolete and can be removed.

### Statusbar for Suggestions

The suggest widget can now also show a status bar at its bottom. Enable it via `editor.suggestStatusBar.enable`. It makes toggling details simpler and shows if a completions support inserting, replacing or both.
...
You should also know that the `editor.suggest.insertMode`-setting allows you to configure that you  prefer insert or replace. When a suggestion supports both, your preference will be the default.

### Move cursor to select suggestions

Last but not least, you can now move the cursor while suggestions show. For instance, trigger suggestions at the end of a word, move left to see more suggestions, and then use replace to overwrite the word.
...
Addresses a **[feature request](https://github.com/microsoft/vscode/issues/26952)** to:
...
- Expand and shrink selection in markdown documents
  ...
  - Mac
    - Expand: ⌃⇧⌘→
    - Shrink: ⌃⇧⌘←
- Applies to headers and their content in a hierarchical way, for example:
  - expanding selection on a header line selects the entire contents of the header
  - expanding selection with your cursor in content expands from the block element to any block element parent to the rest of the content under the parent header to the parent header

![In a markdown document, pressing the keybinding to expand selection selects an html code block on the cursor line then the list its in, then the rest of the content under the subheader, then the subheader itself, then the content of the level 2 subheader, then the level 2 subheader, then the content of the level 1 header, then the header, then the whole contents of the document. Pressing the keybinding to shrink the selection reverses that process.](images/1_51/markdown-smart-select-demo.gif)
