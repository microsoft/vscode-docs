...
The [integrated browser](https://code.visualstudio.com/docs/debugtest/integrated-browser) now supports pinch-to-zoom on macOS. Use the [trackpad pinch gesture](https://support.apple.com/en-us/102482) to magnify web page content up to 3x.

This is a purely visual zoom, so it's different from the standard browser zoom (`kb(workbench.action.zoomIn)` / `kb(workbench.action.zoomOut)`), which reflows the page layout.
