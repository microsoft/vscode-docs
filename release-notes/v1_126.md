...
If a compromised extension version makes it into the Markplace, it can be installed automatically on every VS Code instance with automatic updates enabled before anyone notices and the version is pulled. By turning off automatic updates, you can avoid that exposure, but at the expense of having to manually update every extension.

Now you can have it both ways. VS Code lets you configure a delay before it installs a newly published version on your machine. This delay holds back freshly published versions long enough for problematic releases to be caught and removed.

The `setting(extensions.autoUpdateDelay)` setting controls how many hours an update must have been publicly available before it installs automatically (default: `2`). It applies whenever automatic updates are enabled through the `setting(extensions.autoUpdate)` setting.

The delay is also available as the `ExtensionsAutoUpdateDelay` policy, so administrators can enforce a minimum release age across their organization.
