...
In this release, we introduce a new hybrid Markdown editor in the agents window. It allows to view markdown files, but also edit them in place and add comments that an agent session acts on. By using "Reopen Editor With" you can switch between the text editor and this new markdown editor, both in agents and non-agents windows.
...
![Screenshot of the overlay that appears in the middle of the terminal with the current columns and rows when a terminal resize happens.](images/1_131/terminal-resize-overlay.png)
