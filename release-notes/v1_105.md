...
### OS notification for chat response received

When a chat response arrives while the window is unfocused, an OS badge and notification toast are shown with a preview of the reply. Clicking the notification brings focus to the chat input. You can control this behavior with `setting(chat.notifyWindowOnResponseReceived)`.
...
### Keybindings for navigating user chat messages
...
### Qualified tool names in prompt files

 Tool names in prompt files and chat mode files are now qualified by the MCP server, extension or tool set they are part of.

This helps with tool name conflicts and finding a missing extension or MCP server.

The old names are still functional. A code actions helps migrating to the new names.
...
### Empty State Chat History (Experimental)

You can now see your recent local conversations when the chat is empty, which makes it easier to pick up where you left off or revisit past work. Turn this on to display your chat history directly in the empty state. It is currently off by default.
...
We've introduced an opt-in setting, `chat.tools.terminal.autoReplyToPrompts`, which allows the agent to respond to prompts for in put in the terminal automatically.
...
When a user initiated, long-running task completes while the window is unfocused, an OS badge and notification toast are shown. Clicking the notification focuses the window. You can configure this behavior with `task.notifyWindowOnTaskCompletion`.

![An OS notification shows Task "build" completed in 1 minute 21 secodns](images/1_105/task-notification.png)

### Task terminal title persistence

With the default `terminal.integrated.title` value of `${process}`, a task terminal's name would change when the process did. For tasks, this could cause confusion. Now, we persist the task's name.
...
### Prompt and instruction file contributions

Extensions can now contribute prompt and instruction files.
...
* [@meeraharidasa (Meera Haridasa)](https://github.com/meeraharidasa): Add /azure to the VS Code for Web Docs [PR #8927](https://github.com/microsoft/vscode-docs/pull/8927)
* [@nickzhums](https://github.com/nickzhums): Add App Modernization related documentation for VS Code Java node [PR #8901](https://github.com/microsoft/vscode-docs/pull/8901)
* [@ssss141414](https://github.com/ssss141414): AI Toolkit Model Conversion update GA docs [PR #8904](https://github.com/microsoft/vscode-docs/pull/8904)
