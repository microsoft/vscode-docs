...
### MCP Servers Marketplace (Preview)

VS Code now includes a built-in MCP Marketplace that allows users to browse and install MCP servers directly from the Extensions view. This is powered by the [GitHub MCP registry](https://github.com/mcp) and provides a seamless experience for discovering and managing MCP servers.

**Note**: This feature is currently in preview and therefore not all features are available yet and may have some rough edges.

#### Getting started

The MCP Marketplace is disabled by default and must be manually enabled. When no MCP servers are installed, you'll see a welcome view in the Extensions panel that provides easy access to enable the marketplace:
...
You can also enable it manually using the setting `setting(chat.mcp.gallery.enabled)`.

#### Browsing and installing MCP servers

Once enabled, you can browse MCP servers in several ways:
...
The following screenshot shows the GitHub MCP server in the marketplace:

![Screenshot showing the GitHub MCP server in the MCP Marketplace](images/1_105/mcp-server-editor.png)
