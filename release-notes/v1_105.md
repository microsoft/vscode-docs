...
**Update 1.105.2**: The update adds a [built-in plan agent](#plan-agent), [support for subagents](#isolated-subagents-preview), and [OpenAI Codex integration](#integration-with-openai-codex-preview). It also addresses these [issues](https://github.com/microsoft/vscode/issues?q=is%3Aissue+is%3Aclosed+milestone%3A%22September+2025+Recovery+2%22).
...
The new Agent Sessions view in VS Code Insiders now supports third-party coding agents, starting with OpenAI Codex. You can now manage both local and cloud-based agent sessions from a unified interface, making it easier to track progress and delegate tasks across multiple AI coding assistants.
...
This is an initial experience with OpenAI Codex as the first third-party agent integrated with Copilot. The UI and partner integrations will continue to evolve based on user feedback.

Make sure to download [VS Code Insiders](https://code.visualstudio.com/insiders/) to try out this functionality.

Learn more about [using the Agent Sessions view](https://code.visualstudio.com/docs/copilot/chat/chat-sessions#_agent-sessions-view) to manage local and remote coding agent sessions.
