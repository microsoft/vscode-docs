...
* **[Local history](#local-history)** - Keep track of local file changes without source control.
* **[Settings editor language filter](#settings-editor)** - Displays available language-specific settings.
* **[Audio cues](#audio-cue-improvements)** - You can now adjust the editor audio cue volume.
* **[Terminal find improvements](#terminal)** - Matches are highlighted in the terminal panel and scroll bar.
* **[Built-in CSS formatter](#css-formatter)** - Code formatting for CSS, LESS, and SCSS.
* **[Improved source control decorators](#updated-source-control-decorators)** - Higher visibility using patterns and color contrast.
...
The workbench now supports rendering with reduced animations when the OS reduced motion setting is enabled. This can be overriden or disabeld with the `workbench.reduceMotion` setting.
...
This iteration we continuted to work on support for file nesting in the explorer. As part of this we polished support for moving, renaming, and deleting nested files, and added the setting `explorer.experimental.fileNesting.operateAsGroup` to configure whether edits to a nesting parent should be applied to it's children as well.
