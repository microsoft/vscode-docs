...
Introduced the `workbench.editor.defaultBinaryEditor` setting. This settings allows you to circumvent the binary file warning and instead automatically open the editor type of your choosing when a binary file is detected.
...
In order to be consistent with other languages, selected interpreter version is now moved towards the right side in the status bar beside the language status item **Python**. With the intention of cleaning up the status bar, it's now only displayed when a Python or a `settings.json` file is opened.
