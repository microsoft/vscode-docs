...
Workspace and folder settings in the Settings editor are now preserved until manually reset by the user. This change helps with cases where the user wants to explicitly specify a workspace setting value that is equal to the default value of a setting, but is different from the user setting value. Previously, users had to open the workspace settings JSON file for that case.
...
You may notice that we've smoothed out the authentication flows of both GitHub and Microsoft.

* For GitHub we've removed the initial "Continue" page so your log in experience requires one less step.
* For Microsoft, we try to minimize the amount of times we take you out of VS Code to log in if you've already logged in with Microsoft for Settings Sync or an another extension.
...
In order to make our sour control decorators accesible, we've introduced a new pattern for modified lines and increased the contrast for all decorators.
