...
* **[Symbol links in the Explorer](#symbolic-link-indication-in-explorer)** - Quickly see symbolic links in your project.
* **[Drag & drop, copy & paste across instances](#better-drag-and-drop-support)** - Move files across open VS Code windows.
* **[Enable/disable built-in extensions](#manage-built-in-extensions)** - Manage built-in extensions just like those from the Marketplace.
* **[Breakpoint icons](#breakpoint-icons-in-breakpoints-view)** - High visibility icons show your breakpoint's type and status.
* **['nvs' Node.js version support](#node-debugging)** - Use Node Version Switcher to select your Node.js version.
...
* **[Workbench](#workbench)** - Automatically create subfolders, improved NPM script detection.
* **[Editor](#editor)** - Improved text buffer performance, Emmet filter improvements.
* **[Languages](#languages)** - HTML path completion, Markdown preview smart scrolling.
...
You can now get CSS Emmet completions in a style attribute in an HTML file when the completions are manually triggered via `kb(editor.action.triggerSuggest)`. We are working on getting such completions automatically without the need for manual trigger.
