...
We now also support to drag and drop root folders in explorer to rearange their order as requested in [29715](https://github.com/Microsoft/vscode/issues/29715).
...
We now indicate symbolic links in the explorer by an arrow decoration and additional hover text.

![symlinkg](images/1_21/symlink.png)

### Create multiple folders at once from within the explorer

This can also be used to create a new file:

![symlinkg](images/1_21/explorer-multiFolderCreation.gif)
...
When running scripts in `package.json` files as tasks using `Tasks>Run Tasks` then scripts from `package.json`files in subfolders are now also provided. To exclude folders from script detection use the `npm.exclude` setting.

### Preview: horizontal search

This milestone we have added a configuration `"search.location"` to control the position of the search view. To have more horizontal space set `"search.location": "panel"`. This feature is currently in preview as in next milestone we plan to further improve the layout within the search view to take advantage of the additional horizontal space.
...
Two new actions let you navigate between active changes in the current file: `workbench.action.editor.previousChange` and `workbench.action.editor.nextChange`. Their default keybindings are `Alt+Shift+F5` and `Alt+F5`, respectively.

## Debugging

### Breakpoint icons in breakpoint view
...
**Node Version Switching "nvs" support**

Last milestone we've added support for the Node Version Manager (aka "nvm"). Thanks to a pull request by Alessandro Vergani ([@loghorn](https://github.com/loghorn)) we now support the [Node Version Switching](https://github.com/jasongin/nvs) (aka "nvs") too. Use the `runtimeVersion` attribute in a "node" launch configuration for selecting a specific version of Node.js.

**Please note**: Make sure to have those Node.js versions installed that you want to use with the `runtimeVersion` attribute as the feature will not download and install the version itself. So you will have to run something like `nvs add 7.10.1` from the integrated terminal if you plan to add `"runtimeVersion": "7.10.1"` to your launch configuration.

**Please note**: If VS Code detects that "nvs" is installed, it does **not** fall back to "nvm" if a specific Node.js version cannot be found in "nvs". That means using "nvs" and "nvm" at the same time is not supported.

## Extensions

### Manage Built-In Extensions

With this release, you can now manage (disable/enable) built in extensions those get shipped with VS Code. There is an action `Show Built-in Actions` in Extensions view that shows all built in extensions and you can browse and manage them.
...
The border color of editor highlights can now also be freely choosen:
...
By default the item to be revealed is selected. Inorder to not to select, set the option `select` to `false` eg: `customView.reveal('element', {select: false});`
...
If you want your `TreeItem` to have a folder or file icon from the active file icon theme, you can do that by assigning `iconPath` property of `TreeIem` to `ThemeIcon.Folder` or `ThemeIcon.File` consant.

### Tooltip

Now you can provide tooltip to an item in the view using the property `tooltip` in `TreeItem`.
...
**NOTE:** Only Node scripts are supported.

### Active Viewlet Context Key

There's a new context key available which contains the ID of the currently open viewlet: `activeViewlet`. Example usage: `activeViewlet == workbench.view.scm`.

### Debug Adapter Protocol

**New `CapabilitiesEvent`**

Sometimes it is difficult for a debug adapter to know and return all capabilities from the (very early) `Initialize` request because some might not be known until the runtime/debugger has been started.
The newly introduced `CapabilitiesEvent` can be used at a later time to announce that one or more capabilities have changed.
Since the capabilities are dependent on the frontend and its UI, it might not be possible to change that at random times (or too late).
Consequently the `CapabilitiesEvent` has a hint characteristic: a frontend can only be expected to make a 'best effort' in honouring individual capabilities, but there are no guarantees.
...
In 1.20 we added support for language packs and now the first language packs are available in the [market place](https://marketplace.visualstudio.com/search?term=%22language%20pack%20extension%20for%22&target=VSCode&category=All%20categories&sortBy=Relevance). We will continue to work with the community to add more language packs.

## Language Server Protocol

Released version 3.6.0 of the protocol specification. The version specifies the following new requests and notifications:
...
* [Max Schmitt (@mxschmitt)](https://github.com/mxschmitt)
  * `NOT MERGED - PLS REVIEW` Fixed: Block-scoped declarations not yet supported [PR #98](https://github.com/Microsoft/vscode-generator-code/pull/98)
  *  Added file path space trimming [PR #97](https://github.com/Microsoft/vscode-generator-code/pull/97)
