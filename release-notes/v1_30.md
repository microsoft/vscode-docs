...
![multiline input](images/1_30/multiline-input.png)

### Moved settings editor actions

The settings editor actions ("Open settings.json", "Show modified settings", etc) have been moved from the `...` menu inside the settings editor to the editor action bar. This makes them consistent with other editor types, and "Open settings.json" is now just one click away. The keybindings editor has made the same change.

![settings actions](images/1_30/settings-actions.png)
...
For get a `new` when instantiating a class? TypeScript 3.2 brings a new quick fix to help out:
...
Renames now handle JavaScript and TypeScript destructuring and will introduce and alias if needed:
...
This milestone we worked on improvements to the [GitHub Pull Requests](https://marketplace.visualstudio.com/items?itemName=GitHub.vscode-pull-request-github) extension. Below are some of the   features we added
