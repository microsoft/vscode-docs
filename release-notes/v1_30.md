...
Because this simple syntax is not sufficient to present a meaningful UI to the end user, we've introduced a new `inputs` section in `launch.json` and `tasks.json` where additional configuration attributes are specified. Here is the list of attributes:
...
Tasks can now be rurun using the Rerun Last Task command. By default, a rerun task will re-evaluate it’s run variables, including its inputs. To use the variables that have already been evaluated in the previous run, configure the `"reevaluateOnRerun"` option:
