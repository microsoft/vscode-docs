...
## Improved User Input Variables for Tasks and Debug Configurations

For a long time VS Code supports [variable substitution](https://code.visualstudio.com/docs/editor/variables-reference) in `tasks.json` and `launch.json` via the `${variableName}` syntax.

For `launch.json` it was even possible to use variables that prompt for user input (e.g. `${command:pickNodeProcess}` which opens a QuickPick based process picker). But this feature was not available for `tasks.json` and the command based variables had other limitations too (see issues [#9297](https://github.com/Microsoft/vscode/issues/9297), [#30588](https://github.com/Microsoft/vscode/issues/30588), and [#9544](https://github.com/Microsoft/vscode/issues/9544)).

With this milestone user input variables have been generalized and improved and they are now available for `launch.json` **and** `tasks.json`. This work has also brought the command syntax to `tasks.json`.

For user input variables we've introduced a new variable category `input` which results in this syntax: `${input:variableName}`.
Because this simple syntax is not sufficient to present a meaningful UI to the end user, we've introduced a new `inputs` section in `launch.json` and `tasks.json` where additional configuration attributes are specified. Here is the list of supported attributes:

- `id`: the corresponding variable name for which these attributes are used.
- `type`: the type of user input widget. In this release `promptString` (for a string InputBox) and `pickString` (for a string QuickPick) are supported.
...
```
And here a user input variable for selecting between two options in a debug configuration (`launch.json`):
...
Tasks can now be rerun using the Rerun Last Task command. By default, a rerun task will re-evaluate it’s run variables, including its inputs. To use the variables that have already been evaluated in the previous run, configure the `"reevaluateOnRerun"` option:
...
**Richer DebugSession**<BR>
The [`DebugSession`](https://github.com/Microsoft/vscode/blob/a152a5c7818a13666a5879b4450eb26a4d45fbde/src/vs/vscode.d.ts#L8310) API now surfaces the workspace folder and the _resolved_ debug configuration. _Resolved_ means that all variables have been substituted and platform specific attribute sections have been "flattened" for the matching platform and removed for non-matching platforms.

**Unique ID for Breakpoints**<BR>
...
**Finalized API for controlling debug adapter creation**<BR>
In this milestone we have finalized the extension API for controlling the creation of debug adapters.
...
* A `DebugAdapterExecutable` descriptor makes VS Code launch the debug adapter as an external process per session and communicate with it through stdin/stdout. Via an options bag the working directory and environment variables can be specified.
...
**Finalized Debug Adapter Tracker API**<BR>
In this milestone we have finalized the extension API for tracking the [Debug Adapter Protocol](https://microsoft.github.io/debug-adapter-protocol/) used between VS Code and debugger extensions.
...
**New `shellProcessId` attribute for `runInTerminal` response**<BR>
...
When VS Code started JavaScript wasn't what it is today. For instance, there was no [native promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) and back then the team decided to use `WinJS.Promise`. Fast forwarding to today the situation is different: native promises are real and winjs-promise is dead.

Because of that we are in the progress of migration away from `WinJS.Promise`. This is no easy undertaking because of differences in the API and the runtime-behaviour. We have started at around 10000 references to `WinJS.Promise` and are down to `1471`, of which ~500 are instantiations and not plain type annotation. We are confident to finish this epic soon!
