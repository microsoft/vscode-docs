...
* **[Debug hit count control](#hit-count-expression)** - Set breakpoint hit count conditions.
...
VS Code now supports setting a 'hit count expression' for a breakpoint (in addition to the 'condition expression' introduced some time ago).
...
In this milestone only the built-in Node.js debugger supports hit counts (but we hope other debugger extensions will follow soon).
