...
* **[Simplified Node.js debugging](#simplified-launch-configuration)** - Quickly configure your Node.js debugging sessions.
...
- There is a short delay between when a change of the dependencies in the `package.json` file are detected and when the corresponding typings file is acquired and available for intellisense.
...
Last release we introduced 'Extension Pack' which is represented as an extension that depends on other extensions. In this release, we added a new tab to the extension editor to show these dependencies which gets installed when installing the pack.
