...
Often you have more than one workspace and various Extensions installed to support these workspaces. Most of the times, these extensions are exclusive to a single workspace and not needed for others. For example, if you have `Javascript` and `Go` workspaces, you do not need Javascript linting extension in Go workspace and vice-versa. With this release, you can now run only those extensions which you think are necessary for a workspace and disable rest of them. You can also disable extensions for the complete VS Code application, if you do not want to uninstall them and do not want them to run.
...
Unlike install, uninstalling a pack offers you to uninstall also extensions which it depends on.
...
* `debug.allowBreakpointsEverywhere` - allows breakpoints to be set in any file (and not only in those which are explicitely registered). This is useful if you want to debug a new (transpiled) language with an existing debugger (e.g. Node debugger).
