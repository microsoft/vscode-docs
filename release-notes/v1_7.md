...
This is so cool, that we wanted you to get your hands on it as soon as possible. Our integration of ATA is still rough around the edges, but  we still decided to enable it by default. Should you experience issues, you can switch it off by setting `typescript.experimentalAutomaticTypeAcquisition` to `false` in your user settings.
