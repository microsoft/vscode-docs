...
In order to make seting up debugging as easy as possible we have simplifed the initial 'launch.json' experience for Node:
* Removed unnecessery configuration attributes to reduce noise. These configuration attributes are still available and discoverable using IntelliSense
* Added comments and links to improved documentation
* Added decorations to make it more obvious which attributes should not be changed for the most common scenario

Now each debug extension can be in charge of providing the full content of 'launch.json' thus giving the extension more power to customize the content.

![launchjson](images/1_7/launchjson.png)

### Hit count conditionals

We now support specifying a hit count for a breakpoint - thus a breakpoint will only be hit if the specified hit count condition is met.

In order for the hit count to be respected it has to be supported by the debug extension. At the moment Node debug is supporting this feature and other debug extensions should follow soon. 
