...
In previous releases, you might have noticed that a menu entry might not show a keyboard shortcut even though you have a keybinding assigned. The reason was that some keybindings (for example multiple key chords) cannot be displayed as keyboard shortcuts in menus (including context menus).

As a workaround, we now show those keybindings as part of the label:
...
As announced in the prvious release, we are working on running tasks in the Integrated Terminal instead of the Output panel. In this release, we added support to compose tasks out of simplier tasks. If, for example, you have a workspace with a client and server folder and both contain a build script, you can now have one task that starts both build scripts in separate terminals.
...
Note: A debug adapter backend can update the location of breakpoints during a debug session. This might cause breakpoints to move since breakpoints can not be accepted at every possible location.
...
Starting withthis release, we now support the ':' as a prefix separator in variable names (and we have deprecated the '.'). With this change, the new (and preferred) syntax for 'env', 'command', and 'config' variables becomes `${prefix:id}`.
