...
To give a more visibility to exceptions that occur during debugging, an exception dialog now pops up. This request was raised [here](https://github.com/OmniSharp/omnisharp-vscode/issues/1007), and describes the scenario where the debugger stops but the user is not aware that an exception was thrown (especially if the exception was thrown from library code). Another motivation for adding a dialog was to display exception details.
