...
* **[Expanded keyboard shortcut bindings](#configurable-explorer-key-bindings)** - Create shortcuts for the Explorer as well as VS Code's list/tree UI elements. 
...
A Minimap gives you a high level overview of your source code structure which is very useful for quick navigation and code understanding. To enable VS Code's Minimap, set  `editor.minimap.enabled: true` to turn on the rendering of a Minimap for the current file. Based on the `devicePixelRatio` (for example, regular or retina/high DPI displays), the Minimap will render characters either at 4x2 pixels or at 2x1 pixels. 
