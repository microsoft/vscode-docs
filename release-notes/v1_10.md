...
* **[Preview: Minimap](#preview-minimap)** - See your source code structure with a Minimap overview.
...
* **[Expanded keyboard shortcut bindings](#configurable-explorer-key-bindings)** - Create shortcuts for the Explorer as well as list/tree UI elements. 
* **[Copy text with formatting](#copy-with-syntax-highlighting)** - Copy code examples with full syntax highlighting.
* **[HTML document symbol support](#html)** - Quickly navigate to DOM elements by id and class name.
* **[JSDoc comment templates](#auto-jsdoc-comments)** - VS Code can automatically generate JSDoc templates for your functions.
* **[Language specific settings](#go-make-yaml-markdown)** - Go, Make, YAML and Markdown ship with appropriate default settings for Tabs and word wrap.
* **[Column breakpoints](#column-breakpoints)** - Set multiple breakpoints on the same line of source code.
* **[New Exception Peek UI](#improved-exception-experience)** - Easily view Exception details in the debugger.
...
* **[Tasks](#task-support)** - Assign keyboard shortcuts to tasks, run tasks in separate terminals, new ${lineNumber} variable.
* **[Languages](#languages)** - TypeScript 2.2, disable color preview decorators, better link navigation in Markdown.
* **[Debugging](#debugging)** - Launch using keyboard shortcuts, new variable syntax, debug unsaved files.
* **[Node.js Debugging](#node-debugging)** - Learn about the move to the Node2 debugger.
* **[Extension Authoring](#extension-authoring)** - Modal message dialogs, provide language specific settings, control global command visibility.
...
The option to launch debug configurations using a keyboard shortcut was introduced, as per [this request](https://github.com/Microsoft/vscode/issues/16613). It works similarly to running tasks, with an ability to launch from the **Command Palette**. The keyword `'debug '` or the command **Debug: Select and Start Debugging** from the **Command Palette** is used to select and launch a configuration from `launch.json`.
