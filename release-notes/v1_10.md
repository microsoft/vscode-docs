...
This is particularly useful when debugging minified code which containes mutliple statements on a single line. A column breakpoint can be set using `kb(editor.debug.action.toggleColumnBreakpoint)` or via the context menu during a debug session.
