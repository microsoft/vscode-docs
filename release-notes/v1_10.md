...
You can now bind a keyboard sort cut to any task you want. Simply add a keybinding like this:
...
As announced in the previous release we are working on running tasks in the terminal instead of the output panel. This release we added support to compose tasks out of simplier tasks. If for example you have a workspace with a client and server folder and both contain a build script you can now have one task that starts both build scripts in separate terminals. A `tasks.json` file doing this looks like this:
