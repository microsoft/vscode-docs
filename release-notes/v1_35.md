...
Contributed commands can now control their enablement state using a new `enablement`-property. Its value is a [when](https://code.visualstudio.com/docs/getstarted/keybindings#_when-clause-contexts)-clause, as its already used for keybindings and menu contributions.

The sample below shows how a "Hello Enablement" command is being contributed to the editor's context menu and how its enablement state depends on the edtor having a selection.
...
Note that some menu-implementations, like the command palette, don't show commands that are disabled. Also note that there is some semantic overlap between `enablement` of commands and the `when`-clause of menu items. The former defines enablement for all representations of a command while the latter defines when a command should be included in a menu. For instance an explorer context menu item might only make sense for certain file types, like TypeScript-files, while enablement of that command might depend on another condition, e.g. "TypeScript server running".
...
We have finalized the Comments API that allows extensions to provide commenting support within VS COde. For example, the GitHub Pull Request extension uses the Comments API to provide comments for pull requests opened directly in the editor and users can interact with the comments from the editor and a dedicated Comments Panel.

You can play with the [sample extension](https://github.com/microsoft/vscode-extension-samples/tree/master/comment-sample) to get a better understanding of the API or take a deep look at [GitHub Pull Requests](https://github.com/microsoft/vscode-pull-request-github/pull/1168).
...
![Updates Octicons icons](images/1_35/octicons-update2.png)
...
Thanks to [Andrew Crawley](https://github.com/andrewcrawley) we could extend the Debug Adapter Protocol to better support hardware oriented debuggers that need to surface processor registers and memory contents. Both additions are optional and existing and new debug adapters are not affected by this in any way. But if a debug adapter is interested in showing registers and/or memory contents, it can opt into these features. But please note that the frontend needs to support the additions too: Visual Studio plans to support both, for Visual Studio Code we haven't decided yet.
...
This feature centers around "memory references." A _memory reference_ is an opaque identifier that can be used (along with an optional byte offset) to identify a single location in memory.
A debug adapter has the option of providing a _memory reference_ on DAP responses that represent things a frontend might want to display in a memory view or use as a starting point for disassembly, e.g. `EvaluateResponse` and `Variable`. Since providing _memory references_ could be expensive for a debug adapter, the frontend announces that it can actually use _memory references_ by sending a `supportsMemoryReferences` capability in the `initialize` request.

With a _memory reference_ in hand a frontend can read memory via a new `readMemory` request and it can disassemble code via a new `disassemble` request.
If a debug adapter implements these requests, it can annouce it via the two capabilities `supportsReadMemoryRequest` and `supportsDisassembleRequest`.

## Notable improvements

* The history of the references viewlet can now be cleared
...
* Multiple item paste in explorer is run in parallel for improved performance
