...
* **[New Visual Studio Code logo](#new-visual-studio-code-logo)** - The product logo got a refresh. We hope you like it.
* **[Go to Definition improvements](#go-to-definition-improvements)** - Faster navigation through multiple definitions.
* **[Breadcrumb navigation displayed by default](#breadcrumbs-displayed)** - Quickly navigate to files and symbols with breadcrumbs.
* **[JavaScript/TypeScript smart selection](#smart-selection-for-javascript-and-typescript)** - Expand text selection based on language semantics.
* **[TypeScript extract to type alias refactoring](#extract-to-type-alias-refactoring-for-typescript)** - Extract part type into its own type alias.
* **[Integrated terminal supports true color](#true-color-support)** - Correctly renders true color (24-bit) escape sequences.
* **[Better merge conflict display](#compare-merge-conflict-with-complete-context)** - See merge conflict changes with surrounding source code.
* **[Collapse All command for lists and explorers](#notable-improvements)** - One of many small but handy improvements this milestone.
* **[Remote Development available for Stable](#preview-remote-development)** - You can now use the Remote Development preview in Stable.
...
### Compare merge conflict with complete context
