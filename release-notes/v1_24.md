...
We have introduced a new command `workbench.action.duplicateWorkspaceInNewWindow` to duplicate the current workspace in a new window.
This command should be used as a workaround to our current limitation that one workspace can not be opened in two windows.
...
In this milestone, we began experimenting with a GUI for editing settings, which has been a popular request for a long time. This is an early preview, but if you'd like to try it out, you can find it under the command `Preferences: Open Settings (Preview)`. We would like any feedback you have - you can leave it on [this GitHub issue](https://github.com/Microsoft/vscode/issues/50249).
...
![term quick open menu supports splits](images/1_24/terminal-quick-open.png)
...
A milestone ago we changed the behaviour of F8 to visit all errors and warnings. That means you'll leave the current editor instead of looping around. Not everyone liked this and there we have added new commands:

* 'Go to Next/Previews Problem *in Files*' is the default, visits all problems, and is bound to F8/Shift+F8
* *New* is 'Go to Next/Previews Problem' which loops inside the current editor.

## Debugging

### Preview: debug.toolBarLocation

In this milestone we have started to work on alternative locations for the debug toolbar.
As a first step we have introduced a new setting `debug.toolBarLocation` and an initial attempt for a "docked" debug toolbar (in addition to the old "floating" toolbar):

![debug toolbar](images/1_24/debug_toolbar.png)  @isidorn pls create screenshot where the new debug toolbar is not so cramped at the edge. Show a full (but small) debug viewlet and a bit of the adjacent editor.

Since we are still experimenting with this new toolbar design, please expect more changes in the next milestones.

But we are already certain that the existing `debug.hideActionBar` can be easily subsumed under a value `hidden` of the new `debug.toolBarLocation` setting. So we are now **deprecating** the `debug.hideActionBar` setting`.
...
We have renamed "column breakpoint" to "inline breakpoints" because it seems that the community is much more familar with the term "inline breakpoint" than "column breakpoint".

Consequently we are deprecating the old `editor.debug.action.toggleColumnBreakpoint` command in favor of a new `editor.debug.action.toggleInlineBreakpoint` command.
...
![Quick fix all action](images/1_24/ts-grey-pre.png)
...
![Code after apply the quick fix](images/1_24/ts-grey-post.png)
...
The new syntax aware folding should match the old indentation based folding in most cases, however indentation based folding sometimes created extra folding ranges that you may have gotten used to. To revert to using the old indentaiton based folding, set:
...
- `"single"` quotes.
- `"double"` quotes.
- Or `"auto"` infer quote type from existing imports.
...
### New conetxt key for call stacks elements
...
This release ships an [outline view](#outline-view). Currently, there is logic in place to synthesize a tree from the flat list of document symbols but that logic isn't perfect. We are still discussing the API for this (see [#34968](#https://github.com/Microsoft/vscode/issues/34968)) but you can prepare for this:

* Make sure that your language brain returns a hierarchy of document symbols (this should be the 'natural' representation anyways).
* Make sure you have two ranges in hand: one that covers the full declaration of a symbol and one that points to the identifier of a symbol (with that information we the symbol the cursor selects).

And as always, feel free to join the discussion and to propose API for this: [#34968](#https://github.com/Microsoft/vscode/issues/34968).

### Default argument for keybinding command invocation has changed
...
### Reload webviews using ctrl+r inside the webview dev tools
...
https://github.com/Microsoft/vscode-extension-samples/blob/roblou/searchProviderExample/fsprovider-sample/src/searchProvider.ts
