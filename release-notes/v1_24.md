...
Back in v1.17 the terminal moved over to [rendering using canvas](https://code.visualstudio.com/blogs/2017/10/03/terminal-renderer) in order to boost performance. While this was great for the majority of users, some environments had issues with this at the hardware and OS levels and rendering ended up taking longer. VS Code now measures the average time it takes to render a frame and if it's too long you will get a notification to switch to the new fallback DOM renderer:
...
Running the `code` CLI inside WSL will now suppress output and return to the prompt.
