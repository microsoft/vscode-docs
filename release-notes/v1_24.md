...
### Preview: alternative debug toolbar location
...
We have introduced a new icon for the "Debug: Continue" action in the debug toolbar.
...
### Callstack shows stack frame path

CALL STACK view now shows the path segments to easily distinguish sources with the same name. A common scenario is having to debug mutliple stack frames all named `module.js`.

![callstack](images/1_24/callstack.png)

### Go to next or previous breakpoint

Thanks to user contributed [PR](https://github.com/Microsoft/vscode/pull/50285) now it is easier to navigate between breakpoints using the following two commands:
* Debug: Go to Next Breakpoint
* Debug: Go to Previous Breakpoint

These commands navigate between files and make it easier for breakpoints to be potentially used as bookmarks.

### Variable improvements in launch.json and tasks.json

#### IntelliSense for variables

We now suggest predefined variables as completions inside string values for both the `tasks.json` and `launch.json`. This should make it easier to use and discover various predefined variables.

![variables](images/1_24/variables.png)

#### Variable substitution for keys

Previosuly we would substitute predefined variables only for values inside `launch.json` and `tasks.json` configurations. Now it is possible to have varialbes substituted both for keys and values in `launch.json` and `tasks.json` configurations.
...
Now it is possible to refine for which items commands appear in the CALL STACK view using the `callStackItemType` context key. This context key can have the following values:
* `'stackframe'`: when a stack frame is focused in the CALL STACK view
* `'thread'`: when a thread is focused in the CALL STACK view. Threads are shown only for multi threaded debugging.
* `'seassion'`: when a debugging session is focused in the CALL STACK view. Debugging sessions are only shown for multi session debugging.
