...
We have renamed "column breakpoints" to "inline breakpoints". The rationale behind this change is that the community is much more familar with the term "inline breakpoints".

Due to this we are now deprecating the `editor.debug.action.toggleColumnBreakpoint` command in favor of the new `editor.debug.action.toggleInlineBreakpoint` command. We plan to drop support for the deprecated `editor.debug.action.toggleColumnBreakpoint` in the following milestone.

### Preview: debug.toolBarLocation
Now it is possible to control the location of the debug toolbar. Either the default "floating" in all views, "docked" in the debug view, or "hidden". We are still experimenting with the exact placement of some actions thus this feature is in preview.

Due to this we are now **deprecating** the `debug.hideActionBar` setting in favor of the more general `debug.toolBarLocation`.

![debug toolbar](images/1_24/debug_toolbar.png)
