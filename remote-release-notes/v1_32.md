...
Important: It is the location of the workspace file that signals whether we need to start a remote window or not, so the workspace file should always be where the root folders are.
...
When connected to an SSH remote, we now show the name of the remote in the window title bar and status bar. This makes it much easier to keep track of which machine a window is connected to.
...
We now allow users to specify their user ID when connecting to an SSH box:
...
To lay the foundation for an improved Getting Started experience we continued to improve the SSH Explorer.

The explorer can be enabled with:
...
**SSH CONNECTIONS** lists all configured hosts and shows you the recently opened folders per host. It also indicates the active host or folder. There are inline actions to open a new empty window on a host, open a recently open folder, or remove a folder from the list.

**FORWARDED PORTS** shows the list of established SSH tunnels and allows users to manage tunnels.
...
### Extension debugging in Containers
...
**Note**: Currently the VS Code Server needs to open the extension folder as its workspace folder because mounting inside the dev container is tied to the workspace and not (yet) to the `extensionDevelopmentPath`. Lifting this limitation is covered by [#518](https://github.com/Microsoft/vscode-remote/issues/518).
...
VS Code now uses web sockets to communicate with the remote extension host agent. The remote extension host agent is now also an HTTP server.
