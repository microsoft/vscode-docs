...
* There is an incompatibility between OpenSSH versions when the ssh-agent runs on Windows with version <= 8.8 and the ssh client (on any platform) runs version >= 8.9. The workaround is to upgrade OpenSSH on Windows to 8.9 or later, either using winget or an installer from https://github.com/PowerShell/Win32-OpenSSH/releases.
