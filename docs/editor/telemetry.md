...
With the `setting(telemetry.telemetryLevel)` user [setting](/docs/editor/customizing/settings.md), you can control the different types of telemetry we send with a single setting. Here is a table of the different types of data sent with each value of `setting(telemetry.telemetryLevel)`:
...
For example, if you don't want to send any telemetry data to Microsoft, you can set the `setting(telemetry.telemetryLevel)` user [setting](/docs/editor/customizing/settings.md) to `off`.
...
* [User and Workspace Settings](/docs/editor/customizing/settings.md) - Read about available options to customize VS Code.
* [Key Bindings](/docs/editor/customizing/keybindings.md) - You can easily modify commonly used keyboard shortcuts.
