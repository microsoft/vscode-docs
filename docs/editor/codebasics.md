...
* `kbstyle(Ctrl)` (`kbstyle(Mac: Cmd)`) click on a file in the Explorer
...
When you have more than one editor open you can switch between them quickly by holding the `kbstyle(Ctrl)` (`kbstyle(Mac: Cmd)`) key and pressing `1`, `2`, or `3`.
...
Press `kb(workbench.action.openPreviousEditor)` to view a list of all files that have been opened since VS Code was launched. To open one of these files, hold the `kbstyle(Ctrl)` (`kbstyle(Mac: Cmd)`) key and press `kbstyle(Tab)` to pick the file you want to navigate to.
