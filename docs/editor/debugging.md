...
The following documentation is based on the built-in [Node.js](https://nodejs.org/) debugger, but most of the concepts and features are applicable to other debuggers.
...
Note that the attributes available in these launch configurations vary from debugger to debugger. You can use IntelliSense to find out which attributes exist for a specific debugger. Hover help is available for all attributes. If you see green squigglies in your launch configuration, hover over them to get information on the problem.  Try to fix any problems before launching a debug session.
...
* `type` - the type of debugger to use for this launch configuration. Every installed debug extension introduces a type, for example, `node`, `php` and `go` for the Node, PHP and Go extensions.
...
You can reference environment variables through **${env:Name}** syntax (for example, ${env:PATH}). Be sure to match the environment variable name's casing, for example `${env:Path}` on Windows.
...
In addition to debugging, VS Code supports running the program. The **Run** action is triggered with `kb(workbench.action.debug.run)` and uses the currently selected launch configuration. Many of the launch configuration attributes are supported in 'Run' mode. VS Code maintains a debug session while the program is running. Pressing the **Stop** button terminates the program.
...
To use multi-target debugging:

- Start your first debug session.
- Start your second debug session.

As soon as a second session is running, the VS Code UI switches to _multi-target mode_:
