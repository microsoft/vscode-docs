...
When you see this notification, it indicates that the VS Code file watcher is running out of handles because the workspace is large and contains many files. Before diggin further into the problem, please make sure that such folders as .venv or alike (that are potentially large) are added to files.watcherExclude setting. The current limit can be viewed by running:
