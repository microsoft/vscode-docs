...
---
# Installing {% data variables.product.prodname_vscode %} on Linux

{% data variables.product.prodname_vscode %} is available on Linux through official Debian, RPM, and Snap packages. Community-maintained packages are also available for Arch Linux and Nix.

## Install {% data variables.product.prodname_vscode_shortname %} on Linux
...
1. Install the package with the graphical software center, or install it from the command line:
    ...
    > [!NOTE]
    > Other binaries are available on the [{% data variables.product.prodname_vscode_shortname %} download page](/download).
...
> [!NOTE]
> Due to the manual signing process and publishing system, the Debian repository could lag behind by up to three hours and not immediately contain the latest version of {% data variables.product.prodname_vscode_shortname %}.
...
Microsoft provides the stable 64-bit {% data variables.product.prodname_vscode_shortname %} package for RHEL, Fedora, and CentOS-based distributions in a yum repository.

1. Install the key and yum repository:

    ```bash
    sudo rpm --import https://packages.microsoft.com/keys/microsoft.asc &&
    echo -e "[code]\nname={% data variables.product.prodname_vscode %}\nbaseurl=https://packages.microsoft.com/yumrepos/vscode\nenabled=1\nautorefresh=1\ntype=rpm-md\ngpgcheck=1\ngpgkey=https://packages.microsoft.com/keys/microsoft.asc" | sudo tee /etc/yum.repos.d/vscode.repo > /dev/null
    ```
...
> [!NOTE]
> Due to the manual signing process and publishing system, the yum repository could lag behind by up to three hours and not immediately contain the latest version of {% data variables.product.prodname_vscode_shortname %}.
...
{% data variables.product.prodname_vscode_shortname %} is officially distributed as a Snap package in the [Snap Store](https://snapcraft.io/store).
...
Once installed, the Snap daemon automatically updates {% data variables.product.prodname_vscode_shortname %} in the background. {% data variables.product.prodname_vscode_shortname %} shows an in-product update notification whenever a new update is available.
...
The [Arch User Repository package for {% data variables.product.prodname_vscode_shortname %}](https://aur.archlinux.org/packages/visual-studio-code-bin) is community maintained.
...
The [{% data variables.product.prodname_vscode_shortname %} Nix package](https://github.com/NixOS/nixpkgs/blob/master/pkgs/applications/editors/vscode/vscode.nix) in the nixpkgs repository is community maintained.

To install {% data variables.product.prodname_vscode_shortname %} by using Nix:
...
Download and install the [{% data variables.product.prodname_vscode_shortname %} .rpm package](https://go.microsoft.com/fwlink/?LinkID=760867) manually when repository installation is not available. Auto-update does not work unless the repository is installed.
...
{% data variables.product.prodname_vscode_shortname %} ships weekly [releases](/updates). If the {% data variables.product.prodname_vscode_shortname %} repository is installed correctly, the system package manager handles auto-update the same way it handles other packages on the system.

> [!NOTE]
> Updates are automatic and run in the background for the [Snap package](#snap-package).

## Configure {% data variables.product.prodname_vscode_shortname %} as the default text editor

### xdg-open

Set {% data variables.product.prodname_vscode_shortname %} as the default text editor for text files, `text/plain`, used by `xdg-open`:
...
Debian-based distributions support setting a default **editor** through the [Debian alternatives system](https://wiki.debian.org/DebianAlternatives), without concern for the MIME type. Set {% data variables.product.prodname_vscode_shortname %} as the default editor with this command:

```bash
sudo update-alternatives --set editor /usr/bin/code
```

If {% data variables.product.prodname_vscode_shortname %} was installed with the Snap package, use this command instead:

```bash
sudo update-alternatives --set editor /snap/bin/code
```

If {% data variables.product.prodname_vscode_shortname %} doesn't show up as an alternative to the default `editor`, register it:
...
Another option for Linux development with {% data variables.product.prodname_vscode_shortname %} is to use a Windows machine with the [Windows Subsystem for Linux](https://learn.microsoft.com/windows/wsl/install) (WSL).
...
When paired with the [WSL extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl), {% data variables.product.prodname_vscode_shortname %} provides editing and debugging support while running in the context of a Linux distro on WSL.
...
<details>
<summary>Conflicts with {% data variables.product.prodname_vscode_shortname %} packages from other repositories</summary>

Some distributions, such as [Pop!_OS](https://pop.system76.com), provide their own `code` package. To ensure that the official {% data variables.product.prodname_vscode_shortname %} repository is used, create a file named `/etc/apt/preferences.d/code` with this content:
...
If deleting files from the {% data variables.product.prodname_vscode_shortname %} Explorer fails on Debian, the trash implementation used by {% data variables.product.prodname_vscode_shortname %} might be missing.
...
<details>
<summary>"{% data variables.product.prodname_vscode %} is unable to watch for file changes in this large workspace" (error ENOSPC)</summary>

This notification indicates that the {% data variables.product.prodname_vscode_shortname %} file watcher is running out of file handles. This often happens when opening a workspace that contains many files. Before adjusting platform limits, add large folders, such as Python `.venv`, to the `setting(files.watcherExclude)` setting. Other running applications might also consume file handles, so closing other applications might help.
...
Another option is to exclude specific workspace directories from the {% data variables.product.prodname_vscode_shortname %} file watcher with the `setting(files.watcherExclude)` [setting](/docs/configure/settings.md). The default `setting(files.watcherExclude)` value excludes `node_modules` and some folders under `.git`. Add other directories that {% data variables.product.prodname_vscode_shortname %} should not track.
...
Running `code .` on Ubuntu when {% data variables.product.prodname_vscode_shortname %} is already open in the current directory does not bring {% data variables.product.prodname_vscode_shortname %} into the foreground. This is an operating system behavior that can be changed with `ccsm`.
...
Under **General** > **General Options** > **Focus & Raise Behavior**, set **Focus Prevention Level** to **Off**. This operating system setting applies to all applications, not only {% data variables.product.prodname_vscode_shortname %}.
...
If using X forwarding to use {% data variables.product.prodname_vscode_shortname %} remotely, use the native title bar to manipulate the window. Set `setting(window.titleBarStyle)` to `native`.
