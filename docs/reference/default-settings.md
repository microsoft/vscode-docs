...
You can also view the default values in the Settings editor or see a read-only version of the `defaultSettings.json` via the **Preferences: Open Default Settings (JSON)** command in the Command Palette (`kb(workbench.action.showCommands)`).
...
// Chat
    ...
    // Enable agent mode for Copilot Edits. When this is enabled, a dropdown appears in the Copilot Edits view to toggle agent mode.
    ...
    // Enable reusable prompt files (`*.prompt.md`) in Chat, Edits, and Inline Chat sessions.
...
* [Configure settings in VS Code](/docs/editor/customizing/settings.md)
