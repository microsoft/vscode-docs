...
You can also optionally define an `icon` which is either a string file path of the icon or an object with `light` and `dark` fields providing theme-specific paths. The paths are extension-repository directory relative. An SVG file may be used.
