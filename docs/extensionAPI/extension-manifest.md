...
`qna` | | `marketplace` (default), `string`, `false` | Controls the **Q & A** link in the Marketplace. Set to `marketplace` to enable the default Marketplace Q & A site. Set to `string` to provide a custom Q & A site. Set to `false` to disable Q & A altogether.
