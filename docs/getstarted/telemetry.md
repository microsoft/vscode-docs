...
For example, the GitHub Pull Request extension has a setting to disable its telemetry:
...
If you'd like to review the telemetry events in VS Code as they are sent, you can enable tracing of all telemetry. Set the log level to **Trace** using the **Developer: Set Log Level...** command and then in the Output panel (`kb(workbench.action.output.toggleOutput)`), pick **Log (Telemetry)** from the drop-down.
