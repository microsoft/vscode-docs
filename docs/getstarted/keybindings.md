...
Visual Studio Code lets you perform most tasks directly from the keyboard. This page lists out the default shortcuts (keyboard shortcuts) and describes how you can update them.

>**Note:** If you visit this page on a Mac, you will see the keyboard shortcuts for the Mac.  If you visit using Windows or Linux, you will see the keys for that platform. If you need the keyboard shortcuts for another platform, hover your mouse over the key you are interested in.

## Keyboard Shortcuts editor

VS Code provides a rich keyboard shortcut editing experience with the Keyboard Shortcuts editor. The editor lists all available commands with and without keyboard shortcuts, and enables you to change / remove / reset their keyboard shortcuts using the available actions. You can use the search box to find commands or keyboard shortcuts. You can open this editor by going to the menu under **File** > **Preferences** > **Keyboard Shortcuts** or by using the **Preferences: Open Keyboard Shortcuts** command (`kb(workbench.action.openGlobalKeybindings)`).

![Keyboard Shortcuts](images/keybinding/keyboard-shortcuts.gif)

Most importantly, you can see keyboard shortcuts according to your keyboard layout. For example, keyboard shortcut `kbstyle(Cmd+\)` in US keyboard layout will be shown as `kbstyle(Ctrl+Shift+Alt+Cmd+7)` when layout is changed to German. The dialog to enter keyboard shortcut will assign the correct and desired keyboard shortcut as per your keyboard layout.
...
You can quickly customize the keyboard shortcut for user interface actions. Right-click on any action item in your workbench, and select **Configure Keybinding**. If the action has a `when` clause, it's automatically included, making it easier to set up your keyboard shortcuts just the way you need them.
...
If you have many extensions installed or you have [customized](/docs/getstarted/keybindings.md#advanced-customization) your keyboard shortcuts, you can sometimes have keyboard shortcut conflicts where the same keyboard shortcut is mapped to several commands. This can result in confusing behavior, especially if different keyboard shortcuts are going in and out of scope as you move around the editor.

The **Keyboard Shortcuts** editor has a context menu command **Show Same Keybindings**, which will filter the keyboard shortcuts based on a keyboard shortcut to display conflicts.

![show keyboard shortcut conflicts menu](images/keybinding/show-conflicts-menu.png)

Pick a command with the keyboard shortcut you think is overloaded and you can see if multiple commands are defined, the source of the keyboard shortcuts and when they are active.

![show keyboard shortcut conflicts result](images/keybinding/show-conflicts-result.png)

## Troubleshooting keyboard shortcuts

To troubleshoot keyboard shortcuts problems, you can execute the command **Developer: Toggle Keyboard Shortcuts Troubleshooting**. This will activate logging of dispatched keyboard shortcuts and will open an output panel with the corresponding log file.
...
The first keydown event is for the `MetaLeft` key (`cmd`) and cannot be dispatched. The second keydown event is for the `Slash` key (`/`) and is dispatched as `meta+[Slash]`. There were two keyboard shortcut entries mapped from `meta+[Slash]` and the one that matched was for the command `editor.action.commentLine`, which has the `when` condition `editorTextFocus && !editorReadonly` and is a built-in keyboard shortcut entry.
...
It is possible to create a keyboard shortcut that runs several other commands sequentially using the command `runCommands`.
...
## Removing a specific keyboard shortcut rule

You can write a keyboard shortcut rule that targets the removal of a specific default keyboard shortcut. With the `keybindings.json`, it was always possible to redefine all the keyboard shortcuts of VS Code, but it can be difficult to make a small tweak, especially around overloaded keys, such as `kbstyle(Tab)` or `kbstyle(Escape)`. To remove a specific keyboard shortcut, add a `-` to the `command` and the rule will be a removal rule.
...
>**Note:** This section relates only to keyboard shortcuts, not to typing in the editor.
...
Using scan codes, it is possible to define keyboard shortcuts which do not change with the change of the keyboard layout. For example:
...
VS Code gives you fine control over when your keyboard shortcuts are enabled through the optional `when` clause.  If your keyboard shortcut doesn't have a `when` clause, the keyboard shortcut is globally available at all times. A `when` clause evaluates to either Boolean true or false for enabling keyboard shortcuts.
...
The `editor.action.codeAction` command lets you configure keyboard shortcuts for specific [Refactorings](/docs/editor/refactoring.md) (Code Actions). For example, the keyboard shortcut below triggers the **Extract function** refactoring Code Actions:
...
This is covered in depth in the [Refactoring](/docs/editor/refactoring.md#keyboard-shortcuts-for-code-actions) topic where you can learn about different kinds of Code Actions and how to prioritize them in the case of multiple possible refactorings.
...
Now that you know about our keyboard shortcut support, what's next...
