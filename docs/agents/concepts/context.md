...
* **Start new sessions for new tasks.** A [session](/docs/agents/sessions/manage-sessions.md) is an independent conversation with its own context window and history. Each session starts fresh, so don't reuse a single conversation for unrelated tasks.
