...
1. When the plan is finalized, choose to start the implementation or open the planning prompt in the editor for further review.
    ...
    To implement the plan, you can continue in the same session or start a new [Copilot CLI session](/docs/agents/agent-types/copilot-cli.md) to implement the plan in the background. Before you implement in a Copilot CLI session, you can ask the [rubber duck agent](/docs/agents/agent-types/copilot-cli.md#get-a-second-opinion-with-the-rubber-duck-agent) to review your plan for blind spots and design flaws.
