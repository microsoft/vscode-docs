...
Local agents run interactively within Visual Studio Code on your machine. They work on your current workspace and have access to the full range of tools and models available in VS Code, including extension-provided tools and MCP servers. By [creating custom agents](/docs/agent-customization/custom-agents.md), you can let the agent assume a specific role or persona for a task, such as a code reviewer, tester, or documentation writer.
...
> [!TIP]
> For a hands-on tutorial that demonstrates working with different agent types including background and cloud agents, see the [agents tutorial](/docs/agents/agents-tutorial.md).
...
1. Optionally, [add context to your prompt](/docs/chat/copilot-chat-context.md) to get more accurate responses.
...
The location of the language model depends on the model provider you choose for a request. For example, if you choose a model provided by GitHub Copilot, it will run on remote infrastructure. If you choose a [BYOK model](/docs/agent-customization/language-models.md#bring-your-own-language-model-key) that connects to a model running on your machine or private infrastructure, then the model will run there.
