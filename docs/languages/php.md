...
{% data variables.product.prodname_vscode %} is a great editor for PHP development. You get features like syntax highlighting and bracket matching, IntelliSense (code completion), and snippets out of the box and you can add more functionality through community-created {% data variables.product.prodname_vscode_shortname %} [extensions](/docs/configure/extensions/extension-marketplace.md).

## Linting

{% data variables.product.prodname_vscode_shortname %} uses the official PHP linter (`php -l`) for PHP language diagnostics. This allows {% data variables.product.prodname_vscode_shortname %} to stay current with PHP linter improvements.
...
{% data variables.product.prodname_vscode %} includes a set of common snippets for PHP. To access these, hit `kb(editor.action.triggerSuggest)` to get a context-specific list.
...
PHP debugging with **XDebug** is supported through a [PHP Debug extension](https://marketplace.visualstudio.com/items?itemName=xdebug.php-debug). Follow the extension's instructions for configuring **XDebug** to work with {% data variables.product.prodname_vscode_shortname %}.
...
* [Debugging](/docs/debugtest/debugging.md) - Learn more about {% data variables.product.prodname_vscode_shortname %} debugging
