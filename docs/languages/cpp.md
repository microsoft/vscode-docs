...
The C/C++ extension for Visual Studio Code supports automatic formatting with [clang-format](http://llvm.org/releases/download.html). To use this feature you must install clang-format manually and add its path to your user settings file (```settings.json```).

To configure code formatting, open your ```settings.json``` file from the Explorer, then add the 'c_cpp.clang_format_path' property and set its value to the path where clang-format.exe is installed (for example ```"c_cpp.clang_format_path":"C:\\Program Files (x86)\\LLVM\\bin\\clang-format.exe"```).
...
To access the ```c_cpp_properties.json` file, launch the __Quick Open__ (`kb(workbench.action.quickOpen)`) prompt and begin typing **>C/Cpp: Edit Configurations**, then choose this command from the command list that appears.

### Fuzzy Auto-Complete (preview)

Fuzzy auto-complete is powered by an enhanced tag-parser approach. Although suggestions are not based on semantic analysis of your code, this feature provides a wider selection of matches than the single-file intellisense experience provides today.
...
* GDB on Cygwin and MinGW cannot break a running process. To set a breakpoint in run mode, or to pause the application being debugged, press `Ctrl-C` in the application's terminal.
...
* Core dumps cannote be loaded when debugging with GDB because GDB [does not support the core dump format used in OS X](https://www.sourceware.org/ml/gdb/2014-01/msg00036.html).
