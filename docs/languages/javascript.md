...
MetaDescription: Get the best out of {% data variables.product.prodname_vscode %} for JavaScript development
---
# JavaScript in {% data variables.product.prodname_vscode %}

{% data variables.product.prodname_vscode %} includes built-in JavaScript IntelliSense, debugging, formatting, code navigation, refactorings, and many other advanced language features.
...
{% data variables.product.prodname_vscode_shortname %} provides IntelliSense within your JavaScript projects; for many npm libraries such as `React`, `lodash`, and `express`; and for other platforms such as `node`, serverless, or IoT.

See [Working with JavaScript](/docs/nodejs/working-with-javascript.md) for information about {% data variables.product.prodname_vscode_shortname %}'s JavaScript IntelliSense, how to configure it, and help troubleshooting common IntelliSense problems.

## JavaScript projects (jsconfig.json)

A [jsconfig.json](/docs/languages/jsconfig.md) file defines a JavaScript project in {% data variables.product.prodname_vscode_shortname %}. While `jsconfig.json` files are not required, you will want to create one in cases such as:
...
> [!TIP]
> To check if a JavaScript file is part of JavaScript project, just open the file in {% data variables.product.prodname_vscode_shortname %}  and run the **JavaScript: Go to Project Configuration** command. This command opens the `jsconfig.json` that references the JavaScript file. A notification is shown if the file is not part of any `jsconfig.json` project.

## Snippets

{% data variables.product.prodname_vscode_shortname %} includes basic JavaScript [snippets](/docs/editing/userdefinedsnippets.md) that are suggested as you type;
...
{% data variables.product.prodname_vscode_shortname %} understands many standard [JSDoc](https://jsdoc.app) annotations, and uses these annotations to provide rich [IntelliSense](#intellisense). You can optionally even use the type information from JSDoc comments to [type check your JavaScript](#type-checking).
...
As you write JavaScript function calls, {% data variables.product.prodname_vscode_shortname %} shows information about the function signature and highlights the parameter that you are currently completing:
...
Automatic imports speed up coding by suggesting available variables throughout your project and its dependencies. When you select one of these suggestions, {% data variables.product.prodname_vscode_shortname %} automatically adds an import for it to the top of the file.
...
If you choose one of these auto import suggestions, {% data variables.product.prodname_vscode_shortname %} adds an import for it.

In this example, {% data variables.product.prodname_vscode_shortname %} adds an import for `Button` from [material-ui](https://material-ui.com) to the top of the file:
...
> [!TIP]
> {% data variables.product.prodname_vscode_shortname %} tries to infer the best import style to use. You can explicitly configure the preferred quote style and path style for imports added to your code with the `setting(js/ts.preferences.quoteStyle)` and `setting(js/ts.preferences.importModuleSpecifier)` settings.
...
When you move or rename a file that is imported by other files in your JavaScript project, {% data variables.product.prodname_vscode_shortname %} can automatically update all import paths that reference the moved file:
...
{% data variables.product.prodname_vscode_shortname %}'s built-in JavaScript formatter provides basic code formatting with reasonable defaults.
...
All of {% data variables.product.prodname_vscode_shortname %}'s JavaScript features also work with [JSX](https://reactjs.org/docs/introducing-jsx.html):

![JSX IntelliSense](images/javascript/jsx.png)

You can use JSX syntax in both normal `*.js` files and in `*.jsx` files.

{% data variables.product.prodname_vscode_shortname %} also includes JSX-specific features such as autoclosing of JSX tags:
...
{% data variables.product.prodname_vscode_shortname %} includes some handy refactorings for JavaScript such as **Extract function** and **Extract constant**. Just select the source code you'd like to extract and then click on the lightbulb in the gutter or press (`kb(editor.action.quickFix)`) to see available refactorings.
...
[Linters](https://en.wikipedia.org/wiki/Lint_%28software%29) provides warnings for suspicious looking code. While {% data variables.product.prodname_vscode_shortname %} does not include a built-in JavaScript linter, many JavaScript linter [extensions](/docs/configure/extensions/extension-marketplace.md) are available in the marketplace.
...
{% data variables.product.prodname_vscode_shortname %} comes with great debugging support for JavaScript. Set breakpoints, inspect objects, navigate the call stack, and execute code in the Debug Console. See the [Debugging topic](/docs/debugtest/debugging.md) to learn more.
...
{% data variables.product.prodname_vscode_shortname %} ships with excellent support for JavaScript but you can additionally install debuggers, snippets, linters, and other JavaScript tools through [extensions](/docs/configure/extensions/extension-marketplace.md).
...
* [Working with JavaScript](/docs/nodejs/working-with-javascript.md) - More detailed information about {% data variables.product.prodname_vscode_shortname %}'s JavaScript support and how to troubleshoot common issues.
...
* [TypeScript](/docs/languages/typescript.md) - {% data variables.product.prodname_vscode_shortname %} has great support for TypeScript, which brings structure and strong typing to your JavaScript code.

## Frequently asked questions

<details>
<summary>Does {% data variables.product.prodname_vscode_shortname %} support JSX and React Native?</summary>

{% data variables.product.prodname_vscode_shortname %} supports **JSX** and **React Native**. You will get IntelliSense for **React/JSX** and **React Native** from automatically downloaded type declaration (typings) files from the [npmjs](https://www.npmjs.com) type declaration file repository. Additionally, you can install the popular [React Native extension](https://marketplace.visualstudio.com/items?itemName=vsmobile.vscode-react-native) from  the Marketplace.
...
<details>
<summary>Does {% data variables.product.prodname_vscode_shortname %} support the Dart programming language and the Flutter framework?</summary>

Yes, there are {% data variables.product.prodname_vscode_shortname %} extensions for both [Dart](https://marketplace.visualstudio.com/items?itemName=Dart-Code.dart-code) and [Flutter](https://marketplace.visualstudio.com/items?itemName=Dart-Code.flutter) development. You can learn more at the [Flutter.dev](https://flutter.dev/docs/development/tools/vs-code) documentation.
...
Some users want to use syntax constructs like the proposed pipeline (`|>`) operator. However, these are currently not supported by {% data variables.product.prodname_vscode_shortname %}'s JavaScript language service and are flagged as errors. For users who still want to use these future features, we provide the `setting(js/ts.validate.enable)` [setting](/docs/configure/settings.md).
...
Yes, but some of [Flow's](https://flow.org) language features such as type and error checking may interfere with {% data variables.product.prodname_vscode_shortname %}'s built-in JavaScript support. To learn how to disable {% data variables.product.prodname_vscode_shortname %}'s built-in JavaScript support, see [Disable JavaScript support](/docs/nodejs/working-with-javascript.md#disable-javascript-support).
