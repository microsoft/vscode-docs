...
For the details of how JavaScript IntelliSense works, including being based on type inference, JsDoc annotations, TypeScript declarations, and mixing JavaScript and TypeScript projects, see the [JavaScript language service documentation](https://github.com/Microsoft/TypeScript/wiki/Salsa). Where type inference does not provide the desired information, type information may be provided explicitily with JSDoc annotations. This document describes the [JSDoc annotations](https://github.com/Microsoft/TypeScript/wiki/JsDoc-support-in-JavaScript) currently supported. 

## JavaScript Project (jsconfig.json)

The presence of [`jsconfig.json`](/docs/languages/jsconfig.md) file in a directory indicates that the directory is the root of a JavaScript project. [`jsconfig.json`](/docs/languages/jsconfig.md)  specifies the root files and the options for the language features provided by the [JavaScript language service](https://github.com/Microsoft/TypeScript/wiki/Salsa).

### Location of jsconfig.json

To define our code as a JavaScript project, create `jsconfig.json` at the root of your JavaScript code as shown below. A JavaScript project is the source files of the project and should not include the derived or packaged files (for example, a `dist` directory). 
...
The `exclude` attribute tells the language service which files are and are not part of your source code. If IntelliSense is slow, add folders to your `exclude` list (VS Code will prompt you to do this if it detects slow completions). You will want to `exclude` files generated by a build process (for example, a `dist` directory). These files will cause suggestions to show up twice and will slow down IntelliSense.

Alternatively, you can explicitly set the files in your project using the `files` attribute. If no `files` attribute is present, then this defaults to including all files in the containing directory and subdirectories. When a `files` attribute is specified, only those files are included.
...
You can learn more in the documentation for [typings](https://www.npmjs.com/package/typings) and at [DefinitelyTyped](https://github.com/DefinitelyTyped/DefinitelyTyped), which is a repository of typings files for many major JavaScript libraries and environments.
...
IntelliSense slows when there are too many files. Add more files to the `excludes` property of[`jsconfig.json`](/docs/languages/jsconfig.md) and move your the file closer to your JavaScript sources. If the problem persists open an [issue](https://github.com/Microsoft/vscode/issues/new). 
...
You need to install the typings files for each library in the root of your JavaScript project. This is described [above](/docs/languages/javascript.md#typescript-definition-files-typings).
