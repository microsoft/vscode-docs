...
By default, the Settings editor opens in a [modal overlay](/docs/editing/userinterface.md#modal-editors) on top of the editor area. You can change this behavior with the `setting(workbench.editor.useModal)` setting. When you open the Settings JSON file, it opens as a regular editor tab in the main window.
