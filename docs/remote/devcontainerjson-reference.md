...
| `workspaceFolder` | string | Sets the default path that VS Code should open when connecting to the container. Typically used with `workspaceMount`. Defaults to the automatic source code mount location.<br /><br />⚠️ Only supported for Docker Compose in Codespaces and when using Clone Repository in Container Volume.  |
