...
## Tips for resolving extension issues
...
| Problem | Description | Resolution | GitHub issue |
|---------|-------------|------------|--------------|
| **Browser does not open locally** | Some extensions use external node modules or their own code to find the browser and launch it. This will cause the extension to attempt to launch the browser remotely instead of locally. | The `vscode.env.openExternal` API will automatically forward localhost to resolve this problem. See [here for details](/api/advanced-topics/remote-extensions#opening-something-in-a-local-browser-or-application). | |
| **Clipboard does not work** | Node modules like `clipboardy` will use the clipboard from inside WSL, a container, or on a SSH host and therefore not work. | Use the VS Code clipboard API. See [here for details](/api/advanced-topics/remote-extensions#using-the-clipboard). | |
| **Cannot access local web server from browser or application** | When working inside a container or SSH host, ports are not automatically forwarded, so opening a browser to connect to a server inside the container / SSH host fails. | The `vscode.env.openExternal` API will automatically forward localhost to resolve this problem. See [here for details](/api/advanced-topics/remote-extensions#opening-something-in-a-local-browser-or-application).  | |
| **WebView contents do not appear** | When working inside a  container or SSH host, ports are not automatically forwarded. If your WebView content uses an iframe to connect to a server inside the container / SSH host, this will fail and you will see a blank page instead. | The WebView API now includes a `portMapping` property that you can use to solve this problem. See [here for details](/api/advanced-topics/remote-extensions#accessing-localhost). | |
| **Blocked ports** | When working inside a container or SSH server, ports are not automatically forwarded. | There currently is no API to programmatically forward a port from an extension, but you can document the use of the **Remote-Containers: Forward Port from Container...** and **Remote-SSH: Forward Port from Active Host...** commands as a workaround. |[#531](https://github.com/Microsoft/vscode-remote/issues/531) |
| **Errors storing extension data** | Certain extensions try to persist data by looking for the `~/.config` folder on Linux. In remote settings, the folder requested may not exist so you can see errors like `ENOENT: no such file or directory, open '/root/.config/Code - Insiders/User/fileame-goes-here`. | Use the `context.globalStoragePath` or `context.storagePath` property. See [here for details](/api/advanced-topics/remote-extensions#persisting-extension-data-or-state). | | |
| **Cannot sign in / have to sign in each time I connect to a new endpoint** | Extensions with a sign in may persist secrets using their own code or in files. These will either persist remotely or may fail due to missing dependencies. | Use the `keytar` node module or create a "Helper Extension". See [here for details](/api/advanced-topics/remote-extensions#presisting-secrets). |
| **Cannot access / transfer remote workspace files to local machines** | Extensions that open workspace files in external applications will encounter errors, since the external application will not have access to the workspace files directly. | We are investigating options for how extensions might be able to transfer files from the remote workspace to accomplish this task. | [#640](https://github.com/Microsoft/vscode-remote/issues/640) |
| **Cannot access attached device from extension** | Extensions that access locally attached devices will be unable to connect to them when running remotely. | A "Helper Extension" can be used to integrate with the device. See [here for details](/api/advanced-topics/remote-extensions#accessing-local-apis-using-a-helper-extension). However, we are currently investigating possible ways to transfer workspace files locally so they can be deployed to devices. |[#640](https://github.com/Microsoft/vscode-remote/issues/640) |
