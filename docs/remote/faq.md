...
### What is {% data variables.product.prodname_vscode %} Remote Development?

The {% data variables.product.prodname_vscode %} [Remote Development extension pack](https://aka.ms/vscode-remote/download/extension) allows you to open any folder in a container, on a remote machine (via SSH), or in the Windows Subsystem for Linux and take advantage of {% data variables.product.prodname_vscode_shortname %}'s full feature set. This means that {% data variables.product.prodname_vscode_shortname %} can provide a local-quality development experience — including full IntelliSense (completions), debugging, and more — regardless of where your code is located or hosted.

### What advantages does {% data variables.product.prodname_vscode_shortname %} Remote Development provide over local editing?
...
Compared to using a network share or synchronizing files, {% data variables.product.prodname_vscode_shortname %} Remote Development provides dramatically better performance along with better control over your development environment and tools.

### How do the Remote Development extensions relate to GitHub Codespaces?

[GitHub Codespaces](https://github.com/features/codespaces) is a service that provides managed cloud-hosted development environments accessible from both {% data variables.product.prodname_vscode_shortname %} and a new browser-based editor. The service also allows {% data variables.product.prodname_vscode_shortname %} and the browser-based editor to access self-hosted environments (desktop or server) without requiring an SSH server or even a direct network route. You can read more in the [GitHub Codespaces documentation](https://docs.github.com/github/developing-online-with-codespaces).
...
{% data variables.product.prodname_vscode %} Remote Development allows your local {% data variables.product.prodname_vscode_shortname %} installation to transparently interact with source code and runtime environments on other machines (whether virtual or physical) by moving the execution of certain commands to a "remote server". The **{% data variables.product.prodname_vscode_shortname %} Server** is quickly installed by {% data variables.product.prodname_vscode_shortname %} when you connect to a remote endpoint and can host extensions that interact directly with the remote workspace, machine, and file system.
...
{% data variables.product.prodname_vscode %} Remote Development uses existing, well known transports like [secure shell](https://en.wikipedia.org/wiki/Secure_Shell) to authenticate and secure traffic. No ports need to be publicly opened beyond those used by these well-known, secure transports.

The {% data variables.product.prodname_vscode_shortname %} Server that is injected runs as the same user you used to sign in to the machine, ensuring that {% data variables.product.prodname_vscode_shortname %} and its extensions are not given improper elevated access without permission. The server is started and stopped by {% data variables.product.prodname_vscode_shortname %} and is not wired into any user or global login or startup scripts. {% data variables.product.prodname_vscode_shortname %} manages the server's lifecycle so you do not need to worry about whether or not it is running.

### Can {% data variables.product.prodname_vscode_shortname %} Server be installed or used on its own?

No. The {% data variables.product.prodname_vscode_shortname %} Server is a component of the Remote Development extensions and is managed by a {% data variables.product.prodname_vscode_shortname %} client. It is installed and updated automatically by {% data variables.product.prodname_vscode_shortname %} when it connects to an endpoint and if installed separately could become quickly out of date. It is not intended or [licensed](#license-and-privacy) for use by other clients.

### What are the connectivity requirements for {% data variables.product.prodname_vscode_shortname %} Server?
...
By default, the Remote - SSH will attempt to download on the remote host, and fail back to downloading {% data variables.product.prodname_vscode_shortname %} Server locally and transferring it remotely once a connection is established. You can change this behavior with the `setting(remote.SSH.localServerDownload)` setting to always download locally and then transfer it, or to never download locally.
...
All other communication between the server and the {% data variables.product.prodname_vscode_shortname %} client is accomplished through the following transport channels depending on the extension:
...
You can find a list of locations {% data variables.product.prodname_vscode_shortname %} itself needs access to in the [network connections article](/docs/setup/network.md#common-hostnames).

### Why can't I see my local containers in the Container Tools extension when using the Remote - extensions?

By default, the Container Tools extension will run remotely. While this is a sensible default in some cases, it means the extension may not show local containers when {% data variables.product.prodname_vscode_shortname %} is connected to a remote SSH host, container, or WSL.
...
Support for musl-based [Alpine Linux](https://alpinelinux.org) is available for the Dev Containers and WSL extensions and ARMv7l (AArch32) / ARMv8l (AArch64) is available in Remote - SSH. However, native dependencies in certain extensions may cause them not to function on non-x86_64 glibc distributions. Note that experimental ARMv8l (AArch64) is available in [{% data variables.product.prodname_vscode_shortname %} Insiders](https://code.visualstudio.com/insiders/) only.

See [Remote Development with Linux](/docs/remote/linux.md) for additional details.

### Can I run {% data variables.product.prodname_vscode_shortname %} Server on older Linux distributions?

Starting with {% data variables.product.prodname_vscode_shortname %} release 1.99 (March 2025), the prebuilt servers distributed by {% data variables.product.prodname_vscode_shortname %} are only compatible with Linux distributions that are based on glibc 2.28 or later. These include for example, Debian 10, RHEL 8, or Ubuntu 20.04.

{% data variables.product.prodname_vscode_shortname %} will still allow users to connect to an OS that is not supported by {% data variables.product.prodname_vscode_shortname %} (OSes that don't have glibc >= 2.28 and libstdc++ >= 3.4.25) via the [Remote - SSH](https://aka.ms/vscode-remote/download/ssh) extension, if a sysroot with these required library versions is provided. This approach gives you and your organization more time to migrate to newer Linux distributions.

| {% data variables.product.prodname_vscode_shortname %} version | Base Requirements | Notes |
|--------------|-------------------|-------|
...
2. {% data variables.product.prodname_vscode_shortname %} server uses [patchelf](https://github.com/NixOS/patchelf) during the installation process to consume the required libraries from the sysroot.
...
You can now connect to the remote by using the [Remote - SSH](https://aka.ms/vscode-remote/download/ssh) extension. On successful connection, {% data variables.product.prodname_vscode_shortname %} will show a dialog and banner message about the connection not being supported.

### Can I run {% data variables.product.prodname_vscode_shortname %} Server on 32-bit ARM Linux hosts?

{% data variables.product.prodname_vscode_shortname %} release 1.122 is the last release that supports running the prebuilt servers on 32-bit ARM Linux hosts. Starting with {% data variables.product.prodname_vscode_shortname %} release 1.123, you will need to use a supported x86_64 or ARM64 Linux host for remote development.

### Can I install individual extensions instead of the extension pack?

Yes. The [Remote Development extension pack](https://aka.ms/vscode-remote/download/extension) provides a convenient way for you to access all of the latest remote capabilities as they are released. However, you can always install the individual extensions from the Marketplace or {% data variables.product.prodname_vscode_shortname %} Extensions view.
...
You can find the licenses for the {% data variables.product.prodname_vscode_shortname %} Remote Development extensions here:
...
See the [{% data variables.product.prodname_vscode %} and 'Code - OSS' Differences](https://github.com/microsoft/vscode/wiki/Differences-between-the-repository-and-Visual-Studio-Code) and [Microsoft Extension Licenses](/docs/supporting/oss-extensions.md) articles for more information.
...
### Can I use the {% data variables.product.prodname_vscode_shortname %} Remote Development extensions to build my own product or service?

You can use the extensions with your own internal or private services. You cannot build a public or commercial service on top of the {% data variables.product.prodname_vscode_shortname %} Remote Development extensions or their related components (for example {% data variables.product.prodname_vscode_shortname %} Server). You cannot create other extensions that extend or manipulate the Remote Development extensions. While the license states you may not "provide the software as a stand-alone or integrated offering or combine it with any of your applications for others to use", you can document how to use the extensions in conjunction with your service.

### Can I repackage or reuse {% data variables.product.prodname_vscode_shortname %} Server in my own public service offering?

No. The license states that you may not "provide the software as a stand-alone or integrated offering or combine it with any of your applications for others to use" which means you may not build public products or services on top of the {% data variables.product.prodname_vscode_shortname %} Server.
...
### GDPR and {% data variables.product.prodname_vscode_shortname %} Remote Development

The {% data variables.product.prodname_vscode_shortname %} Remote Development extensions follow the GDPR policies as {% data variables.product.prodname_vscode %} itself. See the [general FAQ](/docs/supporting/faq.md#gdpr-and-vs-code) for more details.
