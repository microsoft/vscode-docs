...
2. In VS Code, select **Remote-SSH: Connect to Host...** from the Command Palette (`kb(workbench.action.showCommands)`) and use the same `user@hostname` as in step 1.
...
Start by selecting **Remote-SSH: Add New SSH Host...** from the Command Palette (`kb(workbench.action.showCommands)`) or clicking on the **Add New** icon in the SSH **Remote Explorer** in the Activity Bar.
...
From this point forward, the host will appear in the list of hosts when you select **Remote-SSH: Connect to Host...** from the Command Palette (`kb(workbench.action.showCommands)`) or in the **SSH Targets** section of the **Remote Explorer**.
...
Once you are connected to a host, if you want to **temporarily forward** a new port for the duration of the session, select **Forward a Port** from the Command Palette (`kb(workbench.action.showCommands)`) or click on the **Forward New Port** icon in the **Remote Explorer** after selecting it from the Activity Bar.
