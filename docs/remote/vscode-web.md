...
{% data variables.product.prodname_vscode %} for the Web provides a free, zero-install Microsoft {% data variables.product.prodname_vscode %} experience running entirely in your browser, allowing you to quickly and safely browse source code repositories and make lightweight code changes. To get started, go to [https://vscode.dev](https://vscode.dev) in your browser.

{% data variables.product.prodname_vscode_shortname %} for the Web has many of the features of {% data variables.product.prodname_vscode_shortname %} Desktop that you love, including search and syntax highlighting while browsing and editing, along with extension support to work on your codebase and make simpler edits. In addition to opening repositories, forks, and pull requests from source control providers like GitHub and Azure Repos, you can also work with code that is stored on your local machine.

{% data variables.product.prodname_vscode_shortname %} for the Web runs entirely in your web browser, so there are certain limitations compared to the desktop experience, which you can read more about [below](#limitations).
...
{% data variables.product.prodname_vscode_shortname %} for the Web provides a browser-based experience for navigating files and repositories and committing lightweight code changes. However, if you need access to a runtime to run, build, or debug your code, you want to use platform features such as a terminal, or you want to run extensions that aren't supported in the web, we recommend moving your work to the desktop application, [GitHub Codespaces](https://github.com/features/codespaces), or using [Remote - Tunnels](#use-your-own-compute-instance-with-remote-tunnels) for the full capabilities of {% data variables.product.prodname_vscode_shortname %}. In addition, {% data variables.product.prodname_vscode_shortname %} Desktop lets you use a full set of keyboard shortcuts not limited by your browser.

When you're ready to switch, you'll be able to ["upgrade"](#continue-working-in-a-different-environment) to the full {% data variables.product.prodname_vscode_shortname %} experience with a few clicks.

You can also switch between the Stable and Insiders versions of {% data variables.product.prodname_vscode_shortname %} for the Web by selecting the gear icon, then **Switch to Insiders Version...**, or by navigating directly to [https://insiders.vscode.dev](https://insiders.vscode.dev).
...
You can create a new local file in the web just as you would in a {% data variables.product.prodname_vscode_shortname %} Desktop environment, using **File** > **New File** from the Command Palette (`kbstyle(F1)`).

## GitHub repos

You can open a GitHub repository in {% data variables.product.prodname_vscode_shortname %} for the Web directly from a URL, following the schema: `https://vscode.dev/github/<organization>/<repo>`. Using the [{% data variables.product.prodname_vscode_shortname %} repository](https://github.com/microsoft/vscode) as an example, this would look like: `https://vscode.dev/github/microsoft/vscode`.
...
>**Note**: The [GitHub Repositories](https://marketplace.visualstudio.com/items?itemName=GitHub.remotehub) extension works in {% data variables.product.prodname_vscode_shortname %} Desktop as well to provide fast repository browsing and editing. Once you have the extension installed, you can open a repo with the **GitHub Repositories: Open Repository...** command.
...
If you're already in {% data variables.product.prodname_vscode_shortname %} for the Web at [https://vscode.dev](https://vscode.dev), you can alternatively navigate to different repos via the [Remote Repositories](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-repositories) extension commands. Select the remote indicator in the lower left of the Status bar, and you'll be presented with the **Open Remote Repository...** command.

![GitHub Repositories](images/vscode-web/remote-repositories.png)

## Azure Repos

You can open Azure Repos just like Github repos in {% data variables.product.prodname_vscode_shortname %} for the Web.
...
You can open any repository, branch, or tag from Azure Repos in {% data variables.product.prodname_vscode_shortname %} for the Web by prefixing `vscode.dev` to the Azure Repos URL.

Alternatively, when you are on an Azure DevOps repository or pull request, you can press (`kbstyle(.)`) to open it in {% data variables.product.prodname_vscode_shortname %} for the Web.

## More custom URLs

Like in the desktop, you can customize {% data variables.product.prodname_vscode_shortname %} for the Web through a rich ecosystem of extensions that support just about every back end, language, and service. `vscode.dev` includes URLs that provide shortcuts to common experiences.
...
| {% data variables.product.prodname_vscode_shortname %} for Education | `/edu` | [{% data variables.product.prodname_vscode_shortname %} for Education landing page](https://vscodeedu.com/) |
...
| Azure | `/azure` | [{% data variables.product.prodname_vscode_shortname %} for Azure](https://code.visualstudio.com/docs/azure/vscodeforweb) |
...
You can share and experience color themes through {% data variables.product.prodname_vscode_shortname %} for the Web through the URL schema: `https://vscode.dev/editor/theme/<extensionId>`.
...
An extension can define multiple themes. You can use the schema `/editor/theme/<extensionId>/<themeName>`. If no `themeName` is specified, {% data variables.product.prodname_vscode_shortname %} for the Web will take the first theme.

As a theme author, you can add the following badge to your extension readme to allow users to easily try out your theme in {% data variables.product.prodname_vscode_shortname %} for the Web (replacing `<extensionId>` with your theme extension's unique identifier):
...
When working on a local file in the web, your work is saved automatically if you have [Auto Save](/docs/editing/codebasics.md#save--auto-save) enabled. You can also save manually as you do when working in desktop {% data variables.product.prodname_vscode_shortname %} (for example **File** > **Save**).
...
The first time that you use **Continue Working On** with uncommitted changes, you will have the option to bring your edits to your selected development environment using **Cloud Changes**, which uses a {% data variables.product.prodname_vscode_shortname %} service to store your pending changes. This is described further in the [GitHub Repositories](/docs/sourcecontrol/github.md#continue-working-on) doc.

## Use your own compute instance with Remote Tunnels

You may develop against another machine in {% data variables.product.prodname_vscode_shortname %} for the Web using the [Remote - Tunnels](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-server) extension.
...
{% data variables.product.prodname_vscode_shortname %} for the Web runs entirely in your web browser's sandbox and offers a very limited execution environment.
...
When working with local files, {% data variables.product.prodname_vscode_shortname %} for the Web loads them through your browser's file system access APIs, which limit the scope of what the browser can access.

## Run anywhere

Similar to [GitHub Codespaces](/docs/remote/codespaces.md), {% data variables.product.prodname_vscode_shortname %} for the Web can run on tablets, like iPads.
...
Since {% data variables.product.prodname_vscode_shortname %} for the Web runs completely within the browser, some experiences will naturally be more constrained when compared to what you can do in the desktop app. For example, the terminal and debugger are not available, which makes sense since you can't compile, run, and debug a Rust or Go application within the browser sandbox.
...
When you install an extension, it is saved in the browser's local storage. You can ensure your extensions are synced across {% data variables.product.prodname_vscode_shortname %} instances, including different browsers and even the desktop, by enabling [Settings Sync](/docs/configure/settings-sync).
...
When extensions are executed in the browser sandbox, they are more restricted. Extensions that are purely declarative, such as most themes, snippets, or grammars, can run unmodified and are available in {% data variables.product.prodname_vscode_shortname %} for the Web without any modification from the extension authors. Extensions that are running code need to be updated to support running in the browser sandbox. You can read more about what is involved to support extensions in the browser in the [web extension authors guide](/api/extension-guides/web-extensions.md).
...
You can use {% data variables.product.prodname_vscode_shortname %} for the Web in the latest versions of Chrome, Edge, Firefox, and Safari. Older versions of each browser may not work - we only guarantee support for the latest version.

> **Tip:** One way to check the compatible browser version is to look at the version of [Playwright](https://playwright.dev/) currently used for testing {% data variables.product.prodname_vscode_shortname %} and review its supported browser versions. You can find the currently used Playwright version in the {% data variables.product.prodname_vscode_shortname %} repo's [package.json](https://github.com/microsoft/vscode/blob/main/package.json) file at `devDependencies/@playwright/test`. Once you know the Playwright version, for example `1.37`, you can then review the **Browser Versions** section in their [Release notes](https://playwright.dev/docs/release-notes).

Webviews might appear differently or have some unexpected behavior in Firefox and Safari. You can view issue queries in the {% data variables.product.prodname_vscode_shortname %} GitHub repo to track issues related to specific browsers, such as with the [Safari label](https://github.com/microsoft/vscode/labels/safari) and [Firefox label](https://github.com/microsoft/vscode/labels/firefox).

There are additional steps you can take to improve your browser experience using {% data variables.product.prodname_vscode_shortname %} for the Web. Review the [Additional browser setup](#additional-browser-setup) section for more information.

### Mobile support

You can use {% data variables.product.prodname_vscode_shortname %} for the Web on mobile devices, but smaller screens may have certain limitations.
...
| `kb(workbench.action.tasks.build)` will not toggle the favorites bar in the browser. | {% data variables.product.prodname_vscode_shortname %} for the Web overrides this and redirects to the "Build" menu in the Command Palette. |
...
There are additional browser configuration steps you can take when working with {% data variables.product.prodname_vscode_shortname %} in a browser.

### Opening new tabs and windows

In certain cases, you may need to open a new tab or window while working in {% data variables.product.prodname_vscode_shortname %} for the Web. {% data variables.product.prodname_vscode_shortname %} might ask you for permission to access the clipboard when reading from it. Depending on your browser, you may grant access to the clipboard or otherwise allow for pop-up windows in different ways:
