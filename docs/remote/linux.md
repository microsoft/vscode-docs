...
MetaDescription: Linux Prerequisites for VS Code Remote - SSH, Remote - Containers, and WSL
...
* **Alpine Linux 64-bit x86 containers or WSL hosts** (3.9+) in Remote - Containers, WSL
...
| Alpine Linux 3.9+ | `musl libgcc libstdc++`. musl >= 1.1.18, glibc not required. | Not yet supported. | Supported in Remote - Containers and WSL. Extensions installed in the container may not work due to `glibc` dependencies in extension native code. |
...
|---------------------|--------------|-------------------|------------------|
| ⚠️ Alpine Linux 3.10 (64-bit) | `alpine:3.10` | `libgcc libstdc++` |  Supported in Remote - Containers and WSL only. Some extensions installed in the container may not work due to `glibc` dependencies in extension native code. |
