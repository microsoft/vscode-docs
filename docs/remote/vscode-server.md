...
The Visual Studio Code Server is a private preview service you can run on a remote development machine, like your desktop PC or a virtual machine (VM). It allows you to securely connect to that remote machine from anywhere through a vscode.dev URL, without the requirement of SSH.
...
Please note the VS Code Server is a private preview. In this early preview, the `code-server` CLI is distinct from the `code` CLI [you use today](https://code.visualstudio.com/docs/editor/command-line#_launching-from-command-line) to launch the desktop VS Code, to install extensions, and more (run `code -h` from the terminal to review the possible commands).
...
The VS Code Server's CLI establishes a tunnel between a VS Code client (vscode.dev) and your remote machine. Tunneling, aka port forwarding, securely transmits data from one network to another.
...
1. Install the VS Code Server on your remote machine. You may install and host it in a machine on-premises or in the cloud, as long as it meets the necessary [system requirements](/docs/remote/linux.md).
...
Upon first run of the VS Code Server, you'll be prompted to accept the terms of the license. You may view the license for the VS Code Server [here](https://aka.ms/vscode-server-license).
...
You can also view your existing remote machines in the Remote Explorer view (which you may focus with the command **View: Show Remote Explorer**).

## Telemetry

If you want to disable telemetry, you can pass in `--disable-telemetry` when launching the VS Code Server, i.e.: `code-server serve --disable-telemetry`. Alternatively, if you would like to specify an initial telemetry level, such as only collecting errors, you may do so by passing in `--telemetry-level` followed by the level (e.g. `error`).

If telemetry is not disabled via the CLI, the VS Code Server will begin respecting the client telemetry settings (i.e. your telemetry setting in vscode.dev) upon successful connection.
...
In the quick start, we ran the VS Code Server with `code-server`, which generated a vscode.dev link connected to your remote machine. The connection was facilitated by the VS Code Server's secure tunneling service. You may achieve the same experience by running `code-server serve`.
...
Pure UI extensions are not supported when using a web-based instance of VS Code, which you may learn more about [here](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds).

### Are there browser limitations?

While working in the browser, there are certain limitations and configuration steps to consider. You may read more about this in the [VS Code for the Web](https://code.visualstudio.com/docs/editor/vscode-web#_additional-browser-setup) documentation.

### Is there a certain number of remote machines I can connect to?

Right now, you may only have 10 remote machines actively running the VS Code Server. If you'd like to connect to a new remote machine and already have 10 others running, you'll need to stop the server on one of your other machines.
...
If you're using `serve-local` mode, you may run the following:
...
You may run: `code-server uninstall`.

### Where can I provide feedback or report an issue?

If you have any issues or feedback, please file an issue in the [VS Code Remote GitHub repo](https://github.com/microsoft/vscode-remote-release/issues). When filing an issue, please include verbose logging, which you may achieve by launching the VS Code Server with the `-v` flag: `server-launcher -v serve`.
