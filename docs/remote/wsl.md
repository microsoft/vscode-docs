...
The **{% data variables.product.prodname_vscode %} WSL** extension lets you use the [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/windows/wsl) as your full-time development environment right from {% data variables.product.prodname_vscode_shortname %}. You can develop in a Linux-based environment, use Linux-specific toolchains and utilities, and run and debug your Linux-based applications all from the comfort of Windows.

The extension runs commands and other extensions directly in WSL so you can edit files located in WSL or the mounted Windows filesystem (for example `/mnt/c`) without worrying about pathing issues, binary compatibility, or other cross-OS challenges.  The extension will install {% data variables.product.prodname_vscode_shortname %} Server inside WSL; the server is independent of any existing {% data variables.product.prodname_vscode_shortname %} installation in WSL.
...
2. Install [{% data variables.product.prodname_vscode %}](https://code.visualstudio.com/) on the **Windows** side (not in WSL).
...
3. Install the [WSL extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl). If you plan to work with other remote extensions in {% data variables.product.prodname_vscode_shortname %}, you may choose to install the [Remote Development extension pack](https://aka.ms/vscode-remote/download/extension).

### Open a remote folder or workspace

#### From the WSL terminal

Opening a folder inside the Windows Subsystem for Linux in {% data variables.product.prodname_vscode_shortname %} is very similar to opening up a Windows folder from the command prompt or PowerShell.

1. Open a **WSL terminal window** (using the start menu item or by typing `wsl` from a command prompt / PowerShell).

2. Navigate to a folder you'd like to open in {% data variables.product.prodname_vscode_shortname %} (including, but not limited to, Windows filesystem mounts like `/mnt/c`)

3. Type **`code .`** in the terminal. When doing this for the first time, you should see {% data variables.product.prodname_vscode_shortname %} fetching components needed to run in WSL. This should only take a short while, and is only needed once.
...
4. After a moment, a new {% data variables.product.prodname_vscode_shortname %} window will appear, and you'll see a notification that {% data variables.product.prodname_vscode_shortname %} is opening the folder in WSL.

   ![WSL Starting notification](images/wsl//wsl-starting-notification.png)

    {% data variables.product.prodname_vscode_shortname %} will now continue to configure itself in WSL and keep you up to date as it makes progress.

5. Once finished, you now see a WSL indicator in the bottom left corner, and you'll be able to use {% data variables.product.prodname_vscode_shortname %} as you would normally!

    ![WSL Status Bar Item](images/wsl/wsl-statusbar-indicator.png)

That's it! Any {% data variables.product.prodname_vscode_shortname %} operations you perform in this window will be executed in the WSL environment, everything from editing and file operations, to debugging, using terminals, and more.

#### From {% data variables.product.prodname_vscode_shortname %}

Alternatively, you can open a WSL window directly from {% data variables.product.prodname_vscode_shortname %}:
...
{% data variables.product.prodname_vscode_shortname %} runs extensions in one of two places: locally on the UI / client side, or in WSL. While extensions that affect the {% data variables.product.prodname_vscode_shortname %} UI, like themes and snippets, are installed locally, most extensions will reside inside WSL.
...
Opening a terminal in WSL from {% data variables.product.prodname_vscode_shortname %} is simple. Once folder is opened in WSL, **any terminal window** you open in {% data variables.product.prodname_vscode_shortname %} (**Terminal > New Terminal**) will automatically run in WSL rather than locally.
...
6. The {% data variables.product.prodname_vscode_shortname %} window (instance) will reload and start building the dev container. A progress notification provides status updates.

    ![Dev Container Progress Notification](images/wsl/dev-container-progress.png)

7. After the build completes, {% data variables.product.prodname_vscode_shortname %} will automatically connect to the container. You can now work with your source code from inside the container.
...
While the Container Tools extension can run both remotely and locally, if it is already installed locally, you will be unable to install on a remote SSH host without first uninstalling it locally. We will address this problem in a future {% data variables.product.prodname_vscode_shortname %} release.
...
The WSL extension and {% data variables.product.prodname_vscode_shortname %} Server require outbound HTTPS (port 443) connectivity to:
...
All other communication between the server and the {% data variables.product.prodname_vscode_shortname %} client is accomplished through an random local TCP port. You can find a list of locations {% data variables.product.prodname_vscode_shortname %} itself needs access to in the [network connections article](/docs/setup/network.md#common-hostnames).
...
- as described in [Network Connections in {% data variables.product.prodname_vscode %}](/docs/setup/network)
