...
After selecing a `project.json` file, the aforementioned status bar text will say "Starting...". OmniSharp (the brain behind VS Code’s C# support) is now starting, loading your project. If you see a red icon, it means something went wrong. The specifics of the problem can be found by looking at the OmniSharp Output Log (`kb(o.showOutput)`).
