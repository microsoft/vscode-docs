...
> **Tip:** The working directory for the new terminal depends on the `setting(terminal.integrated.splitCwd)` [setting](/docs/editor/customizing/settings.md).
...
The content in the terminal is called the buffer, with the section right above the bottom viewport being called "scrollback". The amount of scrollback kept is determined by the `setting(terminal.integrated.scrollback)` [setting](/docs/editor/customizing/settings.md) and defaults to `1000` lines.
...
> **Tip:** If link verification causes performance issues, like in high latency remote environments, disable it via the `setting(terminal.integrated.enableFileLinks)` [setting](/docs/editor/customizing/settings.md).
...
Note that the panel can only be maximized if its [alignment](/docs/editor/customizing/custom-layout.md#panel-alignment) option is set to **Center**.
...
The macOS default terminal uses `kbstyle(Cmd+.)` to perform the same as `kbstyle(Ctrl+C)`. To get this behavior in VS Code, add this [custom keyboard shortcut](/docs/editor/customizing/keybindings.md):
