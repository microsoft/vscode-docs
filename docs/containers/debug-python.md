...
When adding Docker files to a Python project, tasks,s and launch configurations are added to enable debugging the application within a Docker container. To accommodate the various scenarios of Python projects, some apps may require additional configuration.
...
1. Hit `kb(workbench.action.debug.start)` to attach thes debugger.
