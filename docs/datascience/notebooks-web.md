...
MetaDescription: Working with Jupyter notebooks on the web with {% data variables.product.prodname_vscode %}.
MetaSocialImage: images/tutorial/python-social.png
---
# Jupyter Notebooks on the web

{% data variables.product.prodname_vscode %} supports working with [Jupyter Notebooks](https://jupyter-notebook.readthedocs.io/en/latest/) on the desktop, and extends to various browser-based platforms like [GitHub Codespaces](https://github.com/features/codespaces) and [{% data variables.product.prodname_vscode_shortname %} for the Web](/docs/remote/vscode-web.md).
...
- [Remote tunneling with **{% data variables.product.prodname_vscode_shortname %} Server** on {% data variables.product.prodname_vscode_shortname %} for the Web](#remote-tunneling-with-vs-code-server)
- [Connect to a remote **Jupyter server** on {% data variables.product.prodname_vscode_shortname %} for the Web](#connect-to-a-remote-jupyter-server)

## Jupyter notebooks on Codespaces

Developing Jupyter notebooks in {% data variables.product.prodname_vscode_shortname %} can be done entirely through a web-based interface using [GitHub Codespaces](https://github.com/features/codespaces), a cloud-hosted development environment that is secure and configurable with free compute resources (more on Codespaces [monthly usage quotas](https://docs.github.com/billing/managing-billing-for-github-codespaces/about-billing-for-github-codespaces)). This means you can take advantage of all {% data variables.product.prodname_vscode_shortname %} functionalities, including Jupyter notebooks support, without installing anything on your machine. To get started quickly:
...
## Jupyter notebooks on {% data variables.product.prodname_vscode_shortname %} for the Web

You can also use [{% data variables.product.prodname_vscode_shortname %} for the Web](/docs/remote/vscode-web.md) by going to [https://vscode.dev](https://vscode.dev) or [https://github.dev](https://github.dev) (available by typing '.' when viewing a repo on GitHub). With {% data variables.product.prodname_vscode_shortname %} for the Web, you have two options to connect to the following types of kernels for your notebook:

1. [Remote tunneling with {% data variables.product.prodname_vscode_shortname %} Server](#remote-tunneling-with-vs-code-server)
2. [Connect to a remote Jupyter server](#connect-to-a-remote-jupyter-server)

## Remote tunneling with {% data variables.product.prodname_vscode_shortname %} Server

You can securely connect to a remote machine without the requirement of SSH by installing the [{% data variables.product.prodname_vscode_shortname %} Server](/docs/remote/vscode-server.md) on your remote machine and accessing it through {% data variables.product.prodname_vscode_shortname %} for the Web (you can also connect to the server on {% data variables.product.prodname_vscode_shortname %} Desktop). To do so:

1. Install the `code` [CLI](/download) on your remote machine.

   > **Note**: You can skip this step if {% data variables.product.prodname_vscode_shortname %} Desktop is already installed on the remote machine as the `code` CLI is built into {% data variables.product.prodname_vscode_shortname %} Desktop.

2. Create a [secure tunnel](/docs/remote/tunnels.md) with the tunnel command: `code tunnel` (you can also run `code-insiders tunnel` if you wish to use the [daily release](/insiders) of {% data variables.product.prodname_vscode_shortname %}) and follow the prompts to grant access to the server. This will download and start the {% data variables.product.prodname_vscode_shortname %} Server on your remote machine and then create a tunnel to it.
...
To enter your remote Jupyter server URL with older versions of {% data variables.product.prodname_vscode_shortname %} and Jupyter extension:
...
Since {% data variables.product.prodname_vscode_shortname %} for the Web runs entirely in your web browser, there are some limitations compared to the desktop and Codespaces experiences.

- No access the {% data variables.product.prodname_vscode_shortname %} terminal (though you can run [magic commands](https://ipython.readthedocs.io/en/stable/interactive/magics.html) from your notebook cells)
- Limited debugging
...
As with {% data variables.product.prodname_vscode_shortname %} Desktop, you can easily continue your work from {% data variables.product.prodname_vscode_shortname %} for the Web ([https://vscode.dev](https://vscode.dev) or [https://github.dev](https://github.dev)) on Codespaces. When you choose to **Continue Working On** in a new codespace, your uncommitted changes will travel with you. To do so:
