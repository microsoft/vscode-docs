...
When developing a project or learning something new, it can be a significant help to get AI assistance on your questions, large or small. GitHub Copilot enables an interactive chat experience that understands the context of your code, workspace, extensions, settings, and more.
...
* Launch Quick Chat to ask a quick question
...
To get started with GitHub Copilot Chat, you need to:

* Install the [GitHub Copilot Chat](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot-chat) extension. If you've previously installed the [GitHub Copilot](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot) extension, you should already have this extension.
...
If you have installed the Copilot Chat extension, you can access the chat features in the VS Code in different ways:
...
* **Chat smart actions**: Access commonly-used Copilot features from the editor context menu.
...
With the `inlineChat.mode` setting, you can configure how inline chat shows updates in the editor. The default setting value is `live`, which means that the updates are applied directly in the editor, and you can compare the changes by using inline diff viewer. If you prefer to preview the changes side-by-side, you configure the setting to `preview`.
...
Samples
    ...
    Does this type also work for decimals numbers?  -> follow-up
-->
...
The most powerful smart action is `/fix`. Here is a relatively simple TypeScript quick sort algorithm with an error noting that "Argument of type 'number' is not assignable to parameter of type 'never'". Select the light bulb, and then choose **Fix using Copilot**.
