...
| packageIndexDepths | [] | Used to control how many installed packages to be indexed. Accepts values are tuples of [```package name```, ```depth to scan```, ```include all symbols```]. For example: ```[["pandas", 3, true], ["numpy", 2, false]] ``` |
