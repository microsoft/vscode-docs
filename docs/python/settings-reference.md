...
| python.poetryPath | Specifies the location of the [Poetry dependency manager](https://pypi.org/project/poetry/) executable, if installed. The default value is `poetry`, which assumes the executable is in the current path. The Python extension uses this setting to install packages when Poetry is available and there's a `poetry.lock` file in the workspace folder. |
