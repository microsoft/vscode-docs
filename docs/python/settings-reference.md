...
| python.languageServer | Microsoft | Defines type of the language server ([Microsoft]((https://devblogs.microsoft.com/python/introducing-the-python-language-server)), Jedi, None). Note that the language server requires a platform that [supports .NET Core 2.1 or newer](https://docs.microsoft.com/dotnet/core/rid-catalog). |
