...
> **Tip**: You can also set the virtual environment directory in the your `settings.json` file by setting `"python.pythonPath": ".venv"`, where `.venv` is a virtual environment. This is important when you commit `.vscode/settings.json` file as it allows cross-platform support.
