...
Through configuration files, an extension can support syntax highlighting, snippets, and some smart editing features. For more advanced features, your extension needs to extend VS Code through its extensibility [API](/docs/extensionAPI/vscode-api.md) or it can provide a [language server](https://github.com/Microsoft/language-server-protocol/blob/master/protocol.md).
