...
1. Verify that each candidate is available in the public Insiders build. Exclude and report internal-only candidates instead of creating entries for them.
...
1. Verify that each feature is available in the Stable release being documented. Exclude and report internal-only features, and do not infer availability from source-code existence alone.
