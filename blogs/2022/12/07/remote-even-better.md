...
You may not think about it, but VS Code has a built-in [command-line interface](https://code.visualstudio.com/docs/editor/command-line) (CLI) that lets you control how you launch the editor - you can open files, install extensions, and output diagnostics through command-line options:
...
Today, we're thrilled to share our enhanced `code` CLI that lets you both launch VS Code and connect to the machine remotely from VS Code Desktop or vscode.dev. The updated CLI is built into the latest VS Code release and also now available for standalone install, so you can create a secure connection to any computer and connect to it remotely, even if you can't install VS Code Desktop on it.
...
If you'd like to stop a tunnel you're running via the CLI, you may use `kbstyle(Ctrl + C)` to end the active connection. If you've enabled tunneling through the VS Code UI, you may run the command **Remote Tunnels: Turn of Remote Tunnel Access...**.
