...
This sounds reasonable until V8 steps on your toe.

*Text Buffer memory usage after loading a 50MB file*
![]()

*Profile of building a text buffer for a 50MB file*
![]()

With above two images in mind, you can see two severe problems:

1. After string concatenation, the memory usage is almost doubled.
2. The first time when we execute `substring` on the original content string, it takes longer than expected.

When we do string concatenation, V8 will create a cons-string first. Next time you run `substring` or similar API on this cons-string, V8 will flatten the string and this process is time consuming.

This is against our principle #2: doing less. Instead of just holding two buffers, we can hold a list buffers. Each time we receive a 64KB chunk from disk, we push it to the buffers array and create a node which maps to this buffer. The node will be like
