...
For [those PRs](https://github.com/microsoft/vscode/pull/312854), the VS Code team uses an automated eval assessment flow. Adding the _~requires-eval-assessment_ label to a PR kicks off the process: the PR is built, shipped as an eval agent, benchmarked, and the results are posted back on the PR:
