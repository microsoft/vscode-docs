...
This post looks at how we improve that harness with model providers. We work with providers to understand how their models respond to our tools and instructions, turn their recommendations into concrete changes, and validate those changes before rolling them out broadly.

A recent OpenAI experiment for `GPT-5.5` shows what that looks like in practice: small prompt changes that helped the agent explore less, validate sooner, use fewer tokens, and respond faster.

## From prompt changes to experiment setup

After the launch of `GPT-5.5`, we noticed that the model was not as token-efficient inside the VS Code agent harness as OpenAI had referenced in comparable Codex scenarios. That gap gave us concrete room for improvement after launch. We dug into the harness to understand where the model was spending tokens and where it was over-exploring before acting.
...
Treatment A added a compact `<economical_search_and_edit>` section. The practical change was: **start from a concrete anchor, gather only enough local context, avoid broad exploration, act once there is a cheap discriminating check, and avoid rereading unchanged context.**
...
Treatment B added more explicit prompt structure around how the agent should reason, edit, and validate, expressed as `<Before_the_first_edit>` and `<After_the_first_edit>` sections. This prompt was much more than "use more context." It told the agent to **form a local hypothesis before editing**, **avoid broad exploration**, **make a grounded first edit**, and **validate immediately after the first substantive edit**.
...
The two-week scorecard showed that LargePromptSections was the stronger treatment. It improved latency, reduced upper-tail token usage, and reduced tool-call volume while quality, engagement, and reliability guardrails stayed healthy.
...
| --- | ---: | ---: | ---: | ---: | :---: | ---: | ---: | ---: | :---: |
| 10-minute focus survival rate (by user) | 91.54% | 91.17% | -0.40% | 0.0707 | - | 91.13% | -0.44% | 0.0493 | <span style="color: #d13438;">○</span> |
| Commit survival rate (by user) | 83.36% | 82.95% | -0.48% | 0.3200 | - | 83.93% | +0.68% | 0.1533 | - |
| p50 Time to First Chunk (by turn) | 7.3s | 7.2s | -0.81% | 0.4415 | - | 6.8s | -7.21% | 7e-12 | <span style="color: #107c10;">●</span> |
...
| p95 prompt tokens (by turn) | 6.1M | 5.8M | -5.20% | 0.0163 | <span style="color: #107c10;">○</span> | 5.6M | -7.64% | 0.0004 | <span style="color: #107c10;">●</span> |
...
* **Quality**: the guardrail metrics stayed mostly healthy. Commit survival rate moved slightly up for LargePromptSections (+0.68%) and slightly down for EconomicalSearchAndEdit (-0.48%), **neither statistically significant**. The 10-minute focus survival rate moved slightly down for both treatments: -0.44% for LargePromptSections and -0.40% for EconomicalSearchAndEdit, with only the LargePromptSections movement reaching **marginal significance** (p=0.0493).
* **Latency**: LargePromptSections delivered the strongest latency wins, and all three were **highly statistically significant**: p50 Time to First Chunk improved -7.21% (p=7e-12), p50 Time to First Edit improved -5.68% (p=2e-5), and p95 Time to First Edit improved -9.30% (p=1e-10). EconomicalSearchAndEdit moved in the right direction, but the latency effects were weaker: p50 Time to First Chunk -0.81% (not significant), p50 Time to First Edit -2.88% (p=0.0271), and p95 Time to First Edit -1.93% (not significant).
* **Token efficiency**: both treatments reduced median total tokens per user, but those p50 movements were **not statistically significant**: -3.25% for LargePromptSections and -2.54% for EconomicalSearchAndEdit. At the upper tail, LargePromptSections reduced p95 prompt tokens and p95 total tokens by -7.64%, both **highly statistically significant** (p=0.0004 and p=0.0003). EconomicalSearchAndEdit also reduced p95 prompt tokens (-5.20%, p=0.0163) and p95 total tokens (-5.19%, p=0.0157), **both statistically significant**. Both variants reduced average tool calls per turn: -8.54% for LargePromptSections, **highly statistically significant** (p=1e-12), and -3.19% for EconomicalSearchAndEdit, **statistically significant** (p=0.0091).

Based on these findings, we chose Treatment B, LargePromptSections, as the update to the default `GPT-5.5` system prompt. It had the strongest overall profile: clear latency improvements, significant upper-tail token reductions, fewer tool calls, and mostly stable quality guardrails. Treatment A moved several metrics in the right direction, but Treatment B gave us the more consistent result across the measures that matter most for VS Code.
