...
var URL = process.env['token']
  ? 'https://' + process.env['token'] + '@github.com/microsoft/vscode-website'
  : 'https://github.com/microsoft/vscode-website';
...
gulp.task('build-website', function(cb) {
  runSequence('compile', 'clone-vscode-website', 'generate-api-doc', 'compile-all');
});
...
gulp.task('minify-images', function() {
  return gulp
    .src(['**/*.{png,jpg,svg}', '!node_modules/**'], { nocase: true })
    .pipe(imagemin())
    .pipe(gulp.dest('.'));
});
